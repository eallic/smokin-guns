...
// Changes Globals:		-
//===========================================================================
void BotDefaultCharacteristics(bot_character_t *ch,
                               bot_character_t *defaultch) {
    int i;
    ...
    for (i = 0; i < MAX_CHARACTERISTICS; i++) {
        ...
        if (defaultch->c[i].type == CT_FLOAT) {
            ...
            ch->c[i].value.string =
                (char *)GetMemory(strlen(defaultch->c[i].value.string) + 1);
            ...
            if (skill < 0 || token.intvalue == skill) {
                ...
                while (PC_ExpectAnyToken(source, &token)) {
                    ...
                    if (token.type == TT_NUMBER) {
                        ...
                        ch->c[index].value.string =
                            GetMemory(strlen(token.string) + 1);
...
int BotLoadCharacterSkill(char *charfile, float skill) {
    ...
    ch = BotLoadCachedCharacter(charfile, skill,
                                LibVarGetValue("bot_reloadcharacters"));
...
int BotInterpolateCharacters(int handle1, int handle2, float desiredskill) {
    ...
    for (i = 0; i < MAX_CHARACTERISTICS; i++) {
        ...
        if (ch1->c[i].type == CT_FLOAT && ch2->c[i].type == CT_FLOAT) {
            ...
            out->c[i].value._float =
                ch1->c[i].value._float +
            ...
            out->c[i].value.string =
                (char *)GetMemory(strlen(ch1->c[i].value.string) + 1);
