...
int BotReachabilityArea(vec3_t origin, int client) {
    ...
    if (!bsptrace.startsolid && bsptrace.fraction < 1 &&
        bsptrace.ent != ENTITYNUM_NONE) {
        ...
        // if standing on a func_plat or func_bobbing then the bot is assumed to
        // be in the area the reachability points to
        ...
        if (areanum && AAS_AreaReachability(areanum))
            return areanum;
        // trace down till the ground is hit because the bot is standing on some
        // other entity
...
int BotReachabilityArea(vec3_t origin, int testground)
{
        ...
        for (i = 0; i < 2; i++)
        {
                ...
                if (i > 0)
                {
                        ...
                        trace = AAS_TraceClientBBox(origin, end,
PRESENCE_CROUCH, -1); if (!trace.startsolid)
                        {
                                VectorCopy(trace.endpos, org);
                        } //end if
                } //end if
                ...
                for (z = 1; z >= -1; z -= 1)
                {
                        for (x = 1; x >= -1; x -= 1)
                        {
                                for (y = 1; y >= -1; y -= 1)
                                {
                                        ...
                                        numareas = AAS_TraceAreas(org, end,
areas, points, 10); for (j = 0; j < numareas; j++)
                                        {
                                                if
(AAS_AreaReachability(areas[j]))
                                                {
                                                        VectorSubtract(points[j],
org, v); dist = VectorLength(v); if (dist < bestdist)
                                                        {
                                                                bestareanum =
areas[j]; bestdist = dist; } //end if } //end if } //end for } //end for } //end
for if (bestareanum) return bestareanum; } //end for if (!testground) break; }
//end for
//#ifdef DEBUG
...
int BotOnMover(vec3_t origin, int entnum, aas_reachability_t *reach) {
    ...
    if (!trace.startsolid && !trace.allsolid) {
        // NOTE: the reachability face number is the model number of the
        // elevator
...
// Changes Globals:		-
//===========================================================================
int BotAvoidSpots(vec3_t origin, aas_reachability_t *reach,
                  bot_avoidspot_t *avoidspots, int numavoidspots) {
    ...
    for (i = 0; i < numavoidspots; i++) {
        ...
        squareddist =
            DistanceFromLineSquared(avoidspots[i].origin, origin, reach->start);
        ...
        if (squareddist < squaredradius &&
            ...
            squareddist = DistanceFromLineSquared(avoidspots[i].origin,
                                                  reach->start, reach->end);
            // if moving towards the avoid spot
            if (squareddist < squaredradius &&
                VectorDistanceSquared(avoidspots[i].origin, reach->start) >
                    squareddist) {
                type = avoidspots[i].type;
            } // end if
        } // end if
...
int BotGetReachabilityToGoal(vec3_t origin, int areanum, int lastgoalareanum,
    ...
    for (reachnum = AAS_NextAreaReachability(areanum, 0); reachnum;
       ...
       // get the reachability from the number
        ...
        // NOTE: is this actually avoidance of local routing minima between two
        // areas???
        if (lastgoalareanum == goal->areanum && reach.areanum == lastareanum)
            continue;
        // if (AAS_AreaContentsTravelFlags(reach.areanum) & ~travelflags)
        // continue; if the travel isn't valid
        ...
        t += reach.traveltime; // + AAS_AreaTravelTime(areanum, origin,
                               // reach.start);
...
// Changes Globals:		-
//===========================================================================
int BotAddToTarget(vec3_t start, vec3_t end, float maxdist, float *dist,
                   vec3_t target) {
    vec3_t dir;
    float curdist;
...
int BotMovementViewTarget(int movestate, bot_goal_t *goal, int travelflags,
    ...
    while (reachnum && dist < lookahead) {
        ...
        if ((reach.traveltype & TRAVELTYPE_MASK) != TRAVEL_JUMPPAD &&
            ...
            if (BotAddToTarget(reach.start, reach.end, lookahead, &dist,
                               target))
                return qtrue;
        } // end if
        reachnum = BotGetReachabilityToGoal(
            reach.end, reach.areanum, ms->lastgoalareanum, lastareanum,
            ms->avoidreach, ms->avoidreachtimes, ms->avoidreachtries, goal,
            travelflags, NULL, 0, NULL);
...
// Changes Globals:		-
//===========================================================================
int BotPredictVisiblePosition(vec3_t origin, int areanum, bot_goal_t *goal,
    ...
    for (i = 0; i < 20 && (areanum != goal->areanum); i++) {
        //
        reachnum = BotGetReachabilityToGoal(
            end, areanum, lastgoalareanum, lastareanum, avoidreach,
            avoidreachtimes, avoidreachtries, goal, travelflags, NULL, 0, NULL);
        if (!reachnum)
            return qfalse;
        AAS_ReachabilityFromNum(reachnum, &reach);
        //
...
int BotCheckBarrierJump(bot_movestate_t *ms, vec3_t dir, float speed) {
    ...
    // trace right up
    trace =
        AAS_TraceClientBBox(ms->origin, end, PRESENCE_NORMAL, ms->entitynum);
...
int BotWalkInDirection(bot_movestate_t *ms, vec3_t dir, float speed, int type) {
    ...
    if (ms->moveflags & MFL_ONGROUND) {
        ...
        if (type & MOVE_JUMP) {
            ...
            cmdframes = 1;
            stopevent = SE_HITGROUND | SE_HITGROUNDDAMAGE | SE_ENTERWATER |
                        SE_ENTERSLIME | SE_ENTERLAVA;
        } // end if
        else {
            maxframes = 2;
            cmdframes = 2;
            stopevent = SE_HITGROUNDDAMAGE | SE_ENTERWATER | SE_ENTERSLIME |
                        SE_ENTERLAVA;
        } // end else
        ...
        origin[2] += 0.5;
        AAS_PredictClientMovement(&move, ms->entitynum, origin, presencetype,
                                  qtrue, velocity, cmdmove, cmdframes,
                                  maxframes, 0.1f, stopevent, 0,
                                  qfalse); // qtrue);
        // if prediction time wasn't enough to fully predict the movement
        if (move.frames >= maxframes && (type & MOVE_JUMP)) {
            // botimport.Print(PRT_MESSAGE, "client %d: max prediction
            // frames\n", ms->client);
            return qfalse;
        } // end if
        ...
        if (move.stopevent &
            ...
            // botimport.Print(PRT_MESSAGE, "client %d: would be hurt ",
            // ms->client); if (move.stopevent & SE_ENTERSLIME)
            // botimport.Print(PRT_MESSAGE, "slime\n"); if (move.stopevent &
            // SE_ENTERLAVA) botimport.Print(PRT_MESSAGE, "lava\n"); if
            // (move.stopevent & SE_HITGROUNDDAMAGE)
            // botimport.Print(PRT_MESSAGE, "hitground\n");
            return qfalse;
        } // end if
        // if ground was hit
...
void BotCheckBlocked(bot_movestate_t *ms, vec3_t dir, int checkbottom,
    ...
    // Joe Kari: applying the patch of The Doctor:
    // http://forum.smokin-guns.org/viewtopic.php?f=28&t=3075
    // if (!trace.startsolid && (trace.ent != ENTITYNUM_WORLD && trace.ent !=
    // ENTITYNUM_NONE) )
    ...
    // if not in an area with reachability
        ...
        // Joe Kari: again, applying the patch of The Doctor:
        // http://forum.smokin-guns.org/viewtopic.php?f=28&t=3075
        // if (!trace.startsolid && (trace.ent != ENTITYNUM_WORLD && trace.ent
        // != ENTITYNUM_NONE) )
        if (trace.ent != ENTITYNUM_WORLD && trace.ent != ENTITYNUM_NONE) {
...
#ifdef DEBUG
            // botimport.Print(PRT_MESSAGE, "%d: BotCheckBlocked: I'm
            // blocked\n", ms->client);
#endif // DEBUG
        } // end if
    } // end else
...
bot_moveresult_t BotFinishTravel_Walk(bot_movestate_t *ms,
    ...
    {
#ifdef DEBUG
            botimport.Print(PRT_MESSAGE, "BotFinishTravel_Walk: already in reach
area\n"); #endif //DEBUG return result; } //end if*/
...
bot_moveresult_t BotTravel_WalkOffLedge(bot_movestate_t *ms,
    ...
    if (dist < 48) {
        ...
        else if (!AAS_HorizontalVelocityForJump(0, reach->start, reach->end,
                                                &speed)) {
            speed = 400;
        } // end if
    } // end if
...
// Changes Globals:		-
//===========================================================================
int BotAirControl(vec3_t origin, vec3_t velocity, vec3_t goal, vec3_t dir,
                  float *speed) {
    vec3_t org, vel;
    float dist;
    int i;
...
bot_moveresult_t BotTravel_Jump(bot_movestate_t *ms, aas_reachability_t *reach)
{
        ...
        if (dist < 50 || (gapdist && gapdist < 50))
        {
                ...
                //if (AAS_HorizontalVelocityForJump(sv_jumpvel, ms->origin,
reach->end, &horspeed))
                //{
                //	speed = horspeed * 400 /
botlibglobals.sv_maxwalkvelocity->value;
                //} //end if
                hordir[0] = reach->end[0] - ms->origin[0];
                hordir[1] = reach->end[1] - ms->origin[1];
                VectorNormalize(hordir);
        ...
        {
                if (AAS_HorizontalVelocityForJump(sv_jumpvel, reach->start,
reach->end, &horspeed))
                {
                        speed = horspeed * 400 /
botlibglobals.sv_maxwalkvelocity->value; } //end if } //end else
        //elemantary action
        EA_Move(ms->client, hordir, speed);
        VectorCopy(hordir, result.movedir);
        //
...
bot_moveresult_t BotTravel_Jump(bot_movestate_t *ms, aas_reachability_t *reach)
{
        ...
        //check for solids
        trace = AAS_Trace(start, mins, maxs, end, ms->entitynum,
MASK_PLAYERSOLID); if (trace.startsolid) VectorCopy(start, trace.endpos);
        //check for a gap
...
//	if (dist1 && dist1 <= trace.fraction * 80) VectorMA(reach->start,
dist1-20, hordir, trace.endpos);
        //
        ...
        {
                //botimport.Print(PRT_MESSAGE, "between jump start and run to
point\n"); hordir[0] = reach->end[0] - ms->origin[0]; hordir[1] = reach->end[1]
- ms->origin[1]; hordir[2] = 0; VectorNormalize(hordir);
                //elemantary action jump
...
bot_moveresult_t BotTravel_Jump(bot_movestate_t *ms,
    ...
    // if just before the reachability start
    if (DotProduct(dir1, dir2) < -0.8 || dist2 < 5) {
        //		botimport.Print(PRT_MESSAGE, "between jump start and run
        //start point\n");
        hordir[0] = reach->end[0] - ms->origin[0];
        hordir[1] = reach->end[1] - ms->origin[1];
        hordir[2] = 0;
        ...
        //		botimport.Print(PRT_MESSAGE, "going towards run start
        //point\n");
...
// Changes Globals:		-
//===========================================================================
bot_moveresult_t BotTravel_FuncBobbing(bot_movestate_t *ms,
                                       aas_reachability_t *reach) {
    vec3_t dir, dir1, dir2, hordir, bottomcenter, bob_start, bob_end,
        bob_origin;
    float dist, dist1, dist2, speed;
    bot_moveresult_t_cleared(result);
...
// Changes Globals:		-
//===========================================================================
bot_moveresult_t BotFinishTravel_FuncBobbing(bot_movestate_t *ms,
                                             aas_reachability_t *reach) {
...
bot_moveresult_t BotTravel_Grapple(bot_movestate_t *ms,
    ...
    if (ms->moveflags & MFL_ACTIVEGRAPPLE) {
       ...
       //
        ...
        //
        if (dist < 5 &&
            fabs(AngleDiff(result.ideal_viewangles[0], ms->viewangles[0])) <
                2 &&
            fabs(AngleDiff(result.ideal_viewangles[1], ms->viewangles[1])) <
                2) {
#ifdef DEBUG_GRAPPLE
            botimport.Print(PRT_MESSAGE,
       ...
       // check if the grapple missile path is clear
            VectorAdd(ms->origin, ms->viewoffset, org);
            trace = AAS_Trace(org, NULL, NULL, reach->end, ms->entitynum,
                              CONTENTS_SOLID);
...
// Changes Globals:		-
//===========================================================================
bot_moveresult_t BotTravel_BFGJump(bot_movestate_t *ms,
                                   aas_reachability_t *reach) {
    vec3_t hordir;
    float dist, speed;
    bot_moveresult_t_cleared(result);
...
// Changes Globals:		-
//===========================================================================
bot_moveresult_t BotTravel_JumpPad(bot_movestate_t *ms,
                                   aas_reachability_t *reach) {
    vec3_t hordir;
    bot_moveresult_t_cleared(result);
...
// Changes Globals:		-
//===========================================================================
bot_moveresult_t BotFinishTravel_JumpPad(bot_movestate_t *ms,
                                         aas_reachability_t *reach) {
    float speed;
    vec3_t hordir;
    bot_moveresult_t_cleared(result);
...
bot_moveresult_t BotMoveInGoalArea(bot_movestate_t *ms, bot_goal_t *goal) {
    ...
    // botimport.DebugLineShow(debugline, ms->origin, goal->origin,
    // LINECOLOR_BLUE);
    //
    ms->lastreachnum = 0;
    ms->lastareanum = 0;
...
// Changes Globals:		-
//===========================================================================
void BotMoveToGoal(bot_moveresult_t *result, int movestate, bot_goal_t *goal,
                   int travelflags) {
            ...
            if (modelnum >= 0 && modelnum < MAX_MODELS) {
                ...
                if (modeltype == MODELTYPE_FUNC_PLAT) {
                    ...
                    if ((reach.traveltype & TRAVELTYPE_MASK) !=
                        ...
                        reachnum = AAS_NextModelReachability(0, modelnum);
                        if (reachnum) {
                            // botimport.Print(PRT_MESSAGE, "client %d:
                            // accidentally ended up on func_plat\n",
                            // ms->client);
                            AAS_ReachabilityFromNum(reachnum, &reach);
                            ms->lastreachnum = reachnum;
                            ms->reachability_time =
                                AAS_Time() + BotReachabilityTime(&reach);
                        } // end if
                        else {
                            if (botDeveloper) {
                                botimport.Print(PRT_MESSAGE,
                                                "client %d: on func_plat "
                    ...
                    if ((reach.traveltype & TRAVELTYPE_MASK) !=
                            TRAVEL_FUNCBOB ||
                        // NOTE: the face number is the func_bobbing model
                        // number
                        (reach.facenum & 0x0000FFFF) != modelnum) {
                        reachnum = AAS_NextModelReachability(0, modelnum);
                        if (reachnum) {
                            // botimport.Print(PRT_MESSAGE, "client %d:
                            // accidentally ended up on func_bobbing\n",
                            // ms->client);
                            AAS_ReachabilityFromNum(reachnum, &reach);
                            ms->lastreachnum = reachnum;
                            ms->reachability_time =
    ...
    if (ms->moveflags & (MFL_ONGROUND | MFL_SWIMMING | MFL_AGAINSTLADDER)) {
        // botimport.Print(PRT_MESSAGE, "%s: onground, swimming or against
        // ladder\n", ClientName(ms->entitynum-1));
        //
        AAS_ReachabilityFromNum(ms->lastreachnum, &lastreach);
        // reachability area the bot is in
        // ms->areanum = BotReachabilityArea(ms->origin, ((lastreach.traveltype
        ...
        if (reachnum) {
            ...
            // special elevator case
                ...
                // if the bot was going for an elevator and reached the
                // reachability area
                if (ms->areanum == reach.areanum ||
                ...
                if (botDeveloper) {
                    ...
                    {
                            botimport.Print(PRT_MESSAGE, "changed from area %d
       ...
       // or the area changed
        ...
        if (!reachnum) {
            ...
            reachnum = BotGetReachabilityToGoal(
                ms->origin, ms->areanum, ms->lastgoalareanum, ms->lastareanum,
                ms->avoidreach, ms->avoidreachtimes, ms->avoidreachtries, goal,
                travelflags, ms->avoidspots, ms->numavoidspots, &resultflags);
            ...
            if (reachnum) {
                ...
                //
#ifdef AVOIDREACH
                // add the reachability to the reachabilities to avoid for a
                // while
                ...
                Com_Memset(&reach, 0,
                           sizeof(aas_reachability_t)); // make compiler happy
            } // end else
            if (botDeveloper) {
       ...
       //
        ...
        if (botDeveloper) {
            if (result->failure) {
                botimport.Print(PRT_MESSAGE, "client %d: movement failure in ",
                                ms->client);
                AAS_PrintTravelType(reach.traveltype & TRAVELTYPE_MASK);
                botimport.Print(PRT_MESSAGE, "\n");
            } // end if
        ...
        // special handling of jump pads when the bot uses a jump pad without
        // knowing it
        foundjumppad = qfalse;
        ...
        for (i = numareas - 1; i >= 0; i--) {
            if (AAS_AreaJumpPad(areas[i])) {
                // botimport.Print(PRT_MESSAGE, "client %d used a jumppad
                // without knowing, area %d\n", ms->client, areas[i]);
                foundjumppad = qtrue;
                lastreachnum = BotGetReachabilityToGoal(
                    end, areas[i], ms->lastgoalareanum, ms->lastareanum,
                    ms->avoidreach, ms->avoidreachtimes, ms->avoidreachtries,
                    goal, TFL_JUMPPAD, ms->avoidspots, ms->numavoidspots, NULL);
                if (lastreachnum) {
                    ms->lastreachnum = lastreachnum;
                    ms->lastareanum = areas[i];
                    // botimport.Print(PRT_MESSAGE, "found jumppad
                    // reachability\n");
                    break;
                } // end if
                else {
                    for (lastreachnum = AAS_NextAreaReachability(areas[i], 0);
                         lastreachnum; lastreachnum = AAS_NextAreaReachability(
                                           areas[i], lastreachnum)) {
                        // get the reachability from the number
        ...
        //
        if (ms->lastreachnum) {
            // botimport.Print(PRT_MESSAGE, "%s: NOT onground, swimming or
            // against ladder\n", ClientName(ms->entitynum-1));
            AAS_ReachabilityFromNum(ms->lastreachnum, &reach);
            result->traveltype = reach.traveltype;
#ifdef DEBUG
