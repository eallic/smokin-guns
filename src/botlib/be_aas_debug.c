...
void AAS_ShowPolygon(int color, int numpoints, vec3_t *points) {
    ...
    for (i = 0; i < MAX_DEBUGPOLYGONS; i++) {
        if (!debugpolygons[i]) {
            debugpolygons[i] =
                botimport.DebugPolygonCreate(color, numpoints, points);
            break;
        } // end if
    } // end for
...
void AAS_ClearShownDebugLines(void) {
    ...
    for (i = 0; i < MAX_DEBUGLINES; i++) {
        if (debuglines[i]) {
            // botimport.DebugLineShow(debuglines[i], NULL, NULL,
            // LINECOLOR_NONE);
...
// Changes Globals:		-
//===========================================================================
void AAS_DrawPlaneCross(vec3_t point, vec3_t normal, float dist, int type,
                        int color) {
    ...
    start1[n0] = (dist - (start1[n1] * normal[n1] + start1[n2] * normal[n2])) /
                 normal[n0];
    end1[n0] =
        (dist - (end1[n1] * normal[n1] + end1[n2] * normal[n2])) / normal[n0];
    start2[n0] = (dist - (start2[n1] * normal[n1] + start2[n2] * normal[n2])) /
                 normal[n0];
    end2[n0] =
        (dist - (end2[n1] * normal[n1] + end2[n2] * normal[n2])) / normal[n0];
...
void AAS_ShowBoundingBox(vec3_t origin, vec3_t mins, vec3_t maxs) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        // vertical lines
        botimport.DebugLineShow(lines[2], bboxcorners[i], bboxcorners[4 + i],
                                LINECOLOR_RED);
    } // end for
...
void AAS_ShowFace(int facenum) {
    ...
    for (i = 0; i < face->numedges; i++) {
        ...
        AAS_DebugLine(aasworld.vertexes[edge->v[0]],
                      aasworld.vertexes[edge->v[1]], color);
    } // end for
...
void AAS_ShowArea(int areanum, int groundfacesonly) {
    ...
    for (n = 0; n < numareaedges; n++) {
        ...
        if (color == LINECOLOR_RED)
            ...
            color = LINECOLOR_RED;
        botimport.DebugLineShow(debuglines[line], aasworld.vertexes[edge->v[0]],
                                aasworld.vertexes[edge->v[1]], color);
        debuglinevisible[line] = qtrue;
    } // end for*/
...
void AAS_ShowReachability(aas_reachability_t *reach) {
    ...
    if ((reach->traveltype & TRAVELTYPE_MASK) == TRAVEL_JUMP ||
        ...
        AAS_HorizontalVelocityForJump(aassettings.phys_jumpvel, reach->start,
                                      reach->end, &speed);
        //
        ...
        //
        AAS_PredictClientMovement(&move, -1, reach->start, PRESENCE_NORMAL,
        ...
        //
        AAS_PredictClientMovement(&move, -1, reach->start, PRESENCE_NORMAL,
                                  qtrue, velocity, cmdmove, 30, 30, 0.1f,
...
void AAS_FloodAreas_r(int areanum, int cluster, int *done) {
    ...
    for (i = 0; i < area->numfaces; i++) {
        ...
        if (aasworld.areasettings[nextareanum].contents &
