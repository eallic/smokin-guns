        ...
        if (*homedir) {
            if (*gamedir)
                Com_sprintf(logfilename, sizeof(logfilename),
                            "%s%c%s%cbotlib.log", homedir, PATH_SEP, gamedir,
                            PATH_SEP);
            else if (*basegame)
                Com_sprintf(logfilename, sizeof(logfilename),
                            "%s%c%s%cbotlib.log", homedir, PATH_SEP, basegame,
                            PATH_SEP);
            else
                Com_sprintf(logfilename, sizeof(logfilename),
                            "%s%c" BASEGAME "%cbotlib.log", homedir, PATH_SEP,
                            PATH_SEP);
        } else
...
int BotExportTest(int parm0, char *parm1, vec3_t parm2, vec3_t parm3) {
    ...
    */
    ...
    // if (AAS_AgainstLadder(parm2)) botimport.Print(PRT_MESSAGE, "against
    ...
    botimport.Print(PRT_MESSAGE, "\rtravel time to goal (%d) = %d  ",
                    botlibglobals.goalareanum,
                    AAS_AreaTravelTimeToGoalArea(newarea, origin,
                                                 botlibglobals.goalareanum,
                                                 TFL_DEFAULT));
    ...
    if (newarea != area) {
        ...
        botimport.Print(PRT_MESSAGE,
                        ...
                        AAS_AreaCluster(area), AAS_PointPresenceType(origin));
        ...
        botimport.Print(PRT_MESSAGE, "travel time to goal (%d) = %d\n",
                        ...
                        AAS_AreaTravelTimeToGoalArea(
                            newarea, origin, botlibglobals.goalareanum,
    ...
    reachnum = BotGetReachabilityToGoal(parm2, newarea, 1,
                                                                    ms.avoidreach,
    ms.avoidreachtimes, &goal, TFL_DEFAULT); if (!reachnum)
    {
    ...
    */
    ...
    if (parm0 & 2)
        ...
        reachnum = BotGetReachabilityToGoal(origin, newarea,
                                                                  lastgoalareanum,
        lastareanum, avoidreach, avoidreachtimes, avoidreachtries, &goal,
        ...
        */
        ...
        for (i = 0; i < 100; i++) {
            ...
            reachnum = BotGetReachabilityToGoal(
                curorigin, curarea, lastgoalareanum, lastareanum, avoidreach,
                avoidreachtimes, avoidreachtries, &goal,
                TFL_DEFAULT | TFL_FUNCBOB | TFL_ROCKETJUMP, NULL, 0,
                &resultFlags);
            AAS_ReachabilityFromNum(reachnum, &reach);
            AAS_ShowReachability(&reach);
    ...
    */
    ...
    end_time - start_time, CLOCKS_PER_SEC); start_time = clock(); for (i = 0; i
    < 2000; i++)
            {
