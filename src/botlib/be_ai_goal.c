...
itemconfig_t *LoadItemConfig(char *filename) {
    ...
    ic = (itemconfig_t *)GetClearedHunkMemory(
        sizeof(itemconfig_t) + max_iteminfo * sizeof(iteminfo_t));
...
void InitLevelItemHeap(void) {
    ...
    levelitemheap =
        (levelitem_t *)GetClearedMemory(max_levelitems * sizeof(levelitem_t));
    ...
    for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
        ...
        if (spawnflags & 1) {
            ...
            if (!(AAS_PointContents(origin) & CONTENTS_WATER)) {
                ...
                trace = AAS_Trace(origin, ic->iteminfo[i].mins,
                                  ic->iteminfo[i].maxs, end, -1,
                ...
                if (trace.fraction >= 1) {
                    ...
                    goalareanum = AAS_BestReachableFromJumpPadArea(
                        origin, ic->iteminfo[i].mins, ic->iteminfo[i].maxs);
                    Log_Write("item %s reachable from jumppad area %d\r\n",
                              ic->iteminfo[i].classname, goalareanum);
                    // botimport.Print(PRT_MESSAGE, "item %s reachable from
                    // jumppad area %d\r\n", ic->iteminfo[i].classname,
                    // goalareanum);
                    if (!goalareanum)
                        continue;
        ...
        if (!(spawnflags & 1)) {
            if (!AAS_DropToFloor(origin, ic->iteminfo[i].mins,
                                 ic->iteminfo[i].maxs)) {
                botimport.Print(PRT_MESSAGE,
        ...
        if (goalareanum) {
            ...
            li->goalareanum =
                AAS_BestReachableArea(origin, ic->iteminfo[i].mins,
                                      ic->iteminfo[i].maxs, li->goalorigin);
            if (!li->goalareanum) {
                botimport.Print(
                    PRT_MESSAGE,
...
void BotUpdateEntityItems(void) {
    ...
    for (ent = AAS_NextEntity(0); ent; ent = AAS_NextEntity(ent)) {
                ...
                if (ic->iteminfo[li->iteminfo].modelindex != modelindex) {
                    ...
                    if (entinfo.origin[0] != li->origin[0] ||
                        ...
                        li->goalareanum = AAS_BestReachableArea(
                            li->origin, ic->iteminfo[li->iteminfo].mins,
        ...
        // get the item goal area and goal origin
        li->goalareanum =
            AAS_BestReachableArea(li->origin, ic->iteminfo[i].mins,
                                  ic->iteminfo[i].maxs, li->goalorigin);
...
int BotChooseLTGItem(int goalstate, vec3_t origin, int *inventory,
    ...
    // if the bot is in solid or if the area the bot is in has no reachability
    // links
    ...
    for (li = levelitems; li; li = li->next) {
        ...
        if (!li->goalareanum)
            continue;
        // FIXME: is this a good thing? added this for items that never spawned
        // into the game (f.i. CTF flags in obelisk)
        ...
        weight =
            FuzzyWeightUndecided(inventory, gs->itemweightconfig, weightnum);
#else
       ...
       // use weight scale for item_botroam
        ...
        if (weight > 0) {
            ...
            t = AAS_AreaTravelTimeToGoalArea(areanum, origin, li->goalareanum,
                                             travelflags);
    ...
    if (!bestitem) {
        ...
        {
                if (AAS_RandomGoalArea(areanum, travelflags, &goal.areanum,
goal.origin))
                {
                        ...
                        //
#ifdef DEBUG
                        botimport.Print(PRT_MESSAGE, "chosen roam goal area
%d\n", goal.areanum); #endif //DEBUG return qtrue; } //end if } //end if
        */
        return qfalse;
    } // end if
...
int BotChooseNBGItem(int goalstate, vec3_t origin, int *inventory,
                     int travelflags, bot_goal_t *ltg, float maxtime) {
    ...
    if (!areanum || !AAS_AreaReachability(areanum)) {
        ...
        botimport.Print(
            PRT_MESSAGE,
            "bot is in solid or area has no reachability links: %d %d\n",
            areanum, gs->lastreachabilityarea);
#endif
        // use the last valid area the bot was in
        if (gs->lastreachabilityarea > 0) {
    ...
    //
    if (ltg)
        ltg_time = AAS_AreaTravelTimeToGoalArea(areanum, origin, ltg->areanum,
                                                travelflags);
    else
        ltg_time = 99999;
    // the item configuration
        ...
        if (weight > 0) {
            ...
            if (t > 0 && t < maxtime) {
                ...
                if (weight > bestweight) {
                    t = 0;
                    if (ltg && !li->timeout) {
                        // get the travel time from the goal to the long term
                        // goal
                        t = AAS_AreaTravelTimeToGoalArea(
                            li->goalareanum, li->goalorigin, ltg->areanum,
                            travelflags);
                    } // end if
...
// Changes Globals:		-
//===========================================================================
int BotItemGoalInVisButNotVisible(int viewer, vec3_t eye, vec3_t viewangles,
                                  bot_goal_t *goal) {
    aas_entityinfo_t entinfo;
    bsp_trace_t trace;
    vec3_t middle;
