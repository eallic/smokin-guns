    ...
    do {
        ...
        if (def || !strcmp(token.string, "case")) {
            ...
            if (def) {
                ...
                if (!PC_ExpectTokenType(source, TT_NUMBER, TT_INTEGER,
                                        &token)) {
                    FreeFuzzySeperators_r(firstfs);
                    return NULL;
                } // end if
    ...
    if (!LibVarGetValue("bot_reloadcharacters")) {
        ...
        if (avail == -1) {
            botimport.Print(PRT_ERROR,
                            "weightFileList was full trying to load %s\n",
                            filename);
            return NULL;
        } // end if
    } // end if
    ...
    while (PC_ReadToken(source, &token)) {
        if (!strcmp(token.string, "weight")) {
            ...
            config->weights[config->numweights].name =
                (char *)GetClearedMemory(strlen(token.string) + 1);
            ...
            if (!strcmp(token.string, "switch")) {
                ...
                fs = (fuzzyseperator_t *)GetClearedMemory(
                    sizeof(fuzzyseperator_t));
                fs->index = 0;
                fs->value = MAX_INVENTORYVALUE;
                fs->next = NULL;
                fs->child = NULL;
...
float FuzzyWeight_r(int *inventory, fuzzyseperator_t *fs) {
    ...
    if (inventory[fs->index] < fs->value) {
        ...
        if (inventory[fs->index] < fs->next->value) {
            ...
            // the scale factor
            if (fs->next->value ==
                MAX_INVENTORYVALUE) // is fs->next the default case?
                return w2;          // can't interpolate, return default weight
            else
                scale = (float)(inventory[fs->index] - fs->value) /
                        (fs->next->value - fs->value);
        ...
        if (inventory[fs->index] < fs->next->value) {
            ...
            if (fs->next->child)
                ...
                w2 = fs->next->minweight +
                     random() * (fs->next->maxweight - fs->next->minweight);
            // the scale factor
            if (fs->next->value ==
                MAX_INVENTORYVALUE) // is fs->next the default case?
                return w2;          // can't interpolate, return default weight
...
float FuzzyWeightUndecided(int *inventory, weightconfig_t *wc, int weightnum) {
    ...
    return FuzzyWeightUndecided_r(inventory,
                                  wc->weights[weightnum].firstseperator);
#else
    fuzzyseperator_t *s;
...
void ScaleFuzzyBalanceRange(weightconfig_t *config, float scale) {
    ...
    if (scale < 0)
        ...
        scale = 100;
    for (i = 0; i < config->numweights; i++) {
        ScaleFuzzySeperatorBalanceRange_r(config->weights[i].firstseperator,
                                          scale);
    } // end for
...
int InterbreedFuzzySeperator_r(fuzzyseperator_t *fs1, fuzzyseperator_t *fs2,
    ...
    if (fs1->child) {
        if (!fs2->child || !fsout->child) {
            botimport.Print(
                PRT_ERROR, "cannot interbreed weight configs, unequal child\n");
            return qfalse;
        } // end if
        ...
        if (fs2->type != WT_BALANCE || fsout->type != WT_BALANCE) {
            botimport.Print(
                PRT_ERROR,
                "cannot interbreed weight configs, unequal balance\n");
            return qfalse;
        } // end if
    ...
    if (fs1->next) {
        if (!fs2->next || !fsout->next) {
            botimport.Print(PRT_ERROR,
                            "cannot interbreed weight configs, unequal next\n");
            return qfalse;
        } // end if
