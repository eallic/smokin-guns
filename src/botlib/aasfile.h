...
typedef struct aas_plane_s {
    ...
    float dist;    // distance of the plane (normal vector * distance = point in
                // plane)
    int type;
} aas_plane_t;
...
typedef struct aas_face_s {
    ...
    int faceflags; // face flags (no use to create face settings for just this
                   // field)
...
typedef struct aas_node_s {
    int planenum;
    int children[2]; // child nodes of this node, or areas as leaves when
                     // negative when a child is zero it's a solid leaf
} aas_node_t;
