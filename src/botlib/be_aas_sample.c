...
void AAS_InitAASLinkHeap(void) {
    ...
    if (!aasworld.linkheap) {
        ...
        aasworld.linkheap =
            (aas_link_t *)GetHunkMemory(max_aaslinks * sizeof(aas_link_t));
    } // end if
    ...
    aasworld.linkheap[max_aaslinks - 1].prev_ent =
        &aasworld.linkheap[max_aaslinks - 2];
...
int AAS_AreaPresenceType(int areanum) {
    ...
    if (areanum <= 0 || areanum >= aasworld.numareas) {
        botimport.Print(PRT_ERROR,
                        "AAS_AreaPresenceType: invalid area number\n");
        return 0;
    } // end if
...
// Changes Globals:		-
//===========================================================================
vec_t AAS_BoxOriginDistanceFromPlane(vec3_t normal, vec3_t mins, vec3_t maxs,
                                     int side) {
    vec3_t v1, v2;
    int i;
...
qboolean AAS_AreaEntityCollision(int areanum, vec3_t start, vec3_t end,
    ...
    bsptrace.fraction = 1;
    collision = qfalse;
    for (link = aasworld.arealinkedentities[areanum]; link;
         link = link->next_ent) {
    ...
    while (1) {
        ...
        if (nodenum < 0) {
            ...
            if (-nodenum > aasworld.numareasettings) {
                botimport.Print(
                    PRT_ERROR, "AAS_TraceBoundingBox: -nodenum out of range\n");
                return trace;
            } // end if
#endif // AAS_SAMPLE_DEBUG
       ...
       // got the right presence type
            if (!(aasworld.areasettings[-nodenum].presencetype &
                  presencetype)) {
                ...
                // always take the plane with normal facing towards the trace
                // start
                ...
                if (passent >= 0) {
                    if (AAS_AreaEntityCollision(-nodenum, tstack_p->start,
                                                tstack_p->end, presencetype,
                                                passent, &trace)) {
                        if (!trace.startsolid) {
                            VectorSubtract(end, start, v1);
                            VectorSubtract(trace.endpos, start, v2);
                            trace.fraction =
                                VectorLength(v2) / VectorLength(v1);
                        } // end if
                        return trace;
                    } // end if
                } // end if
       ...
       // the node to test against
        ...
        switch (
            plane->type) { /*FIXME: wtf doesn't this work? obviously the axial
                              node planes aren't always facing positive!!!
        ...
        // go down the tree both at the front and back of the node
            ...
            //plane->dist, plane->type, LINECOLOR_RED); side the front part of
            // the line is on
            side = front < 0;
            ...
            // now put the part near the start of the line on the stack so we
            // will continue with thats part first. This way we'll find the
            // first hit of the bbox
...
int AAS_TraceAreas(vec3_t start, vec3_t end, int *areas, vec3_t *points,
    ...
    while (1) {
        ...
        if (nodenum < 0) {
            ...
            if (-nodenum > aasworld.numareasettings) {
                botimport.Print(PRT_ERROR,
                                "AAS_TraceAreas: -nodenum = %d out of range\n",
                                -nodenum);
                return numareas;
            } // end if
#endif // AAS_SAMPLE_DEBUG
       ...
       // AAS_PointAreaNum(start));
        ...
        if (nodenum > aasworld.numnodes) {
            botimport.Print(PRT_ERROR,
                            "AAS_TraceAreas: nodenum out of range\n");
            return numareas;
        } // end if
#endif // AAS_SAMPLE_DEBUG
       // the node to test against
        ...
        switch (plane->type) { /*FIXME: wtf doesn't this work? obviously the
                                  node planes aren't always facing positive!!!
                                      //check for axial planes
                                      case PLANE_X:
                                      {
                                              front = cur_start[0] -
                                  plane->dist; back = cur_end[0] - plane->dist;
                                              break;
                                      } //end case
                                      case PLANE_Y:
                                      {
                                              front = cur_start[1] -
                                  plane->dist; back = cur_end[1] - plane->dist;
                                              break;
                                      } //end case
                                      case PLANE_Z:
                                      {
                                              front = cur_start[2] -
                                  plane->dist; back = cur_end[2] - plane->dist;
                                              break;
                                      } //end case*/
        default:               // gee it's not an axial plane
        {
...
aas_face_t *AAS_TraceEndFace(aas_trace_t *trace) {
    ...
    for (i = 0; i < area->numfaces; i++) {
        ...
        if ((face->planenum & ~1) == (trace->planenum & ~1)) {
            ...
            /*			if (firstface)
                                    {
                                            if (firstface->numedges <
               face->numedges)
                                            {
                                                    if
               (AAS_InsideFace(firstface,
                                                            aasworld.planes[face->planenum].normal,
               trace->endpos))
                                                    {
                                                            return firstface;
                                                    } //end if
            ...
            if (AAS_InsideFace(face, aasworld.planes[face->planenum].normal,
                               trace->endpos, 0.01f))
                return face;
        } // end if
    } // end for
...
#define AAS_BoxOnPlaneSide(absmins, absmaxs, p)                                \
    (((p)->type < 3)                                                           \
         ? (((p)->dist <= (absmins)[(p)->type])                                \
                ? (1)                                                          \
                : (((p)->dist >= (absmaxs)[(p)->type]) ? (2) : (3)))           \
         : (AAS_BoxOnPlaneSide2(                                               \
               (absmins), (absmaxs),                                           \
               (p)))) // end of the function AAS_BoxOnPlaneSide
//===========================================================================
...
aas_link_t *AAS_AASLinkEntity(vec3_t absmins, vec3_t absmaxs, int entnum) {
    ...
    while (1) {
        ...
        if (nodenum < 0) {
            ...
            for (link = aasworld.arealinkedentities[-nodenum]; link;
                 link = link->next_ent) {
