...
qboolean ReadNumber(source_t *source, fielddef_t *fd, void *p) {
    ...
    if (token.type == TT_PUNCTUATION) {
        if (fd->type & FT_UNSIGNED) {
            SourceError(source, "expected unsigned value, found %s",
                        token.string);
            return 0;
        } // end if
        // if not a minus sign
    ...
    if (token.subtype & TT_FLOAT) {
        ...
        if (fd->type & FT_BOUNDED) {
            if (floatval < fd->floatmin || floatval > fd->floatmax) {
                SourceError(source, "float out of range [%f, %f]", fd->floatmin,
                            fd->floatmax);
                return 0;
            } // end if
        } // end if
    ...
    if ((fd->type & FT_TYPE) == FT_CHAR || (fd->type & FT_TYPE) == FT_INT) {
        ...
        if (intval < intmin || intval > intmax) {
            SourceError(source, "value %ld out of range [%ld, %ld]", intval,
                        intmin, intmax);
            return 0;
        } // end if
    } // end if
        ...
        if (fd->type & FT_BOUNDED) {
            if (intval < fd->floatmin || intval > fd->floatmax) {
                SourceError(source, "value %ld out of range [%f, %f]", intval,
        ...
        while (num-- > 0) {
            ...
            if (fd->type & FT_ARRAY) {
                ...
                if (!strcmp(token.string, "}"))
                    break;
                if (strcmp(token.string, ",")) {
                    SourceError(source, "expected a comma, found %s",
                                token.string);
                    return qfalse;
                } // end if
            } // end if
...
// Changes Globals:		-
//===========================================================================
int WriteStructWithIndent(FILE *fp, structdef_t *def, char *structure,
                          int indent) {
    int i, num;
    void *p;
    fielddef_t *fd;
    ...
    for (i = 0; def->fields[i].name; i++) {
        ...
        while (num-- > 0) {
            ...
            case FT_STRUCT: {
                if (!WriteStructWithIndent(fp, fd->substruct, structure,
                                           indent))
                    return qfalse;
                p = (char *)p + fd->substruct->size;
                break;
