...
#define TFL_DEFAULT                                                            \
    TFL_WALK | TFL_CROUCH | TFL_BARRIERJUMP | TFL_JUMP | TFL_LADDER |          \
        TFL_WALKOFFLEDGE | TFL_SWIM | TFL_WATERJUMP | TFL_TELEPORT |           \
...
typedef struct bsp_trace_s
{
        ...
        qboolean		startsolid;	// if true, the initial point
was in a solid area float			fraction;	// time
completed, 1.0 = didn't hit anything vec3_t			endpos;
// final position cplane_t		plane;		// surface normal at
impact float			exp_dist;	// expanded plane distance int
sidenum;	// number of the brush side hit bsp_surface_t	surface;
// hit surface int				contents;	// contents on
other side of surface hit int				ent;		//
number of entity hit } bsp_trace_t;
//
*/
...
#define RSE_USETRAVELTYPE                                                      \
    2 // stop as soon as on of the given travel types is used
