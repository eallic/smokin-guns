...
int AAS_AlternativeRouteGoals(vec3_t start, int startareanum, vec3_t goal,
    ...
    goaltraveltime = AAS_AreaTravelTimeToGoalArea(startareanum, start,
    ...
    for (i = 1; i < aasworld.numareas; i++) {
        ...
        starttime =
            AAS_AreaTravelTimeToGoalArea(startareanum, start, i, travelflags);
        if (!starttime)
            continue;
        // if the travel time from the start to the area is greater than the
        // shortest goal travel time
        if (starttime > (float)1.1 * goaltraveltime)
            continue;
        // travel time from the area to the goal area
        goaltime =
            AAS_AreaTravelTimeToGoalArea(i, NULL, goalareanum, travelflags);
        if (!goaltime)
            continue;
        // if the travel time from the area to the goal is greater than the
        // shortest goal travel time
    ...
    for (i = 1; i < aasworld.numareas; i++) {
        ...
        // now we've got a cluster with areas through which an alternative route
        // could go get the 'center' of the cluster
        VectorClear(mid);
...
void AAS_InitAlternativeRouting(void) {
    ...
    midrangeareas =
        (midrangearea_t *)GetMemory(aasworld.numareas * sizeof(midrangearea_t));
    if (clusterareas)
