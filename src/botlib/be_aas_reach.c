...
// Changes Globals:		-
//===========================================================================
int AAS_GetJumpPadInfo(int ent, vec3_t areastart, vec3_t absmins,
                       vec3_t absmaxs, vec3_t velocity) {
    int modelnum, ent2;
    ...
    for (ent2 = AAS_NextBSPEntity(0); ent2; ent2 = AAS_NextBSPEntity(ent2)) {
        if (!AAS_ValueForBSPEpairKey(ent2, "targetname", targetname,
                                     MAX_EPAIRKEY))
            continue;
...
int AAS_BestReachableFromJumpPadArea(vec3_t origin, vec3_t mins, vec3_t maxs) {
    ...
    for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
        ...
        //
        // botimport.Print(PRT_MESSAGE, "found a trigger_push with velocity %f
        // %f %f\n", velocity[0], velocity[1], velocity[2]);
        //
        VectorSet(cmdmove, 0, 0, 0);
        Com_Memset(&move, 0, sizeof(aas_clientmove_t));
        AAS_ClientMovementHitBBox(&move, -1, areastart, PRESENCE_NORMAL, qfalse,
                                  velocity, cmdmove, 0, 30, 0.1f, bboxmins,
                                  bboxmaxs, bot_visualizejumppads);
        if (move.frames < 30) {
            bestareanum = 0;
            bestareavolume = 0;
            for (link = areas; link; link = link->next_area) {
...
// Changes Globals:		-
//===========================================================================
int AAS_BestReachableArea(vec3_t origin, vec3_t mins, vec3_t maxs,
                          vec3_t goalorigin) {
    int areanum, i, j, k, l;
    aas_link_t *areas;
    ...
    if (areanum) {
        ...
        if (!trace.startsolid) {
            ...
            // it can very well happen that the AAS_PointAreaNum function tells
            // that a point is in an area and that starting an
            // AAS_TraceClientBBox from that point will return trace.startsolid
            // qtrue
#if 0
...
int AAS_AreaReachability(int areanum) {
    if (areanum < 0 || areanum >= aasworld.numareas) {
        // Joe Kari: added #ifdef DEBUG , so it will stop bugging users with
        // such a recurring and flooding error.
#ifdef DEBUG
...
float AAS_MaxJumpHeight(float phys_jumpvel) {
    ...
    return 0.5 * phys_gravity * (phys_jumpvel / phys_gravity) *
...
int AAS_AreaClusterPortal(int areanum) {
    return (aasworld.areasettings[areanum].contents &
        ...
        for (j = 0; j < area2->numfaces; j++) {
            ...
            if (face1num == face2num) {
                ...
                if (AAS_PointContents(start) &
                    ...
                    VectorMA(lreach->start, -INSIDEUNITS, plane->normal,
                             lreach->end);
                    ...
                    // if (!(AAS_PointContents(start) & MASK_WATER))
                    // lreach->traveltime += 500; link the reachability
...
int AAS_Reachability_EqualFloorHeight(int area1num, int area2num) {
    ...
    for (i = 0; i < area1->numfaces; i++) {
        ...
        for (j = 0; j < area2->numfaces; j++) {
            face2 =
                &aasworld.faces[abs(aasworld.faceindex[area2->firstface + j])];
            ...
            for (edgenum1 = 0; edgenum1 < face1->numedges; edgenum1++) {
                for (edgenum2 = 0; edgenum2 < face2->numedges; edgenum2++) {
                    ...
                    // NOTE: if there's nearby solid or a gap area after this
                    // area disabled this crap if (AAS_NearbySolidOrGap(start,
                    // end)) height += 200; NOTE: disabled because it disables
                    // reachabilities to very small areas if
                    // (AAS_PointAreaNum(end) != area2num) continue; get the
                    // longest lowest edge
    ...
    if (foundreach) {
        ...
        */
        ...
        // if (AAS_AreaGroundFaceArea(lreach->areanum) < 500) lreach->traveltime
        // += 100;
        //
        reach_equalfloor++;
        return qtrue;
...
int AAS_Reachability_Step_Barrier_WaterJump_WalkOffLedge(int area1num,
    ...
    float length, ground_bestlength, water_bestlength, ground_bestdist,
        water_bestdist;
    vec3_t v1, v2, v3, v4, tmpv, p1area1, p1area2, p2area1, p2area2;
    vec3_t normal, ort, edgevec, start, end, dir;
    vec3_t ground_beststart = {0, 0, 0}, ground_bestend = {0, 0, 0},
           ground_bestnormal = {0, 0, 0};
    vec3_t water_beststart = {0, 0, 0}, water_bestend = {0, 0, 0},
           water_bestnormal = {0, 0, 0};
    ...
    for (i = 0; i < area1->numfaces; i++) {
        ...
        for (k = 0; k < groundface1->numedges; k++) {
            ...
            for (j = 0; j < area2->numfaces; j++) {
                groundface2 =
                    &aasworld
                         .faces[abs(aasworld.faceindex[area2->firstface + j])];
                ...
                for (l = 0; l < groundface2->numedges; l++) {
                    edge2num =
                        abs(aasworld.edgeindex[groundface2->firstedge + l]);
                    edge2 = &aasworld.edges[edge2num];
                    // vertexes of the edge
                    VectorCopy(aasworld.vertexes[edge2->v[0]], v3);
                    VectorCopy(aasworld.vertexes[edge2->v[1]], v4);
                    // check the distance between the two points and the
                    // vertical plane through the edge of area1
                    ...
                    // NOTE: since gravity is vertical this is just the z
                    // coordinate
                    ...
                    // get the length of the overlapping part of the edges of
                    // the two areas
    ...
    if (ground_foundreach) {
        ...
        if (ground_bestdist >= 0 &&
            ...
            VectorMA(ground_beststart, INSIDEUNITS_WALKSTART, ground_bestnormal,
                     lreach->start);
            VectorMA(ground_bestend, INSIDEUNITS_WALKEND, ground_bestnormal,
                     lreach->end);
    ...
    //~~~~~~~~|
    //        |
    //        |          higher than step height and water up to waterjump
    //        height -> TRAVEL_WATERJUMP
    //--------|
    //
    //~~~~~~~~~~~~~~~~~~
    //        ---------
    ...
    if (water_foundreach) {
        ...
        if (aasworld.areasettings[AAS_PointAreaNum(testpoint)].areaflags &
            AREA_LIQUID) {
            // don't create rediculous water jump reachabilities from areas very
            // far below the water surface
            if (water_bestdist < aassettings.phys_maxwaterjump + 24) {
                // waterjumping from or towards a crouch only area is not
                // possible in Quake2
                if ((aasworld.areasettings[area1num].presencetype &
                     PRESENCE_NORMAL) &&
                    (aasworld.areasettings[area2num].presencetype &
                     PRESENCE_NORMAL)) {
                    ...
                    VectorMA(water_bestend, INSIDEUNITS_WATERJUMP,
                             water_bestnormal, lreach->end);
            ...
            if (!water_foundreach || (ground_bestdist - water_bestdist < 16)) {
                ...
                if (!AAS_AreaCrouch(area1num) && !AAS_AreaCrouch(area2num)) {
                    ...
                    VectorMA(ground_beststart, INSIDEUNITS_WALKSTART,
                             ground_bestnormal, lreach->start);
                    VectorMA(ground_bestend, INSIDEUNITS_WALKEND,
                             ground_bestnormal, lreach->end);
                    lreach->traveltype = TRAVEL_BARRIERJUMP;
                    lreach->traveltime =
                        aassettings
                            .rs_barrierjump; // AAS_BarrierJumpTravelTime();
                    lreach->next = areareachability[area1num];
                    areareachability[area1num] = lreach;
    ...
    if (ground_foundreach) {
        if (ground_bestdist < 0) {
            ...
            if (!aassettings.rs_maxfallheight ||
                fabs(ground_bestdist) < aassettings.rs_maxfallheight) {
                // trace a bounding box vertically to check for solids
                VectorMA(ground_bestend, INSIDEUNITS, ground_bestnormal,
                         ground_bestend);
                ...
                if (!trace.startsolid && trace.fraction >= 1.0) {
                    ...
                    if (AAS_PointAreaNum(trace.endpos) == area2num) {
                        ...
                        numareas = AAS_TraceAreas(start, end, areas, NULL,
                                                  ARRAY_LEN(areas));
                        for (i = 0; i < numareas; i++)
                            if (AAS_AreaClusterPortal(areas[i]))
                                break;
                        if (i >= numareas) {
                            // create a walk off ledge reachability from area1
                            // to area2
                            lreach = AAS_AllocReachability();
                            if (!lreach)
                                return qfalse;
                            ...
                            lreach->traveltime =
                                ...
                                fabs(ground_bestdist) * 50 /
                                    aassettings.phys_gravity;
                            // if falling from too high and not falling into
                            // water
                            if (!AAS_AreaSwim(area2num) &&
                            ...
                            //
                            reach_walkoffledge++;
                            // NOTE: don't create a weapon (rl, bfg) jump
                            // reachability here because it interferes with
                            // other reachabilities like the ladder reachability
                            return qtrue;
                        } // end if
                    } // end if
                } // end if
...
float AAS_ClosestEdgePoints(vec3_t v1, vec3_t v2, vec3_t v3, vec3_t v4,
                                                        aas_plane_t *plane1,
aas_plane_t *plane2, vec3_t beststart, vec3_t bestend, float bestdist)
{
        ...
        if (dir2[0])
        {
                ...
                p1[0] = (DotProduct(v1, dir2) - (a2 * dir2[0] + b2 * dir2[1])) /
dir2[0]; p1[1] = a2 * p1[0] + b2;
                //point on the edge vector of area2 closest to v2
                p2[0] = (DotProduct(v2, dir2) - (a2 * dir2[0] + b2 * dir2[1])) /
dir2[0]; p2[1] = a2 * p2[0] + b2; } //end if else
        {
                //point on the edge vector of area2 closest to v1
                p1[0] = v3[0];
                p1[1] = v1[1];
        ...
        if (dir1[0])
        {
                ...
                p3[0] = (DotProduct(v3, dir1) - (a1 * dir1[0] + b1 * dir1[1])) /
dir1[0]; p3[1] = a1 * p3[0] + b1;
                //point on the edge vector of area1 closest to v4
                p4[0] = (DotProduct(v4, dir1) - (a1 * dir1[0] + b1 * dir1[1])) /
dir1[0]; p4[1] = a1 * p4[0] + b1; } //end if else
        {
                //point on the edge vector of area1 closest to v3
                p3[0] = v1[0];
                p3[1] = v3[1];
...
plane2->normal[2]; p2[2] = (plane2->dist - DotProduct(plane2->normal, p2)) /
plane2->normal[2]; p3[2] = (plane1->dist - DotProduct(plane1->normal, p3)) /
plane1->normal[2]; p4[2] = (plane1->dist - DotProduct(plane1->normal, p4)) /
plane1->normal[2];
        //
        founddist = qfalse;
        //
...
float AAS_ClosestEdgePoints(vec3_t v1, vec3_t v2, vec3_t v3, vec3_t v4,
                            ...
                            vec3_t beststart2, vec3_t bestend2,
                            float bestdist) {
    vec3_t dir1, dir2, p1, p2, p3, p4;
    ...
    if (dir2[0]) {
        ...
        // point on the edge vector of area2 closest to v1
        p1[0] =
            (DotProduct(v1, dir2) - (a2 * dir2[0] + b2 * dir2[1])) / dir2[0];
        p1[1] = a2 * p1[0] + b2;
        // point on the edge vector of area2 closest to v2
        p2[0] =
            (DotProduct(v2, dir2) - (a2 * dir2[0] + b2 * dir2[1])) / dir2[0];
        p2[1] = a2 * p2[0] + b2;
    } // end if
    else {
        // point on the edge vector of area2 closest to v1
    ...
    if (dir1[0]) {
        ...
        // point on the edge vector of area1 closest to v3
        p3[0] =
            (DotProduct(v3, dir1) - (a1 * dir1[0] + b1 * dir1[1])) / dir1[0];
        p3[1] = a1 * p3[0] + b1;
        // point on the edge vector of area1 closest to v4
        p4[0] =
            (DotProduct(v4, dir1) - (a1 * dir1[0] + b1 * dir1[1])) / dir1[0];
        p4[1] = a1 * p4[0] + b1;
    } // end if
    else {
        // point on the edge vector of area1 closest to v3
...
int AAS_Reachability_Jump(int area1num, int area2num) {
            ...
            for (k = 0; k < face1->numedges; k++) {
                ...
                for (l = 0; l < face2->numedges; l++) {
                    ...
                    bestdist = AAS_ClosestEdgePoints(
                        v1, v2, v3, v4, plane1, plane2, beststart, bestend,
    ...
    if (bestdist > 4 && bestdist < maxjumpdistance) {
        //		Log_Write("shortest distance between %d and %d is
        ...
        if (bestdist <= 48 && fabs(beststart[2] - bestend[2]) < 8) {
            ...
            // get the horizontal speed for the jump, if it isn't possible to
            // calculate this speed (the jump is not possible) then there's no
            // jump reachability created
            if (!AAS_HorizontalVelocityForJump(phys_jumpvel, beststart, bestend,
                                               &speed))
                return qfalse;
            speed *= 1.05f;
        ...
        if (!AAS_AreaClusterPortal(area1num) &&
            !AAS_AreaClusterPortal(area2num))
        ...
        for (i = 0; i < 3; i++) {
            ...
            //
            AAS_PredictClientMovement(&move, -1, beststart, PRESENCE_NORMAL,
                                      qtrue, velocity, cmdmove, 3, 30, 0.1f,
                                      stopevent, 0, qfalse);
            ...
            teststart[2] += 1;
            numareas = AAS_TraceAreas(move.endpos, teststart, areas, NULL,
                                      ARRAY_LEN(areas));
       ...
       // create a new reachability link
        ...
        if ((traveltype & TRAVELTYPE_MASK) == TRAVEL_WALKOFFLEDGE &&
            ...
            lreach->traveltime = aassettings.rs_startwalkoffledge +
                                 height * 50 / aassettings.phys_gravity;
        } else {
            lreach->traveltime = aassettings.rs_startjump +
                                 VectorDistance(bestend, beststart) * 240 /
       ...
       //
                                                                                            ...
                                                                                            | AREACONTENTS_LAVA))
            {
                    for (i = 20; i <= 120; i += 20)
                    {
                            ...
                            //trace without entity collision
                            trace = AAS_TraceClientBBox(start, end,
            PRESENCE_NORMAL, -1);
                            //
                            ...
                            //
                            if (start[2] - trace.endpos[2] > maxjumpheight)
            continue; if (aasworld.areasettings[area2num].contents &
            (AREACONTENTS_SLIME | AREACONTENTS_LAVA)) continue;
                            //
                            //create a new reachability link
                            lreach = AAS_AllocReachability();
                            ...
                            reach_jump++;
                            //
                            Log_Write("jump far to ladder reach between %d and
            %d\r\n", area2num, area1num);
                            //
                            break;
                    } //end for
...
void AAS_Reachability_Teleport(void) {
    ...
    for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
        ...
        if (!strcmp(classname, "trigger_multiple")) {
            ...
            // #endif REACH_DEBUG
            VectorClear(angles);
            AAS_BSPModelMinsMaxsOrigin(atoi(model + 1), angles, mins, maxs,
                                       origin);
            //
            ...
            for (dest = AAS_NextBSPEntity(0); dest;
                 dest = AAS_NextBSPEntity(dest)) {
                if (!AAS_ValueForBSPEpairKey(dest, "classname", classname,
                                             MAX_EPAIRKEY))
                    continue;
            ...
            if (!AAS_ValueForBSPEpairKey(dest, "target", target,
                                         MAX_EPAIRKEY)) {
        ...
        if (!AAS_AreaTeleporter(area2num) && !AAS_AreaJumpPad(area2num)) {
            ...
            else*/
            {
                ...
                AAS_PredictClientMovement(
                    &move, -1, destorigin, PRESENCE_NORMAL, qfalse, velocity,
                    cmdmove, 0, 30, 0.1f,
                    SE_HITGROUND | SE_ENTERWATER | SE_ENTERSLIME |
                        SE_ENTERLAVA | SE_HITGROUNDDAMAGE | SE_TOUCHJUMPPAD |
                        SE_TOUCHTELEPORTER,
                    0, qfalse); // qtrue);
        ...
        // origin[0], origin[1], origin[2]); botimport.Print(PRT_MESSAGE,
        // "teleporter brush mins = %f %f %f\n", mins[0], mins[1], mins[2]);
...
void AAS_Reachability_Elevator(void) {
    ...
    for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
        ...
        if (!strcmp(classname, "func_plat")) {
            ...
            //
            // botimport.Print(PRT_MESSAGE, "platbottom[2] = %1.1f plattop[2] =
            ...
            for (i = 0; i < 9; i++) {
                if (i < 8) // check at the sides of the plat
                {
                    ...
                    // get a grounded or swim area near the plat in the bottom
                    // position
                    area1num = AAS_PointAreaNum(bottomorg);
                    for (k = 0; k < 16; k++) {
                        if (area1num) {
                            if (AAS_AreaGrounded(area1num) ||
                ...
                for (n = 0; n < 3; n++) {
                    ...
                    for (j = 0; j < 8; j++) {
                        ...
                        // get a grounded or swim area near the plat in the top
                        // position
                        ...
                        if (!AAS_AreaGrounded(area2num))
                            continue;
                        // if there already exists reachability between the
                        // areas
                        if (AAS_ReachabilityExists(area1num, area2num))
                            continue;
                        // if the reachability start is within the elevator
                        // bounding box
                        ...
                        //
                        for (p = 0; p < 3; p++)
                            if (dir[p] < origin[p] + mins[p] ||
                                dir[p] > origin[p] + maxs[p])
                                break;
                        if (p >= 3)
                            continue;
                        // create a new reachability link
                        ...
                        lreach->traveltype |= AAS_TravelFlagsForTeam(ent);
                        lreach->traveltime =
                            aassettings.rs_startelevator + height * 100 / speed;
                        lreach->next = areareachability[area1num];
                        areareachability[area1num] = lreach;
    ...
    for (i = 1; i < aasworld.numareas; i++) {
            ...
            for (k = 0; k < face->numedges; k++) {
                ...
                for (l = 0; l < numpoints; l++) {
                    ...
                    dist = AAS_ClosestEdgePoints(
                        v1, v2, v3, v4, faceplane, plane, beststart, bestend,
                        beststart2, bestend2, bestdist);
                    if (dist < bestdist) {
                        bestfacenum = facenum;
        ...
        if (bestfaceplane != NULL)
            testpoint[2] = (bestfaceplane->dist -
                            DotProduct(bestfaceplane->normal, testpoint)) /
...
void AAS_Reachability_FuncBobbing(void) {
    ...
    aas_lreachability_t *startreach, *endreach, *nextstartreach, *nextendreach,
        *lreach;
    ...
    for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
        ...
        //
        Log_Write("funcbob model %d, start = {%1.1f, %1.1f, %1.1f} end = "
                  "{%1.1f, %1.1f, %1.1f}\n",
                  modelnum, move_start[0], move_start[1], move_start[2],
                  move_end[0], move_end[1], move_end[2]);
        //
#ifndef BSPC
        /*
        ...
        for (i = 0; i < 2; i++) {
            ...
            if (i == 0) {
                firststartreach = AAS_FindFaceReachabilities(
                    start_edgeverts, 4, &start_plane, qtrue);
                firstendreach = AAS_FindFaceReachabilities(end_edgeverts, 4,
                                                           &end_plane, qfalse);
            } // end if
            else {
                firststartreach = AAS_FindFaceReachabilities(end_edgeverts, 4,
                                                             &end_plane, qtrue);
                firstendreach = AAS_FindFaceReachabilities(
                    start_edgeverts, 4, &start_plane, qfalse);
            } // end else
            //
            // create reachabilities from start to end
            for (startreach = firststartreach; startreach;
                 startreach = nextstartreach) {
                nextstartreach = startreach->next;
                //
                // trace = AAS_TraceClientBBox(startreach->start,
                // move_start_top, PRESENCE_NORMAL, -1); if (trace.fraction < 1)
                // continue;
                //
                for (endreach = firstendreach; endreach;
                     endreach = nextendreach) {
                    nextendreach = endreach->next;
                    //
                    // trace = AAS_TraceClientBBox(endreach->end, move_end_top,
                    // PRESENCE_NORMAL, -1); if (trace.fraction < 1) continue;
                    //
                    ...
                    if (i == 0)
                        lreach->edgenum = ((int)move_start[axis] << 16) |
                                          ((int)move_end[axis] & 0x0000ffff);
                    else
                        lreach->edgenum = ((int)move_end[axis] << 16) |
                                          ((int)move_start[axis] & 0x0000ffff);
...
absmins[1], absmins[2]); botimport.Print(PRT_MESSAGE, "absmaxs = %f %f %f\n",
absmaxs[0], absmaxs[1], absmaxs[2]); #endif REACH_DEBUG VectorAdd(absmins,
absmaxs, origin); VectorScale (origin, 0.5, origin);
        ...
        {
                if (!AAS_ValueForBSPEpairKey(ent2, "targetname", targetname,
MAX_EPAIRKEY)) continue; if (!strcmp(targetname, target)) break; } //end for if
(!ent2)
        {
                botimport.Print(PRT_MESSAGE, "trigger_push without target entity
%s\n", target); continue; } //end if AAS_VectorForBSPEpairKey(ent2, "origin",
ent2origin);
        //
        height = ent2origin[2] - origin[2];
...
*/
        ...
        botimport.Print(PRT_MESSAGE,
                        ...
                        velocity[0], velocity[1], velocity[2]);
        // if there is a horizontal velocity check for a reachability without
        // air control
            ...
            if (area2num && i < 20) {
                for (link = areas; link; link = link->next_area) {
                    ...
                    if (AAS_ReachabilityExists(link->areanum, area2num))
                        continue;
                    // create a rocket or bfg jump reachability from area1 to
                    // area2
                    lreach = AAS_AllocReachability();
                    if (!lreach) {
                        AAS_UnlinkFromAreas(areas);
                    ...
                    lreach->edgenum = sqrt(velocity[0] * velocity[0] +
        ...
        for (area2num = 1; area2num < aasworld.numareas; area2num++) {
            ...
            for (i = 0; i < area2->numfaces; i++) {
                ...
                zvel = velocity[2];
                // get the horizontal speed for the jump, if it isn't possible
                // to calculate this speed
                ret = AAS_HorizontalVelocityForJump(zvel, areastart, facecenter,
                                                    &speed);
                if (ret && speed < 150) {
                    ...
                    // if (hordist < 1.6 * facecenter[2] - areastart[2])
                    {
                        ...
                        //
                        AAS_PredictClientMovement(
                            &move, -1, areastart, PRESENCE_NORMAL, qfalse,
                            velocity, cmdmove, 30, 30, 0.1f,
                            SE_ENTERWATER | SE_ENTERSLIME | SE_ENTERLAVA |
                                SE_HITGROUNDDAMAGE | SE_TOUCHJUMPPAD |
                                SE_TOUCHTELEPORTER | SE_HITGROUNDAREA,
                            area2num, visualize);
                        // if prediction time wasn't enough to fully predict the
                        // movement don't enter slime or lava and don't fall
                        // from too high
                        if (move.frames < 30 &&
                            !(move.stopevent & (SE_ENTERSLIME | SE_ENTERLAVA |
                                                SE_HITGROUNDDAMAGE)) &&
                            (move.stopevent &
                             (SE_HITGROUNDAREA | SE_TOUCHJUMPPAD |
                              SE_TOUCHTELEPORTER))) {
                            ...
                            if (!link) {
                                for (link = areas; link;
                                    ...
                                    if (AAS_ReachabilityExists(link->areanum,
                                                               area2num))
                                        continue;
                                    // create a jumppad reachability from area1
                                    // to area2
                                    lreach = AAS_AllocReachability();
                                    if (!lreach) {
                                        AAS_UnlinkFromAreas(areas);
                                    ...
                                    // NOTE: the facenum is the Z velocity
                                    lreach->facenum = velocity[2];
                                    // NOTE: the edgenum is the horizontal
                                    // velocity
                                    lreach->edgenum =
                                        sqrt(cmdmove[0] * cmdmove[0] +
                                             cmdmove[1] * cmdmove[1]);
                                    VectorCopy(areastart, lreach->start);
...
int AAS_Reachability_Grapple(int area1num, int area2num) {
    ...
    aas_area_t *area1, *area2;
    aas_lreachability_t *lreach;
    vec3_t areastart = {0, 0, 0}, facecenter, start, end, dir,
           down = {0, 0, -1};
    vec_t *v;
    ...
    for (i = 0; i < area2->numfaces; i++) {
        ...
        v = aasworld
                .vertexes[aasworld
                              .edges[abs(aasworld.edgeindex[face2->firstedge])]
                              .v[0]];
        ...
        VectorMA(facecenter, -500, aasworld.planes[face2->planenum].normal,
                 end);
        //
        bsptrace = AAS_Trace(start, NULL, NULL, end, 0, CONTENTS_SOLID);
        // the grapple won't stick to the sky and the grapple point should be
        // near the AAS wall
        if ((bsptrace.surface.flags & SURF_SKY) ||
            (bsptrace.fraction * 500 > 32))
            continue;
        ...
        if (numareas >= 20)
            continue;
        for (j = 0; j < numareas; j++) {
            if (aasworld.areasettings[areas[j]].contents &
                AREACONTENTS_CLUSTERPORTAL)
                break;
        } // end for
        if (j < numareas)
        ...
        lreach->traveltime =
            aassettings.rs_startgrapple + VectorLength(dir) * 0.25;
        lreach->next = areareachability[area1num];
        areareachability[area1num] = lreach;
        //
        reach_grapple++;
            ...
            if (AAS_VectorForBSPEpairKey(ent, "origin", origin)) {
                ...
                if (!(spawnflags & 1)) {
                    if (!AAS_DropToFloor(origin, mins, maxs)) {
                        botimport.Print(
                            PRT_MESSAGE, "%s in solid at (%1.1f %1.1f %1.1f)\n",
            ...
            if (ret && speed < 300) {
                ...
                // if (hordist < 1.6 * (facecenter[2] - areastart[2]))
                {
                    ...
                    */
                    //
                    AAS_PredictClientMovement(
                        &move, -1, areastart, PRESENCE_NORMAL, qtrue, velocity,
                        cmdmove, 30, 30, 0.1f,
                        SE_ENTERWATER | SE_ENTERSLIME | SE_ENTERLAVA |
                            SE_HITGROUNDDAMAGE | SE_TOUCHJUMPPAD |
                        ...
                        area2num, visualize);
                    // if prediction time wasn't enough to fully predict the
                    // movement don't enter slime or lava and don't fall from
                    // too high
                    if (move.frames < 30 &&
                        !(move.stopevent & (SE_ENTERSLIME | SE_ENTERLAVA |
                                            SE_HITGROUNDDAMAGE)) &&
                        (move.stopevent &
                         (SE_HITGROUNDAREA | SE_TOUCHJUMPPAD))) {
                        // create a rocket or bfg jump reachability from area1
                        // to area2
                        lreach = AAS_AllocReachability();
                        if (!lreach)
                            return qfalse;
            ...
            for (j = 0; j < area->numfaces; j++) {
                ...
                for (l = 0; l < face2->numedges; l++) {
                    ...
                    if (abs(edge1num) == abs(edge2num)) {
                        ...
                        if (aasworld.areasettings[otherareanum].areaflags &
                            AREA_GROUNDED) {
                            // check for a possible gap
                            gap = qfalse;
                            for (n = 0; n < area2->numfaces; n++) {
                                face3num =
                                    aasworld.faceindex[area2->firstface + n];
                                ...
                                for (m = 0; m < face3->numedges; m++) {
                                    edge3num =
                                        aasworld
                                            .edgeindex[face3->firstedge + m];
                                    // but the edge should be shared by all
                                    // three faces
                                    if (abs(edge3num) == abs(edge1num)) {
                                        ...
                                        // FIXME: there are more situations to
                                        // be handled
                                        gap = qtrue;
                                        break;
                                    } // end if
                        ...
                        testend[2] -= 1000;
                        trace = AAS_TraceClientBBox(mid, testend,
                                                    PRESENCE_CROUCH, -1);
                        //
                        if (trace.startsolid) {
                        ...
                        if (AAS_ReachabilityExists(areanum, reachareanum)) {
                            // Log_Write("area %d: reachability already
                            // exists\r\n", areanum);
                            break;
                        } // end if
                        if (!AAS_AreaGrounded(reachareanum) &&
                            !AAS_AreaSwim(reachareanum)) {
                            // Log_Write("area %d, reach area %d: not grounded
                        ...
                        if (aasworld.areasettings[reachareanum].contents &
                            (AREACONTENTS_SLIME | AREACONTENTS_LAVA)) {
                            // Log_Write("area %d, reach area %d: lava or
                            // slime\r\n", areanum, reachareanum);
                            break;
                        } // end if
                        // if not going through a cluster portal
                        numareas = AAS_TraceAreas(mid, testend, areas, NULL,
                                                  ARRAY_LEN(areas));
                        ...
                        if (p < numareas)
                            break;
                        // if a maximum fall height is set and the bot would
                        // fall down further
                        if (aassettings.rs_maxfallheight &&
                            fabs(mid[2] - trace.endpos[2]) >
...
void AAS_StoreReachability(void) {
    ...
    aasworld.reachability = (aas_reachability_t *)GetClearedMemory(
        (numlreachabilities + 10) * sizeof(aas_reachability_t));
...
//===========================================================================
//
// TRAVEL_WALK					100%	equal floor height +
// steps TRAVEL_CROUCH				100% TRAVEL_BARRIERJUMP
// 100% TRAVEL_JUMP					 80% TRAVEL_LADDER
// 100%	+ fall down from ladder + jump up to ladder TRAVEL_WALKOFFLEDGE
// 90%	walk off very steep walls? TRAVEL_SWIM 100% TRAVEL_WATERJUMP
// 100% TRAVEL_TELEPORT				100% TRAVEL_ELEVATOR
// 100% TRAVEL_GRAPPLEHOOK			100% TRAVEL_DOUBLEJUMP
// 0% TRAVEL_RAMPJUMP				  0% TRAVEL_STRAFEJUMP
// 0% TRAVEL_ROCKETJUMP			100%	(currently limited towards areas
// with items) TRAVEL_BFGJUMP				  0%	(currently
// disabled) TRAVEL_JUMPPAD				100% TRAVEL_FUNCBOB
// 100%
//
// Parameter:			-
...
int AAS_ContinueInitReachability(float time) {
    ...
    // loop over the areas
    for (i = aasworld.numreachabilityareas; i < aasworld.numareas && i < todo;
         i++) {
        ...
        for (j = 1; j < aasworld.numareas; j++) {
            if (i == j)
                continue;
            // never create reachabilities from teleporter or jumppad areas to
            // regular areas
            if (aasworld.areasettings[i].contents &
                (AREACONTENTS_TELEPORTER | AREACONTENTS_JUMPPAD)) {
                if (!(aasworld.areasettings[j].contents &
                      (AREACONTENTS_TELEPORTER | AREACONTENTS_JUMPPAD))) {
                    continue;
                } // end if
            } // end if
            ...
            if (AAS_Reachability_EqualFloorHeight(i, j))
                continue;
            // check for step, barrier, waterjump and walk off ledge
            // reachabilities
        ...
        if (Sys_MilliSeconds() - start_time > (int)reachability_delay)
            break;
        //
        if (aasworld.numreachabilityareas * 1000 / aasworld.numareas >
            lastpercentage)
            break;
    } // end for
    //
    ...
    // if this is the last step in the reachability calculations
        ...
        lastpercentage =
            aasworld.numreachabilityareas * 1000 / aasworld.numareas;
