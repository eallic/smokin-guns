...
float AAS_WeaponJumpZVelocity(vec3_t origin, float radiusdamage) {
    ...
    start[2] += forward[2] * rocketoffset[0] + right[2] * rocketoffset[1] +
                rocketoffset[2];
...
// Changes Globals:		-
//===========================================================================
void AAS_Accelerate(vec3_t velocity, float frametime, vec3_t wishdir,
...
// Changes Globals:		-
//===========================================================================
int AAS_ClipToBBox(aas_trace_t *trace, vec3_t start, vec3_t end,
...
int AAS_ClientMovementPrediction(struct aas_clientmove_s *move, int entnum,
                                 ...
                                 vec3_t velocity, vec3_t cmdmove, int cmdframes,
                                 int maxframes, float frametime, int stopevent,
                                 int stopareanum, vec3_t mins, vec3_t maxs,
                                 int visualize) {
    ...
    for (n = 0; n < maxframes; n++) {
        ...
        if (onground || swimming) {
            ...
            AAS_ApplyFriction(frame_test_vel, friction, phys_stopspeed,
                              frametime);
        ...
        if (n < cmdframes) {
            ...
            if (onground) {
                ...
                if (!swimming && cmdmove[2] > 1) {
                    // jump velocity minus the gravity for one frame + 5 for
                    // safety
                    frame_test_vel[2] =
                        phys_jumpvel - (gravity * 0.1 * frametime) + 5;
                    jump_frame = n;
            ...
            if (wishspeed > maxvel)
                wishspeed = maxvel;
            VectorScale(frame_test_vel, 1 / frametime, frame_test_vel);
            AAS_Accelerate(frame_test_vel, frametime, wishdir, wishspeed,
                           accelerate);
            VectorScale(frame_test_vel, frametime, frame_test_vel);
            /*
            for (i = 0; i < ax; i++)
            {
                    ...
                    if (velchange > phys_maxacceleration) velchange =
            phys_maxacceleration; else if (velchange < -phys_maxacceleration)
            velchange = -phys_maxacceleration; newvel = frame_test_vel[i] +
            velchange;
                    //
                    if (frame_test_vel[i] <= maxvel && newvel > maxvel)
            frame_test_vel[i] = maxvel; else if (frame_test_vel[i] >= -maxvel &&
            newvel < -maxvel) frame_test_vel[i] = -maxvel; else
            frame_test_vel[i] = newvel; } //end for
            */
        } // end if
        if (crouch) {
        ...
        do {
            ...
            //
            if (stopevent & (SE_ENTERAREA | SE_TOUCHJUMPPAD |
                             SE_TOUCHTELEPORTER | SE_TOUCHCLUSTERPORTAL)) {
            ...
            if (stopevent & SE_HITBOUNDINGBOX) {
                if (AAS_ClipToBBox(&trace, org, trace.endpos, presencetype,
                                   mins, maxs)) {
            ...
            if (trace.fraction < 1.0) {
                ...
                if (plane->normal[2] == 0 &&
                    ...
                    steptrace = AAS_TraceClientBBox(start, stepend,
                                                    presencetype, entnum);
                    //
                    if (!steptrace.startsolid) {
                        plane2 = AAS_PlaneFromNum(steptrace.planenum);
                        if (DotProduct(plane2->normal, up) >
                            phys_maxsteepness) {
                            VectorSubtract(end, steptrace.endpos,
                                           left_test_vel);
                            left_test_vel[2] = 0;
                            frame_test_vel[2] = 0;
                            // #ifdef AAS_MOVE_DEBUG
                ...
                if (!step) {
                    ...
                    if (stopevent & SE_HITGROUNDDAMAGE) {
                        ...
                        if (delta) {
                            ...
                            if (swimming)
                                delta = 0;
                            // never take falling damage if completely
                            // underwater
                            /*
        ...
        if (onground) {
            ...
            // if solid is found the bot cannot walk any further and will not
            // fall into a gap
            if (!gaptrace.startsolid) {
                // if it is a gap (lower than one step height)
                if (gaptrace.endpos[2] <
                    org[2] - aassettings.phys_maxstep - 1) {
...
int AAS_PredictClientMovement(struct aas_clientmove_s *move, int entnum,
    ...
    return AAS_ClientMovementPrediction(move, entnum, origin, presencetype,
                                        onground, velocity, cmdmove, cmdframes,
                                        maxframes, frametime, stopevent,
                                        stopareanum, mins, maxs, visualize);
...
// Changes Globals:		-
//===========================================================================
int AAS_ClientMovementHitBBox(struct aas_clientmove_s *move, int entnum,
                              vec3_t origin, int presencetype, int onground,
                              vec3_t velocity, vec3_t cmdmove, int cmdframes,
                              int maxframes, float frametime, vec3_t mins,
                              vec3_t maxs, int visualize) {
    return AAS_ClientMovementPrediction(move, entnum, origin, presencetype,
                                        onground, velocity, cmdmove, cmdframes,
                                        maxframes, frametime, SE_HITBOUNDINGBOX,
                                        0, mins, maxs, visualize);
...
void AAS_TestMovementPrediction(int entnum, vec3_t origin, vec3_t dir) {
    ...
    AAS_PredictClientMovement(&move, entnum, origin, PRESENCE_NORMAL, qtrue,
                              velocity, cmdmove, 13, 13, 0.1f, SE_HITGROUND, 0,
                              qtrue); // SE_LEAVEGROUND);
...
int AAS_HorizontalVelocityForJump(float zvel, vec3_t start, vec3_t end,
    ...
    maxjump =
        0.5 * phys_gravity * (zvel / phys_gravity) * (zvel / phys_gravity);
