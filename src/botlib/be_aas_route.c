...
static ID_INLINE int AAS_ClusterAreaNum(int cluster, int areanum) {
    ...
    if (areacluster > 0)
        ...
        /*#ifdef ROUTING_DEBUG
                        if (aasworld.portals[-areacluster].frontcluster !=
        cluster && aasworld.portals[-areacluster].backcluster != cluster)
                        {
                                botimport.Print(PRT_ERROR, "portal %d: does not
        belong to cluster %d\n" , -areacluster, cluster); } //end if #endif
        //ROUTING_DEBUG*/
...
void AAS_RemoveRoutingCacheInCluster(int clusternum) {
    ...
    for (i = 0; i < cluster->numareas; i++) {
        for (cache = aasworld.clusterareacache[clusternum][i]; cache;
             cache = nextcache) {
...
void AAS_RemoveRoutingCacheUsingArea(int areanum) {
    ...
    if (clusternum > 0) {
        ...
        // if this is a portal remove all cache in both the front and back
        // cluster
        AAS_RemoveRoutingCacheInCluster(
            aasworld.portals[-clusternum].frontcluster);
        AAS_RemoveRoutingCacheInCluster(
            aasworld.portals[-clusternum].backcluster);
    } // end else
    ...
    if (areanum <= 0 || areanum >= aasworld.numareas) {
        if (botDeveloper) {
            botimport.Print(PRT_ERROR,
                            "AAS_EnableRoutingArea: areanum %d out of range\n",
                            areanum);
        } // end if
        return 0;
    } // end if
...
void AAS_InitAreaContentsTravelFlags(void) {
    ...
    aasworld.areacontentstravelflags =
        (int *)GetClearedMemory(aasworld.numareas * sizeof(int));
    //
    for (i = 0; i < aasworld.numareas; i++) {
...
void AAS_CreateReversedReachability(void) {
    ...
    ptr = (char *)GetClearedMemory(
        aasworld.numareas * sizeof(aas_reversedreachability_t) +
    ...
    for (i = 1; i < aasworld.numareas; i++) {
        ...
        if (settings->numreachableareas >= 128)
            botimport.Print(PRT_WARNING,
                            "area %d has more than 128 reachabilities\n", i);
    ...
    for (i = 0; i < aasworld.numareas; i++) {
        ...
        for (l = 0; l < settings->numreachableareas; l++) {
            ...
            for (n = 0, revlink = revreach->first; revlink;
                ...
                //
                aasworld.areatraveltimes[i][l][n] =
                    AAS_AreaTravelTime(i, end, reach->start);
            } // end for
        } // end for
    } // end for
#ifdef DEBUG
...
int AAS_FreeOldestCache(void)
{
        ...
        for (i = 0; i < aasworld.numclusters; i++)
        {
                ...
                {
                        for (cache = aasworld.clusterareacache[i][j]; cache;
cache = cache->next)
                        {
                                //never remove cache leading towards a portal
                                if
(aasworld.areasettings[cache->areanum].cluster < 0) continue;
                                //if this cache is older than the cache we found
so far if (cache->time < besttime)
                                {
                                        bestcache = cache;
                                        bestcluster = i;
                                        bestarea = j;
        ...
        if (bestcache)
        {
                ...
                else aasworld.clusterareacache[bestcluster][bestarea] =
cache->next; if (cache->next) cache->next->prev = cache->prev;
                AAS_FreeRoutingCache(cache);
                freed = qtrue;
        } //end if
        besttime = 999999999;
        ...
        {
                //refresh portal cache
                for (cache = aasworld.portalcache[i]; cache; cache =
cache->next)
                {
...
int AAS_FreeOldestCache(void) {
    ...
    for (cache = aasworld.oldestcache; cache; cache = cache->time_next) {
        ...
        if (cache->type == CACHETYPE_AREA &&
            aasworld.areasettings[cache->areanum].cluster < 0) {
            continue;
        }
        break;
    }
    if (cache) {
        ...
        if (cache->type == CACHETYPE_AREA) {
            ...
            if (cache->prev)
                ...
                aasworld.clusterareacache[cache->cluster][clusterareanum] =
                    cache->next;
            if (cache->next)
...
aas_routingcache_t *AAS_AllocRoutingCache(int numtraveltimes) {
    ...
    cache->reachabilities = (unsigned char *)cache +
...
void AAS_InitClusterAreaCache(void) {
    ...
    ptr = (char *)GetClearedMemory(aasworld.numclusters *
                                       sizeof(aas_routingcache_t **) +
                                   size * sizeof(aas_routingcache_t *));
    ...
    for (i = 1; i < aasworld.numareas; i++) {
        ...
        for (j = 1; j < aasworld.numareas; j++) {
            ...
            if (!AAS_AreaReachability(j))
                continue;
            AAS_AreaTravelTimeToGoalArea(i, aasworld.areas[i].center, j,
                                         TFL_DEFAULT);
            // t = AAS_AreaTravelTimeToGoalArea(i, aasworld.areas[i].center, j,
            // TFL_DEFAULT); Log_Write("traveltime from %d to %d is %d", i, j,
            // t);
        } // end for
    } // end for
...
void AAS_WriteRouteCache(void) {
    ...
    for (i = 0; i < aasworld.numareas; i++)
    {
            ...
            AAS_DecompressVis( aasworld.areavisibility[i], aasworld.numareas,
    aasworld.decompressedvis ); size = AAS_CompressVis(
    aasworld.decompressedvis, aasworld.numareas, aasworld.decompressedvis );
            botimport.FS_Write(&size, sizeof(int), fp);
...
aas_routingcache_t *AAS_ReadCache(fileHandle_t fp) {
    ...
    cache->size = size;
    botimport.FS_Read((unsigned char *)cache + sizeof(size),
                      size - sizeof(size), fp);
    cache->reachabilities =
        (unsigned char *)cache + sizeof(aas_routingcache_t) -
...
int AAS_ReadRouteCache(void) {
    ...
    for (i = 0; i < routecacheheader.numareacache; i++) {
        ...
        if (aasworld.clusterareacache[cache->cluster][clusterareanum])
            aasworld.clusterareacache[cache->cluster][clusterareanum]->prev =
                cache;
        aasworld.clusterareacache[cache->cluster][clusterareanum] = cache;
    } // end for
    // read the visareas
    /*
    aasworld.areavisibility = (byte **) GetClearedMemory(aasworld.numareas *
    sizeof(byte *)); aasworld.decompressedvis = (byte *)
    GetClearedMemory(aasworld.numareas * sizeof(byte)); for (i = 0; i <
    aasworld.numareas; i++)
    {
            botimport.FS_Read(&size, sizeof(size), fp );
            if (size) {
...
void AAS_InitReachabilityAreas(void) {
    ...
    aasworld.reachabilityareas = (aas_reachabilityareas_t *)GetClearedMemory(
        aasworld.reachabilitysize * sizeof(aas_reachabilityareas_t));
    aasworld.reachabilityareaindex = (int *)GetClearedMemory(
        aasworld.reachabilitysize * MAX_REACHABILITYPASSAREAS * sizeof(int));
    numreachareas = 0;
    for (i = 0; i < aasworld.reachabilitysize; i++) {
        ...
        case TRAVEL_WATERJUMP:
            ...
            numareas = AAS_TraceAreas(reach->start, end, areas, NULL,
                                      MAX_REACHABILITYPASSAREAS);
            break;
        case TRAVEL_WALKOFFLEDGE:
            VectorCopy(reach->end, start);
            start[2] = reach->start[2];
            numareas = AAS_TraceAreas(start, reach->end, areas, NULL,
                                      MAX_REACHABILITYPASSAREAS);
            break;
        case TRAVEL_GRAPPLEHOOK:
            numareas = AAS_TraceAreas(reach->start, reach->end, areas, NULL,
                                      MAX_REACHABILITYPASSAREAS);
            break;
...
//
// Parameter:			areacache		: routing cache to
// update Returns:				- Changes Globals: -
//===========================================================================
void AAS_UpdateAreaRoutingCache(aas_routingcache_t *areacache) {
    ...
    unsigned short int t,
        startareatraveltimes[128]; // NOTE: not more than 128 reachabilities per
                                   // area allowed
    aas_routingupdate_t *updateliststart, *updatelistend, *curupdate,
        *nextupdate;
    aas_reachability_t *reach;
    aas_reversedreachability_t *revreach;
    aas_reversedlink_t *revlink;
    ...
    numreachabilityareas =
        aasworld.clusters[areacache->cluster].numreachabilityareas;
    //
    ...
    while (updateliststart) {
        ...
        //
        for (i = 0, revlink = revreach->first; revlink;
             revlink = revlink->next, i++) {
            linknum = revlink->linknum;
            reach = &aasworld.reachability[linknum];
            // if there is used an undesired travel type
            if (AAS_TravelFlagForType_inline(reach->traveltype) &
                badtravelflags)
                continue;
            ...
            clusterareanum =
                AAS_ClusterAreaNum(areacache->cluster, nextareanum);
            if (clusterareanum >= numreachabilityareas)
                continue;
            // time already travelled plus the traveltime through
            // the current area plus the travel time from the reachability
            t = curupdate->tmptraveltime +
                // AAS_AreaTravelTime(curupdate->areanum, curupdate->start,
                // reach->end) +
...
// Changes Globals:		-
//===========================================================================
aas_routingcache_t *AAS_GetAreaRoutingCache(int clusternum, int areanum,
                                            int travelflags) {
    int clusterareanum;
...
void AAS_UpdatePortalRoutingCache(aas_routingcache_t *portalcache) {
    ...
    // if the start area is a cluster portal, store the travel time for that
    // portal
    ...
    while (updateliststart) {
        ...
        //
        cache = AAS_GetAreaRoutingCache(curupdate->cluster, curupdate->areanum,
        ...
        for (i = 0; i < cluster->numportals; i++) {
            ...
            //
            clusterareanum =
                AAS_ClusterAreaNum(curupdate->cluster, portal->areanum);
            ...
            if (!portalcache->traveltimes[portalnum] ||
                ...
                // add travel time through the actual portal area for the next
                // update
...
// Changes Globals:		-
//===========================================================================
aas_routingcache_t *AAS_GetPortalRoutingCache(int clusternum, int areanum,
                                              int travelflags) {
    aas_routingcache_t *cache;
...
// Changes Globals:		-
//===========================================================================
int AAS_AreaRouteToGoalArea(int areanum, vec3_t origin, int goalareanum,
    ...
    if (areanum <= 0 || areanum >= aasworld.numareas) {
        if (botDeveloper) {
            botimport.Print(
                PRT_ERROR,
                "AAS_AreaTravelTimeToGoalArea: areanum %d out of range\n",
                areanum);
        } // end if
        return qfalse;
    } // end if
    if (goalareanum <= 0 || goalareanum >= aasworld.numareas) {
        if (botDeveloper) {
            botimport.Print(
                PRT_ERROR,
                "AAS_AreaTravelTimeToGoalArea: goalareanum %d out of range\n",
                goalareanum);
        } // end if
        return qfalse;
    } // end if
    ...
    {
#ifdef DEBUG
            //Log_Write("WARNING: AAS_AreaTravelTimeToGoalArea: frame routing
updates overflowed"); #endif return 0; } //end if
    */
    //
    clusternum = aasworld.areasettings[areanum].cluster;
    ...
    // NOTE: there might be a shorter route via another cluster!!! but we don't
    // care
    ...
    for (i = 0; i < cluster->numportals; i++) {
        ...
        areacache =
            AAS_GetAreaRoutingCache(clusternum, portal->areanum, travelflags);
        ...
        // NOTE: for now we just add the largest travel time through the portal
        // area 		because we can't directly calculate the exact travel time 		to be
        //more specific we don't know which reachability was used to travel 		into
        //the portal area
        t += aasworld.portalmaxtraveltimes[portalnum];
        //
        if (origin) {
...
int AAS_AreaTravelTimeToGoalArea(int areanum, vec3_t origin, int goalareanum,
    ...
    if (AAS_AreaRouteToGoalArea(areanum, origin, goalareanum, travelflags,
                                &traveltime, &reachnum)) {
        return traveltime;
    }
    return 0;
...
// Changes Globals:		-
//===========================================================================
int AAS_AreaReachabilityToGoalArea(int areanum, vec3_t origin, int goalareanum,
                                   int travelflags) {
    int traveltime, reachnum;
...
int AAS_PredictRoute(struct aas_predictroute_s *route, int areanum,
    ...
    for (i = 0; curareanum != goalareanum && (!maxareas || i < maxareas) &&
                i < aasworld.numareas;
         i++) {
        reachnum = AAS_AreaReachabilityToGoalArea(curareanum, curorigin,
        ...
        for (j = 0; j < reachareas->numareas + 1; j++) {
            ...
            if (stopevent & RSE_ENTERCONTENTS) {
                if (aasworld.areasettings[testareanum].contents &
                    stopcontents) {
                    route->stopevent = RSE_ENTERCONTENTS;
                    route->endarea = testareanum;
                    route->endcontents =
                        aasworld.areasettings[testareanum].contents;
                    VectorCopy(reach->end, route->endpos);
                    route->time +=
                        AAS_AreaTravelTime(areanum, origin, reach->start);
                    route->time += reach->traveltime;
...
int AAS_NextAreaReachability(int areanum, int reachnum) {
    ...
    if (areanum <= 0 || areanum >= aasworld.numareas) {
        botimport.Print(PRT_ERROR,
                        "AAS_NextAreaReachability: areanum %d out of range\n",
                        areanum);
        return 0;
    } // end if
...
int AAS_NextModelReachability(int num, int modelnum) {
    ...
    for (i = num; i < aasworld.reachabilitysize; i++) {
        if ((aasworld.reachability[i].traveltype & TRAVELTYPE_MASK) ==
            TRAVEL_ELEVATOR) {
            if (aasworld.reachability[i].facenum == modelnum)
                return i;
        } // end if
        else if ((aasworld.reachability[i].traveltype & TRAVELTYPE_MASK) ==
                 TRAVEL_FUNCBOB) {
...
// Changes Globals:		-
//===========================================================================
int AAS_RandomGoalArea(int areanum, int travelflags, int *goalareanum,
                       vec3_t goalorigin) {
    int i, n, t;
    vec3_t start, end;
    aas_trace_t trace;
    ...
    for (i = 0; i < aasworld.numareas; i++) {
        ...
        if (AAS_AreaReachability(n)) {
            t = AAS_AreaTravelTimeToGoalArea(
                areanum, aasworld.areas[areanum].center, n, travelflags);
            // if the goal is reachable
            if (t > 0) {
                if (AAS_AreaSwim(n)) {
                    *goalareanum = n;
                    VectorCopy(aasworld.areas[n].center, goalorigin);
                    // botimport.Print(PRT_MESSAGE, "found random goal area
                    // %d\n", *goalareanum);
                    return qtrue;
                } // end if
                VectorCopy(aasworld.areas[n].center, start);
                if (!AAS_PointAreaNum(start))
                    Log_Write("area %d center %f %f %f in solid?", n, start[0],
                              start[1], start[2]);
...
int AAS_NearestHideArea(int srcnum, vec3_t origin, int areanum, int enemynum,
                        vec3_t enemyorigin, int enemyareanum, int travelflags) {
    ...
    while (updateliststart) {
        ...
        reach = &aasworld.reachability[aasworld.areasettings[curupdate->areanum]
                                           .firstreachablearea];
        //
        for (i = 0; i < numreach; i++, reach++) {
            ...
            AAS_ProjectPointOntoVector(enemyorigin, curupdate->start,
                                       reach->end, p);
            for (j = 0; j < 3; j++)
            ...
            // if we weren't visible when starting, make sure we don't move into
            // their view
