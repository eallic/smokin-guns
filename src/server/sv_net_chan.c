...
static void SV_Netchan_Encode(client_t *client, msg_t *msg,
                              const char *clientCommandString) {
    long i, index;
    byte key, *string;
    int srdc, sbit;
    qboolean soob;
...
static void SV_Netchan_Decode(client_t *client, msg_t *msg) {
    ...
    string = (byte *)client->reliableCommands[reliableAcknowledge &
...
void SV_Netchan_TransmitNextInQueue(client_t *client) {
    ...
    Com_DPrintf("#462 Netchan_TransmitNextFragment: popping a queued message "
                "for transmit\n");
...
Return number of ms until next message can be sent based on throughput given by
client rate, -1 if no packet was sent.
=================
*/
...
void SV_Netchan_Transmit(client_t *client, msg_t *msg) {
    ...
    if (client->netchan.unsentFragments || client->netchan_start_queue) {
        ...
        // store the msg, we can't store it encoded, as the encoding depends on
        // stuff we still have to finish sending
        MSG_Copy(&netbuf->msg, netbuf->msgBuffer, sizeof(netbuf->msgBuffer),
                 msg);
#ifdef LEGACY_PROTOCOL
        if (client->compat) {
            Q_strncpyz(netbuf->clientCommandString,
