...
static void SV_Startup(void) {
    ...
    if (com_dedicated->integer) {
        ...
        svs.numSnapshotEntities =
            sv_maxclients->integer * 4 * MAX_SNAPSHOT_ENTITIES;
    }
...
void SV_SpawnServer(char *server, qboolean killBots) {
    ...
    // if not running a dedicated server CL_MapLoading will connect the client
    // to the server also print some status stuff
    CL_MapLoading();
    ...
    svs.snapshotEntities =
        Hunk_Alloc(sizeof(entityState_t) * svs.numSnapshotEntities, h_high);
    ...
    // Joe Kari: this is what cause the 'nextmap' to bug, I remove that if
    // SMOKINGUNS is defined
    ...
    for (i = 0; i < sv_maxclients->integer; i++) {
        ...
        if (svs.clients[i].state >= CS_CONNECTED) {
            ...
            denied = VM_ExplicitArgPtr(
                gvm, VM_Call(gvm, GAME_CLIENT_CONNECT, i, qfalse,
                             isBot)); // firstTime = qfalse
            if (denied) {
    ...
    if (sv_pure->integer) {
        ...
        // if a dedicated pure server we need to touch the cgame because it
        // could be in a seperate pk3 file and the client will need to load the
        // latest cgame.qvm
        if (com_dedicated->integer) {
            SV_TouchCGame();
        }
...
void SV_Init(void) {
    ...
    sv_hostname =
        Cvar_Get("sv_hostname", "noname", CVAR_SERVERINFO | CVAR_ARCHIVE);
    sv_maxclients =
        Cvar_Get("sv_maxclients", "8", CVAR_SERVERINFO | CVAR_LATCH);
    ...
    sv_floodProtect =
        Cvar_Get("sv_floodProtect", "1", CVAR_ARCHIVE | CVAR_SERVERINFO);
    ...
    for (index = 2; index < MAX_MASTER_SERVERS; index++)
        sv_master[index] =
            Cvar_Get(va("sv_master%d", index + 1), "", CVAR_ARCHIVE);
    ...
    // initialize bot cvars so they are listed and can be set before loading the
    // botlib
    SV_BotInitCvars();
