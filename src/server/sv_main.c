...
void SV_MasterHeartbeat(const char *message) {
    static netadr_t adr[MAX_MASTER_SERVERS][2]; // [2] for v4 and v6 address for
...
static leakyBucket_t *SVC_BucketForAddress(netadr_t address, int burst,
    ...
    for (i = 0; i < MAX_BUCKETS; i++) {
        ...
        if (bucket->lastTime > 0 &&
            (interval > (burst * period) || interval < 0)) {
            if (bucket->prev != NULL) {
...
void SVC_Info(netadr_t from) {
     ...
     * original Quake3 version which led to the Infostring bug discovered by
     * Luigi Auriemma. See http://aluigi.altervista.org/ for the advisory.
     */
...
static void SVC_RemoteCommand(netadr_t from, msg_t *msg) {
    qboolean valid;
    char remaining[1024];
    // TTimo - scaled down to accumulate, but not overflow anything network
    // wise, print wise etc. (OOB messages are the bottleneck here)
#define SV_OUTPUTBUF_LENGTH (1024 - 16)
    char sv_outputbuf[SV_OUTPUTBUF_LENGTH];
    char *cmd_aux;

    // Prevent using rcon as an amplifier and make dictionary attacks
    // impractical
    if (SVC_RateLimitAddress(from, 10, 1000)) {
        Com_DPrintf("SVC_RemoteCommand: rate limit from %s exceeded, dropping "
                    "request\n",
                    NET_AdrToString(from));
        return;
    }
    ...
    if (!strlen(sv_rconPassword->string)) {
        ...
        // get the command directly, "rcon <pass> <command>" to avoid quoting
        // issues extract the command by walking since the cmd formatting can
        // fuckup (amount of spaces), using a dumb step by step parsing
        cmd_aux = Cmd_Cmd();
        cmd_aux += 4;
        while (cmd_aux[0] == ' ')
            cmd_aux++;
