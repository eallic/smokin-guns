...
void SV_GetChallenge(netadr_t from) {
    ...
    if (SVC_RateLimitAddress(from, 10, 1000)) {
        Com_DPrintf(
            "SV_GetChallenge: rate limit from %s exceeded, dropping request\n",
    ...
    // reject client if the gamename string sent by the client doesn't match
    // ours
    if (gameMismatch) {
    ...
    // always generate a new challenge number, so the client cannot circumvent
    // sv_maxping
    ...
    if (challenge->adr.type == NA_IP && !Sys_IsLANAddress(from)) {
        ...
        // let them in, assuming the id server is down
            ...
            // the 0 is for backwards compatibility with obsolete
            // sv_allowanonymous flags getIpAuthorize <challenge> <IP> <game> 0
            // <auth-flag>
            NET_OutOfBandPrint(NS_SERVER, svs.authorizeAddress,
                               "getIpAuthorize %i %i.%i.%i.%i %s 0 %s",
                               challenge->challenge, from.ip[0], from.ip[1],
                               from.ip[2], from.ip[3], game,
...
static qboolean SV_TrustedClient(char *ip, char *info) {
    ...
    if (svs.authorizeAddress.type != NA_BAD) {
        ...
        if (guid != NULL) {
            ...
            Com_DPrintf("getTrustedClient: %s;%s;%s\n", ip,
                        sv_strictAuth->string, name);
            NET_OutOfBandPrint(NS_SERVER, svs.authorizeAddress,
                               "getTrustedClient %s;%s;%s", ip,
                               sv_strictAuth->string, name);
        }
    }
...
void SV_AuthorizeIpPacket(netadr_t from) {
    ...
    if (!Q_stricmp(s, "accept")) {
        NET_OutOfBandPrint(
            NS_SERVER, challengeptr->adr, "challengeResponse %d %d %d",
            challengeptr->challenge, challengeptr->clientChallenge,
            com_protocol->integer);
        return;
    }
...
static qboolean SV_IsBanned(netadr_t *from, qboolean isexception) {
    ...
    if (!isexception) {
        // If this is a query for a ban, first check whether the client is
        // excepted
...
void SV_DirectConnect(netadr_t from) {
    ...
    if (!NET_IsLocalAddress(from)) {
        ...
        if (challengeptr->wasrefused) {
            // Return silently, so that error messages written by the server
...
gotnewcl:
    ...
    *newcl = temp;
    ...
    denied = VM_Call(gvm, GAME_CLIENT_CONNECT, clientNum, qtrue,
                     qfalse); // firstTime = qtrue
    if (denied) {
        // we can't just use VM_ArgPtr, because that is only valid inside a
        // VM_Call
    ...
    // Tequila comment: Trusted client should present a valid GUID, but only
    // IPV4 clients are supported for now
    if (from.type == NA_IP && !Sys_IsLANAddress(from) &&
        !SV_TrustedClient(ip, userinfo)) {
        NET_OutOfBandPrint(
            NS_SERVER, from,
...
void SV_ClientEnterWorld(client_t *client, usercmd_t *cmd) {
    ...
    if (svs.initialized && sv_autorecord->integer &&
...
client Fill up msg with data, return number of download blocks added
==================
*/
int SV_WriteDownloadToClient(client_t *cl, msg_t *msg) {
    ...
    if (!cl->download) {
        ...
        if (!(sv_allowDownload->integer & DLF_ENABLE) ||
            (sv_allowDownload->integer & DLF_NO_UDP) || idPack ||
            unreferenced ||
            (cl->downloadSize =
                 FS_SV_FOpenFileRead(cl->downloadName, &cl->download)) < 0) {
            // cannot auto-download file
            if (unreferenced) {
                Com_Printf("clientDownload: %d : \"%s\" is not referenced and "
                ...
                Com_Printf("clientDownload: %d : \"%s\" cannot download id pk3 "
                           "files\n",
                           (int)(cl - svs.clients), cl->downloadName);
                {
                ...
                if (sv_pure->integer) {
                    Com_sprintf(
                        errorMessage, sizeof(errorMessage),
                        "Could not download \"%s\" because autodownloading is "
                    ...
                    Com_sprintf(errorMessage, sizeof(errorMessage),
                                "Could not download \"%s\" because "
                                "autodownloading is disabled on the server.\n\n"
                                "The server you are connecting to is not a "
                                "pure server, "
                                "set autodownload to No in your settings and "
                                "you might be "
                                "able to join the game anyway.\n",
                                cl->downloadName);
                }
            } else {
                // NOTE TTimo this is NOT supposed to happen unless bug in our
                // filesystem scheme?
                //   if the pk3 is referenced, it must have been found somewhere
                //   in the filesystem
                Com_Printf(
    ...
    while (cl->downloadCurrentBlock - cl->downloadClientBlock <
        ...
        cl->downloadBlockSize[curindex] = FS_Read(
            cl->downloadBlocks[curindex], MAX_DOWNLOAD_BLKSIZE, cl->download);
    ...
    if (cl->downloadCount == cl->downloadSize && !cl->downloadEOF &&
        ...
        cl->downloadBlockSize[cl->downloadCurrentBlock % MAX_DOWNLOAD_WINDOW] =
            0;
        cl->downloadCurrentBlock++;
    ...
    // window, automatically start retransmitting
...
Send one round of fragments, or queued messages to all clients that have data
pending. Return the shortest time interval for sending next packet to client
==================
*/
...
=================
*/
static void SV_Disconnect_f(client_t *cl) { SV_DropClient(cl, "disconnected"); }

/*
=================
SV_VerifyPaks_f

If we are pure, disconnect the client if they do no meet the following
conditions:
...
static void SV_VerifyPaks_f(client_t *cl) {
    ...
    if (sv_pure->integer != 0) {
        ...
        nChkSum1 = nChkSum2 = 0;
        // we run the game, so determine which cgame and ui the client "should"
        // be running
        ...
        if (!pArg) {
            ...
            // we may get incoming cp sequences from a previous checksumFeed,
            // which we need to ignore since serverId is a frame count, it
            // always goes up
        ...
        while (bGood) {
            ...
            // must be at least 6: "cl_paks cgame ui @ firstref ...
            // numChecksums" numChecksums is encoded
            if (nClientPaks < 6) {
                bGood = qfalse;
                break;
            }
            ...
            // store number to compare against (minus one cause the last is the
            // number of checksums)
            nClientPaks = i - 1;
            ...
            // check if the client has provided any pure checksums of pk3 files
        ...
        if (bGood) {
            ...
            SV_DropClient(
                cl, "Unpure client detected. Invalid .PK3 files referenced!");
        }
    }
}
...
void SV_UserinfoChanged(client_t *cl) {
    ...
    // if the client is on the same subnet as the server and we aren't running
    // an internet public server, assume they don't need a rate choke
    ...
    if (i != cl->snapshotMsec) {
        // Reset last sent snapshot so we avoid desync between server frame time
        // and snapshot send time
        cl->lastSnapshotTime = 0;
        cl->snapshotMsec = i;
    }
...
#ifdef USE_VOIP
static void SV_UpdateVoipIgnore(client_t *cl, const char *idstr,
                                qboolean ignore) {
    if ((*idstr >= '0') && (*idstr <= '9')) {
...
static void SV_UserMove(client_t *cl, msg_t *msg, qboolean delta) {
    ...
    key ^= MSG_HashKey(cl->reliableCommands[cl->reliableAcknowledge &
    ...
    // if not, then we are getting remaining parasite usermove commands, which
    // we should ignore
    if (sv_pure->integer != 0 && cl->pureAuthentic == 0 && !cl->gotCP) {
        if (cl->state == CS_ACTIVE) {
            // we didn't get a cp yet, don't assume anything and just send the
            // gamestate all over again
...
static void SV_UserVoip(client_t *cl, msg_t *msg) {
    ...
    for (i = 0, client = svs.clients; i < sv_maxclients->integer;
        ...
        client
            ->voipPacket[(client->queuedVoipIndex + client->queuedVoipPackets) %
...
void SV_ExecuteClientMessage(client_t *cl, msg_t *msg) {
    ...
    // server commands which the server thinks are not yet acknowledged in
    // SV_UpdateServerCommandsToClient
    if (cl->reliableAcknowledge <
        cl->reliableSequence - MAX_RELIABLE_COMMANDS) {
        // usually only hackers create messages like this
        // it is more annoying for them to let them hanging
#ifndef NDEBUG
        SV_DropClient(cl, "DEBUG: illegible client message");
    ...
    // downloadName is set back to "" but we still need to read the next message
    // to move to next download or send gamestate I don't like this hack though,
    // it must have been working fine at some point, suspecting the fix is
    // somewhere else
    if (serverId != sv.serverId && !*cl->downloadName &&
        !strstr(cl->lastClientCommandString, "nextdl")) {
        if (serverId >= sv.restartedServerId &&
            serverId < sv.serverId) { // TTimo - use a comparison here to catch
                                      // multiple map_restart
            // they just haven't caught the map_restart yet
            Com_DPrintf(
                "%s : ignoring pre map_restart / outdated client message\n",
                cl->name);
            return;
        }
