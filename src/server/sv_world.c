...
testing, the world is carved up with an evenly spaced, axially aligned bsp tree.
Entities are kept in chains either at the final leafs, or at the first node that
splits them, which prevents having to deal with multiple fragments of a single
entity.
...
===============
*/
static worldSector_t *SV_CreateworldSector(int depth, vec3_t mins,
                                           vec3_t maxs) {
    worldSector_t *anode;
    vec3_t size;
...
void SV_LinkEntity(sharedEntity_t *gEnt) {
    ...
    if (gEnt->r.bmodel) {
        gEnt->s.solid =
            SOLID_BMODEL; // a solid_box will never create this value
    ...
    num_leafs = CM_BoxLeafnums(gEnt->r.absmin, gEnt->r.absmax, leafs,
...
void SV_ClipToEntity(trace_t *trace, const vec3_t start, const vec3_t mins,
    ...
    CM_TransformedBoxTrace(trace, (float *)start, (float *)end, (float *)mins,
                           (float *)maxs, clipHandle, contentmask, origin,
                           angles, capsule);
...
static void SV_ClipMoveToEntities(moveclip_t *clip) {
    ...
    num =
        SV_AreaEntities(clip->boxmins, clip->boxmaxs, touchlist, MAX_GENTITIES);
    ...
    for (i = 0; i < num; i++) {
        ...
        CM_TransformedBoxTrace(&trace, (float *)clip->start, (float *)clip->end,
                               (float *)clip->mins, (float *)clip->maxs,
...
int SV_PointContents(const vec3_t p, int passEntityNum) {
    ...
    for (i = 0; i < num; i++) {
        ...
        c2 = CM_TransformedPointContents(p, clipHandle, hit->r.currentOrigin,
                                         angles);
