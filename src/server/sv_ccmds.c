...
static void SV_Map_f(void) {
    ...
    // TheDoctor: patch demos: stop server-side demo recording when sv_autrecord
    // is set
...
static void SV_MapRestart_f(void) {
    ...
    if (sv_autorecord->integer) {
        for (j = 0, cl = svs.clients; j < sv_maxclients->integer; j++, cl++) {
            if (cl && cl->state && cl->state >= CS_CONNECTED &&
                cl->demorecording) {
                CL_StopRecord(cl);
            }
        }
    ...
    // had been changed from their default values will generate broadcast
    // updates
    sv.state = SS_LOADING;
    sv.restarting = qtrue;
    ...
    for (i = 0; i < sv_maxclients->integer; i++) {
        ...
        denied = VM_ExplicitArgPtr(
            gvm, VM_Call(gvm, GAME_CLIENT_CONNECT, i, qfalse, isBot));
        if (denied) {
        ...
        if (client->state == CS_ACTIVE)
            ...
            // If we don't reset client->lastUsercmd and are restarting during
            // map load, the client will hang because we'll use the last Usercmd
            // from the previous map, which is wrong obviously.
    ...
    if (!cl) {
        if (!Q_stricmp(Cmd_Argv(1), "all")) {
            for (i = 0, cl = svs.clients; i < sv_maxclients->integer;
                ...
                cl->lastPacketTime =
                    svs.time; // in case there is a funny zombie
            }
...
static void SV_Ban_f(void) {
    ...
    if (svs.authorizeAddress.type != NA_BAD) {
        ...
        NET_OutOfBandPrint(
            NS_SERVER, svs.authorizeAddress, "banUser %i.%i.%i.%i %s",
...
static void SV_RehashBans_f(void) {
    ...
    Com_sprintf(filepath, sizeof(filepath), "%s/%s", FS_GetCurrentGameDir(),
                sv_banFile->string);
    ...
    if ((filelen = FS_SV_FOpenFileRead(filepath, &readfrom)) >= 0) {
        ...
        for (index = 0; index < SERVER_MAXBANS && curpos + 2 < endpos;
             index++) {
            // find the end of the address string
            for (maskpos = curpos + 2; maskpos < endpos && *maskpos != ' ';
                 maskpos++)
                ;
    ...
    if ((writeto = FS_SV_FOpenFileWrite(filepath))) {
        ...
        for (index = 0; index < serverBansCount; index++) {
            ...
            Com_sprintf(writebuf, sizeof(writebuf), "%d %s %d\n",
                        curban->isexception, NET_AdrToString(curban->ip),
                        curban->subnet);
...
static void SV_AddBanToList(qboolean isexception) {
    ...
    // first check whether a conflicting ban exists that would supersede the new
    // one.
    for (index = 0; index < serverBansCount; index++) {
        curban = &serverBans[index];
        ...
        if (curban->subnet >= mask) {
            if (!curban->isexception && isexception &&
                ...
                Com_Printf(
                    ...
                    isexception ? "Exception" : "Ban", NET_AdrToString(ip),
                    mask, curban->isexception ? "exception" : "ban", addy2,
                    curban->subnet);
                return;
            }
        }
    ...
    while (index < serverBansCount) {
        ...
        if (curban->subnet > mask && (!curban->isexception || isexception) &&
            NET_CompareBaseAdrMask(curban->ip, ip, mask))
...
static void SV_BanAddr_f(void) { SV_AddBanToList(qfalse); }

static void SV_ExceptAddr_f(void) { SV_AddBanToList(qtrue); }

static void SV_BanDel_f(void) { SV_DelBanFromList(qfalse); }

static void SV_ExceptDel_f(void) { SV_DelBanFromList(qtrue); }
...
static void SV_ClientID_f(void) {
    ...
    for (i = 0, cl = svs.clients; i < sv_maxclients->integer; i++, cl++) {
        ...
        // Name (should come last, easiest to parse it this way cause people may
        // have all sort of char in her names)
        Com_Printf("%s^7\n", cl->name);
    }
    Com_Printf("\n");
}
...
==================
*/
void SV_Heartbeat_f(void) { svs.nextHeartbeatTime = -9999999; }
...
=================
*/
static void SV_KillServer_f(void) { SV_Shutdown("killserver"); }

/*
=======================================================================// patch
demos: server-side recorded client demos
...
void SVCL_WriteDemoMessage(client_t *cl, msg_t *msg, int headerBytes) {
    ...
    if (!cl->savedemo && ps->pm_type != PM_INTERMISSION &&
...
void CL_StopRecord(client_t *cl) {
    ...
    if (!sv_autorecord->integer) {
        ...
        if (cl->savedemo) {
            ...
            //S_COLOR_WHITE "'s game was recorded as " S_COLOR_GREEN "http:/
            ///demos.smokin-guns.org/" S_COLOR_YELLOW "%s\n\"", cl->name,
            //demoname);
        } else {
...
void CL_Record(client_t *cl, char *s) {
    ...
    //		return;
    //	}
    //  // sync 0 doesn't prevent recording, so not forcing it off .. everyone
    //  does g_sync 1 ; record ; g_sync 0 ..
    //	if ( NET_IsLocalAddress( cl->serverAddress ) && !Cvar_VariableValue(
    //"g_synchronousClients" ) ) { 		Com_Printf (S_COLOR_YELLOW "WARNING: You
    //should set 'g_synchronousClients 1' for smoother demo recording\n");
    //	}
    ...
    if (s) {
        ...
        for (number = 0; number <= 9999; number++) {
            ...
            Com_sprintf(name, sizeof(name), "demos/%s_%s_%i%i%i%i.dm_%d",
                        prefix, sv_mapname->string, a, b, c, d,
                        PROTOCOL_VERSION);
            Com_sprintf(name_zip, sizeof(name_zip),
                        "demos/%s_%s_%i%i%i%i.dm_%d.zip", prefix,
                        sv_mapname->string, a, b, c, d, PROTOCOL_VERSION);
            Q_strlwr(name);
            Q_strlwr(name_zip);
    ...
    cl->savedemo = qfalse; // demo will not be saved if sv_autorecord 1 and cl's
                           // score is too low
...
void SVCL_Record_f(void) {
    ...
    if (Cmd_Argc() == 3) {
        s = Cmd_Argv(2);
        //		Com_sprintf (name, sizeof(name), "demos/%s.dm_%d", s,
        //PROTOCOL_VERSION );
        CL_Record(cl, s);
    } else {
        CL_Record(cl, 0);
