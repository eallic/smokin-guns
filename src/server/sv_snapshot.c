...
static void SV_WriteSnapshotToClient(client_t *client, msg_t *msg,
    ...
    if (client->olddemoframe) {
        MSG_WriteDeltaPlayerstate(msg_demo, &client->olddemoframe->ps,
                                  &frame->ps);
    } else {
...
static qboolean recentlySeen(client_t *viewer_cl, int ent_clnum) {
    if (viewer_cl->tracetimer[ent_clnum] >
        sv.time + 210 + 10) { // if the sv.time has been reset
...
#define offsetrandom2(MIN, MAX)                                                \
    (((float)(rand() + 0.5f) / ((float)RAND_MAX + 1.0f)) * ((MAX) - (MIN)) +   \
     (MIN))
/*
===============
...
static void SV_AddEntitiesVisibleFromPoint(vec3_t origin,
    ...
    for (e = 0; e < sv.num_entities; e++) {
        ...
        // broadcast only non-players. Warning: in case sv_antiwallhack equals
        // 1, bots will be disappearing on dm_train when touching doors, unless
        // you compensate with "recentlySeen".
        if (ent->r.svFlags & SVF_BROADCAST &&
            (ent->s.eType != ET_PLAYER || sv_antiwallhack->integer == 0 ||
             recentlySeen(cl, ent->s.clientNum))) {
        ...
        // if it's a portal entity, add everything visible from its camera
        // position
...
static void SV_BuildClientSnapshot(client_t *client) {
    ...
    for (i = 0; i < entityNumbers.numSnapshotEntities; i++) {
        ...
        *state = ent->s;
        ...
        // this should never hit, map should always be restarted first in
        // SV_Frame
...
static void SV_WriteVoipToClient(client_t *cl, msg_t *msg) {
    ...
    if (cl->queuedVoipPackets) {
        ...
        for (i = 0; i < cl->queuedVoipPackets; i++) {
            packet = cl->voipPacket[(i + cl->queuedVoipIndex) %
                                    ARRAY_LEN(cl->voipPacket)];
...
void SV_SendClientMessages(void) {
    ...
    for (i = 0; i < sv_maxclients->integer; i++) {
        ...
        if (c->netchan.unsentFragments || c->netchan_start_queue) {
            c->rateDelayed = qtrue;
            continue; // Drop this snapshot if the packet queue is still full or
                      // delta compression will break
        }
        ...
        if (!(c->netchan.remoteAddress.type == NA_LOOPBACK ||
            ...
            if (svs.time - c->lastSnapshotTime <
                c->snapshotMsec * com_timescale->value)
