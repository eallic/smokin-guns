...
void UI_LoadBestScores(const char *map, int game) {
    ...
    // Tequila: Minor fix to not try to load anything when map is not defined
    // here
    ...
    Com_sprintf(fileName, MAX_QPATH, "demos/%s_%d.%s%d", map, game, DEMOEXT,
                protocol);
    if (trap_FS_FOpenFile(fileName, &f, FS_READ) >= 0) {
...
void UI_ClearScores(void) {
    ...
    if (count > 0) {
        ...
        for (i = 0; i < count; i++) {
            len = strlen(gameFile);
            if (trap_FS_FOpenFile(va("games/%s", gameFile), &f, FS_WRITE) >=
                0) {
...
static void UI_CalcPostGameStats(void) {
    ...
    newHigh =
        (newInfo.redScore > newInfo.blueScore && newInfo.score > oldInfo.score);
