...
static void UI_ForceTorsoAnim(playerInfo_t *pi, int anim) {
    ...
    if (anim == TORSO_TAUNT_PISTOL) {
        pi->torsoAnimationTimer = UI_TIMER_GESTURE;
    }

    if (anim == TORSO_PISTOL_ATTACK || anim == TORSO_RIFLE_ATTACK) {
...
static void UI_TorsoSequencing(playerInfo_t *pi) {
    ...
    if (pi->weapon != pi->currentWeapon) {
        if (currentAnim != TORSO_PISTOL_DROP) {
            pi->torsoAnimationTimer = UI_TIMER_WEAPON_SWITCH;
            UI_ForceTorsoAnim(pi, TORSO_PISTOL_DROP);
        }
    }
    ...
    if (currentAnim == TORSO_TAUNT_PISTOL) {
        UI_SetTorsoAnim(pi, TORSO_PISTOL_STAND);
        return;
    }

    if (currentAnim == TORSO_PISTOL_ATTACK ||
        currentAnim == TORSO_RIFLE_ATTACK) {
        UI_SetTorsoAnim(pi, TORSO_PISTOL_STAND);
        return;
    }
    ...
    if (currentAnim == TORSO_PISTOL_DROP) {
        ...
        pi->torsoAnimationTimer = UI_TIMER_WEAPON_SWITCH;
        UI_ForceTorsoAnim(pi, TORSO_PISTOL_RAISE);
        return;
    }

    if (currentAnim == TORSO_PISTOL_RAISE) {
        UI_SetTorsoAnim(pi, TORSO_PISTOL_STAND);
        return;
    }
}
...
static void UI_RunLerpFrame(playerInfo_t *ci, lerpFrame_t *lf,
    ...
    if (lf->frameTime == lf->oldFrameTime) {
        ...
        lf->backlerp = 1.0 - (float)(dp_realtime - lf->oldFrameTime) /
...
void UI_DrawPlayer(float x, float y, float w, float h, playerInfo_t *pi,
    ...
    refdef.fov_x =
        (int)((float)refdef.width / uiInfo.uiDC.xscale / 640.0f * 90.0f);
    ...
    if (dp_realtime <= pi->muzzleFlashTime) {
        ...
        if (pi->flashDlightColor[0] || pi->flashDlightColor[1] ||
            ...
            trap_R_AddLightToScene(
                flash.origin, 200 + (rand() & 31), pi->flashDlightColor[0],
...
static qboolean UI_RegisterClientSkin(playerInfo_t *pi, const char *modelName,
    ...
    Com_sprintf(filename, sizeof(filename),
                "models/wq3_players/%s/head_%s.skin", modelName, skinName);
...
void UI_PlayerInfo_SetInfo(playerInfo_t *pi, int legsAnim, int torsoAnim,
    ...
    if (weaponNum != pi->currentWeapon || currentAnim == TORSO_PISTOL_RAISE ||
        currentAnim == TORSO_PISTOL_DROP) {
        pi->pendingTorsoAnim = torsoAnim;
    } else if ((currentAnim == TORSO_TAUNT_PISTOL ||
                currentAnim == TORSO_PISTOL_ATTACK) &&
