...
// use this to get a demo build without an explicit demo build, i.e. to get the
...
static const char *netSources[] = {"Local",     "Internet",  "Internet1",
...
void AssetCache(void) {
    ...
    uiInfo.uiDC.Assets.scrollBar_horz =
        trap_R_RegisterShaderNoMip(ASSET_SCROLLBAR_HORZ);
    ...
    for (n = 0; n < NUM_CROSSHAIRS; n++) {
        uiInfo.uiDC.Assets.crosshairShader[n] =
            trap_R_RegisterShaderNoMip(va("hud/crosshair%c", 'a' + n));
    }
    uiInfo.uiDC.Assets.menu_width =
        trap_R_RegisterShaderNoMip(ASSET_MENU_WIDTH);
    uiInfo.uiDC.Assets.menu_height =
        trap_R_RegisterShaderNoMip(ASSET_MENU_HEIGHT);
}
...
void Text_PaintWithCursor(float x, float y, float scale, vec4_t color,
    ...
    if (text) {
        ...
        while (s && *s && count < len) {
            ...
            if (Q_IsColorString(s)) {
                ...
                if (count == cursorPos &&
                    ...
                    Text_PaintChar(x, y - yadj, glyph2->imageWidth,
                                   glyph2->imageHeight, useScale, glyph2->s,
                                   glyph2->t, glyph2->s2, glyph2->t2,
                                   glyph2->glyph);
                }
        ...
        if (cursorPos == len && !((uiInfo.uiDC.realTime / BLINK_DIVISOR) & 1)) {
            yadj = useScale * glyph2->top;
            Text_PaintChar(x, y - yadj, glyph2->imageWidth, glyph2->imageHeight,
                           useScale, glyph2->s, glyph2->t, glyph2->s2,
                           glyph2->t2, glyph2->glyph);
        }
...
void _UI_Refresh(int realtime) {
    ...
    if (Menu_Count() > 0 && (trap_Key_GetCatcher() & KEYCATCH_UI)) {
        UI_DrawHandlePic(uiInfo.uiDC.cursorx - 12, uiInfo.uiDC.cursory - 12, 24,
                         24, uiInfo.uiDC.Assets.cursor);
    }
    ...
    if (uiInfo.uiDC.debug) {
        ...
        // FIXME
        Text_PaintCenter(
            80, 50, 0.6f, colorWhite,
            va("(%d,%d)", uiInfo.uiDC.cursorx, uiInfo.uiDC.cursory), 0);
...
=================
*/
void _UI_Shutdown(void) { trap_LAN_SaveCachedServers(); }
...
void UI_LoadMenus(const char *menuFile, qboolean reset) {
    ...
    if (!handle) {
        ...
        if (!handle) {
            trap_Error(S_COLOR_RED "default menu file not found: ui/menus.txt, "
                                   "unable to continue!");
        }
    }
...
static const char *handicapValues[] = {
    "None", "95", "90", "85", "80", "75", "70", "65", "60", "55", "50",
    "45",   "40", "35", "30", "25", "20", "15", "10", "5",  NULL};
...
static void UI_DrawClanLogo(rectDef_t *rect, float scale, vec4_t color) {
    ...
    if (i >= 0 && i < uiInfo.teamCount) {
        ...
        if (uiInfo.teamList[i].teamIcon == -1) {
            ...
            uiInfo.teamList[i].teamIcon_Name = trap_R_RegisterShaderNoMip(
                va("%s_name", uiInfo.teamList[i].imageName));
        }
    ...
    if (i >= 0 && i < uiInfo.teamCount) {
        ...
        if (uiInfo.teamList[i].cinematic >= -2) {
            if (uiInfo.teamList[i].cinematic == -1) {
                uiInfo.teamList[i].cinematic = trap_CIN_PlayCinematic(
                    va("%s.roq", uiInfo.teamList[i].imageName), 0, 0, 0, 0,
                    (CIN_loop | CIN_silent));
            }
            if (uiInfo.teamList[i].cinematic >= 0) {
                trap_CIN_RunCinematic(uiInfo.teamList[i].cinematic);
                trap_CIN_SetExtents(uiInfo.teamList[i].cinematic, rect->x,
...
static void UI_DrawPreviewCinematic(rectDef_t *rect, float scale,
                                    vec4_t color) {
    if (uiInfo.previewMovie > -2) {
        uiInfo.previewMovie = trap_CIN_PlayCinematic(
            va("%s.roq", uiInfo.movieList[uiInfo.movieIndex]), 0, 0, 0, 0,
            (CIN_loop | CIN_silent));
        if (uiInfo.previewMovie >= 0) {
            trap_CIN_RunCinematic(uiInfo.previewMovie);
            trap_CIN_SetExtents(uiInfo.previewMovie, rect->x, rect->y, rect->w,
                                rect->h);
...
static void UI_DrawTeamName(rectDef_t *rect, float scale, vec4_t color,
    ...
    if (i >= 0 && i < uiInfo.teamCount) {
        Text_Paint(
            ...
            va("%s: %s", (blue) ? "Blue" : "Red", uiInfo.teamList[i].teamName),
            0, 0, textStyle);
    }
}
...
static void UI_DrawTeamMember(rectDef_t *rect, float scale, vec4_t color,
    ...
    // 1 - Human
    // 2..NumCharacters - Bot
    int value = trap_Cvar_VariableValue(
        va(blue ? "ui_blueteam%i" : "ui_redteam%i", num));
    const char *text;
    if (value <= 0) {
        text = "Closed";
    } else if (value == 1) {
...
static void UI_DrawMapPreview(rectDef_t *rect, float scale, vec4_t color,
    ...
    if (uiInfo.mapList[map].levelShot > 0) {
        ...
        UI_DrawHandlePic(rect->x, rect->y, rect->w, rect->h,
                         trap_R_RegisterShaderNoMip("ui/bg/main"));
    }
}
...
static void UI_DrawMapTimeToBeat(rectDef_t *rect, float scale, vec4_t color,
    ...
    time = uiInfo.mapList[ui_currentMap.integer]
               .timeToBeat[uiInfo.gameTypes[ui_gameType.integer].gtEnum];
    ...
    if (uiInfo.mapList[map].cinematic >= -1) {
        if (uiInfo.mapList[map].cinematic == -1) {
            uiInfo.mapList[map].cinematic = trap_CIN_PlayCinematic(
                va("%s.roq", uiInfo.mapList[map].mapLoadName), 0, 0, 0, 0,
                (CIN_loop | CIN_silent));
        }
...
static void UI_DrawPlayerModel(rectDef_t *rect) {
    ...
    if (updateModel) {
        ...
        UI_PlayerInfo_SetInfo(&info, LEGS_IDLE, TORSO_PISTOL_STAND, viewangles,
                              vec3_origin, WP_PEACEMAKER, qfalse);
...
static void UI_DrawTier(rectDef_t *rect, float scale, vec4_t color,
    ...
    Text_Paint(rect->x, rect->y, scale, color,
               va("Tier: %s", uiInfo.tierList[i].tierName), 0, 0, textStyle);
}
...
static void UI_DrawTierMap(rectDef_t *rect, int index) {
    ...
    if (uiInfo.tierList[i].mapHandles[index] == -1) {
        uiInfo.tierList[i].mapHandles[index] = trap_R_RegisterShaderNoMip(
...
static void UI_DrawTierMapName(rectDef_t *rect, float scale, vec4_t color,
    ...
    Text_Paint(rect->x, rect->y, scale, color,
               UI_EnglishMapName(uiInfo.tierList[i].maps[j]), 0, 0, textStyle);
}
...
static void UI_DrawTierGameType(rectDef_t *rect, float scale, vec4_t color,
    ...
    Text_Paint(rect->x, rect->y, scale, color,
               uiInfo.gameTypes[uiInfo.tierList[i].gameTypes[j]].gameType, 0, 0,
               textStyle);
}
...
static void UI_DrawOpponent(rectDef_t *rect) {
    ...
    if (updateOpponentModel) {
        ...
        UI_PlayerInfo_SetInfo(&info2, LEGS_IDLE, TORSO_PISTOL_STAND, viewangles,
                              vec3_origin, WP_PEACEMAKER, qfalse);
...
static void UI_DrawAllMapsSelection(rectDef_t *rect, float scale, vec4_t color,
    ...
    int map = (net) ? ui_currentNetMap.integer : ui_currentMap.integer;
    if (map >= 0 && map < uiInfo.mapCount) {
        Text_Paint(rect->x, rect->y, scale, color, uiInfo.mapList[map].mapName,
                   0, 0, textStyle);
    }
}
...
static int UI_OwnerDrawWidth(int ownerDraw, float scale) {
    ...
    case UI_BLUETEAM4:
    case UI_BLUETEAM5:
        value = trap_Cvar_VariableValue(
            va("ui_blueteam%i", ownerDraw - UI_BLUETEAM1 + 1));
        if (value <= 0) {
            text = "Closed";
        } else if (value == 1) {
            text = "Human";
    ...
    case UI_REDTEAM5:
        value = trap_Cvar_VariableValue(
            va("ui_redteam%i", ownerDraw - UI_REDTEAM1 + 1));
        if (value <= 0) {
            text = "Closed";
        } else if (value == 1) {
            text = "Human";
    ...
    case UI_MAPDESC_LINE6:
        text = uiInfo.mapList[ui_currentNetMap.integer]
                   .description[ownerDraw - UI_MAPDESC_LINE1];
        s = va("%s", text);
        break;
...
static void UI_DrawCrosshair(rectDef_t *rect, float scale, vec4_t color) {
    ...
    UI_DrawHandlePic(
        rect->x, rect->y - rect->h, rect->w, rect->h,
    ...
    if (uiInfo.serverStatus.refreshActive) {
        ...
        Text_Paint(rect->x, rect->y, scale, newColor,
                   va("Getting info for %d servers (ESC to cancel)",
                      trap_LAN_GetServerCount(UI_SourceForLAN())),
                   0, 0, textStyle);
    } else {
        char buff[64];
        Q_strncpyz(buff,
                   UI_Cvar_VariableString(
                       va("ui_lastServerRefresh_%i", ui_netSource.integer)),
                   64);
        Text_Paint(rect->x, rect->y, scale, color, va("Refresh Time: %s", buff),
                   0, 0, textStyle);
    }
}
...
static void UI_DrawServerMOTD(rectDef_t *rect, float scale, vec4_t color) {
    if (uiInfo.serverStatus.motdLen) {
                ...
                if (uiInfo.serverStatus.motdOffset <
                    ...
                    uiInfo.serverStatus.motdPaintX +=
                        Text_Width(&uiInfo.serverStatus
                                        .motd[uiInfo.serverStatus.motdOffset],
                                   scale, 1);
        ...
        Text_Paint_Limit(
            &maxX, uiInfo.serverStatus.motdPaintX, rect->y + rect->h - 3, scale,
            color, &uiInfo.serverStatus.motd[uiInfo.serverStatus.motdOffset], 0,
            0);
        if (uiInfo.serverStatus.motdPaintX2 >= 0) {
            float maxX2 = rect->x + rect->w - 2;
            Text_Paint_Limit(&maxX2, uiInfo.serverStatus.motdPaintX2,
...
#define UI_GL_INFO_TEXTLIMIT 60
    Text_Paint(rect->x + 2, rect->y, scale, color,
               va("VENDOR: %s", uiInfo.uiDC.glconfig.vendor_string), 0,
               UI_GL_INFO_TEXTLIMIT, textStyle);
    Text_Paint(rect->x + 2, rect->y + 15, scale, color,
               ...
               0, UI_GL_INFO_TEXTLIMIT, textStyle);
    ...
    // in TA this was not directly crashing, but displaying a nasty broken
    // shader right in the middle brought down the string size to 1024, there's
    // not much that can be shown on the screen anyway
    ...
    i = 0;
    while (i < numLines) {
        Text_Paint(rect->x + 2, y, scale, color, lines[i++], 0,
                   UI_GL_INFO_TEXTLIMIT / 2 - 1, textStyle);
        if (i < numLines) {
            Text_Paint(rect->x + rect->w / 2, y, scale, color, lines[i++], 0,
                       UI_GL_INFO_TEXTLIMIT / 2, textStyle);
        }
        y += 10;
...
// FIXME: table drive
//
static void UI_OwnerDraw(itemDef_t *item, float x, float y, float w, float h,
    ...
    case UI_MAPDESC_LONGNAME:
        Text_Paint(rect.x, rect.y, scale, color,
                   uiInfo.mapList[ui_currentNetMap.integer].mapName, 0, 0,
                   textStyle);
        break;

    case UI_MAPDESC_AUTHOR:
        Text_Paint(rect.x, rect.y, scale, color,
                   uiInfo.mapList[ui_currentNetMap.integer].author, 0, 0,
                   textStyle);
        break;
    ...
    case UI_JOIN_RED:
        ...
        Text_Paint(rect.x, rect.y, scale, color, s, 0, 0, textStyle);
        break;
    ...
    case UI_MAPDESC_LINE6:
        Text_Paint(rect.x, rect.y, scale, color,
                   uiInfo.mapList[ui_currentNetMap.integer]
                       .description[ownerDraw - UI_MAPDESC_LINE1],
                   0, 0, textStyle);
        break;
...
static qboolean UI_OwnerDrawVisible(int flags) {
    ...
    while (flags) {
        ...
        if (flags & UI_SHOW_LEADER) {
            // these need to show when this client can give orders to a player
            // or a group
            if (!uiInfo.teamLeader) {
                vis = qfalse;
            } else {
        ...
        if (flags & UI_SHOW_NOTLEADER) {
            // these need to show when this client is assigning their own status
            // or they are NOT the leader
            if (uiInfo.teamLeader) {
                ...
                // these need to show when this client can give orders to a
                // player or a group
            }
            flags &= ~UI_SHOW_NOTLEADER;
        }
        if (flags & UI_SHOW_FAVORITESERVERS) {
            // this assumes you only put this type of display flag on something
            // showing in the proper context
...
static qboolean UI_NetGameType_HandleKey(int flags, float *special, int key) {
    if (key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER ||
        ...
        Menu_SetFeederSelection(NULL, FEEDER_ALLMAPS, ui_mapIndex.integer,
                                NULL);
        return qtrue;
    }
    return qfalse;
...
static qboolean UI_Crosshair_HandleKey(int flags, float *special, int key) {
    if (key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER ||
        key == K_KP_ENTER || key == K_MWHEELUP || key == K_MWHEELDOWN) {
...
static float UI_GetValue(int ownerDraw) { return 0; }
...
static int QDECL UI_ServersQsortCompare(const void *arg1, const void *arg2) {
    return trap_LAN_CompareServers(
        UI_SourceForLAN(), uiInfo.serverStatus.sortKey,
...
static void UI_StartSinglePlayer(void) {
        ...
        trap_Cvar_SetValue( "g_gametype", Com_Clamp( 0, GT_MAX_GAME_TYPE-1,
tierList[i].gameTypes[j] ) ); trap_Cmd_ExecuteText( EXEC_APPEND, va( "wait ;
wait ; map %s\n", tierList[i].maps[j] ) ); skill = trap_Cvar_VariableValue(
"g_spSkill" );

        if (j == MAPS_PER_TIER-1) {
                k =
UI_TeamIndexFromName(UI_Cvar_VariableString("ui_opponentName")); Com_sprintf(
buff, sizeof(buff), "wait ; addbot %s %i %s 250 %s\n",
UI_AIFromName(teamList[k].teamMembers[0]), skill, "",
teamList[k].teamMembers[0]); } else { k =
UI_TeamIndexFromName(UI_Cvar_VariableString("ui_opponentName")); for (i = 0; i <
PLAYERS_PER_TEAM; i++) { Com_sprintf( buff, sizeof(buff), "wait ; addbot %s %i
%s 250 %s\n", UI_AIFromName(teamList[k].teamMembers[i]), skill, "Blue",
teamList[k].teamMembers[i]); trap_Cmd_ExecuteText( EXEC_APPEND, buff );
                }

                k = UI_TeamIndexFromName(UI_Cvar_VariableString("ui_teamName"));
                for (i = 1; i < PLAYERS_PER_TEAM; i++) {
                        Com_sprintf( buff, sizeof(buff), "wait ; addbot %s %i %s
250 %s\n", UI_AIFromName(teamList[k].teamMembers[i]), skill, "Red",
teamList[k].teamMembers[i]); trap_Cmd_ExecuteText( EXEC_APPEND, buff );
                }
...
static void UI_LoadDemos(void) {
    ...
    uiInfo.demoCount =
        trap_FS_GetFileList("demos", demoExt, demolist, ARRAY_LEN(demolist));
    ...
    for (j = 0; j < 2; j++) {
        ...
        if (!j) {
            if (protocolLegacy > 0 && uiInfo.demoCount < MAX_DEMOS) {
                Com_sprintf(demoExt, sizeof(demoExt), ".%s%d", DEMOEXT,
                            protocolLegacy);
                uiInfo.demoCount += trap_FS_GetFileList(
                    "demos", demoExt, demolist, ARRAY_LEN(demolist));
                demoname = demolist;
            } else
                break;
        }
...
static void UI_RunMenuScript(char **args) {
    ...
    if (String_Parse(args, &name)) {
        if (Q_stricmp(name, "StartServer") == 0 ||
            ...
            trap_Cvar_Set("ui_mapIndex", va("%d", ui_mapIndex.integer));
            Menu_SetFeederSelection(NULL, FEEDER_MAPS, ui_mapIndex.integer,
                                    "skirmish");
            ...
            if (Int_Parse(args, &sortColumn)) {
                // if same column we're already sorting on then flip the
                // direction
            ...
            if (uiInfo.mapCycleIndex >= 0 &&
                uiInfo.mapCycleIndex < uiInfo.mapCycleCount) {
                trap_Cmd_ExecuteText(
                    EXEC_APPEND, va("callvote mapcycle %s\n",
            ...
            if (ui_netSource.integer != UIAS_FAVORITES) {
                ...
                name[0] = addr[0] = '\0';
                Q_strncpyz(name, Info_ValueForKey(buff, "hostname"),
                           sizeof(name));
            ...
            Q_strncpyz(name, Info_ValueForKey(info, "hostname"),
                       MAX_NAME_LENGTH);
            ...
            addr[0] = '\0';
            Q_strncpyz(addr, UI_Cvar_VariableString("ui_specifyAddress"),
                       MAX_NAME_LENGTH);
            ...
            if (String_Parse(args, &orders)) {
                int selectedPlayer =
                    trap_Cvar_VariableValue("cg_selectedPlayer");
            ...
            trap_Cvar_Set("ui_sv_mapname",
                          Info_ValueForKey(serverinfo, "mapname"));
            trap_Cvar_Set("ui_sv_gametype",
                          Info_ValueForKey(serverinfo, "g_gametype"));
            trap_Cvar_Set("ui_sv_scorelimit",
                          Info_ValueForKey(serverinfo, "scorelimit"));
            trap_Cvar_Set("ui_sv_fraglimit",
                          Info_ValueForKey(serverinfo, "fraglimit"));
            trap_Cvar_Set("ui_sv_duellimit",
                          Info_ValueForKey(serverinfo, "duellimit"));
            trap_Cvar_Set("ui_sv_timelimit",
                          Info_ValueForKey(serverinfo, "timelimit"));
            trap_Cvar_Set("ui_sv_redteamname",
                          Info_ValueForKey(serverinfo, "g_redteam"));
            trap_Cvar_Set("ui_sv_blueteamname",
                          Info_ValueForKey(serverinfo, "g_blueteam"));
            trap_Cvar_Set("ui_sv_protocol",
                          Info_ValueForKey(serverinfo, "protocol"));
            trap_Cvar_Set("ui_sv_maxclients",
                          Info_ValueForKey(serverinfo, "sv_maxclients"));
            trap_Cvar_Set("ui_sv_hostname",
                          Info_ValueForKey(serverinfo, "sv_hostname"));
            trap_Cvar_Set("ui_sv_needpass",
                          Info_ValueForKey(serverinfo, "g_needpass"));
            trap_Cvar_Set("ui_sv_dmflags",
                          Info_ValueForKey(serverinfo, "dmflags"));
            trap_Cvar_Set("ui_sv_bot_minplayers",
                          Info_ValueForKey(serverinfo, "bot_minplayers"));
            trap_Cvar_Set("ui_sv_version",
                          Info_ValueForKey(serverinfo, "version"));
            trap_Cvar_Set("ui_sv_sg_version",
                          Info_ValueForKey(serverinfo, "sg_version"));
        }
        else {
...
static int UI_MapCountByGameType(qboolean singlePlayer) {
    ...
    c = 0;
    game = uiInfo
               .gameTypes[singlePlayer ? ui_gameType.integer
                                       : ui_netGameType.integer]
               .gtEnum;
    ...
    if (force) {
        ...
        for (i = 0; i < count; i++) {
            trap_LAN_GetServerInfo(ui_netSource.integer, i, info,
                                   MAX_STRING_CHARS);
            // The second string of the comparison should translate in BASEGAME
            // - and it does for normal SG
            if (!Q_stricmp(
                    Info_ValueForKey(info, "game"),
                    serverFilters[ui_serverFilterType.integer].basedir)) {
    ...
    for (i = 0; i < count; i++) {
        ...
        if (ping > 0 || ui_netSource.integer == UIAS_FAVORITES) {
            ...
            if (uiInfo.joinGameTypes[ui_joinGameType.integer].gtEnum != -1) {
                ...
                if (game !=
                    uiInfo.joinGameTypes[ui_joinGameType.integer].gtEnum) {
                    trap_LAN_MarkServerVisible(lanSource, i, qfalse);
                    continue;
                }
            }
    ...
    if (trap_LAN_ServerStatus(serverAddress, info->text, sizeof(info->text))) {
        ...
        if (info->numLines < MAX_SERVERSTATUS_LINES - 3) {
            ...
            while (p && *p) {
                ...
                *p++ = '\0';
                name = p;
                Com_sprintf(&info->pings[len], sizeof(info->pings) - len, "%d",
                            i);
    ...
    for (i = 0; i < MAX_SERVERSTATUSREQUESTS; i++) {
        ...
        if (uiInfo.pendingServerStatus.server[i].valid) {
            ...
            if (UI_GetServerStatusInfo(
                    uiInfo.pendingServerStatus.server[i].adrstr, &info)) {
                //
                numFound++;
                ...
                for (j = 0; j < info.numLines; j++) {
                    ...
                    if (stristr(name, uiInfo.findPlayerName)) {
                        // add to found server list if we have space (always
                        // leave space for a line with the number found)
                        if (uiInfo.numFoundPlayerServers <
                            MAX_FOUNDPLAYER_SERVERS - 1) {
                            //
                            Q_strncpyz(
                                uiInfo.foundPlayerServerAddresses
                                    [uiInfo.numFoundPlayerServers - 1],
                ...
                Com_sprintf(
                    uiInfo.foundPlayerServerNames[uiInfo.numFoundPlayerServers -
                                                  1],
                    sizeof(uiInfo.foundPlayerServerNames
                               [uiInfo.numFoundPlayerServers - 1]),
                    "searching %d/%d...", uiInfo.pendingServerStatus.num,
                    numFound);
        ...
        if (!uiInfo.pendingServerStatus.server[i].valid ||
            ...
            // reuse pending slot
            uiInfo.pendingServerStatus.server[i].valid = qfalse;
            // if we didn't try to get the status of all servers in the main
            // browser yet
            if (uiInfo.pendingServerStatus.num <
                ...
                trap_LAN_GetServerAddressString(
                    lanSource,
                    uiInfo.serverStatus
                        .displayServers[uiInfo.pendingServerStatus.num],
                    uiInfo.pendingServerStatus.server[i].adrstr,
...
static void UI_FixMapIndex(int force_map) {
    ...
    if (force_map >= 0 && force_map < uiInfo.mapCount) {
        sharedMap = force_map;
        // Com_Printf( "sharedMap forced to %d for
    ...
    if (sharedMap >= uiInfo.mapCount) {
        ...
        Com_sprintf(mapname, sizeof(mapname), "%s",
                    uiInfo.mapList[sharedMap].mapLoadName);
    }
    // Com_Printf( "Trying to fix sharedMap for map %s, was
    // %d\n",mapname,sharedMap);
    ...
    *handle = -1;
    if (feederID == FEEDER_HEADS) {
        ...
        if (index >= 0 && index < uiInfo.serverStatus.numDisplayServers) {
            ...
            if (ping == -1) {
                // if we ever see a ping that is out of date, do a server
                // refresh UI_UpdatePendingPings();
            }
            switch (column) {
            case SORT_HOST:
                if (ping <= 0) {
            ...
            if (uiInfo.characterList[index].headImage == -1) {
                uiInfo.characterList[index].headImage =
                    trap_R_RegisterShaderNoMip(
                        uiInfo.characterList[index].imageName);
            }
...
static void UI_FeederSelection(float feederID, int index) {
    ...
    if (feederID == FEEDER_Q3HEADS) {
        ...
        int actual, map;
        map = (feederID == FEEDER_ALLMAPS) ? ui_currentNetMap.integer
                                           : ui_currentMap.integer;
        if (uiInfo.mapList[map].cinematic >= 0) {
        ...
        if (feederID == FEEDER_MAPS) {
            ...
            // We pass here also when switching gametype now but that's
            // supported
...
static qboolean GameType_Parse(char **p, qboolean join) {
    ...
    while (1) {
        ...
        if (token[0] == '{') {
            ...
            if (join) {
                if (!String_Parse(p,
                                  &uiInfo.joinGameTypes[uiInfo.numJoinGameTypes]
                                       .gameType) ||
                    !Int_Parse(p, &uiInfo.joinGameTypes[uiInfo.numJoinGameTypes]
                                       .gtEnum)) {
                    return qfalse;
                }
            } else {
                if (!String_Parse(
                        p, &uiInfo.gameTypes[uiInfo.numGameTypes].gameType) ||
                    !Int_Parse(p,
                               &uiInfo.gameTypes[uiInfo.numGameTypes].gtEnum)) {
                    return qfalse;
                }
            }
...
static void UI_BuildQ3Model_List(void) {
    ...
    for (i = 0; i < numdirs && uiInfo.q3HeadCount < MAX_PLAYERMODELS;
        ...
        numfiles = trap_FS_GetFileList(va("models/wq3_players/%s", dirptr),
                                       "tga", filelist, 2048);
        fileptr = filelist;
        for (j = 0; j < numfiles && uiInfo.q3HeadCount < MAX_PLAYERMODELS;
             j++, fileptr += filelen + 1) {
...
void _UI_Init(qboolean inGameLoad) {
    ...
    if (trap_Cvar_VariableValue("cg_farclip") != 0 &&
        trap_Cvar_VariableValue("ui_farclip") == 0) {
        // trap_Cvar_SetValue("ui_farclip",
        // trap_Cvar_VariableValue("cg_farclipValue")*2-1);
    }
...
void _UI_SetActiveMenu(uiMenuCommand_t menu) {
    ...
    if (Menu_Count() > 0) {
        ...
        case UIMENU_MAIN:
            ...
            // trap_S_RegisterSound("sound/misc/menu_background.wav", qfalse) ,
            // CHAN_LOCAL_SOUND );
        ...
        if (width > xmax) {
            ...
            *s2 = '\0';
            ...
            if (c_bcp == '\0') {
                ...
                // even if the word is too long, we would overflow it (see
                // above) so just print it now if needed
                s2++;
                if (*s2 != '\0') // if we are printing an overflowing line we
                                 // have s2 == s3
...
static void UI_DisplayDownloadInfo(const char *downloadName, float centerPoint,
    ...
    if (downloadCount < 4096 || !downloadTime) {
        ...
        Text_PaintCenter(leftWidth, 345, 0.3f, colorWhite,
                         va("(%s of %s copied)", dlSizeBuf, totalSizeBuf), 0);
        Text_PaintCenter(leftWidth, 420, 0.3f, colorWhite,
                         va("%s bytes/Sec", xferRateBuf), 0);
    } else {
        if ((uiInfo.uiDC.realTime - downloadTime) / 1000) {
            xferRate =
                downloadCount / ((uiInfo.uiDC.realTime - downloadTime) / 1000);
        } else {
            xferRate = 0;
        }
        ...
        if (downloadSize && xferRate) {
            int n = downloadSize /
                    xferRate; // estimated time for entire d/l in secs

            // We do it in K (/1024) because we'd overflow around 4MB
            UI_PrintTime(
                dlTimeBuf, sizeof dlTimeBuf,
                (n - (((downloadCount / 1024) * n) / (downloadSize / 1024))) *
                    1000);

            Text_PaintCenter(leftWidth, 330, 0.3f, colorWhite, dlTimeBuf, 0);
            Text_PaintCenter(leftWidth, 345, 0.3f, colorWhite,
                             va("(%s of %s copied)", dlSizeBuf, totalSizeBuf),
                             0);
        } else {
            Text_PaintCenter(leftWidth, 330, 0.3f, colorWhite, "estimating", 0);
            if (downloadSize) {
                Text_PaintCenter(
                    leftWidth, 345, 0.3f, colorWhite,
                    va("(%s of %s copied)", dlSizeBuf, totalSizeBuf), 0);
            } else {
                Text_PaintCenter(leftWidth, 345, 0.3f, colorWhite,
...
void UI_DrawConnectScreen(qboolean overlay) {
    ...
    if (trap_GetConfigString(CS_SERVERINFO, info, sizeof(info))) {
        Text_PaintCenter(centerPoint, yStart, scale, colorWhite,
                         va("Loading %s", Info_ValueForKey(info, "mapname")),
                         0);
    }
    ...
    Text_PaintCenter_AutoWrapped(
        centerPoint, yStart + 276, 630, 25, scale, colorYellow,
        Info_ValueForKey(cstate.updateInfoString, "motd"), 0);
    // print any server info (server full, bad version, etc)
    if (cstate.connState < CA_CONNECTED) {
        Text_PaintCenter_AutoWrapped(centerPoint, yStart + 176, 630, 20, scale,
                                     colorWhite, cstate.messageString, 0);
    }
...
static void UI_StopServerRefresh(void) {
    ...
    if (count - uiInfo.serverStatus.numDisplayServers > 0) {
        Com_Printf("%d servers not listed due to packet loss or pings higher "
                   "than %d\n",
        ...
        if (strlen(ptr)) {
            if (ui_netSource.integer == UIAS_GLOBALS)
                ...
                trap_Cmd_ExecuteText(EXEC_NOW,
                                     va("globalservers %d %s full empty\n",
            ...
            if (ui_netSource.integer == UIAS_GLOBALS)
                ...
                trap_Cmd_ExecuteText(
                    EXEC_NOW, va("globalservers %d %d full empty\n",
