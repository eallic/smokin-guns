...
qboolean PC_Script_Parse(int handle, const char **out) {
    ...
    // command, arg.. basically we want everything between the { } as it will be
...
void Window_Paint(Window *w, float fadeAmount, float fadeClamp,
    ...
    if (w->style == WINDOW_STYLE_FILLED) {
        ...
        if (w->cinematic == -1) {
            w->cinematic =
                DC->playCinematic(w->cinematicName, fillRect.x, fillRect.y,
...
qboolean Item_SetFocus(itemDef_t *item, float x, float y) {
    ...
    menuDef_t *parent;
    // sanity check, non-null, not a decoration and does not already have the
    // focus
...
int Item_ListBox_ThumbDrawPosition(itemDef_t *item) {
    ...
    if (itemCapture == item) {
        if (item->window.flags & WINDOW_HORIZONTAL) {
            ...
            max = item->window.rect.x + item->window.rect.w -
                  2 * SCROLLBAR_SIZE - 1;
            ...
            max = item->window.rect.y + item->window.rect.h -
                  2 * SCROLLBAR_SIZE - 1;
    ...
    if (item->window.flags & WINDOW_HORIZONTAL) {
        if (!(item->window.flags &
            ...
            if (listPtr->elementStyle == LISTBOX_IMAGE) {
                ...
                if (Rect_ContainsPoint(&r, x, y)) {
                    listPtr->cursorPos =
                        (int)((x - r.x) / listPtr->elementWidth) +
                        listPtr->startPos;
        ...
        if (Rect_ContainsPoint(&r, x, y)) {
            listPtr->cursorPos = (int)((y - 2 - r.y) / listPtr->elementHeight) +
                                 listPtr->startPos;
            if (listPtr->cursorPos > listPtr->endPos) {
                listPtr->cursorPos = listPtr->endPos;
            }
...
qboolean Item_TextField_HandleKey(itemDef_t *item, int key) {
    ...
    if (item->cvar) {
        ...
        if (key & K_CHAR_FLAG) {
            ...
            if (key == K_END ||
                key == K_KP_END) { // ( tolower(key) == 'e' && trap_Key_IsDown(
                                   // K_CTRL ) ) ) {
                item->cursorPos = len;
...
static void Scroll_ListBox_AutoFunc(void *p) {
    ...
    if (DC->realTime > si->nextScrollTime) {
        ...
        // this is done a bit sideways as the autoscroll "knows" that the item
        // is a listbox so it calls it directly
...
static void Scroll_ListBox_ThumbFunc(void *p) {
    ...
    if (si->item->window.flags & WINDOW_HORIZONTAL) {
        ...
        r.y = si->item->window.rect.y + si->item->window.rect.h -
              SCROLLBAR_SIZE - 1;
        r.h = SCROLLBAR_SIZE;
        r.w = si->item->window.rect.w - (SCROLLBAR_SIZE * 2) - 2;
        max = Item_ListBox_MaxScroll(si->item);
        //
        pos = (DC->cursorx - r.x - SCROLLBAR_SIZE / 2) * max /
              (r.w - SCROLLBAR_SIZE);
        if (pos < 0) {
            pos = 0;
        } else if (pos > max) {
        ...
        r.x = si->item->window.rect.x + si->item->window.rect.w -
              SCROLLBAR_SIZE - 1;
        ...
        max = Item_ListBox_MaxScroll(si->item);
        //
        pos = (DC->cursory - r.y - SCROLLBAR_SIZE / 2) * max /
              (r.h - SCROLLBAR_SIZE);
        if (pos < 0) {
            pos = 0;
        } else if (pos > max) {
    ...
    case ITEM_TYPE_LISTBOX: {
        ...
        if (flags & (WINDOW_LB_LEFTARROW | WINDOW_LB_RIGHTARROW)) {
            ...
            scrollInfo.scrollDir =
                (flags & WINDOW_LB_LEFTARROW) ? qtrue : qfalse;
...
qboolean Item_Slider_HandleKey(itemDef_t *item, int key, qboolean down) {
    ...
    if (item->window.flags & WINDOW_HASFOCUS && item->cvar &&
        Rect_ContainsPoint(&item->window.rect, DC->cursorx, DC->cursory)) {
        if (key == K_MOUSE1 || key == K_ENTER || key == K_MOUSE2 ||
            key == K_MOUSE3 || key == K_MWHEELUP || key == K_MWHEELDOWN) {
            editFieldDef_t *editDef = item->typeData;
            if (editDef) {
                rectDef_t testRect;
                width = SLIDER_WIDTH;
                ...
                if (Rect_ContainsPoint(&testRect, DC->cursorx, DC->cursory)) {
                    ...
                    if (key == K_MWHEELUP || key == K_MWHEELDOWN) {
                        float step =
                            (editDef->maxVal - editDef->minVal) * 0.01f;
                        value = DC->getCVarValue(item->cvar);
                        if (key == K_MWHEELUP)
                            value += step;
                        else
    ...
    while (menu->cursorItem > -1) {
        ...
        if (Item_SetFocus(menu->items[menu->cursorItem], DC->cursorx,
            ...
            Menu_HandleMouseMove(
                menu, menu->items[menu->cursorItem]->window.rect.x + 1,
...
void Menus_HandleOOBClick(menuDef_t *menu, int key, qboolean down) {
    if (menu) {
        int i;
        // basically the behaviour we are looking for is if there are windows in
        // the stack.. see if the cursor is within any of them.. if not close
        // them otherwise activate them and pass the key on.. force a mouse move
        // to activate focus and script stuff
...
void Menu_HandleKey(menuDef_t *menu, int key, qboolean down) {
    ...
    // see if the mouse is within the window bounds and if so is this a mouse
    // click
    ...
    case K_MWHEELDOWN:
        if (item) {
            if (item->type == ITEM_TYPE_TEXT) {
                if (Rect_ContainsPoint(Item_CorrectedTextRect(item),
...
void Item_Text_Paint(itemDef_t *item) {
    ...
    /*
       ...
       &DC->Assets.shadowColor[3], DC->Assets.fadeClamp, &item->window.nextTime,
       DC->Assets.fadeCycle, qfalse); DC->drawText(item->textRect.x +
       DC->Assets.shadowX, item->textRect.y + DC->Assets.shadowY,
       item->textscale, DC->Assets.shadowColor, textPtr, adjust);
            }
    */
...
void Item_TextField_Paint(itemDef_t *item) {
    ...
    if (item->window.flags & WINDOW_HASFOCUS && g_editingField) {
        ...
        DC->drawText(item->textRect.x + item->textRect.w + offset,
                     item->textRect.y, item->textscale, newColor,
                     buff + editPtr->paintOffset, 0, editPtr->maxPaintChars,
                     item->textStyle);
    }
}
...
void Item_YesNo_Paint(itemDef_t *item) {
    ...
    if (item->text) {
        ...
        DC->drawText(item->textRect.x + item->textRect.w + 8, item->textRect.y,
                     item->textscale, newColor, (value != 0) ? "Yes" : "No", 0,
                     0, item->textStyle);
    } else {
        DC->drawText(item->textRect.x, item->textRect.y, item->textscale,
                     newColor, (value != 0) ? "Yes" : "No", 0, 0,
                     item->textStyle);
    }
}
...
static bind_t g_bindings[] = {{"+scores", K_TAB, -1, -1, -1},
                              {"+button2", 'e', -1, -1, -1},
                              {"+speed", K_SHIFT, -1, -1, -1},
...
void Controls_GetConfig(void) {
    ...
    // s_controls.smoothmouse.curvalue  = UI_ClampCvar( 0, 1,
    // Controls_GetCvarValue( "m_filter" ) ); s_controls.alwaysrun.curvalue    =
    // UI_ClampCvar( 0, 1, Controls_GetCvarValue( "cl_run" ) );
    // s_controls.autoswitch.curvalue   = UI_ClampCvar( 0, 1,
    // Controls_GetCvarValue( "cg_autoswitch" ) );
    // s_controls.sensitivity.curvalue  = UI_ClampCvar( 2, 30,
    // Controls_GetCvarValue( "sensitivity" ) ); s_controls.joyenable.curvalue
    // = UI_ClampCvar( 0, 1, Controls_GetCvarValue( "in_joystick" ) );
    // s_controls.joythreshold.curvalue = UI_ClampCvar( 0.05, 0.75,
    // Controls_GetCvarValue( "joy_threshold" ) ); s_controls.freelook.curvalue
...
void Controls_SetDefaults(void) {
    ...
    // s_controls.invertmouse.curvalue  = Controls_GetCvarDefault( "m_pitch" ) <
    // 0; s_controls.smoothmouse.curvalue  = Controls_GetCvarDefault( "m_filter"
    // ); s_controls.alwaysrun.curvalue    = Controls_GetCvarDefault( "cl_run"
    // ); s_controls.autoswitch.curvalue   = Controls_GetCvarDefault(
    // "cg_autoswitch" ); s_controls.sensitivity.curvalue  =
    // Controls_GetCvarDefault( "sensitivity" ); s_controls.joyenable.curvalue
    // = Controls_GetCvarDefault( "in_joystick" );
    // s_controls.joythreshold.curvalue = Controls_GetCvarDefault(
    // "joy_threshold" ); s_controls.freelook.curvalue     =
    // Controls_GetCvarDefault( "cl_freelook" );
}
...
void Item_Slider_Paint(itemDef_t *item) {
    ...
    DC->drawHandlePic(x - (SLIDER_THUMB_WIDTH / 2), y - 2, SLIDER_THUMB_WIDTH,
...
void Item_Bind_Paint(itemDef_t *item) {
    ...
    if (item->text) {
        ...
        DC->drawText(item->textRect.x, item->textRect.y, item->textscale,
                     newColor, "FIXME", 0, maxChars, item->textStyle);
    }
}
...
void Item_ListBox_Paint(itemDef_t *item) {
    ...
    // the listbox is horizontal or vertical and has a fixed size scroll bar
    // going either direction elements are enumerated from the DC and either
    // text or image handles are acquired from the DC as well textscale is used
    // to size the text, textalignx and textaligny are used to size image
    // elements there is no clipping available so only the last completely
    // visible item is painted
            ...
            for (i = listPtr->startPos; i < count; i++) {
                ...
                if (listPtr->numColumns > 0) {
                    ...
                    for (j = 0; j < listPtr->numColumns; j++) {
                        text = DC->feederItemText(item->special, i, j,
                                                  &optionalImage);
                        if (optionalImage >= 0) {
                            DC->drawHandlePic(
                    ...
                    text =
                        DC->feederItemText(item->special, i, 0, &optionalImage);
                    if (optionalImage >= 0) {
                        // DC->drawHandlePic(x + 4 + listPtr->elementHeight, y,
                        // listPtr->columnInfo[j].width,
                        // listPtr->columnInfo[j].width, optionalImage);
                    } else if (text) {
                        DC->drawText(x + 4, y + listPtr->elementHeight,
                                     item->textscale, item->window.foreColor,
                ...
                if (i == item->cursorPos) {
                    DC->fillRect(
                        x + 2, y + 2, item->window.rect.w - SCROLLBAR_SIZE - 4,
...
void Item_OwnerDraw_Paint(itemDef_t *item) {
    ...
    if (DC->ownerDrawItem) {
        ...
        if (item->numColors > 0 && DC->getValue) {
            // if the value is within one of the ranges then set color to that,
        ...
        if (item->text) {
            Item_Text_Paint(item);
            if (item->text[0]) {
                // +8 is an offset kludge to properly align owner draw items
                // that have text combined with them
                DC->ownerDrawItem(item, item->textRect.x + item->textRect.w + 8,
                                  item->window.rect.y, item->window.rect.w,
                                  item->window.rect.h, 0, item->textaligny,
                                  item->window.ownerDraw,
                                  item->window.ownerDrawFlags, item->alignment,
                                  item->special, item->textscale, color,
                                  item->window.background, item->textStyle);
            } else {
                DC->ownerDrawItem(item, item->textRect.x + item->textRect.w,
                                  item->window.rect.y, item->window.rect.w,
                                  item->window.rect.h, 0, item->textaligny,
                                  item->window.ownerDraw,
                                  item->window.ownerDrawFlags, item->alignment,
            ...
            DC->ownerDrawItem(item, item->window.rect.x, item->window.rect.y,
        ...
        for (i = 0; i < menu->itemCount; i++) {
            if (menu->items[i]->special == feeder) {
                Item_ListBox_HandleKey(menu->items[i],
                                       (down) ? K_DOWNARROW : K_UPARROW, qtrue,
                                       qtrue);
                return;
            }
        }
            ...
            if (menu->items[i]->special == feeder) {
                if (index == 0) {
                    listBoxDef_t *listPtr =
                        (listBoxDef_t *)menu->items[i]->typeData;
                    listPtr->cursorPos = 0;
                    listPtr->startPos = 0;
                }
                ...
                //  Tequila comment: Update map list positions to show the
                //  selected item
                if (feeder == FEEDER_ALLMAPS || feeder == FEEDER_MAPS) {
                    ...
                    int pad = (count - max) >> 1;
                    // Com_Printf("Focusing list on index %d (max for startPos
                    // %d, count %d, computed pad %d)\n",index,max,count,pad);
                    if (index >= max)
                        listPtr->startPos = max;
                    else if (index > pad)
...
void Menu_HandleMouseMove(menuDef_t *menu, float x, float y) {
    ...
    // need a better overall solution as i don't like going through everything
    // twice
                ...
                if (pass == 1) {
                    ...
                    if (overItem->type == ITEM_TYPE_TEXT && overItem->text) {
                        if (!Rect_ContainsPoint(
                                Item_CorrectedTextRect(overItem), x, y)) {
                            continue;
                        }
                    }
...
void Menu_Paint(menuDef_t *menu, qboolean forcePaint) {
    ...
    if (menu->fullScreen) {
        ...
        // FIXME: make sure we have a default shader if fullscreen is set with
        // no background
        DC->drawHandlePic(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT,
                          menu->window.background);
    } else if (menu->window.background) {
        // this allows a background shader without being full screen
        // UI_DrawHandlePic(menu->window.rect.x, menu->window.rect.y,
        // menu->window.rect.w, menu->window.rect.h, menu->backgroundShader);
    }
    ...
    if (debugMode) {
        ...
        DC->drawRect(menu->window.rect.x, menu->window.rect.y,
                     menu->window.rect.w, menu->window.rect.h, 1, color);
    }
}
...
qboolean ItemParse_alAvailableDevicesList(itemDef_t *item, int handle) {
    ...
    trap_Cvar_VariableStringBuffer("s_alAvailableDevices", devices,
                                   sizeof(devices));
    text_p = devices;
    ...
    while (1) {
        ...
        if (!token[0]) {
            multiPtr->cvarList[multiPtr->count] =
                multiPtr->cvarStr[multiPtr->count] = String_Alloc(device);
            multiPtr->count++;
...
static void Item_ApplyHacks(itemDef_t *item) {
    ...
    if (item->type == ITEM_TYPE_EDITFIELD && item->cvar &&
        !Q_stricmp(item->cvar, "ui_favoriteAddress")) {
        editFieldDef_t *editField = (editFieldDef_t *)item->typeData;

        // enough to hold an IPv6 address plus null
        if (editField->maxChars < 48) {
            Com_Printf("Extended create favorite address edit field length to "
    ...
    if (item->type == ITEM_TYPE_EDITFIELD && item->cvar &&
        (!Q_stricmp(item->cvar, "ui_Name") ||
         !Q_stricmp(item->cvar, "ui_findplayer"))) {
        editFieldDef_t *editField = (editFieldDef_t *)item->typeData;
        ...
        if (editField->maxChars < MAX_NAME_LENGTH) {
            ...
            Com_Printf(
                "Extended player name field using cvar %s to %d characters\n",
...
qboolean MenuParse_itemDef(itemDef_t *item, int handle) {
    ...
    if (menu->itemCount < MAX_MENUITEMS) {
        ...
        PC_SourceError(handle,
                       "itemDef exceeds the MAX_MENUITEMS (=" XSTRING(
                           MAX_MENUITEMS) ") maximum menu item definition");
    return qtrue;
}
        ...
        if (Rect_ContainsPoint(&menu->window.rect, x, y)) {
            ...
            for (i = 0; i < menu->itemCount; i++) {
                ...
                if (Rect_ContainsPoint(&menu->items[i]->window.rect, x, y)) {
                    ...
                    if (overItem->type == ITEM_TYPE_TEXT && overItem->text) {
                        if (Rect_ContainsPoint(Item_CorrectedTextRect(overItem),
                                               x, y)) {
                            return qtrue;
                        } else {
                            continue;
