...
static void IN_PrintKey(const SDL_keysym *keysym, keyNum_t key, qboolean down) {
    ...
    Com_Printf("0x%02x \"%s\"", keysym->scancode, SDL_GetKeyName(keysym->sym));
...
===============
*/
static const char *IN_TranslateSDLToQ3Key(SDL_keysym *keysym, keyNum_t *key,
                                          qboolean down) {
    ...
    *key = 0;
    ...
    if (down && strlen(Key_KeynumToString(*key)) == 1 && keysym->unicode == 0) {
        if (in_keyboardDebug->integer)
...
static io_connect_t IN_GetIOHandle(void) // mac os x mouse accel hack
{
    ...
    status = IOServiceOpen(iohidsystem, mach_task_self(),
                           kIOHIDParamConnectType, &iohandle);
...
static void IN_ActivateMouse(void) {
    ...
    if (!mouseActive) // mac os x mouse accel hack
    {
        ...
        if (in_disablemacosxmouseaccel->integer) {
            ...
            if (mouseDev != 0) {
                if (IOHIDGetAccelerationWithKey(
                        mouseDev, CFSTR(kIOHIDMouseAccelerationType),
                        &originalMouseSpeed) == kIOReturnSuccess) {
                    Com_Printf("previous mouse acceleration: %f\n",
                               originalMouseSpeed);
                    if (IOHIDSetAccelerationWithKey(
                            mouseDev, CFSTR(kIOHIDMouseAccelerationType),
                            -1.0) != kIOReturnSuccess) {
                        Com_Printf(
                            "Could not disable mouse acceleration (failed at "
                            "IOHIDSetAccelerationWithKey).\n");
                        Cvar_Set("in_disablemacosxmouseaccel", 0);
                    }
                } else {
                    Com_Printf("Could not disable mouse acceleration (failed "
                               "at IOHIDGetAccelerationWithKey).\n");
                    Cvar_Set("in_disablemacosxmouseaccel", 0);
                }
                IOServiceClose(mouseDev);
            } else {
                Com_Printf("Could not disable mouse acceleration (failed at "
                           "IO_GetIOHandle).\n");
                Cvar_Set("in_disablemacosxmouseaccel", 0);
            }
        }
...
static void IN_DeactivateMouse(void) {
        ...
        if (originalMouseSpeed != -1.0) {
            ...
            if (mouseDev != 0) {
                ...
                if (IOHIDSetAccelerationWithKey(
                        mouseDev, CFSTR(kIOHIDMouseAccelerationType),
                        originalMouseSpeed) != kIOReturnSuccess)
                    Com_Printf("Could not re-enable mouse acceleration (failed "
                               "at IOHIDSetAccelerationWithKey).\n");
                IOServiceClose(mouseDev);
            } else
                Com_Printf("Could not re-enable mouse acceleration (failed at "
                           "IO_GetIOHandle).\n");
        }
    }
#endif
    ...
    if (mouseActive) {
        ...
        if (SDL_GetAppState() & SDL_APPMOUSEFOCUS)
            SDL_WarpMouse(cls.glconfig.vidWidth / 2,
...
static int joy_keys[16] = {K_LEFTARROW, K_RIGHTARROW, K_UPARROW, K_DOWNARROW,
...
static int hat_keys[16] = {K_JOY29, K_JOY30, K_JOY31, K_JOY32, K_JOY25, K_JOY26,
                           K_JOY27, K_JOY28, K_JOY21, K_JOY22, K_JOY23, K_JOY24,
                           K_JOY17, K_JOY18, K_JOY19, K_JOY20};
    ...
    if (hats != stick_state.oldhats) {
        for (i = 0; i < 4; i++) {
            if (((Uint8 *)&hats)[i] != ((Uint8 *)&stick_state.oldhats)[i]) {
                ...
                case SDL_HAT_UP:
                    Com_QueueEvent(0, SE_KEY, hat_keys[4 * i + 0], qfalse, 0,
                                   NULL);
                    break;
                case SDL_HAT_RIGHT:
                    Com_QueueEvent(0, SE_KEY, hat_keys[4 * i + 1], qfalse, 0,
                                   NULL);
                    break;
                case SDL_HAT_DOWN:
                    Com_QueueEvent(0, SE_KEY, hat_keys[4 * i + 2], qfalse, 0,
                                   NULL);
                    break;
                case SDL_HAT_LEFT:
                    Com_QueueEvent(0, SE_KEY, hat_keys[4 * i + 3], qfalse, 0,
                                   NULL);
                    break;
                ...
                switch (((Uint8 *)&hats)[i]) {
                case SDL_HAT_UP:
                    Com_QueueEvent(0, SE_KEY, hat_keys[4 * i + 0], qtrue, 0,
                                   NULL);
                    break;
                case SDL_HAT_RIGHT:
                    Com_QueueEvent(0, SE_KEY, hat_keys[4 * i + 1], qtrue, 0,
                                   NULL);
                    break;
                case SDL_HAT_DOWN:
                    Com_QueueEvent(0, SE_KEY, hat_keys[4 * i + 2], qtrue, 0,
                                   NULL);
                    break;
                case SDL_HAT_LEFT:
                    Com_QueueEvent(0, SE_KEY, hat_keys[4 * i + 3], qtrue, 0,
                                   NULL);
                    break;
                case SDL_HAT_RIGHTUP:
                    Com_QueueEvent(0, SE_KEY, hat_keys[4 * i + 0], qtrue, 0,
...
static void IN_ProcessEvents(void) {
    ...
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_MOUSEMOTION:
            if (mouseActive)
                Com_QueueEvent(0, SE_MOUSE, e.motion.xrel, e.motion.yrel, 0,
                               NULL);
            break;
        ...
        case SDL_MOUSEBUTTONUP: {
            ...
            Com_QueueEvent(0, SE_KEY, b,
                           (e.type == SDL_MOUSEBUTTONDOWN ? qtrue : qfalse), 0,
                           NULL);
        } break;
...
void IN_Init(void) {
    ...
    in_disablemacosxmouseaccel =
        Cvar_Get("in_disablemacosxmouseaccel", "1", CVAR_ARCHIVE);
#endif
