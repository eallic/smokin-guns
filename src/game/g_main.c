...
static cvarTable_t gameCvarTable[] = {
    ...
    {&g_fraglimit, "fraglimit", "20",
     CVAR_SERVERINFO | CVAR_ARCHIVE | CVAR_NORESTART, 0, qtrue},
    {&g_timelimit, "timelimit", "0",
     CVAR_SERVERINFO | CVAR_ARCHIVE | CVAR_NORESTART, 0, qtrue},
    {&g_scorelimit, "scorelimit", "10",
     CVAR_SERVERINFO | CVAR_ARCHIVE | CVAR_NORESTART, 0, qtrue},
    {&g_duellimit, "duellimit", "3",
     CVAR_SERVERINFO | CVAR_ARCHIVE | CVAR_NORESTART, 0, qtrue},

    {&du_enabletrio, "du_enabletrio", "0",
     CVAR_SERVERINFO | CVAR_ARCHIVE | CVAR_NORESTART, 0, qtrue},
    {&du_forcetrio, "du_forcetrio", "0",
     CVAR_SERVERINFO | CVAR_ARCHIVE | CVAR_NORESTART, 0, qtrue},

    {&br_teamrole, "br_teamrole", "0", CVAR_ARCHIVE | CVAR_NORESTART, 0, qtrue},
    {&g_moneyRespawn, "g_moneyRespawn", "1", CVAR_ARCHIVE | CVAR_NORESTART, 0,
     qtrue},
    {&g_maxMoney, "g_maxMoney", "200",
     CVAR_SERVERINFO | CVAR_ARCHIVE | CVAR_LATCH, 0, qtrue},

    {&g_newShotgunPattern, "g_newShotgunPattern", "1",
     CVAR_SERVERINFO | CVAR_ARCHIVE | CVAR_LATCH, 0, qtrue},
    {&g_roundNoMoveTime, "g_roundNoMoveTime", "3",
     CVAR_SERVERINFO | CVAR_ARCHIVE | CVAR_NORESTART, 0, qtrue},
    ...
    {&g_maxteamkills, "g_maxteamkills", "0", CVAR_ARCHIVE | CVAR_NORESTART, 0,
     qtrue},
    {&g_teamkillsforgettime, "g_teamkillsforgettime", "0",
     CVAR_ARCHIVE | CVAR_NORESTART, 0, qtrue},
    {&g_teamkillschecktime, "g_teamkillschecktime", "0",
     CVAR_ARCHIVE | CVAR_NORESTART, 0, qtrue},
    ...
    {&g_allowVote, "g_allowVote", "1", CVAR_ARCHIVE | CVAR_SERVERINFO, 0,
     qfalse},
    {&g_allowVoteKick, "g_allowVoteKick", "1", CVAR_ARCHIVE | CVAR_SERVERINFO,
     0, qfalse},
    ...
    {&g_redteam, "g_redteam", DEFAULT_REDTEAM_NAME,
     CVAR_ARCHIVE | CVAR_SERVERINFO | CVAR_USERINFO, 0, qtrue, qtrue},
    {&g_blueteam, "g_blueteam", DEFAULT_BLUETEAM_NAME,
     CVAR_ARCHIVE | CVAR_SERVERINFO | CVAR_USERINFO, 0, qtrue, qtrue},
    ...
    // unlagged - server options
    {&g_delagHitscan, "g_delagHitscan", "1", CVAR_ARCHIVE | CVAR_SERVERINFO, 0,
     qtrue},
    {&g_truePing, "g_truePing", "1", CVAR_ARCHIVE, 0, qtrue},
    // it's CVAR_SYSTEMINFO so the client's sv_fps will be automagically set to
    // its value
    {&sv_fps, "sv_fps", "20", CVAR_SYSTEMINFO | CVAR_ARCHIVE, 0, qfalse},
    // unlagged - server options
    // Spoon
    {&g_roundtime, "g_roundtime", "4", CVAR_ARCHIVE | CVAR_SERVERINFO, 0,
     qtrue},
    {&sg_rtppoints, "sg_rtppoints", "4", CVAR_ROM, 0, qtrue},
    {&g_deathcam, "g_deathcam", "1", CVAR_ARCHIVE | CVAR_SERVERINFO, 0, qtrue},
    {&g_chaseonly, "g_chaseonly", "0", CVAR_ARCHIVE | CVAR_SERVERINFO, 0,
     qtrue},
    {&g_specsareflies, "g_specsareflies", "1", CVAR_ARCHIVE | CVAR_SERVERINFO,
     0, qtrue},
    ...
    {&g_version, "sg_version", PRODUCT_VERSION " " PRODUCT_RELEASE,
     CVAR_ROM | CVAR_SERVERINFO, 0, qtrue},
    ...
    {&g_bulletDamageALDRmidrangefactor, "g_bulletDamageALDRmidrangefactor", "2",
     CVAR_ARCHIVE, 0, qtrue},
    {&g_bulletDamageALDRmidpointfactor, "g_bulletDamageALDRmidpointfactor",
     "0.66", CVAR_ARCHIVE, 0, qtrue},
    {&g_bulletDamageALDRminifactor, "g_bulletDamageALDRminifactor", "0.5",
     CVAR_ARCHIVE, 0, qtrue},
...
void G_InitGame(int levelTime, int randomSeed, int restart) {
    ...
    level.snd_fry =
        G_SoundIndex("sound/player/fry.wav"); // FIXME standing in lava / slime
    ...
    if (g_gametype.integer == GT_DUEL) {
        ...
        for (i = 0; i < MAX_GENTITIES; i++) {
            if (g_entities[i].mappart &&
                !Q_stricmp(g_entities[i].classname,
                           "info_player_intermission")) {
                g_maxmapparts++;
                // Tequila comment: Read the intermission name from here now
                if (g_entities[i].mappartname && g_entities[i].mappartname[0] &&
                    g_entities[i].mappart > 0 &&
                    g_entities[i].mappart <= MAX_MAPPARTS) {
                    Q_strncpyz(intermission_names[g_entities[i].mappart - 1],
                               g_entities[i].mappartname,
                               MAX_MAPPARTS_NAME_LENGTH);
        ...
        for (i = 0; i < MAX_GENTITIES; i++) {
            ...
            if (!Q_stricmp(ent->classname, "team_CTF_redplayer") &&
                startpoints == 0) {
                startpoints = 1;
                i = 0;
            } else if (!Q_stricmp(ent->classname, "team_CTF_blueplayer") &&
                       startpoints == 1) {
                startpoints = 2;
                i = 0;
            } else if (!Q_stricmp(ent->classname, "team_CTF_redspawn") &&
                       startpoints == 2) {
                startpoints = 3;
                i = 0;
            } else if (!Q_stricmp(ent->classname, "team_CTF_bluespawn") &&
                       startpoints == 3) {
                startpoints = 4;
                break;
            }
...
int QDECL SortRanks(const void *a, const void *b) {
    ...
    if (ca->sess.sessionTeam == TEAM_SPECTATOR &&
        cb->sess.sessionTeam == TEAM_SPECTATOR && a_realspec && b_realspec) {
...
void CalculateRanks(void) {
    ...
    if (g_gametype.integer >= GT_TEAM) {
        // in team games, rank is just the order of the teams, 0=red, 1=blue,
        // 2=tied
        ...
        for (i = 0; i < level.numPlayingClients; i++) {
            ...
            if (i == 0 || newScore != score) {
                ...
                level.clients[level.sortedClients[i - 1]]
                    .ps.persistant[PERS_RANK] = rank | RANK_TIED_FLAG;
                level.clients[level.sortedClients[i]].ps.persistant[PERS_RANK] =
                    rank | RANK_TIED_FLAG;
            }
            score = newScore;
...
void FindIntermissionPoint(int mappart) {
    ...
    if (g_gametype.integer != GT_DUEL) {
        ...
        while (1) {
            ...
            if (ent == NULL) {
                ent = G_Find(NULL, FOFS(classname), "info_player_intermission");
                // G_Printf("Warning: No info_player_intermission for current
                // mappart found. Using default\n");
                break;
            }
    ...
    if (!ent) { // the map creator forgot to put in an intermission point...
        SelectSpawnPoint(vec3_origin, level.intermission_origin,
                         level.intermission_angle, qfalse, mappart, NULL);
    } else {
        ...
        if (ent->target) {
            ...
            if (target) {
                VectorSubtract(target->s.origin, level.intermission_origin,
                               dir);
...
void LogExit(const char *string) {
    ...
    for (i = 0; i < numSorted; i++) {
        ...
        G_LogPrintf("score: %i  ping: %i  client: %i  date: %s  name: %s\n",
                    ...
                    va("%02i.%02i.%04i %02i:%02i", q.tm_mday, q.tm_mon + 1,
...
void CheckExitRules(void) {
    ...
    if (g_timelimit.integer &&
        (!level.warmupTime || g_gametype.integer >= GT_RTP)) {

        if ((level.time - level.startTime >= g_timelimit.integer * 60000)
            // Joe Kari: no Timelimit if the round has already begun (more than
            // 15 seconds)
            && (g_gametype.integer < GT_RTP ||
                level.time < g_roundstarttime + 15000 ||
                level.time > g_roundendtime)) {
...
void ClearItems(void) {
    ...
    for (i = 0; i < MAX_GENTITIES; i++) {
        ...
        //		don't try to initialize them "too" much after the first
        //spawn
...
static void Check_Gatling(playerState_t *ps) {
    ...
    if (ps->weapon == WP_GATLING && ps->stats[STAT_GATLING_MODE]) {
        ...
        for (i = 0; i < MAX_GENTITIES; i++) {
            ...
            if (g_entities[i].s.eType == ET_TURRET &&
                g_entities[i].s.torsoAnim == ps->clientNum &&
...
void Setup_NewRound(void) {
    ...
    if (g_round > 0) {
        PushMinilogf("ENDROUND: %i", (g_round));
    }
...
static void BankRobbed(void) {
    ...
    if (level.nextroundstart <=
        level.time) // only add score if round hasn't been already set up
        AddScoreRTPTeam(g_robteam, 1);

    if (g_robteam == TEAM_BLUE) {
        trap_SendServerCommand(
            -1, va("cp \"%s robbed the bank.\"", g_blueteam.string));
        G_LogPrintf("ROUND: Won: %s\n", g_blueteam.string);
        PushMinilogf("TEAMWON: outlaws => %i", level.teamScores[TEAM_BLUE]);
    } else {
        trap_SendServerCommand(
            -1, va("cp \"%s robbed the bank.\"", g_redteam.string));
        G_LogPrintf("ROUND: Won: %s\n", g_redteam.string);
...
void CheckRound(void) {
    ...
    if (g_goldescaped && g_gametype.integer == GT_BR) {
        if (!g_robbed) {
            ...
            g_robbed = qtrue;
            return;
        } else if (level.time > level.nextroundstart &&
                   level.warmupTime != -1 && level.nextroundstart != -1) {
            Setup_NewRound();
            return;
        }
    }
    ...
    if (TeamCount(-1, TEAM_RED) == 0 || TeamCount(-1, TEAM_BLUE) == 0 ||
        ...
        if (level.nextroundstart == -1 &&
            level.warmupTime != -1) { // the round has just been won
            ...
            if (!tied) { // there is someone left in the game -- they won
                ...
                if (g_defendteam != winner) {
                    if (winner == TEAM_BLUE) {
                        trap_SendServerCommand(
                            -1, va("cp \"%s won.\"", g_blueteam.string));
                        G_LogPrintf("ROUND: Won: %s\n", g_blueteam.string);
                        PushMinilogf("TEAMWON: outlaws => %i",
                                     level.teamScores[TEAM_BLUE]);
                    } else {
                        trap_SendServerCommand(
                            -1, va("cp \"%s won.\"", g_redteam.string));
                        G_LogPrintf("ROUND: Won: %s\n", g_redteam.string);
                        PushMinilogf("TEAMWON: lawmen => %i",
                                     level.teamScores[TEAM_RED]);
                    }
                } else {
                    if (winner == TEAM_BLUE) {
                        trap_SendServerCommand(
                            -1, va("cp \"%s defended the bank.\"",
                                   g_blueteam.string));
                        G_LogPrintf("ROUND: Won: %s\n", g_blueteam.string);
                        PushMinilogf("TEAMWON: outlaws => %i",
                                     level.teamScores[TEAM_BLUE]);
...
void Du_PrintVSMessage(void) {
    if (du_introend - DU_INTRO_CAM - DU_INTRO_DRAW + DU_VS_MESSAGE <=
            level.time &&
        !du_vs_message) {
        int i;
        ...
        for (i = 0; i < g_maxmapparts; i++) {
            ...
            for (j = 0; j < du_mapparts[i].numplayers; j++) {
                ...
                case 2:
                    trap_SendServerCommand(
                        player, va("cp \"%s vs %s\"", players[0], players[1]));
                    break;
                case 3:
                    trap_SendServerCommand(player, va("cp \"%s vs %s vs %s\"",
                                                      players[0], players[1],
                                                      players[2]));
                    break;
                case 4:
                    trap_SendServerCommand(
                        player, va("cp \"%s & %s vs %s & %s\"", players[0],
...
void CheckDuel(void) {
    ...
    // if we don't have two players, go back to "waiting for players"
    if ((TeamCount(-1, TEAM_FREE) + TeamCount(-1, TEAM_SPECTATOR)) < 2) {
        if (level.warmupTime != -1) {
    ...
    if (!du_validround && du_nextroundstart <= level.time &&
        du_nextroundstart) {
        int lastmappart = 0;
        ...
        for (i = 0; i < g_maxmapparts; i++) {
            ...
            for (j = 0; j < du_mapparts[i].numplayers; j++) {
                ...
                if (intermission_names[i][0])
                    trap_SendServerCommand(
                        du_mapparts[i].players[j],
                        va("cp \"%s\"", intermission_names[i]));
        ...
        for (j = 0; j < level.maxclients; j++) {
            ...
            // if player was a winner, and round has been over and he's not
            // selected move him to the specs
            if (client->sess.sessionTeam == TEAM_FREE &&
                !du_clientmapparts[j].inuse &&
    ...
    // if round is running check if important things have happened, uiuiuiui
    // "important things", are you a asshole or what? "uiuiuiuiu" damn it!
    if (du_validround && du_nextroundstart == 0) {
        ...
        for (i = 0; i < g_maxmapparts; i++) {
            ...
            int survivor =
                -1; // clientNum of the survivor, important to know who won
            ...
            if (count == du_mapparts[i].numplayers - 1) {
                ...
                for (j = 0; j < du_mapparts[i].numplayers; j++) {
                    ...
                    if (player != survivor) {
                        ...
                        // don't let him join the specs if all duels are
                        // finished
                        if (Du_UsedMapparts() &&
                            level.clients[player].pers.connected ==
                                CON_CONNECTED) {
...
void CheckMapRestart(void) {
    // check because we run 3 game frames before calling Connect and/or
...
void SetLeader(int team, int client) {
    ...
    if (g_gametype.integer >= GT_RTP) {
        ...
        // level.clients[client].pers.netname) ); PrintTeam(team2, va("print
        // \"%s is the new team leader\n\"", level.clients[client].pers.netname)
        // );
    } else {
        if (level.clients[client].pers.connected == CON_DISCONNECTED) {
            PrintTeam(team, va("print \"%s is not connected\n\"",
        ...
        for (i = 0; i < level.num_entities; i++, ent++) {
            ...
            if (level.time - ent->eventTime > EVENT_VALID_MSEC) {
                ...
                if (ent->freeAfterEvent) {
                    // tempEntities or dropped items completely go away after
                    // their event
                    G_FreeEntity(ent);
                    continue;
                } else if (ent->unlinkAfterEvent) {
                    // items that will respawn will hide themselves after their
                    // pickup event
...
// unlagged - backward reconciliation #2
            ...
            if (ent->s.eType == ET_BREAKABLE && ent->wait &&
                ent->wait <= level.time &&
...
// unlagged - backward reconciliation #4
    ...
    void trap_Trace_New(trace_t * results, const vec3_t start,
    ...
    int trap_Trace_New2(trace_t * results, const vec3_t start,
                        const vec3_t mins, const vec3_t maxs, const vec3_t end,
                        int passEntityNum, int contentmask) {
        int shaderNum;
