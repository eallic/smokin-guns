...
void BotPrintTeamGoal(bot_state_t *bs) {
    ...
    case LTG_TEAMHELP: {
        BotAI_Print(PRT_MESSAGE,
                    "%s: I'm gonna help a team mate for %1.0f secs\n", netname,
                    t);
        break;
    }
    case LTG_TEAMACCOMPANY: {
        BotAI_Print(PRT_MESSAGE,
                    "%s: I'm gonna accompany a team mate for %1.0f secs\n",
                    netname, t);
        break;
    }
    case LTG_RUSHBASE: {
        BotAI_Print(PRT_MESSAGE,
                    "%s: I'm gonna rush to the base for %1.0f secs\n", netname,
                    t);
        break;
    }
    case LTG_DEFENDKEYAREA: {
        BotAI_Print(PRT_MESSAGE,
                    "%s: I'm gonna defend a key area for %1.0f secs\n", netname,
                    t);
        break;
    }
    case LTG_GETITEM: {
    ...
    if (match->subtype & ST_TIME) {
        ...
        if (trap_BotFindMatch(timestring, &timematch, MTCONTEXT_TIME)) {
            if (timematch.type == MSG_FOREVER) {
                ...
                trap_BotMatchVariable(&timematch, TIME, timestring,
                                      MAX_MESSAGE_SIZE);
...
int BotGetPatrolWaypoints(bot_state_t *bs, bot_match_t *match) {
    ...
    //
    while (1) {
        if (!trap_BotFindMatch(keyarea, &keyareamatch,
        ...
        trap_BotMatchVariable(&keyareamatch, KEYAREA, keyarea,
                              MAX_MESSAGE_SIZE);
        ...
        if (keyareamatch.subtype & ST_BACK) {
            ...
            trap_BotMatchVariable(&keyareamatch, MORE, keyarea,
                                  MAX_MESSAGE_SIZE);
        } else {
            break;
        }
...
int BotAddressedToBot(bot_state_t *bs, bot_match_t *match) {
    ...
    if (match->subtype & ST_ADDRESSED) {
        trap_BotMatchVariable(match, ADDRESSEE, addressedto,
                              sizeof(addressedto));
        // the name of this bot
        ClientName(bs->client, botname, 128);
        //
        while (trap_BotFindMatch(addressedto, &addresseematch,
                                 MTCONTEXT_ADDRESSEE)) {
            if (addresseematch.type == MSG_EVERYONE) {
                return qtrue;
            } else if (addresseematch.type == MSG_MULTIPLENAMES) {
                trap_BotMatchVariable(&addresseematch, TEAMMATE, name,
                                      sizeof(name));
                if (strlen(name)) {
                ...
                trap_BotMatchVariable(&addresseematch, MORE, addressedto,
                                      MAX_MESSAGE_SIZE);
            } else {
                trap_BotMatchVariable(&addresseematch, TEAMMATE, name,
                                      MAX_MESSAGE_SIZE);
        ...
        // Com_sprintf(buf, sizeof(buf), "not addressed to me but %s",
        // addressedto); trap_EA_Say(bs->client, buf);
        return qfalse;
    } else {
        ...
        if (!trap_BotFindMatch(match->string, &tellmatch,
                               MTCONTEXT_REPLYCHAT) ||
    ...
    if (match->subtype & ST_THERE) {
        ...
        if (entinfo.valid) {
            ...
            if (areanum) { // && trap_AAS_AreaReachability(areanum)) {
                ...
                // if (BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles,
                // 360, client)) {
                bs->teamgoal.entitynum = client;
                bs->teamgoal.areanum = areanum;
                VectorCopy(entinfo.origin, bs->teamgoal.origin);
...
void BotMatch_CheckPoint(bot_state_t *bs, bot_match_t *match) {
    ...
    if (BotAddressedToBot(bs, match)) {
        Com_sprintf(buf, sizeof(buf), "%1.0f %1.0f %1.0f", cp->goal.origin[0],
                    cp->goal.origin[1], cp->goal.origin[2]);
...
float BotNearestVisibleItem(bot_state_t *bs, char *itemname, bot_goal_t *goal) {
    ...
    do {
        ...
        if (dist < bestdist) {
            ...
            BotAI_Trace(&trace, bs->eye, NULL, NULL, tmpgoal.origin, bs->client,
...
int BotMatchMessage(bot_state_t *bs, char *message) {
    ...
    if (!trap_BotFindMatch(message, &match,
                           MTCONTEXT_MISC | MTCONTEXT_INITIALTEAMCHAT |
                               MTCONTEXT_CTF)) {
        return qfalse;
    }
    ...
    case MSG_FORMATIONPOSITION: // tell someone his/her position in the
                                // formation
    {
        trap_EA_SayTeam(
            bs->client,
