...
char *BotWeaponNameForMeansOfDeath(int mod) {
    ...
    case MOD_PEACEMAKER:
        return "Peacemaker";
    case MOD_WINCHESTER66:
        return "Winchester";
    case MOD_LIGHTNING:
        return "Colt Lightning";
    case MOD_SHARPS:
        return "Sharps";
...
int BotValidChatPosition(bot_state_t *bs) {
    ...
    point[2] -= 24;
    if (trap_PointContents(point, bs->entitynum) &
...
int BotChat_EnterGame(bot_state_t *bs) {
    ...
    if (gametype == GT_DUEL)
        return qfalse;
    rnd = trap_Characteristic_BFloat(bs->character,
...
int BotChat_Death(bot_state_t *bs) {
    ...
    if (bs->lastchat_time > FloatTime() - TIME_BETWEENCHATTING)
        return qfalse;
    rnd = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_DEATH,
                                     0, 1);
    // don't chat in tournament mode
    if (gametype == GT_DUEL)
        return qfalse;
    ...
    if (TeamPlayIsOn() && BotSameTeam(bs, bs->lastkilledby)) {
        ...
        if (bs->botdeathtype == MOD_WATER)
            BotAI_BotInitialChat(bs, "death_drown", BotRandomOpponentName(bs),
                                 NULL);
        else if (bs->botdeathtype == MOD_SLIME)
            BotAI_BotInitialChat(bs, "death_slime", BotRandomOpponentName(bs),
                                 NULL);
        else if (bs->botdeathtype == MOD_LAVA)
            BotAI_BotInitialChat(bs, "death_lava", BotRandomOpponentName(bs),
                                 NULL);
        else if (bs->botdeathtype == MOD_FALLING)
            BotAI_BotInitialChat(bs, "death_cratered",
                                 BotRandomOpponentName(bs), NULL);
            ...
            BotAI_BotInitialChat(bs, "death_suicide", BotRandomOpponentName(bs),
                                 NULL);
            ...
            // choose between insult and praise
                     ...
                     trap_Characteristic_BFloat(
                         bs->character, CHARACTERISTIC_CHAT_INSULT, 0, 1)) {
                BotAI_BotInitialChat(
                    bs, "death_insult",
                    name,                                           // 0
                    BotWeaponNameForMeansOfDeath(bs->botdeathtype), // 1
...
int BotChat_Kill(bot_state_t *bs) {
    ...
    if (bs->lastchat_time > FloatTime() - TIME_BETWEENCHATTING)
        return qfalse;
    rnd = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_KILL, 0,
                                     1);
    // don't chat in tournament mode
    if (gametype == GT_DUEL)
        return qfalse;
...
int BotChat_Random(bot_state_t *bs) {
    ...
    if (bs->ltgtype == LTG_TEAMHELP || bs->ltgtype == LTG_TEAMACCOMPANY ||
        ...
        return qfalse;
    //
    rnd = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_RANDOM,
                                     0, 1);
    ...
    //
    if (random() < trap_Characteristic_BFloat(bs->character,
...
float BotChatTime(bot_state_t *bs) {
    ...
    // cpm = trap_Characteristic_BInteger(bs->character,
    // CHARACTERISTIC_CHAT_CPM, 1, 4000);
