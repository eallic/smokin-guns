...
// unlagged - true ping
        ...
        Com_sprintf(entry, sizeof(entry), " %i %i %i %i %i %i %i %i",
                    ...
                    g_entities[level.sortedClients[i]].s.powerups, cl->realspec,
                    cl->won);
        j = strlen(entry);
    ...
    trap_SendServerCommand(
        ent - g_entities, va("scores %i %i %i%s", i, level.teamScores[TEAM_RED],
...
Now it is just an utility function, no "print" cmd sent to an entity, no more
mixing number (ID) and name.
...
void Cmd_Give_f(gentity_t *ent) {
    ...
    if (give_all || Q_stricmp(name, "ammo") == 0) {
        ...
        ent->client->ps.ammo[WP_BULLETS_CLIP] =
            bg_weaponlist[WP_PEACEMAKER].maxAmmo * 2;
        ent->client->ps.ammo[WP_SHELLS_CLIP] =
            bg_weaponlist[WP_REMINGTON_GAUGE].maxAmmo * 2;
        ent->client->ps.ammo[WP_CART_CLIP] =
            bg_weaponlist[WP_WINCHESTER66].maxAmmo * 2;
        ent->client->ps.ammo[WP_GATLING_CLIP] =
            bg_weaponlist[WP_GATLING].maxAmmo * 2;
        ent->client->ps.ammo[WP_SHARPS_CLIP] =
            bg_weaponlist[WP_SHARPS].maxAmmo * 2;
        if (!give_all)
            return;
    }
...
void Cmd_LevelShot_f(gentity_t *ent) {
    if (!ent->client->pers.localClient) {
        trap_SendServerCommand(ent - g_entities,
                               "print \"The levelshot command must be executed "
                               "by a local client\n\"");
        return;
    }
...
void BroadcastTeamChange(gclient_t *client, int oldTeam) {
    ...
    if (client->sess.sessionTeam == TEAM_RED) {
        trap_SendServerCommand(-1,
                               va("cp \"%s" S_COLOR_YELLOW " joined %s.\n\"",
        ...
        // Joe Kari: no need to shout, especially if I want to spec someone
        // suspicious, I don't want to get caught.
        // trap_SendServerCommand( -1, va("cp \"%s" S_COLOR_WHITE " joined the
        // spectators.\n\"", client->pers.netname));
        trap_SendServerCommand(-1, va("print \"%s" S_COLOR_WHITE
                                      " joined the spectators.\n\"",
                                      client->pers.netname));
    } else if (client->sess.sessionTeam == TEAM_FREE) {
        trap_SendServerCommand(-1, va("cp \"%s" S_COLOR_YELLOW
                                      " joined the gunfight.\n\"",
                                      client->pers.netname));
    }
}
...
void SetTeam(gentity_t *ent, char *s) {
    ...
    if (!Q_stricmp(s, "scoreboard") || !Q_stricmp(s, "score")) {
        ...
        if (g_teamForceBalance.integer) {
            ...
            // It's ok, the team we are switching to has less or same number of
            // players
        }
    ...
    if (oldTeam < TEAM_SPECTATOR && !ent->client->specwatch &&
        ent->client->ps.stats[STAT_HEALTH] > 0) {
        ent->client->ps.persistant[PERS_SCORE] +=
            1; // switching team is not suicide
        ent->flags &= ~FL_GODMODE;
    ...
    if ((team == TEAM_RED && oldTeam != TEAM_RED_SPECTATOR) ||
        ...
        // if there is no team leader or the team leader is a bot and this
        // client is not a bot
        if (teamLeader == -1 ||
    ...
    } else if ((team == TEAM_RED || team == TEAM_RED_SPECTATOR) &&
               !(oldTeam == TEAM_RED || oldTeam == TEAM_RED_SPECTATOR)) {
        PushMinilogf("JOIN: %i => lawmen", clientNum);
    } else if ((team == TEAM_BLUE || team == TEAM_BLUE_SPECTATOR) &&
               !(oldTeam == TEAM_BLUE || oldTeam == TEAM_BLUE_SPECTATOR)) {
    ...
    if (g_gametype.integer >= GT_RTP) {
        ...
        if ((oldTeam == TEAM_SPECTATOR && team == TEAM_FREE &&
             !client->realspec) ||
            (oldTeam == TEAM_FREE && team == TEAM_SPECTATOR &&
             !client->realspec)) {
            ClientSpawn(ent);
        } else
            ClientBegin(clientNum);
...
qboolean ChaseCam_CheckClient(int clientnum, gentity_t *ent) {
    ...
    if (g_gametype.integer == GT_DUEL &&
        ent->client->specmappart != g_entities[clientnum].mappart)
        return qfalse;
...
void ChaseCam_Start(gentity_t *ent, int dir) {
    ...
    do {
        ...
        VectorCopy(level.clients[clientnum].ps.viewangles,
                   ent->client->ps.viewangles);
        VectorCopy(level.clients[clientnum].ps.delta_angles,
...
void ChaseCam_Change(gentity_t *ent) {
    ...
    if (ent->client->sess.spectatorState == SPECTATOR_CHASECAM) {
        ...
        ent->client->sess.spectatorState = SPECTATOR_CHASECAM;
        trap_SendServerCommand(ent - g_entities,
                               va("print \"Free Chasecam\n\""));
    }

    if (ent->client->sess.spectatorState != SPECTATOR_FREE) {
        VectorCopy(
            level.clients[ent->client->sess.spectatorClient].ps.viewangles,
            ent->client->ps.viewangles);
        VectorCopy(
            level.clients[ent->client->sess.spectatorClient].ps.delta_angles,
            ent->client->ps.delta_angles);
        VectorCopy(
            level.clients[ent->client->sess.spectatorClient].ps.delta_angles,
            ent->s.angles);
...
void Cmd_Team_f(gentity_t *ent) {
    ...
    if (trap_Argc() != 2) {
        ...
        case TEAM_RED_SPECTATOR:
            trap_SendServerCommand(
                ent - g_entities, va("print \"Team: %s\n\"", g_redteam.string));
            break;
        case TEAM_FREE:
...
void G_Say(gentity_t *ent, gentity_t *target, int mode, const char *chatText) {
    ...
    // It is probably more funny to display "(muted)" than turn it off
    // completely ^^
    if (ent->client->pers.muted) {
        if (ent->client->pers.muted > 1)
            return;
        chatText = "^9(muted)";
    }
    ...
    case SAY_TELL:
        if (target && target->inuse && target->client &&
            ...
            Com_sprintf(name, sizeof(name), EC "[%s%c%c" EC "]" EC ": ",
                        ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE);
        color = COLOR_MAGENTA;
        // Joe Kari: "tell" call G_Say() twice, one for the target and one for
        // self... but we don't need to log twice.
        if (ent->s.number != target->s.number)
            PushMinilogf("SAYTELL: %i => %i > %s", ent->s.number,
                         target->s.number, chatText);
        break;
    }
...
static void Cmd_Tell_f(gentity_t *ent) {
    ...
    if (trap_Argc() < 3) {
        trap_SendServerCommand(ent - g_entities,
                               "print \"Usage: tell <player id> <message>\n\"");
        return;
    }
...
==================
Cmd_Secret_f
Joe Kari: add to the minilog a SECRET entry, allow user to register to third
party admin-bot.
==================
*/
static void Cmd_Secret_f(gentity_t *ent) {
    ...
    str = ConcatArgs(1);
    // Do not even log it with G_Printf(), this should be as discreet as it
    // could...
    PushMinilogf("SECRET: %i > %s", ent->s.number, str);
}

static void G_VoiceTo(gentity_t *ent, gentity_t *other, int mode,
                      const char *id, qboolean voiceonly) {
    int color;
    char *cmd;
...
static char *gc_orders[] = {
    "hold your position", "hold this position", "come here", "cover me",
...
void Cmd_GameCommand_f(gentity_t *ent) {
    ...
    G_LogPrintf("tell: %s to %s: %s\n", ent->client->pers.netname,
                target->client->pers.netname, gc_orders[order]);
...
static const char *gameNames[] = {
    "Deathmatch",      "Duel",           "Single Player",
    "Team Deathmatch", "Round Teamplay", "Bank Robbery",
};
...
void Cmd_CallVote_f(gentity_t *ent) {
    ...
    if (ent->client->pers.muted) {
        trap_SendServerCommand(ent - g_entities,
                               "print \"You try to call a vote, but nobody can "
                               "hear your voice.\n\"");
        trap_SendServerCommand(ent - g_entities, "cp \"...\n\"");
        return;
    }

    // Joe Kari: a tips to avoid stOopid punk with fast computer to call a vote
    // before anyone have time to join, allowing them to do everything they
    // want... now you have to wait some time.
    if (level.time < (g_voteMinLevelTime.integer * 1000)) {
        diff = (g_voteMinLevelTime.integer * 1000 - level.time) / 1000;
        trap_SendServerCommand(ent - g_entities,
                               va("print \"This level has just begun, wait %i "
                                  "seconds before calling a vote...\n\"",
                                  diff));
        trap_SendServerCommand(
            ent - g_entities,
            va("cp \"Level has just begun, wait %i seconds\n\"", diff));
        return;
    }
    if (level.voteTime) {
        trap_SendServerCommand(ent - g_entities,
        ...
        trap_SendServerCommand(ent - g_entities,
                               "cp \"A vote is already in progress\n\"");
        return;
    }
    ...
    if (ent->client->pers.voteCount >= g_maxVote.integer) {
        trap_SendServerCommand(
            ent - g_entities,
            "print \"You have reached your maximum number of votes.\n\"");
        trap_SendServerCommand(ent - g_entities,
                               "cp \"Maximum number of votes reached\n\"");
        return;
    }

    // Tequila: g_voteDelay can now be set to the number of seconds a player
    // must wait between 2 votes
    if (ent->client->pers.lastVoteTime > 0 &&
        level.time <
            ent->client->pers.lastVoteTime + g_voteDelay.integer * 1000) {
        diff = (ent->client->pers.lastVoteTime + g_voteDelay.integer * 1000 -
                level.time) /
               1000;
        trap_SendServerCommand(
            ent - g_entities,
        ...
        trap_SendServerCommand(
            ent - g_entities,
            va("cp \"Can't call too many vote, wait %i seconds\n\"", diff));
        return;
    }

    if (ent->client->sess.sessionTeam == TEAM_SPECTATOR &&
        (g_gametype.integer != GT_DUEL || ent->client->realspec)) {
        trap_SendServerCommand(ent - g_entities,
                               "print \"Can't call a vote as spectator.\n\"");
        trap_SendServerCommand(ent - g_entities,
                               "cp \"Can't call a vote as spectator\n\"");
        return;
    }
    ...
    for (c = arg2; *c; ++c) {
        ...
        case ';':
            trap_SendServerCommand(
                ent - g_entities,
                va("print \"Invalid vote string argument: %s\n\"", arg2));
            return;
            break;
        }
    }
    ...
    if (!Q_stricmp(arg1, "map_restart") &&
        ...
        // Validate argument, it must be an integer
               ...
               !strcmp(arg2, va("%i", atoi(arg2)))) {
        // Validate argument, it must be an integer (DEPRECATED)
        ...
        trap_SendServerCommand(
            ent - g_entities,
            va("print \"Invalid vote string command: %s\n\"", ConcatArgs(1)));
        trap_SendServerCommand(
            ent - g_entities,
            "print \"Vote commands are: map_restart, nextmap, map <mapname>, "
            "g_gametype <n>, mute <player>, unmute <player>, kick <player>, "
            "kicknum <client id> [reason], g_doWarmup <integer>, timelimit "
            "<time>, fraglimit <frags>, mapcycle <mapcyclename>.\n\"");
    ...
    if ((!Q_stricmp(arg1, "kick") ||
         !Q_stricmp(arg1, "kicknum") | !Q_stricmp(arg1, "clientkick")) &&
        !g_allowVoteKick.integer) {
        trap_SendServerCommand(ent - g_entities,
                               "print \"Not allowed to vote kick here.\n\"");
        trap_SendServerCommand(ent - g_entities, "cp \"No vote kick here\n\"");
        return;
    }
    ...
    if (!Q_stricmp(arg1, "g_gametype")) {
        ...
        if (i == GT_SINGLE_PLAYER || i < GT_FFA || i > GT_RTP) {
            trap_SendServerCommand(
                ent - g_entities, va("print \"Invalid gametype: %s\n\"", arg2));
            return;
        }

        Com_sprintf(level.voteString, sizeof(level.voteString), "%s %d", arg1,
                    i);
        Com_sprintf(level.voteDisplayString, sizeof(level.voteDisplayString),
                    "%s %s", arg1, gameNames[i]);
    } else if (!Q_stricmp(arg1, "map")) {
        // special case for map changes, we want to reset the nextmap setting
        // this allows a player to change maps, but not upset the map rotation
        // Joe Kari : calling a vote for a /map command is not supposed to
        // change 'nextmap' cvar We removed this code, this is not usefull
        // anymore since I have modified server/sv_init.c to never turn
        // 'nextmap' to 'map_restart 0'
        Com_sprintf(level.voteString, sizeof(level.voteString), "%s %s", arg1,
                    arg2);
        Com_sprintf(level.voteDisplayString, sizeof(level.voteDisplayString),
                    "%s", level.voteString);
    } else if (!Q_stricmp(arg1, "mapcycle")) { // Joe Kari: allow player to vote
                                               // for a mapcycle
        ...
        trap_Cvar_VariableStringBuffer("g_mapcycles", mapcycles,
                                       sizeof(mapcycles));
        ...
        if (!match) {
            trap_SendServerCommand(
                ent - g_entities,
                "print \"Unknown mapcycle. Available mapcycles are:\n\"");
            p = mapcycles;
        ...
        Com_sprintf(level.voteString, sizeof(level.voteString), "vstr %s",
                    token);
        Com_sprintf(level.voteDisplayString, sizeof(level.voteDisplayString),
                    "mapcycle %s", token);

    } else if (!Q_stricmp(arg1, "kick")) {
        // Tequila: Replace kick command by safer sendaway one and merging other
        // arguments
        // Com_sprintf( level.voteString, sizeof( level.voteString ), "sendaway
        // \"%s\"", ConcatArgs(2) );
        ...
        if (clientnum < 0) {
            trap_SendServerCommand(ent - g_entities,
                                   va("print \"%s not found.\n\"", arg_str));
            return;
        }
        Com_sprintf(level.voteString, sizeof(level.voteString),
                    "kicknum %i \"Kicked by vote.\"", clientnum);
        Com_sprintf(level.voteDisplayString, sizeof(level.voteString),
                    "kick \"%s\"", arg_str);
    } else if (!Q_stricmp(arg1, "kicknum")) {
        arg_str = ConcatArgs(3);
        Com_sprintf(level.voteString, sizeof(level.voteString), "kicknum %s %s",
                    arg2, arg_str);
        Com_sprintf(level.voteDisplayString, sizeof(level.voteString),
                    "kicknum %s %s", arg2, arg_str);
        ...
        Com_sprintf(level.voteString, sizeof(level.voteString), "mute %i",
                    clientnum);
        Com_sprintf(level.voteDisplayString, sizeof(level.voteString),
                    "mute \"%s\"", arg_str);
        ...
        Com_sprintf(level.voteString, sizeof(level.voteString), "unmute %i",
                    clientnum);
        Com_sprintf(level.voteDisplayString, sizeof(level.voteString),
                    "unmute \"%s\"", arg_str);
    } else if (!Q_stricmp(arg1, "timelimit")) {
...
#define MAX_TIMELIMIT 43200
        if (i < 0) {
            trap_SendServerCommand(
                ent - g_entities,
                va("print \"Invalid timelimit: %s, must be positive\n\"",
                   arg2));
            return;
        } else if (i >= MAX_TIMELIMIT) {
            trap_SendServerCommand(ent - g_entities,
                                   va("print \"Invalid timelimit: %s, must be "
                                      "lower than %i minutes\n\"",
                                      arg2, MAX_TIMELIMIT));
            return;
        }
...
void Cmd_CallTeamVote_f(gentity_t *ent) {
    ...
    if (level.teamVoteTime[cs_offset]) {
        ...
        trap_SendServerCommand(ent - g_entities,
                               "cp \"A team vote is already in progress\n\"");
        return;
    }
            ...
            if (i >= 3 || !arg2[i]) {
                ...
                for (i = 0; i < level.maxclients; i++) {
                    ...
                    if (level.clients[i].sess.sessionTeam != team)
                        continue;
                    Q_strncpyz(netname, level.clients[i].pers.netname,
                               sizeof(netname));
                    Q_CleanStr(netname);
...
void Cmd_Stats_f(gentity_t *ent) {
    /*
            ...
            //trap_SendServerCommand( ent-g_entities, va("print \"visited %d of
       %d areas\n\"", n, max)); trap_SendServerCommand( ent-g_entities,
       va("print \"%d%% level coverage\n\"", n * 100 / max));
    */
}
...
void Cmd_DropWeapon_f(gentity_t *ent, int weapon) {
    ...
    // Joe Kari : it looks like using sess.spectatorState is buggy, a playing
    // player have a value of SPECTATOR_FREE instead of SPECTATOR_NOT, so I will
    // use sess.sessionTeam instead
    ...
    if ((ucmd->buttons & BUTTON_ATTACK) ||
        (ucmd->buttons & BUTTON_ALT_ATTACK) || (client->ps.eFlags & EF_RELOAD))
        return;
...
void Cmd_BuyItem_f(gentity_t *ent, qboolean cgame) {
    ...
    if (!(ent->client->ps.stats[STAT_FLAGS] & SF_CANBUY) &&
        ...
        if (!cgame)
            trap_SendServerCommand(ent - g_entities,
                                   va("print \"You're not in a buy-zone!\n\""));
        return;
    }
    ...
    if (level.time - g_roundstarttime > BUY_TIME &&
        ...
        if (!cgame)
            trap_SendServerCommand(ent - g_entities,
                                   va("print \"60 seconds have passed ... you "
                                      "can't buy anything\n\""));
        return;
    }
    ...
    if (ent->client->sess.sessionTeam >= TEAM_SPECTATOR) {
        if (!cgame)
            trap_SendServerCommand(
                ent - g_entities,
                va("print \"You can't buy while in spectator mode!\n\""));
        return;
    }
    ...
    case IT_WEAPON:
        ...
        if (bg_weaponlist[item->giTag].wp_sort == WPS_PISTOL &&
            ...
            if (!(ps->stats[STAT_FLAGS] & SF_SEC_PISTOL) &&
                ...
                int weapon_drop = BG_SearchTypeWeapon(
                    WPS_PISTOL, ps->stats[STAT_WEAPONS], weapon);
                if (weapon_drop)
                    weapon = weapon_drop;
            }
            G_ThrowWeapon(weapon, ent);
        ...
        if (item->giTag == WP_DYNAMITE || item->giTag == WP_KNIFE ||
            ...
            if (ps->ammo[item->giTag] >= bg_weaponlist[item->giTag].maxAmmo) {
                if (!cgame)
                    trap_SendServerCommand(
                        ent - g_entities,
                        va("print \"You can't carry any more!\n\""));
                return;
            }
        ...
        // give ammo to player
        if (bg_weaponlist[item->giTag].clip &&
            ps->ammo[bg_weaponlist[item->giTag].clip] <
                bg_weaponlist[item->giTag].maxAmmo) {
            ps->ammo[bg_weaponlist[item->giTag].clip] =
                bg_weaponlist[item->giTag].maxAmmo;
        }
        break;
    ...
    case IT_ARMOR:
        if (ps->stats[STAT_ARMOR] && !cgame) {
            trap_SendServerCommand(
                ent - g_entities,
                va("print \"You already have a boiler plate!\n\""));
            return;
        }
        break;
    case IT_POWERUP:
    ...
    default:
        trap_SendServerCommand(ent - g_entities,
                               va("print \"This item is not buyable\n\""));
        return;
    }
    ...
    if (item->giType == IT_POWERUP && item->giTag == PW_BELT) {
        ...
        ent->client->ps.ammo[WP_BULLETS_CLIP] =
            bg_weaponlist[WP_REM58].maxAmmo * belt;
        ent->client->ps.ammo[WP_SHELLS_CLIP] =
            bg_weaponlist[WP_REMINGTON_GAUGE].maxAmmo * belt;
        ent->client->ps.ammo[WP_CART_CLIP] =
            bg_weaponlist[WP_WINCHESTER66].maxAmmo * belt;
        ent->client->ps.ammo[WP_GATLING_CLIP] =
            bg_weaponlist[WP_GATLING].maxAmmo * belt;
        ent->client->ps.ammo[WP_SHARPS_CLIP] =
            bg_weaponlist[WP_SHARPS].maxAmmo * belt;
    }
