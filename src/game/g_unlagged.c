...
Clear out the given client's history (should be called when the teleport bit is
flipped)
============
*/
void G_ResetHistory(gentity_t *ent) {
    ...
    for (i = ent->client->historyHead, time = level.time; i >= 0;
         i--, time -= 50) {
        ...
        VectorCopy(ent->client->legs_angles,
                   ent->client->history[i].legs_angles);
        VectorCopy(ent->client->torso_angles,
                   ent->client->history[i].torso_angles);
        VectorCopy(ent->client->ps.viewangles,
                   ent->client->history[i].viewangles);
...
void G_StoreHistory(gentity_t *ent) {
    ...
    VectorCopy(ent->client->legs_angles,
               ent->client->history[head].legs_angles);
    VectorCopy(ent->client->torso_angles,
               ent->client->history[head].torso_angles);
    VectorCopy(ent->client->ps.viewangles,
               ent->client->history[head].viewangles);
...
static void TimeShiftAnglesLerp(float frac, vec3_t start, vec3_t end,
                                vec3_t result) {
...
=================
*/
void G_TimeShiftClient(gentity_t *ent, int time, qboolean debug,
                       gentity_t *debugger) {
    int j, k;
    char msg[2048];

    // this will dump out the head index, and the time for all the stored
    // positions
    /*
            if ( debug ) {
                    char	str[MAX_STRING_CHARS];

                    Com_sprintf(str, sizeof(str), "print \"head: %d, %d %d %d %d
    ...
    */
    ...
    if (j != k) {
        ...
        if (ent->client->saved.leveltime != level.time) {
            ...
            VectorCopy(ent->client->legs_angles,
                       ent->client->saved.legs_angles);
            VectorCopy(ent->client->torso_angles,
                       ent->client->saved.torso_angles);
            VectorCopy(ent->client->ps.viewangles,
                       ent->client->saved.viewangles);
        ...
        if (j != ent->client->historyHead) {
            float frac = (float)(time - ent->client->history[j].leveltime) /
                         (float)(ent->client->history[k].leveltime -
                                 ent->client->history[j].leveltime);

            // interpolate between the two origins to give position at time
            // index "time"
            TimeShiftLerp(frac, ent->client->history[j].currentOrigin,
                          ent->client->history[k].currentOrigin,
                          ent->r.currentOrigin);

            // lerp these too, just for fun (and ducking)
            TimeShiftLerp(frac, ent->client->history[j].mins,
                          ent->client->history[k].mins, ent->r.mins);

            TimeShiftLerp(frac, ent->client->history[j].maxs,
                          ent->client->history[k].maxs, ent->r.maxs);

            // lerp angles
            TimeShiftAnglesLerp(frac, ent->client->history[k].currentAngles,
                                ent->client->history[j].currentAngles,
                                ent->r.currentAngles);

            TimeShiftAnglesLerp(frac, ent->client->history[k].legs_angles,
                                ent->client->history[j].legs_angles,
                                ent->client->legs_angles);

            TimeShiftAnglesLerp(frac, ent->client->history[k].torso_angles,
                                ent->client->history[j].torso_angles,
                                ent->client->torso_angles);

            TimeShiftAnglesLerp(frac, ent->client->history[k].viewangles,
                                ent->client->history[j].viewangles,
                                ent->client->ps.viewangles);

            BG_CopyLerpFrame(&ent->client->history[k].legs, &ent->client->legs);
            BG_CopyLerpFrame(&ent->client->history[k].torso,
                             &ent->client->torso);
            ...
            if (debug && debugger != NULL) {
                ...
                Com_sprintf(
                    msg, sizeof(msg),
                    "print \"^1Rec: time: %d, j: %d, k: %d, origin: %0.2f "
                    "%0.2f %0.2f\n"
                    "^2frac: %0.4f, origin1: %0.2f %0.2f %0.2f, origin2: %0.2f "
                    "%0.2f %0.2f\n"
                    "^7level.time: %d, est time: %d, level.time delta: %d, est "
                    "real ping: %d\n\"",
                    time, ent->client->history[j].leveltime,
                    ent->client->history[k].leveltime, ent->r.currentOrigin[0],
                    ent->r.currentOrigin[1], ent->r.currentOrigin[2], frac,
                    ...
                    ent->client->history[k].currentOrigin[2], level.time,
                    level.time + debugger->client->frameOffset,
                    level.time - time,
            ...
            VectorCopy(ent->client->history[k].currentOrigin,
                       ent->r.currentOrigin);
            VectorCopy(ent->client->history[k].mins, ent->r.mins);
            VectorCopy(ent->client->history[k].maxs, ent->r.maxs);

            // locational damage
            VectorCopy(ent->client->history[k].currentAngles,
                       ent->r.currentAngles);
            VectorCopy(ent->client->history[k].legs_angles,
                       ent->client->legs_angles);
            VectorCopy(ent->client->history[k].torso_angles,
                       ent->client->torso_angles);
            VectorCopy(ent->client->history[k].viewangles,
                       ent->client->ps.viewangles);
            BG_CopyLerpFrame(&ent->client->history[k].legs, &ent->client->legs);
            BG_CopyLerpFrame(&ent->client->history[k].torso,
                             &ent->client->torso);
        ...
        // this only happens when the client is using a negative timenudge,
        // because that number is added to the command time
        ...
        if (debug && debugger != NULL) {
            Com_sprintf(msg, sizeof(msg),
                        "print \"^1No rec: time: %d, j: %d, k: %d, origin: "
                        "%0.2f %0.2f %0.2f\n"
                        "^2frac: %0.4f, origin1: %0.2f %0.2f %0.2f, origin2: "
                        "%0.2f %0.2f %0.2f\n"
                        "^7level.time: %d, est time: %d, level.time delta: %d, "
                        "est real ping: %d\n\"",
                        time, level.time, level.time, ent->r.currentOrigin[0],
                        ent->r.currentOrigin[1], ent->r.currentOrigin[2], 0.0f,
                        ent->r.currentOrigin[0], ent->r.currentOrigin[1],
                        ent->r.currentOrigin[2], ent->r.currentOrigin[0],
                        ent->r.currentOrigin[1], ent->r.currentOrigin[2],
...
void G_TimeShiftAllClients(int time, gentity_t *skip) {
    ...
    for (i = 0; i < MAX_CLIENTS; i++, ent++) {
        if (ent->client && ent->inuse &&
            ent->client->sess.sessionTeam < TEAM_SPECTATOR && ent != skip) {
...
void G_DoTimeShiftFor(gentity_t *ent) {
    ...
    // if it's enabled server-side and the client wants it or wants it for this
    // weapon
...
qboolean G_PredictPlayerSlideMove(gentity_t *ent, float frametime) {
    ...
    for (bumpcount = 0; bumpcount < numbumps; bumpcount++) {
        ...
        trap_Trace(&trace, origin, ent->r.mins, ent->r.maxs, end, ent->s.number,
                   ent->clipmask);
        ...
        for (i = 0; i < numplanes; i++) {
            ...
            // slide along the plane
            G_PredictPlayerClipVelocity(endVelocity, planes[i],
                                        endClipVelocity);
            ...
            for (j = 0; j < numplanes; j++) {
                ...
                G_PredictPlayerClipVelocity(clipVelocity, planes[j],
                                            clipVelocity);
                G_PredictPlayerClipVelocity(endClipVelocity, planes[j],
                                            endClipVelocity);
...
void G_PredictPlayerStepSlideMove(gentity_t *ent, float frametime) {
    ...
    trap_Trace(&trace, start_o, ent->r.mins, ent->r.maxs, up, ent->s.number,
               ent->clipmask);
    if (trace.allsolid) {
