...
/*QUAKED target_speaker (1 0 0) (-8 -8 -8) (8 8 8) looped-on looped-off global
activator "noise"		wav file to play
...
cost. "wait" : Seconds between auto triggerings, 0 = don't auto trigger "random"
...
void Use_Target_Speaker(gentity_t *ent, gentity_t *other,
    ...
    if (ent->spawnflags & 3) { // looping sound toggles
        if (ent->s.loopSound) {
            ...
            ent->s.eFlags |=
                EF_SOUNDOFF; // make it possible to restart at round restart
        }
        else
...
void target_laser_think(gentity_t *self) {
    ...
    if (tr.entityNum) {
        ...
        G_Damage(&g_entities[tr.entityNum], self, self->activator,
                 self->movedir, tr.endpos, self->damage, DAMAGE_NO_KNOCKBACK,
                 MOD_TARGET_LASER);
    }
