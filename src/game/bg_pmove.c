...
float pm_ladderfriction =
    3000; // Friction is high enough so you don't slip down
...
void PM_FireWeapon(int anim, qboolean weapon2, qboolean altfire,
                   qboolean delay) {
    ...
    if (anim) {
        if (weapon2)
            pm->ps->weapon2Anim =
                ((pm->ps->weapon2Anim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT) |
                anim;
        else
            pm->ps->weaponAnim =
...
===================
*/
static void PM_StartWeaponAnim(int anim, qboolean weapon2, qboolean reload,
                               qboolean sec) {
    int *weaponAnim;
    int weaponNum;
...
static void PM_Friction(void) {
    ...
    {
        drop +=
            speed * pm_ladderfriction * pml.frametime; // Add ladder friction!
    }
...
static void PM_GroundTraceMissed(void) {
    ...
    if (pm->ps->groundEntityNum != ENTITYNUM_NONE) {
        ...
        // force into it if we didn't do the trace, the player would be
        // backflipping down staircases
        VectorCopy(pm->ps->origin, point);
        point[2] -= 64;
...
static void PM_Footsteps(void) {
    ...
    if ((pml.ladder && !pm->cmd.forwardmove && !pm->cmd.upmove &&
         !pm->cmd.rightmove) || // if on ladder
        (!pm->cmd.forwardmove && !pm->cmd.rightmove &&
         !pml.ladder)) { // if not on ladder
        if (pm->xyspeed < 5) {
...
static void PM_BeginWeaponChange(int weapon) {
    ...
    if (pm->ps->weaponstate == WEAPON_DROPPING ||
        (pm->ps->weapon2state == WEAPON_DROPPING && pm->ps->weapon2)) {
        return;
    }
    ...
    if (weapon == WP_AKIMBO) {
        ...
        count_pistols +=
            pm->ps->stats[STAT_WEAPONS] & (1 << WP_SCHOFIELD) ? 1 : 0;
        count_pistols +=
            pm->ps->stats[STAT_WEAPONS] & (1 << WP_PEACEMAKER) ? 1 : 0;
    ...
    if ((!pm->ps->stats[STAT_OLDWEAPON] &&
         (!pm->ps->weapon2 || weapon != WP_AKIMBO) &&
...
static void PM_FinishWeaponChange(void) {
    ...
    if (weapon == WP_AKIMBO) {
        ...
        if (pm->ps->weapon2 &&
            bg_weaponlist[pm->ps->weapon].wp_sort == WPS_PISTOL) {

            if (!sec_pistol) {
                weapon1 = BG_SearchTypeWeapon(
                    WPS_PISTOL, pm->ps->stats[STAT_WEAPONS], pm->ps->weapon2);
            } else {
                weapon1 = BG_SearchTypeWeapon(WPS_PISTOL,
                                              pm->ps->stats[STAT_WEAPONS], 0);
            }
            weapon2 = 0; // don't change weapon2

        } else {
            weapon1 =
                BG_SearchTypeWeapon(WPS_PISTOL, pm->ps->stats[STAT_WEAPONS], 0);

            if (!sec_pistol) {
                weapon2 = BG_SearchTypeWeapon(
                    WPS_PISTOL, pm->ps->stats[STAT_WEAPONS], weapon1);
            } else {
                weapon2 = BG_SearchTypeWeapon(WPS_PISTOL,
                                              pm->ps->stats[STAT_WEAPONS], 0);
            }
        }
    } else {
    ...
    if (bg_weaponlist[weapon1].wp_sort == WPS_GUN)
        ...
        PM_StartTorsoAnim(TORSO_PISTOL_RAISE);
    ...
    if (!pm->ps->ammo[pm->ps->weapon] && pm->ps->weapon == WP_REMINGTON_GAUGE) {
        pm->ps->weaponTime -= 28 * bg_weaponlist[WP_REMINGTON_GAUGE]
                                       .animations[WP_ANIM_CHANGE]
                                       .frameLerp;
...
static void PM_TorsoAnimation(void) {
    ...
    if ((pm->ps->legsAnim & ~ANIM_TOGGLEBIT) != BOTH_JUMP &&
        ...
        pm->ps->weaponstate == WEAPON_JUMPING && pm->ps->legsTimer <= 0) {
        pm->ps->weaponstate = WEAPON_READY;
        pm->ps->weapon2state = WEAPON_READY;
    }
...
==============
*/
static qboolean PM_ReloadStart(qboolean weapon2, int *weaponTime,
                               int *weaponstate) {
    int weapon = weapon2 ? pm->ps->weapon2 : pm->ps->weapon;
    ...
    if (pm->ps->weaponstate == WEAPON_DROPPING ||
         ...
         * initiates a PM_BeginWeaponChange() or PM_FinishWeaponChange() to
         * another weapon at the same time. So we cancel the reloading attempt.
         */
        return qfalse;
    }
    ...
    if (pm->ps->ammo[clip] <= 0)
        return qfalse;

    if (pm->ps->ammo[ammo] >= bg_weaponlist[weapon].clipAmmo)
        return qfalse;
    ...
    if (weapon != WP_WINCHESTER66 && weapon != WP_PEACEMAKER &&
        ...
        if (reloading) {
            ...
            if (pm->ps->ammo[ammo] + 1 >= bg_weaponlist[weapon].clipAmmo)
                return qfalse;
            // Prevent any reload if no ammo left
            if (pm->ps->ammo[clip] <= 1)
                return qfalse;
        }
    }
    ...
    if (pm->ps->weapon2) {
        ...
        if (weapon2) {
            ...
            if (oldweaponstate != WEAPON_RELOADING) {
                ...
                pm->ps->stats[STAT_FLAGS] |=
                    SF_RELOAD2; // start animation later
...
static void PM_ReloadEnd(qboolean weapon2) {
    ...
    if (weapon != WP_WINCHESTER66 && weapon != WP_PEACEMAKER &&
        ...
        // the weapon will not get full by this reload -> just add ammo, no
        // checks
        if (pm->ps->ammo[clip] < bg_weaponlist[weapon].clipAmmo &&
            weapon != WP_GATLING) {
            int over;

            over = pm->ps->ammo[ammo] + pm->ps->ammo[clip] -
                   bg_weaponlist[weapon].clipAmmo;
            if (over < 0) {
...
=================
*/
static void PM_WeaponIdle(int *weapon, int *weaponTime, int *weaponstate,
                          qboolean weapon2) {
    *weaponTime = 0;
    ...
    if (*weapon == WP_MOLOTOV && pm->ps->stats[STAT_WP_MODE] < 0) {
        ...
        else if (MOLOTOV_BURNTIME + pm->ps->stats[STAT_WP_MODE] <=
                     PM_AnimLength(WP_ANIM_SPECIAL, qfalse) &&
    ...
    if (pm->ps->stats[IDLE_TIMER] <= 0 || *weapon == WP_GATLING) {
        ...
        if (weapon2 || !pm->ps->weapon2)
            pm->ps->stats[IDLE_TIMER] =
                2500 + rand() % 5000 + PM_AnimLength(WP_ANIM_IDLE, weapon2);
    }
}
...
static qboolean PM_PlanarCheck(void) {
    ...
    // Tequila comment: Use bigger gatling min/max to avoid bad situation when
    // leaving the gatling
    pm->trace(&trace, origin, gatling_mins2, gatling_maxs2, origin,
...
==============
*/
static void PM_WeaponAction(int *weapon, int *weaponTime, int *weaponstate,
                            qboolean weapon2) {
    int addTime, delay = 0;
    int reload = weapon2 ? EF_RELOAD2 : EF_RELOAD;
    int ammo = *weapon;
    ...
    if (pm->ps->eFlags & reload) {
        ...
        case WP_LIGHTNING:
            delay =
                6 * bg_weaponlist[*weapon].animations[WP_ANIM_RELOAD].frameLerp;
            break;
        case WP_WINCH97:
            delay =
                7 * bg_weaponlist[*weapon].animations[WP_ANIM_RELOAD].frameLerp;
            break;
        case WP_PEACEMAKER:
            delay = 16 *
                    bg_weaponlist[*weapon].animations[WP_ANIM_RELOAD].frameLerp;
            break;
        }
    ...
    // now end reload sequence by starting reload animation if necessary
    // (akimbos)
    ...
    *weaponTime = old_weaponTime;
    ...
    if (pm->cmd.buttons & BUTTON_USE_HOLDABLE &&
        !(pm->ps->oldbuttons & BUTTON_USE_HOLDABLE) &&
    ...
    if (bg_weaponlist[*weapon].wp_sort == WPS_PISTOL) {
        ...
        if ((pm->cmd.buttons & BUTTON_ATTACK) &&
            (pm->ps->oldbuttons & BUTTON_ATTACK) &&
        ...
        if ((pm->cmd.buttons & BUTTON_ALT_ATTACK) &&
            (pm->ps->oldbuttons & BUTTON_ALT_ATTACK) &&
            ((pm->ps->weapon2 && weapon2) || !pm->ps->weapon2)) {
    ...
    *weaponstate = WEAPON_FIRING;
    ...
    // fire weapon
    if ((pm->cmd.buttons & BUTTON_ALT_ATTACK ||
         (*weapon == WP_KNIFE && pm->ps->stats[STAT_WP_MODE])) &&
        !pm->ps->weapon2) {
...
static void PM_Weapon(void) {
    ...
    if (pm->ps->ammo[WP_DYNAMITE] &&
        !(pm->ps->stats[STAT_WEAPONS] & (1 << WP_DYNAMITE)))
        pm->ps->stats[STAT_WEAPONS] |= (1 << WP_DYNAMITE);
    if (pm->ps->ammo[WP_MOLOTOV] &&
    ...
    if (pm->ps->stats[STAT_WP_MODE] < 0 &&
        (pm->ps->weapon == WP_DYNAMITE ||
         pm->ps->weapon ==
             WP_MOLOTOV)) { // no other weapons have negative WP_MODE values
        pm->ps->stats[STAT_WP_MODE] -= pml.msec;

        if (pm->ps->stats[STAT_WP_MODE] <=
                -(PM_AnimLength(WP_ANIM_ALT_FIRE, qfalse) +
                  PM_AnimLength(WP_ANIM_SPECIAL, qfalse)) &&
            pm->ps->weapon == WP_DYNAMITE) {
    ...
    if (pm->cmd.buttons & BUTTON_ALT_ATTACK) {
        ...
        case WP_MOLOTOV:
            if (pm->ps->stats[STAT_WP_MODE] >= 0 &&
                pm->ps->ammo[pm->ps->weapon] && pm->ps->weaponTime <= 0 &&
                pm->waterlevel < 3) {
    ...
    // spoon modified it so you can't change weapon if lowering or raising
    // (dunno why) i've written it back, because of the last used weapon func.
        ...
        if (pm->cmd.weapon == WP_AKIMBO && pm->ps->weapon2) {
            ...
            if (pm->ps->weapon == pm->ps->weapon2 &&
                ...
                automatic = qtrue;
                   ...
                   (pm->ps->weapon &&
                    pm->ps
                        ->weapon2)) { // if not akimbo and weapon2 -> change it

            // NB:
            // In case, pm->cmd.weapon == WP_SEC_PISTOL,
            // pm->ps->weapon != pm->cmd.weapon will always be true.
            // Then, a change weapon will happen even if it can result in the
            // same pistol type
            ...
            if (left_pistol) {
                ...
                weapon_pistol = BG_SearchTypeWeapon(
                    WPS_PISTOL, pm->ps->stats[STAT_WEAPONS], 0);
                if (weapon_pistol)
    ...
    PM_WeaponAction(&pm->ps->weapon, &pm->ps->weaponTime, &pm->ps->weaponstate,
                    qfalse);

    if (pm->ps->weapon2)
        PM_WeaponAction(&pm->ps->weapon2, &pm->ps->weapon2Time,
                        &pm->ps->weapon2state, qtrue);
}
...
void PM_UpdateViewAngles(playerState_t *ps, const usercmd_t *cmd) {
    ...
    if (pm->ps->stats[STAT_GATLING_MODE] && pm->ps->weapon == WP_GATLING) {
        ...
        if (!goback) {
            ...
            VectorSet(mins, -16, -16,
                      -18); // set this value a bit higher
                            // to be able to walk on some higher planes-24);
            VectorSet(maxs, 16, 16, 28);

            pm->trace(&trace, oldorigin, mins, maxs, ps->origin, ps->clientNum,
...
static void PM_LadderMove(void) {
    ...
    if (!scale) {
        ...
        for (i = 0; i < 3; i++) {
            ...
            wishvel[i] =
                scale * pml.forward[i] * fw + scale * pml.right[i] * rt;
        }
...
void CheckLadder(void) {
    ...
    pm->trace(&trace, origin, pm->mins, pm->maxs, spot, pm->ps->clientNum,
              MASK_PLAYERSOLID);
...
void PmoveSingle(pmove_t *pmove) {
    ...
    if (pm->cmd.buttons & BUTTON_CHOOSE_MODE &&
        !(pm->ps->oldbuttons & BUTTON_CHOOSE_MODE)) {
        pm->ps->stats[STAT_FLAGS] |= SF_WP_CHOOSE;
    }
