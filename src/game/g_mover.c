...
qboolean G_MoverPush(gentity_t *pusher, vec3_t move, vec3_t amove,
    ...
    *obstacle = NULL;
    ...
    // Tequila comment: Fix "CM_AdjustAreaPortalState: negative reference count"
    // case in SteamBoat Binary mod don't round float in the same way than QVM
    // mod
    ...
    for (e = 0; e < listedEntities; e++) {
        ...
        if ((check->s.eType != ET_ITEM && check->s.eType != ET_PLAYER &&
             check->s.eType != ET_TURRET && !check->physicsObject)) {
            continue;
        }
...
void SetMoverState(gentity_t *ent, moverState_t moverState, int time) {
    ...
    if (ent->moverState == MOVER_STATIC)
        return;
    ...
    if (ent->moverState == MOVER_POS1) {
        ...
        if (ent->s.eFlags & EF_ROTATING_DOOR) {
            ...
            if (ent->spawnflags & DOOR_ROTATING_ONE_WAY) {
                ...
                if (len) {
                    if (activator->client->ps.origin[len] >
                        ent->r.currentOrigin[len]) {
                        if (ent->r.maxs[apos] < -ent->r.mins[apos]) {
                            ent->apos2[axis] += ent->count;
                        } else {
                            ent->apos2[axis] -= ent->count;
                        }
...
void InitMover(gentity_t *ent) {
    ...
    if (ent->s.eFlags & EF_ROTATING_DOOR) {
        ...
        // For God only know the reason, if ent->s.pos.trDuration is not set, it
        // causes trouble...
...
in reverse.  It is used to temporarily or permanently close off an area when
triggered (not useful for touch or takedamage doors). NOMONSTER	monsters will
not trigger this door SILENT		supresses the default door sounds.

"model2"	.md3 model to also draw
"angle"		determines the opening direction
"targetname" if set, no touch field will be spawned and a remote button or
trigger field activates the door. "speed"		movement speed (100
default) "wait"		wait before returning (3 default, -1 = never return)
...
void SP_func_door(gentity_t *ent) {
    ...
    if (!(ent->spawnflags & 32)) {
        ...
        ent->sound1to2 = ent->sound2to1 =
            G_SoundIndex("sound/movers/doors/dr1_strt.wav");
        ent->soundPos1 = ent->soundPos2 =
            G_SoundIndex("sound/movers/doors/dr1_end.wav");
...
START_OPEN	the door to moves to its destination when spawned, and operate
in reverse.  It is used to temporarily or permanently close off an area when
triggered (not useful for touch or takedamage doors). CRUSHER : door will not
reverse direction when blocked and will keep damaging player until he dies or
gets out of the way. RETURN: door will return automatically after having been
opened TRIGGER_DOOR: door can't be used by an activate button but a trigger
...
"model2"	.md3 model to also draw
"targetname" if set, no touch field will be spawned and a remote button or
trigger field activates the door. "speed"		movement speed (130
default) "distance"	how many degrees the door will open, negative to reverse
direction (default 90) "wait"		wait before returning (3 default, -1 =
never return) "lip"		lip remaining at end of move (8 default) "dmg"
damage to inflict when blocked (2 default) "color"		constantLight
color "light"		constantLight radius "health"	if set, the door must be
shot open
*/
...
void func_breakable_die(gentity_t *self, gentity_t *inflictor,
...
void SP_func_breakable(gentity_t *ent) {
    ...
    // Tequila comment: set breakable center as origin for G_BreakableRespawn
    // needs
...
void G_BreakableRespawn(gentity_t *ent) {
    ...
    // cycle through all players and see if the breakable respawn would be
    // visible to them
    for (i = 0; i < level.maxclients; i++) {
        ...
        // Tequila comment: Minor server optimization, don't check if breakable
        // is in a bot FOV They really don't care to "see" a breakable respawn,
        // so we won't delay the respawn because of bot proximity.
...
void G_MoverContents(qboolean change) {
    ...
    for (i = 0; i < MAX_GENTITIES; i++) {
        ...
        if (ent->s.eType == ET_MOVER && (ent->r.contents & CONTENTS_BODY)) {
...
(frame) target : sets the direction of the wind (direction: apos.trBase) wind :
strength of the wind (default 5) (torsoAnim) boost : rise-up-velocity of the
sprites (default 5) (legsAnim)
*/
...
void SP_func_plat(gentity_t *ent) {
    ...
    if (!(ent->spawnflags & 1)) {
        ...
        ent->sound1to2 = ent->sound2to1 =
            G_SoundIndex("sound/movers/plats/pt1_strt.wav");
        ent->soundPos1 = ent->soundPos2 =
            G_SoundIndex("sound/movers/plats/pt1_end.wav");
...
void Reached_Train(gentity_t *ent) {
    ...
    // preliminaries...
    relative_move = ent->spawnflags & TRAIN_RELATIVE_MOVE ||
                    next->spawnflags & TRAIN_RELATIVE_MOVE;
    rotation_only = ent->spawnflags & TRAIN_ROTATION_ONLY ||
                    next->spawnflags & TRAIN_ROTATION_ONLY;
    rot_sync_speedup = ent->spawnflags & TRAIN_ROT_SYNC_SPEEDUP ||
                       next->spawnflags & TRAIN_ROT_SYNC_SPEEDUP;
    rot_sync_slowdown = ent->spawnflags & TRAIN_ROT_SYNC_SLOWDOWN ||
                        next->spawnflags & TRAIN_ROT_SYNC_SLOWDOWN;
    mov_sync_speedup = ent->spawnflags & TRAIN_MOV_SYNC_SPEEDUP ||
                       next->spawnflags & TRAIN_MOV_SYNC_SPEEDUP;
    mov_sync_slowdown = ent->spawnflags & TRAIN_MOV_SYNC_SLOWDOWN ||
                        next->spawnflags & TRAIN_MOV_SYNC_SLOWDOWN;
    ...
    if (rotation_only) {
        ...
        // Standard case, coordinate are taken from the current path_corner
        // origin
    ...
    if (ent->s.pos.trDuration < 1) {
        // Tequila comment: As trDuration is used later in a division, we need
        // to avoid that case now With null trDuration, the calculated rocks
        // bounding box becomes infinite and the engine think for a short time
        // any entity is riding that mover but not the world entity... In rare
        // case, I found it can also stuck every map entities after func_door
        // are used. The desired effect with very very big speed is to have
        // instant move, so any not null duration lower than a frame duration
        // should be sufficient. Afaik, the negative case don't have to be
        // supported.
        ent->s.pos.trDuration = 1;

        // Tequila comment: Don't send entity to clients so it becomes really
        // invisible Joe Kari: can cause bugs, commented out... (especially it
        // limits/bugs rotating train)
        // ent->r.svFlags |= SVF_NOCLIENT;
    }
    ...
    if (ent->s.apos.trDuration > ent->s.pos.trDuration) {
        ...
        if (rot_sync_speedup && mov_sync_slowdown) {
            ...
            ent->s.pos.trDuration =
                (ent->s.pos.trDuration + ent->s.apos.trDuration) / 2;
            ent->s.apos.trDuration = ent->s.pos.trDuration;
...
and models. "model2"	.md3 model to also draw "color"		constantLight
color "light"		constantLight radius
*/
    void SP_func_static(gentity_t * ent) {

        // Joe Kari: new func_static (totally rewritten) with far-clipping and
        // level of detail support with remainder-style comment
        ...
        // because a func_static could have is own origin, this is needed to
        // replace its coord in world coord (automatically made for game, but
        // not for cgame)
        VectorCopy(ent->r.currentOrigin, ent->s.origin);

        // this is needed for CG_Mover() to display the element (yes func_static
        // was used like a mover)
        ent->s.eType = ET_MOVER;

        // just a new value to ensure that the game will never perform "think"
        // for this entity
        ...
        // get the value for the key "farclip" and copy it to powerups (7 lower
        // bits), legsAnim and torsoAnim field

        G_SpawnString("farclip", "none", &value);
        sscanf(value, "%31s %i %i", tmp_char, &ent->s.legsAnim,
               &ent->s.torsoAnim);

        ent->s.powerups = FARCLIP_NONE;

        if (!Q_stricmp("none", tmp_char))
            ent->s.powerups = FARCLIP_NONE;
        else if (!Q_stricmp("always", tmp_char))
            ent->s.powerups = FARCLIP_ALWAYS;
        else if (!Q_stricmp("sphere", tmp_char))
            ent->s.powerups = FARCLIP_SPHERE;
        else if (!Q_stricmp("cube", tmp_char))
            ent->s.powerups = FARCLIP_CUBE;
        else if (!Q_stricmp("ellipse_z", tmp_char))
            ent->s.powerups = FARCLIP_ELLIPSE_Z;
        else if (!Q_stricmp("cylinder_z", tmp_char))
            ent->s.powerups = FARCLIP_CYLINDER_Z;
        else if (!Q_stricmp("box_z", tmp_char))
            ent->s.powerups = FARCLIP_BOX_Z;
        else if (!Q_stricmp("cone_z", tmp_char))
            ent->s.powerups = FARCLIP_CONE_Z;
        else if (!Q_stricmp("pyramid_z", tmp_char))
            ent->s.powerups = FARCLIP_PYRAMID_Z;
        else if (!Q_stricmp("circle_infinite_z", tmp_char))
            ent->s.powerups = FARCLIP_CIRCLE_INFINITE_Z;
        else if (!Q_stricmp("square_infinite_z", tmp_char))
            ent->s.powerups = FARCLIP_SQUARE_INFINITE_Z;
        else if (!Q_stricmp("ellipse_x", tmp_char))
            ent->s.powerups = FARCLIP_ELLIPSE_X;
        else if (!Q_stricmp("cylinder_x", tmp_char))
            ent->s.powerups = FARCLIP_CYLINDER_X;
        else if (!Q_stricmp("box_x", tmp_char))
            ent->s.powerups = FARCLIP_BOX_X;
        else if (!Q_stricmp("cone_x", tmp_char))
            ent->s.powerups = FARCLIP_CONE_X;
        else if (!Q_stricmp("pyramid_x", tmp_char))
            ent->s.powerups = FARCLIP_PYRAMID_X;
        else if (!Q_stricmp("circle_infinite_x", tmp_char))
            ent->s.powerups = FARCLIP_CIRCLE_INFINITE_X;
        else if (!Q_stricmp("square_infinite_x", tmp_char))
            ent->s.powerups = FARCLIP_SQUARE_INFINITE_X;
        else if (!Q_stricmp("ellipse_y", tmp_char))
            ent->s.powerups = FARCLIP_ELLIPSE_Y;
        else if (!Q_stricmp("cylinder_y", tmp_char))
            ent->s.powerups = FARCLIP_CYLINDER_Y;
        else if (!Q_stricmp("box_y", tmp_char))
            ent->s.powerups = FARCLIP_BOX_Y;
        else if (!Q_stricmp("cone_y", tmp_char))
            ent->s.powerups = FARCLIP_CONE_Y;
        else if (!Q_stricmp("pyramid_y", tmp_char))
            ent->s.powerups = FARCLIP_PYRAMID_Y;
        else if (!Q_stricmp("circle_infinite_y", tmp_char))
            ent->s.powerups = FARCLIP_CIRCLE_INFINITE_Y;
        else if (!Q_stricmp("square_infinite_y", tmp_char))
            ent->s.powerups = FARCLIP_SQUARE_INFINITE_Y;

        // get the value for the key "closeclip" and copy it to powerups (7
        // lower bits), legsAnim and torsoAnim field farclip has priority over
        // closeclip, we only try it if there is no farclip
        ...
        if (ent->s.powerups == FARCLIP_NONE) {
            ...
            if (ent->s.powerups)
                ent->s.powerups |= CLOSECLIP_BINARY_MASK;
        }

        if (ent->s.powerups) {
            // "legsAnim" has only 8 bits over network, so the far clip value is
            // rounded to multiple of 64
            ent->s.legsAnim = (ent->s.legsAnim + 32) / 64;
            if (ent->s.legsAnim < 1)
                ent->s.legsAnim = 1;

            // "torsoAnim" has only 8 bits over network, so the far clip value
            // is rounded to multiple of 64
            ent->s.torsoAnim = (ent->s.torsoAnim + 32) / 64;
            if (ent->s.torsoAnim < 1)
                ent->s.torsoAnim = 1;
        }

        // G_Printf( "^4%s (%i) : %i - %i\n" , tmp_char , ent->s.powerups ,
        // ent->s.legsAnim , ent->s.torsoAnim ) ;

        // get the value for the key "lod" (Level Of Detail) and copy it to
        // upper bits of powerups (bits 9 to 12)

        G_SpawnString("lod", "none", &value);
        sscanf(value, "%5s %i", tmp_char, &tmp_int);

        if (!Q_stricmp("min", tmp_char))
            ent->s.powerups |= MAPLOD_BINARY_MASK | MAPLOD_GTE_BINARY_MASK |
                               ((tmp_int & 3) << 8);
        else if (!Q_stricmp("max", tmp_char))
            ent->s.powerups |= MAPLOD_BINARY_MASK | ((tmp_int & 3) << 8);
...
"delta"		angle's delta in degree around the origine angle (only if func =
sin) "freq"		frequency of the movement (only if func = sin) "color"
constantLight color "light"		constantLight radius
*/

// Joe Kari : New func_rotating allowing the "sin" function (greater flexibility
// than pendulum)
...
to the model to allow rotation in other directions. Pendulum frequency is a
physical constant based on the length of the beam and gravity. "model2"	.md3
model to also draw "speed"		the number of degrees each way the
pendulum swings, (30 default) "phase"		the 0.0 to 1.0 offset in the
cycle to start at "dmg"		damage to inflict when blocked (2 default)
"color"		constantLight color
"light"		constantLight radius
*/
