...
// for the voice chats
#include "../ui/ui_menudef.h" // sos001205 - for q3_ui also

// from aasfile.h
#define AREACONTENTS_MOVER 1024
#define AREACONTENTS_MODELNUMSHIFT 24
#define AREACONTENTS_MAXMODELNUM 0xFF
#define AREACONTENTS_MODELNUM                                                  \
...
// NOTE: not using a cvars which can be updated because the game should be
// reloaded anyway
...
int BotTeam(bot_state_t *bs) {
    ...
    if (level.clients[bs->client].sess.sessionTeam == TEAM_RED ||
        level.clients[bs->client].sess.sessionTeam == TEAM_RED_SPECTATOR)
        return TEAM_RED;
    if (level.clients[bs->client].sess.sessionTeam == TEAM_BLUE ||
        level.clients[bs->client].sess.sessionTeam == TEAM_BLUE_SPECTATOR)
        return TEAM_BLUE;
...
int BotBRCarryingMoneyBag(bot_state_t *bs) {
    ...
    if (bs->inventory[INVENTORY_MONEY] > 0)
        return qtrue;
    return qfalse;
}
...
// TheDoctor: adding molotov, dynamite and gatling
int BotChooseBestWeapon(playerState_t *ps, int oldweapon) {
    int i;

    // first look for weapons which already have ammo in it
    // TheDoctor: allow bots to use all weapons
    for (i = WP_MOLOTOV; i > 0; i--) {
        ...
        if (ps->stats[STAT_WEAPONS] & (1 << i) &&
            ...
            if (bg_weaponlist[i].wp_sort == WPS_PISTOL &&
                (ps->stats[STAT_FLAGS] & SF_SEC_PISTOL ||
                 BG_CountTypeWeapons(WPS_PISTOL, ps->stats[STAT_WEAPONS]) >=
                     2)) {
                return WP_AKIMBO;
            }
            return i;
...
==================
*/
qboolean BotAI_FindNearestNode(qboolean valid, bot_state_t *bs, vec3_t org,
                               vec3_t node, float mindist, vec3_t avoid);
void BotSetupForMovement(bot_state_t *bs) {
...
#ifdef DEBUG
    // DEBUG by TheDoctor: treat gatling as activatable entity => no bsp entity
...
#endif
    // the first goal for the bot in br, if he has the goldbag is the escape
    // area
    /*if(g_gametype.integer == GT_BR &&
            bs->cur_ps.powerups[PW_GOLD] &
            !(bs->flags & BFL_ESCAPE)){
            int i;

            for(i=0; i<MAX_GENTITIES; i++){
                    if(!Q_stricmp("trigger_escape", g_entities[i].classname)){
                            vec3_t origin;

                            VectorAdd(g_entities[i].r.absmax,
    g_entities[i].r.absmin, origin); VectorScale(origin, 0.5f, origin);
                            BotAI_FindNearestNode(qfalse, bs, origin,
    bs->escape_point, 0, NULL);
                    }
            }
            bs->flags |= BFL_ESCAPE;
            bs->flags &= ~BFL_MONEY;
            Com_Printf("escaping: %f %f %f\n", bs->escape_point[0],
                    bs->escape_point[1], bs->escape_point[2]);

            trap_BotResetLastAvoidReach(bs->ms);
            trap_BotEmptyGoalStack(bs->gs);
            BotClearActivateGoalStack(bs);
    } else if(!bs->cur_ps.powerups[PW_GOLD])*/
    bs->flags &= ~BFL_ESCAPE;
    ...
    if ((bs->cur_ps.pm_flags & PMF_TIME_KNOCKBACK) &&
        (bs->cur_ps.pm_time > 0)) {
        initmove.or_moveflags |= MFL_TELEPORTED;
    }
    // set the waterjump flag
    if ((bs->cur_ps.pm_flags & PMF_TIME_WATERJUMP) &&
        (bs->cur_ps.pm_time > 0)) {
        initmove.or_moveflags |= MFL_WATERJUMP;
    }
    // Blame TheDoctor ;)
    if ((bs->cur_ps.torsoAnim == BOTH_LADDER ||
         bs->cur_ps.torsoAnim == BOTH_LADDER_STAND) &&
        (bs->cur_ps.pm_time > 0)) {
...
void BotUpdateInventory(bot_state_t *bs) {
    ...
    // weapons
    bs->inventory[INVENTORY_KNIFE] =
        (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_KNIFE)) *
        bs->cur_ps.ammo[WP_KNIFE];
    bs->inventory[INVENTORY_PEACEMAKER] =
        (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_PEACEMAKER)) *
        bs->cur_ps.ammo[WP_PEACEMAKER];
    bs->inventory[INVENTORY_REM58] =
        (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_REM58)) *
        bs->cur_ps.ammo[WP_REM58];
    bs->inventory[INVENTORY_SCHOFIELD] =
        (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_SCHOFIELD)) *
        bs->cur_ps.ammo[WP_SCHOFIELD];
    bs->inventory[INVENTORY_WINCHESTER66] =
        (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_WINCHESTER66)) *
        bs->cur_ps.ammo[WP_WINCHESTER66];
    bs->inventory[INVENTORY_LIGHTNING] =
        (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_LIGHTNING)) *
        bs->cur_ps.ammo[WP_LIGHTNING];
    bs->inventory[INVENTORY_WINCH97] =
        (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_WINCH97)) *
        bs->cur_ps.ammo[WP_LIGHTNING];
    bs->inventory[INVENTORY_SHARPS] =
        (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_SHARPS)) *
        bs->cur_ps.ammo[WP_SHARPS];
    bs->inventory[INVENTORY_REMGAUGE] =
        (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_REMINGTON_GAUGE)) *
        bs->cur_ps.ammo[WP_REMINGTON_GAUGE];
    bs->inventory[INVENTORY_SAWEDOFF] =
        (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_KNIFE)) *
        bs->cur_ps.ammo[WP_SAWEDOFF];
    bs->inventory[INVENTORY_GATLING] =
        (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_GATLING)) *
        bs->cur_ps.ammo[WP_GATLING];
    bs->inventory[INVENTORY_DYNAMITE] =
        (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_DYNAMITE)) *
        bs->cur_ps.ammo[WP_DYNAMITE];
    bs->inventory[INVENTORY_MOLOTOV] =
        (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_MOLOTOV)) *
        bs->cur_ps.ammo[WP_MOLOTOV];

    // ammo
    bs->inventory[INVENTORY_BULLETS] = bs->cur_ps.ammo[WP_BULLETS_CLIP];
    bs->inventory[INVENTORY_SHELLS] = bs->cur_ps.ammo[WP_SHELLS_CLIP];
    bs->inventory[INVENTORY_SHARPS_AMMO] = bs->cur_ps.ammo[WP_SHARPS_CLIP];
    bs->inventory[INVENTORY_GATLING_AMMO] = bs->cur_ps.ammo[WP_GATLING_CLIP];
    bs->inventory[INVENTORY_CART] = bs->cur_ps.ammo[WP_CART_CLIP];

    // powerups
    bs->inventory[INVENTORY_HEALTH] = bs->cur_ps.stats[STAT_HEALTH];
    bs->inventory[INVENTORY_BELT] = bs->cur_ps.powerups[PW_BELT];
    bs->inventory[INVENTORY_MONEY] = bs->cur_ps.stats[PW_GOLD];
    bs->inventory[INVENTORY_SCOPE] = bs->cur_ps.stats[PW_SCOPE];
...
void BotUpdateBattleInventory(bot_state_t *bs, int enemy) {
    ...
    // FIXME: add num visible enemies and num visible team mates to the
    // inventory
}
...
==================
*/
qboolean BotIsDead(bot_state_t *bs) { return (bs->cur_ps.pm_type == PM_DEAD); }
...
qboolean BotIntermission(bot_state_t *bs) {
    ...
    if (level.intermissiontime)
        return qtrue;
    return (bs->cur_ps.pm_type == PM_FREEZE ||
            bs->cur_ps.pm_type == PM_INTERMISSION);
}
...
float BotAggression(bot_state_t *bs) {
    // if the bot has quad
    if (bs->inventory[INVENTORY_ARMOR]) {
    ...
    // if the bot can use the sharps
    if (bs->inventory[INVENTORY_SHARPS] > 0)
        return 100;

    // if the bot can use the lightning
    if (bs->inventory[INVENTORY_LIGHTNING] > 0)
        return 95;

    // if the bot can use the peacemaker
    if (bs->inventory[INVENTORY_PEACEMAKER] > 0)
        return 90;

    // if the bot can use the shotgun
    if (bs->inventory[INVENTORY_REMGAUGE] > 0)
        return 90;

    // if the bot can use the sawedoff
    if (bs->inventory[INVENTORY_SAWEDOFF] > 0)
        return 85;

    // if the bot can use the gatling
    if (bs->inventory[INVENTORY_GATLING] > 0)
        return 80;

    // if the bot can use the winchester 66
    if (bs->inventory[INVENTORY_WINCHESTER66] > 0)
        return 60;

    // if the bot can use the schofield
    if (bs->inventory[INVENTORY_SCHOFIELD] > 0)
        return 45;

    // if the bot can use the rem58
    if (bs->inventory[INVENTORY_REM58] > 0)
        return 40;

    // if the bot can use the dynamite
    if (bs->inventory[INVENTORY_DYNAMITE] > 0)
        return 45;

    if (bs->inventory[INVENTORY_MOLOTOV] > 0)
        return 40;

    // if the bot can use the dynamite
    if (bs->inventory[INVENTORY_KNIFE] > 1)
        return 45;
...
int BotWantsToRetreat(bot_state_t *bs) {
    aas_entityinfo_t entinfo;
    // Tequila: Bot should retreat when it has the money bag in BR mode
    if (gametype == GT_BR) {
...
int BotWantsToChase(bot_state_t *bs) {
    // Tequila: Bot should not chase when it has the money bag in BR mode
    if (gametype == GT_BR) {
        if (BotBRCarryingMoneyBag(bs))
            return qfalse;
    }
...
==================
*/
int BotWantsToHelp(bot_state_t *bs) { return qtrue; }
...
int BotWantsToCamp(bot_state_t *bs) {
    ...
    if (bs->ltgtype == LTG_TEAMHELP || bs->ltgtype == LTG_TEAMACCOMPANY ||
        bs->ltgtype == LTG_DEFENDKEYAREA || bs->ltgtype == LTG_GETFLAG ||
        bs->ltgtype == LTG_RUSHBASE || bs->ltgtype == LTG_CAMP ||
    ...
    if (BotAggression(bs) < 50)
        return qfalse;
    if ((!bs->inventory[INVENTORY_SHARPS] ||
         bs->inventory[INVENTORY_SHARPS_AMMO] < 5) &&
        (bs->inventory[INVENTORY_LIGHTNING] < 8) &&
        (bs->inventory[INVENTORY_WINCHESTER66] < 8) &&
        (bs->inventory[INVENTORY_GATLING] <= 0 ||
         !bs->inventory[INVENTORY_GATLING_AMMO])) {
        return qfalse;
    }
    // find the closest camp spot
    besttraveltime = 99999;
    for (cs = trap_BotGetNextCampSpotGoal(0, &goal); cs;
         cs = trap_BotGetNextCampSpotGoal(cs, &goal)) {
        traveltime = trap_AAS_AreaTravelTimeToGoalArea(
            bs->areanum, bs->origin, goal.areanum, TFL_DEFAULT);
...
void BotRoamGoal(bot_state_t *bs, vec3_t goal) {
    ...
    for (i = 0; i < 10; i++) {
        ...
        BotAI_Trace(&trace, bs->origin, NULL, NULL, bestorg, bs->entitynum,
                    MASK_SOLID);
        ...
        if (len > 200) {
            ...
            BotAI_Trace(&trace, bestorg, NULL, NULL, belowbestorg,
...
int BotMoveInDir(bot_moveresult_t *moveresult, bot_state_t *bs,
                 vec3_t direction, int distance, int movetype, int tfl) {
    vec3_t newpoint;
    aas_entityinfo_t entinfo;
    bot_goal_t goal;
...
bot_moveresult_t BotAttackMove(bot_state_t *bs, int tfl) {
    ...
    if (dist > 60 && bs->weaponnum == WP_GATLING &&
        g_entities[bs->client].client->ps.stats[STAT_GATLING_MODE]) {
        return moveresult;
    }
    ...
    attack_skill = trap_Characteristic_BFloat(
        bs->character, CHARACTERISTIC_ATTACK_SKILL, 0, 1);
    jumper =
        trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_JUMPER, 0, 1);
    jumper = 0.1f;
    croucher = trap_Characteristic_BFloat(bs->character,
                                          CHARACTERISTIC_CROUCHER, 0, 1);
    // TheDoctor: run!!!
    if (bs->enemy && entinfo.weapon == WP_KNIFE) {
        croucher = 0.0f;
        jumper = 0.0f;
    } else {
        // TheDoctor: knife = run
        ...
        // TheDoctor: reload = jump'n'run
        else if ((bs->flags & BFL_RELOAD) || (bs->flags & BFL_RELOAD2)) {
            croucher = 0.05f;
            jumper = 0.95f;
        }
        // TheDoctor: fall = crouch
        if (bs->cur_ps.groundEntityNum == ENTITYNUM_NONE) {
            croucher = 1.0f;
    ...
    if (bs->attackcrouch_time < FloatTime() - 1) {
        ...
        else if (bs->attackcrouch_time < FloatTime() - 1 &&
                 random() < croucher) {
    ...
    if (bs->weaponnum == WP_KNIFE ||
        (entinfo.weapon == WP_KNIFE && dist < 200) ||
        ((entinfo.flags & BFL_RELOAD || bs->flags & BFL_RELOAD2) &&
         dist < 140) ||
        dist < 70) {
        // TheDoctor: attack!
        movetype = MOVE_WALK;
        ...
        if (dist < 60 ||
            (g_entities[bs->client].client->ps.ammo[WP_KNIFE] > 1 &&
             dist < 1500)) {
            if (dist < 20) {
        ...
        if (dist > 40 || random() > 0.66) {
            ...
            //			if (trap_BotMoveInDirection(bs->ms, forward,
            //400, movetype)) { 				return moveresult;
            //			}
        } else {
            if (BotMoveInDir(&moveresult, bs, backward, 400, movetype, tfl)) {
                return moveresult;
            }
            //			if (trap_BotMoveInDirection(bs->ms, backward,
            //400, movetype)) { 				return moveresult;
            //			}
        }
    }
    ...
    //  TheDoctor: Factor lowered
    strafechange_time = 0.4 + (1 - attack_skill) * 0.3;
    if (attack_skill > 0.7)
        strafechange_time += crandom() * 0.3;
    ...
    if (bs->cur_ps.torsoAnim == BOTH_LADDER ||
        bs->cur_ps.torsoAnim == BOTH_LADDER_STAND) {
        // TheDoctor: does not work?
        VectorMA(sideward, 1, backward, sideward);
    ...
    //		CrossProduct(hordir, up, sideward);
    //		//reverse the vector depending on the strafe direction
    //		if (bs->flags & BFL_STRAFERIGHT) VectorNegate(sideward,
    //sideward); 		if (dist > attack_dist + attack_range || !goback) {
    //			VectorAdd(sideward, forward, sideward);
    //		}
    //		else if (dist < attack_dist - attack_range) {
    //			VectorAdd(sideward, backward, sideward);
    ...
    //		if (trap_BotMoveInDirection(bs->ms, sideward, 400, movetype)) {
    //			return moveresult;
    //		} else if (trap_BotMoveInDirection(bs->ms, forward, 400,
    //movetype)) { 			return moveresult;
    //		}
    //	}
...
int BotSameTeam(bot_state_t *bs, int entnum) {
    ...
    if (gametype >= GT_TEAM) {
        if (level.clients[bs->client].sess.sessionTeam ==
            level.clients[entnum].sess.sessionTeam)
            return qtrue;
    }
...
qboolean BotIsARealEnemy(int client, int other) {
    ...
    if (g_gametype.integer >= GT_TEAM) {
        ...
        if ((c_team == TEAM_RED || c_team == TEAM_BLUE) && c_team == o_team) {
            return qfalse;
        }
    } else {
...
returns visibility in the range [0, 1] taking fog and water surfaces into
account
==================
*/
float BotEntityVisible(int viewer, vec3_t eye, vec3_t viewangles, float fov,
                       int ent) {
    int i, j, contents_mask, passent, hitent, infog, inwater, otherinfog, pc;
    float squaredfogdist, waterfactor, glassfactor, vis, bestvis;
    bsp_trace_t trace;
    aas_entityinfo_t entinfo;
    vec3_t dir, entangles, start, end, middle, tmpstart;
    ...
    if (!InFieldOfVision(viewangles, fov, entangles, ent))
        return 0;
    //
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (trace.contents &
            ...
            if (1) {
                ...
                contents_mask &=
                    ~(CONTENTS_LAVA | CONTENTS_SLIME | CONTENTS_WATER);
                BotAI_Trace(&trace, trace.endpos, NULL, NULL, end, passent,
                            contents_mask);
                waterfactor = 0.5;
            }
        }
        ...
        if (trace.fraction >= 1 || trace.ent == hitent) {
            // Check if we would have hit a door or window (BODY)
            BotAI_Trace(&trace, start, NULL, NULL, end, passent,
                        (contents_mask | CONTENTS_BODY));
            glassfactor = 1.0;
            // Propagate from each BODY to the next until we hit
            // the target (at most 10 levels to catch infinite loops)
            for (j = 0; j < 10 && trace.fraction < 1 && trace.ent != hitent;
                 j++) {
                // Only keep going if we hit glass (water is here only for
                // completeness, looking into lakes is something completely
                // different !)
                if (trace.surface.value & (SURF_GLASS | SURF_WATER)) {
                    glassfactor *= 0.9f;
                    // The following math forwards the trace vector through
                    // the glass surface, assuming the glass has a
                    // perpendicular thickness of 1.0, otherwise the
                    // thickness needs to be multiplied to the fabs() exprssion
                    // OTOH, eventually, the glass is passed anyways.
                    VectorSubtract(trace.endpos, start, tmpstart);
                    VectorNormalize(tmpstart);
                    VectorMA(
                        trace.endpos,
                        fabs(1.0 / DotProduct(tmpstart, trace.plane.normal)),
                        tmpstart, tmpstart);
                    // After we propageted through the glass, we try again
                    BotAI_Trace(&trace, tmpstart, NULL, NULL, end, passent,
                                (contents_mask | CONTENTS_BODY));
                } else {
                    // all other surfaces are assumed to be opaque
                    // Com_Printf("Blocked from the bot after %d layers. ",j+1);
                    return 0;
                }
            }
            // Com_Printf("Bot can see you through %d layers. ",j);
            // check for fog, assuming there's only one fog brush where
            // either the viewer or the entity is in or both are in
            // TODO: Test the fog treatment.
            //  Compare to ID's Q3A: there, all the "middle"
            ...
            //  For Smokin' Guns this could not be tested since there are no
            //  maps with fog (and water) Note also, that the current glass
            //  detection logic also overwrites trace.
            ...
            if (infog && otherinfog) {
                ...
                VectorCopy(middle, start);
                BotAI_Trace(&trace, start, NULL, NULL, eye, viewer,
                            CONTENTS_FOG);
                ...
                // if the entity and the viewer are not in fog assume there's no
                // fog in between
                squaredfogdist = 0;
            }
            // decrease visibility with the view distance through fog
            vis = 1 /
                  ((squaredfogdist * 0.001) < 1 ? 1 : (squaredfogdist * 0.001));
            // if entering water or looking through glass, visibility is reduced
            vis *= waterfactor;
            vis *= glassfactor;
            //
            if (vis > bestvis)
...
int BotFindEnemy(bot_state_t *bs, int curenemy) {
    ...
    alertness = trap_Characteristic_BFloat(bs->character,
                                           CHARACTERISTIC_ALERTNESS, 0, 1);
    easyfragger = trap_Characteristic_BFloat(bs->character,
    ...
    for (i = 0; i < maxclients && i < MAX_CLIENTS; i++) {
        ...
        // unlagged - misc
        //  this has nothing to do with lag compensation, but it's great for
        //  testing
...
// unlagged - misc
        ...
        if (curenemy < 0 && (healthdecrease || EntityIsShooting(&entinfo)))
            ...
            f = 90 + 90 -
                (90 - (squaredist > Square(810) ? Square(810) : squaredist) /
                          (810 * 9));
        // check if the enemy is visible
        vis = BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, f, i);
        if (vis <= 0)
            continue;
        // if the enemy is quite far away, not shooting and the bot is not
        // damaged
        if (curenemy < 0 && squaredist > Square(100) && !healthdecrease &&
...
#ifdef DEBUG
    // DEBUG by TheDoctor: treat gatling as enemy => SEGFAULT
    for (i = 0; i < MAX_GENTITIES; i++) {
        if (!Q_stricmp("gatling", g_entities[i].classname)) {
            VectorSubtract(g_entities[i].r.currentOrigin, bs->origin, forward);
            // the distance towards the gatling
...
void BotAimAtEnemy(bot_state_t *bs) {
    ...
    //
    aim_skill = trap_Characteristic_BFloat(bs->character,
    ...
    if (aim_skill > 0.95) {
        // don't aim too early
        reactiontime =
            0.5 * trap_Characteristic_BFloat(bs->character,
    ...
    enemyvisible = BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360,
                                    bs->enemy);
    // if the enemy is visible
    if (enemyvisible > (1.0 - aim_skill)) {
        // Com_Printf("Bot is aiming at you. ");
        //
        ...
        start[2] += wi.offset[2];
        //
        BotAI_Trace(&trace, start, mins, maxs, bestorigin, bs->entitynum,
                    MASK_SHOT);
        // if the enemy is NOT hit
        if (trace.fraction <= 1 && trace.ent != entinfo.number) {
            bestorigin[2] += 16;
        }
        // if it is not an instant hit weapon the bot might want to predict the
        // enemy
        if (wi.speed) {
            //
            ...
            VectorSubtract(entinfo.origin, bs->enemyorigin, dir);
            // if the enemy is NOT pretty far away and strafing just small steps
            // left and right
            if (!(dist > 100 && VectorLengthSquared(dir) < Square(32))) {
                ...
                if (aim_skill > 0.8 &&
                    ...
                    trap_AAS_PredictClientMovement(
                        &move, bs->enemy, origin, PRESENCE_CROUCH, qfalse, dir,
                        cmdmove, 0, dist * 10 / wi.speed, 0.1f, 0, 0, qfalse);
                    VectorCopy(move.endpos, bestorigin);
                    // BotAI_Print(PRT_MESSAGE, "%1.1f predicted speed = %f,
                    // frames = %f\n", FloatTime(), VectorLength(dir), dist * 10
                    // / wi.speed);
                }
                // if not that skilled do linear prediction
                    ...
                    // botimport.Print(PRT_MESSAGE, "speed = %f, wi->speed =
                    // %f\n", speed, wi->speed); best spot to aim at
                    VectorMA(entinfo.origin, (dist / wi.speed) * speed, dir,
                             bestorigin);
                }
            }
        }
        ...
        if (aim_skill > 0.6 && wi.proj.damagetype & DAMAGETYPE_RADIAL) {
            ...
            if (entinfo.origin[2] < bs->origin[2] + 16) {
                ...
                BotAI_Trace(&trace, entinfo.origin, NULL, NULL, end,
                ...
                BotAI_Trace(&trace, start, NULL, NULL, groundtarget,
                            bs->entitynum, MASK_SHOT);
                ...
                if (fabs(trace.endpos[2] - groundtarget[2]) < 50) {
                    ...
                    if (VectorLengthSquared(dir) < Square(60)) {
                        ...
                        if (VectorLengthSquared(dir) > Square(100)) {
                            // check if the bot is visible from the ground
                            // target
                            trace.endpos[2] += 1;
                            BotAI_Trace(&trace, trace.endpos, NULL, NULL,
                                        entinfo.origin, entinfo.number,
                                        MASK_SHOT);
                            if (trace.fraction >= 1) {
                                // botimport.Print(PRT_MESSAGE, "%1.1f aiming at
                                // ground\n", AAS_Time());
        ...
        if (aim_skill > 0.5) {
            ...
            if (wi.number == WP_SAWEDOFF || wi.number == WP_WINCH97 ||
                wi.number == WP_DYNAMITE || wi.number == WP_MOLOTOV) {
                // create the chase goal
                ...
                VectorSet(goal.maxs, 8, 8, 8);
                //
                if (trap_BotPredictVisiblePosition(bs->lastenemyorigin,
                                                   bs->lastenemyareanum, &goal,
                                                   TFL_DEFAULT, target)) {
                    VectorSubtract(target, bs->eye, dir);
    ...
    //
    if (enemyvisible > (1.0 - aim_skill)) {
        BotAI_Trace(&trace, bs->eye, NULL, NULL, bestorigin, bs->entitynum,
                    MASK_SHOT);
    ...
    //
    if (wi.number == WP_SHARPS || wi.number == WP_WINCHESTER66 ||
        wi.number == WP_LIGHTNING || wi.number == WP_PEACEMAKER) {
        // distance towards the enemy
        dist = VectorLength(dir);
        if (dist > 150)
            dist = 150;
    ...
    bs->ideal_viewangles[PITCH] +=
        6 * wi.vspread * crandom() * (1 - aim_accuracy);
    bs->ideal_viewangles[PITCH] = AngleMod(bs->ideal_viewangles[PITCH]);
    bs->ideal_viewangles[YAW] +=
        6 * wi.hspread * crandom() * (1 - aim_accuracy);
    ...
    VectorSubtract(entinfo.origin, bs->origin, forward);
    // TheDoctor: the distance towards the enemy
    // arctan arcsin
    //
    //
    //       o
    ...
    if (bs->weaponnum == WP_KNIFE) { // throw knife
        ...
        ////height = 0;
        ////angle
        ///= 3.14159388/2-asin(((DEFAULT_GRAVITY*dist)/1200)*sqrt(2/((1200*1200-2*height*DEFAULT_GRAVITY)+sqrt((1200*1200-2*height*DEFAULT_GRAVITY)*(1200*1200-2*height*DEFAULT_GRAVITY)
        ///- (4*DEFAULT_GRAVITY*DEFAULT_GRAVITY)*(dist*dist+height*height)))));
        // BotAI_Print(PRT_MESSAGE, "dist=%f, angle=%f,
...
void BotCheckBuy(bot_state_t *bs) {
    ...
    if (g_gametype.integer >= GT_RTP) {
        ...
        if (level.time - g_roundstarttime > BUY_TIME)
            return;

        // if already bought something this round
        // wenn flag gesetzt -> runde muss gerade sein
        // wenn nich -> runde muss ungerade sein
        if (((bs->flags & BFL_BOUGHT) && g_round % 2) ||
            (!(bs->flags & BFL_BOUGHT) && !(g_round % 2)))
            return;
    }

    // bots try to buy things at anytime in dm
    // if(g_gametype.integer <= GT_TEAM)
    //	return;
    ...
    for (i = 1, mark = -1, mark2 = -1;; i++) {
        ...
        if (((bs->cur_ps.stats[STAT_WEAPONS] & (1 << item->giTag)) &&
             bg_itemlist[item->giTag].weapon_sort != WPS_PISTOL) ||
            (bg_itemlist[item->giTag].weapon_sort != WPS_PISTOL &&
             (bs->cur_ps.stats[STAT_FLAGS] & SF_SEC_PISTOL)))
            continue;
    ...
    for (i = 1, mark = -1, mark2 = -1;; i++) {
        ...
        if (random() > 0.50 && item->giTag == WP_DYNAMITE)
            continue;
    ...
    if (bought) {
        // TheDoctor: update the buytime only in case we buy something
        bs->buytime = level.time + 3000 + rand() % 4000;
    } else {
        // Tequila: But why not delay next check a little to avoid chance to
        // come back here during the next frame
        bs->buytime = level.time + 1000 + rand() % 1000;
    }
}
...
void BotCheckReload(bot_state_t *bs, int weapon, int reloadbit) {
    ...
    if (bg_weaponlist[weapon].clip == -1)
        return;

    // if out of ammo, do reload
    if (!bs->cur_ps.ammo[weapon] &&
        bs->cur_ps.ammo[bg_weaponlist[weapon].clip]) {

        // if we have akimbos and one is loaded
        if (bs->cur_ps.weapon2 && (bs->cur_ps.ammo[bs->cur_ps.weapon] ||
                                   bs->cur_ps.ammo[bs->cur_ps.weapon2]))
            return;
        ...
        if(!(bs->flags & BFL_SEEK)){
                bs->flags |= BFL_RETREAT;
                AIEnter_Battle_Retreat(bs, "reload: wants to retreat");
        }*/
        return;
    }

    // if not default ammo and no enemy do reload
    if (bs->cur_ps.ammo[bg_weaponlist[weapon].clip] &&
        bs->cur_ps.ammo[weapon] != bg_weaponlist[weapon].clipAmmo &&
        bs->enemy < 0) {
        int i;

        // go through all clients and look if there's noone in the near
        // environment
        for (i = 0; i < MAX_CLIENTS; i++) {
            ...
            if (Distance(entinfo.origin, bs->eye) < 500 &&
                BotIsARealEnemy(bs->client, i))
                return;
        }
...
void BotCheckAttack(bot_state_t *bs) {
    ...
    if ((bs->flags & BFL_RELOAD) || (bs->flags & BFL_RELOAD2))
        return;
    ...
    reactiontime = trap_Characteristic_BFloat(
        bs->character, CHARACTERISTIC_REACTIONTIME, 0, 1);
    if (bs->enemysight_time > FloatTime() - reactiontime)
        return;
    if (bs->teleport_time > FloatTime() - reactiontime)
        return;
    ...
    firethrottle = trap_Characteristic_BFloat(
        bs->character, CHARACTERISTIC_FIRETHROTTLE, 0, 1);
    ...
    //
    vectoangles(dir, angles);
    if (!InFieldOfVision(bs->viewangles, fov, angles, attackentity))
        return;
    ...
    start[2] +=
        forward[2] * wi.offset[0] + right[2] * wi.offset[1] + wi.offset[2];
    // end point aiming at
    ...
    // if fire has to be release to activate weapon
    {
        ...
        if ((bg_weaponlist[bs->cur_ps.weapon].wp_sort == WPS_PISTOL &&
             !bs->cur_ps.weapon2 && Distance(bs->eye, bs->aimtarget) < 500) ||
            (bs->cur_ps.weapon2 && bs->cur_ps.weaponTime > 0) ||
            (bs->cur_ps.weapon2 && !bs->cur_ps.ammo[bs->cur_ps.weapon]))
            bit = BFL_ATTACK2;
...
==================
*/
int BotModelMinsMaxs(int modelindex, int eType, int contents, vec3_t mins,
                     vec3_t maxs) {
    gentity_t *ent;
    int i;
...
==================
*/
int BotFuncButtonActivateGoal(bot_state_t *bs, int bspent,
                              bot_activategoal_t *activategoal) {
    ...
    dist = fabs(movedir[0]) * size[0] + fabs(movedir[1]) * size[1] +
           fabs(movedir[2]) * size[2];
    dist *= 0.5;
    //
    ...
    if (health) {
        ...
        if (bsptrace.fraction >= 1.0 || bsptrace.ent == entitynum) {
            ...
            activategoal->goal.entitynum =
                entitynum; // NOTE: this is the entity number of the shootable
                           // button
            ...
            // create a goal from where the button is visible and shoot at the
            // button from there add bounding box size to the dist
            trap_AAS_PresenceTypeBoundingBox(PRESENCE_CROUCH, bboxmins,
                                             bboxmaxs);
            for (i = 0; i < 3; i++) {
                if (movedir[i] < 0)
            ...
            if (i < 0) {
                // FIXME: trace forward and maybe in other directions to find a
                // valid area
            }
            if (i >= 0) {
                //
                ...
                for (i = 0; i < 3; i++) {
                    if (movedir[i] < 0)
                        activategoal->goal.maxs[i] +=
                            fabs(movedir[i]) * fabs(extramaxs[i]);
                    else
                        activategoal->goal.mins[i] +=
                            fabs(movedir[i]) * fabs(extramins[i]);
                } // end for
                //
...
==================
*/
int BotFuncBreakableGoal(bot_state_t *bs, int entitynum,
                         bot_activategoal_t *activategoal) {
    int weapon;
    vec3_t origin;
    ...
    //
    activategoal->goal.entitynum =
        entitynum; // NOTE: this is the entity number of the breakable
    activategoal->goal.number = 0;
    activategoal->goal.flags = 0;
    VectorCopy(bs->origin, activategoal->goal.origin);
    activategoal->goal.areanum = bs->areanum;
    ...
    // if the breakable can only be destroyed with a certain weapon
    if (g_entities[entitynum].s.weapon) {
        weapon = g_entities[entitynum].s.weapon;
        activategoal->weapon = weapon;

        if (!(bs->cur_ps.stats[STAT_WEAPONS] & (1 << weapon)) ||
            (!bs->cur_ps.ammo[weapon] &&
             (!bg_weaponlist[weapon].clip ||
              !bs->cur_ps.ammo[bg_weaponlist[weapon].clip]))) {
            return qfalse;
        }
    } else {
...
==================
*/
int BotFuncDoorActivateGoal(bot_state_t *bs, int bspent,
                            bot_activategoal_t *activategoal) {
    ...
    //
    activategoal->goal.entitynum =
        entitynum; // NOTE: this is the entity number of the shootable door
    activategoal->goal.number = 0;
    activategoal->goal.flags = 0;
    VectorCopy(bs->origin, activategoal->goal.origin);
    activategoal->goal.areanum = bs->areanum;
...
==================
*/
int BotTriggerMultipleActivateGoal(bot_state_t *bs, int bspent,
                                   bot_activategoal_t *activategoal) {
    int i, areas[10], numareas, modelindex, entitynum;
    char model[128];
...
==================
*/
int BotPushOntoActivateGoalStack(bot_state_t *bs,
                                 bot_activategoal_t *activategoal) {
    int i, best;
    float besttime;
    ...
    if (best != -1) {
        memcpy(&bs->activategoalheap[best], activategoal,
               sizeof(bot_activategoal_t));
...
static void GetAvoidDir(int num, vec3_t origin, vec3_t maxs, vec3_t mins,
                        vec3_t result) {
    vec3_t nvec1, nvec2; // nearest vector from the brush to the player
    int i, mark;
    vec3_t dir[4];
    vec3_t pdir;
    vec3_t maxs2, mins2;
    ...
    for (i = 0; i < MAX_GENTITIES; i++) {
        ...
        if (i != num && !Q_stricmp(ent->classname, "func_breakable") &&
            Distance(origin, ent->r.currentOrigin) < 300) {
            VectorAdd(ent->r.mins, ent->r.currentOrigin, maxs2);
            VectorAdd(ent->r.maxs, ent->r.currentOrigin, mins2);
            sec = qtrue;
            // Com_Printf("second\n");
            break;
        }
    ...
    // get nearest point
    for (i = 0; i < 2; i++) {
        if (fabs(origin[i] - maxs[i]) < fabs(origin[i] - mins[i]))
            nvec1[i] = maxs[i];
        else
            nvec1[i] = mins[i];
    }
    nvec1[2] = (maxs[2] + mins[2]) * 0.5;

    if (sec) {
        for (i = 0; i < 2; i++) {
            if (fabs(origin[i] - maxs2[i]) < fabs(origin[i] - mins2[i]))
                nvec2[i] = maxs2[i];
            else
                nvec2[i] = mins2[i];
        }
        nvec2[2] = (maxs2[2] + mins2[2]) * 0.5;
    }

    // see if we can see a clear direction to go to
    for (i = 0; i < 2; i++) {
        if ((maxs[i] > origin[i] && mins[i] < origin[i]) ||
            (mins[i] > origin[i] && maxs[i] < origin[i])) {
        ...
        if (sec && ((maxs2[i] > origin[i] && mins2[i] < origin[i]) ||
                    (mins2[i] > origin[i] && maxs2[i] < origin[i]))) {
            ...
            // Com_Printf("easy choice\n");
            return;
        }
    }

    // if that didn't show which direction is more likely to be safe
    VectorSet(dir[0], 1, 0, 0);
    VectorSet(dir[1], 0, 1, 0);
    VectorSet(dir[2], -1, 0, 0);
    VectorSet(dir[3], 0, -1, 0);
    if (sec) {
        VectorAdd(nvec1, nvec2, nvec1);
        VectorScale(nvec1, 0.5, nvec1);
    }
    VectorSubtract(origin, nvec1, pdir);
    VectorNormalize(pdir);

    for (i = 0, mark = 0; i < 4; i++) {
        if (Distance(pdir, dir[i]) < Distance(pdir, dir[mark]))
            mark = i;
    }

    VectorCopy(dir[mark], result);
}
...
int BotGetActivateGoal(bot_state_t *bs, int entitynum,
                       bot_activategoal_t *activategoal) {
    int i, ent, cur_entities[10], spawnflags, modelindex,
        areas[MAX_ACTIVATEAREAS * 2], numareas, t;
    ...
    // if it is a door
    if (!Q_stricmp(classname, "func_door") ||
        !Q_stricmp(classname, "func_door_rotating")) {
            ...
            if (modelindex) {
                ...
                for (i = 0; i < numareas; i++) {
                    ...
                    if (areainfo.contents & AREACONTENTS_MOVER) {
                        activategoal->areas[activategoal->numareas++] =
                            areas[i];
                    }
                }
    ...
    if (!Q_stricmp(classname, "func_breakable")) {
        if (!(bs->flags & BFL_AVOID) &&
            BotFuncBreakableGoal(bs, entitynum, activategoal)) {
    ...
    if (!trap_AAS_ValueForBSPEpairKey(ent, "targetname", targetname[0],
                                      sizeof(targetname[0]))) {
        if (bot_developer.integer) {
            BotAI_Print(PRT_ERROR,
                        "BotGetActivateGoal: entity with model \"%s\" has no "
                        "targetname\n",
                        model);
        }
        return 0;
    ...
    for (i = 0; i >= 0 && i < 10;) {
        for (ent = cur_entities[i]; ent; ent = trap_AAS_NextBSPEntity(ent)) {
            if (!trap_AAS_ValueForBSPEpairKey(ent, "target", target,
                                              sizeof(target)))
                continue;
        ...
        if (!trap_AAS_ValueForBSPEpairKey(ent, "classname", classname,
                                          sizeof(classname))) {
            if (bot_developer.integer) {
                BotAI_Print(PRT_ERROR,
                            "BotGetActivateGoal: entity with target \"%s\" has "
                            "no classname\n",
                            targetname[i]);
            }
            continue;
        ...
        if (!strcmp(classname, "func_button")) {
            ...
            if (bs->activatestack && bs->activatestack->inuse &&
                bs->activatestack->goal.entitynum ==
                    activategoal->goal.entitynum &&
            ...
            if (trap_AAS_AreaReachability(bs->areanum)) {
                ...
                t = trap_AAS_AreaTravelTimeToGoalArea(
                    bs->areanum, bs->origin, activategoal->goal.areanum,
                    bs->tfl);
        ...
        // the actual button or trigger might be linked through a target_relay
        // or target_delay
        else if (!strcmp(classname, "target_relay") ||
                 !strcmp(classname, "target_delay")) {
            if (trap_AAS_ValueForBSPEpairKey(ent, "targetname",
                                             targetname[i + 1],
                                             sizeof(targetname[0]))) {
                i++;
...
#ifdef OBSTACLEDEBUG
    BotAI_Print(PRT_ERROR,
                "BotGetActivateGoal: no valid activator for entity with target "
                "\"%s\"\n",
                targetname[0]);
#endif
    return 0;
...
==================
*/
void BotPrintActivateGoalInfo(bot_state_t *bs, bot_activategoal_t *activategoal,
                              int bspent) {
    ...
    if (activategoal->shoot) {
        Com_sprintf(
            ...
            activategoal->goal.origin[1], activategoal->goal.origin[2],
...
void BotAIBlocked(bot_state_t *bs, bot_moveresult_t *moveresult, int activate) {
    ...
    vec3_t hordir, sideward, direction, left, right, angles, forward, backward,
        up = {0, 0, 1};
    ...
    if ((bs->weaponnum == WP_GATLING &&
         g_entities[bs->client].client->ps.stats[STAT_GATLING_MODE]) ||
        !moveresult->blocked && (bs->origin[0] != bs->myorigin[0] &&
                                 bs->origin[1] != bs->myorigin[1])) {
    ...
    // this happens if he trace hits a wall before an entity, even though, the
    // blocking reason is the entity
    if (moveresult->blockentity == 0) {
        for (i = 0; i < level.num_entities && moveresult->blockentity == 0;
             i++) {
            ent = &g_entities[i];
            if (!ent) {
                break;
            }
            if ((ent->r.contents & CONTENTS_MOVER) &&
                !!Q_stricmp(ent->classname, "func_door_rotate")) {
                VectorSubtract(ent->r.mins, g_entities[bs->entitynum].r.maxs,
                               absmins);
                VectorSubtract(ent->r.maxs, g_entities[bs->entitynum].r.mins,
                               absmaxs);
                VectorAdd(absmins, ent->r.currentOrigin, absmins);
                VectorAdd(absmaxs, ent->r.currentOrigin, absmaxs);

                for (j = 0; j < 3; j++) {
                    if (bs->origin[j] < absmins[j] ||
                        bs->origin[j] > absmaxs[j]) {
                        break;
                    }
                    if (j == 3) {
                        BotAI_Print(PRT_MESSAGE,
                                    "checking for doors nearby - found one\n");
    ...
    if (!Q_stricmp(g_entities[moveresult->blockentity].classname,
                   "func_breakable")) {
#ifdef DEBUG
    ...
    } else if (bs->origin[0] != bs->myorigin[0] &&
               bs->origin[1] != bs->myorigin[1]) { // DEBUG DOCTOR
        // generally, look into the direction of the breakable
        VectorCopy(moveresult->movedir, direction);
        direction[2] = 0;
        VectorNormalize(direction);
    ...
    //	if (activate && entinfo.modelindex > 0 && entinfo.modelindex <=
    //max_bspmodelindex && (FloatTime() - bs->last_blocked_time > 2) ) {
    //		// find the bsp entity which should be activated in order to get
    //the blocking entity out of the way 		bspent = BotGetActivateGoal(bs,
    //entinfo.number, &activategoal); 		if (bspent) { 			if (bs->activatestack &&
    //!bs->activatestack->inuse) 				bs->activatestack = NULL;
    //			// if not already trying to activate this entity
    //			if (!BotIsGoingToActivateEntity(bs,
    //activategoal.goal.entitynum)) { 				BotAI_Print(PRT_MESSAGE, " and will\n");
    //				BotGoForActivateGoal(bs, &activategoal);
    //			} else {
    //				BotAI_Print(PRT_MESSAGE, " and already going
    //to\n");
    //			}
    //			// if ontop of an obstacle or
    //			// if the bot is not in a reachability area it'll still
    //			// need some dynamic obstacle avoidance, otherwise
    //return 			if (!(moveresult->flags & MOVERESULT_ONTOPOFOBSTACLE) &&
...
int BotAIPredictObstacles(bot_state_t *bs, bot_goal_t *goal) {
    ...
    trap_AAS_PredictRoute(&route, bs->areanum, bs->origin, goal->areanum,
                          bs->tfl, 100, 1000,
    ...
    if (route.stopevent & RSE_ENTERCONTENTS) {
        ...
        if (route.endcontents & AREACONTENTS_MOVER) {
            ...
            modelnum = (route.endcontents & AREACONTENTS_MODELNUM) >>
                       AREACONTENTS_MODELNUMSHIFT;
            if (modelnum) {
                //
                entitynum = BotModelMinsMaxs(modelnum, ET_MOVER, 0, NULL, NULL);
                if (entitynum) {
                    // NOTE: BotGetActivateGoal already checks if the door is
                    // open or not
                    ...
                    if (bspent) {
                        ...
                        if (bs->activatestack && !bs->activatestack->inuse)
                            bs->activatestack = NULL;
                        // if not already trying to activate this entity
                        if (!BotIsGoingToActivateEntity(
                                bs, activategoal.goal.entitynum)) {
                            //
                            // BotAI_Print(PRT_MESSAGE, "blocked by mover model
                            // %d, entity %d ?\n", modelnum, entitynum);
                            //
                            BotGoForActivateGoal(bs, &activategoal);
                            return qtrue;
...
void BotCheckConsoleMessages(bot_state_t *bs) {
    ...
    while ((handle = trap_BotNextConsoleMessage(bs->cs, &m)) != 0) {
        // if the chat state is flooded with messages the bot will read them
        // quickly
        ...
        if (!BotMatchMessage(bs, m.message)) {
            ...
            if (m.type == CMS_CHAT && !bot_nochat.integer) {
                //
                if (!trap_BotFindMatch(m.message, &match,
                                       MTCONTEXT_REPLYCHAT)) {
                ...
                //
                trap_BotMatchVariable(&match, NETNAME, netname,
                                      sizeof(netname));
                trap_BotMatchVariable(&match, MESSAGE, message,
                                      sizeof(message));
                ...
                if (bot_testrchat.integer) {
                    ...
                    if (trap_BotReplyChat(bs->cs, message, context,
                                          CONTEXT_REPLY, NULL, NULL, NULL, NULL,
                                          NULL, NULL, botname, netname)) {
                        BotAI_Print(PRT_MESSAGE, "------------------------\n");
                    } else {
                        BotAI_Print(PRT_MESSAGE, "**** no valid reply ****\n");
                    }
                }
                // if at a valid chat position and not chatting already and not
                // in teamplay
                else if (bs->ainode != AINode_Stand &&
                         BotValidChatPosition(bs) && !TeamPlayIsOn()) {
                    chat_reply = trap_Characteristic_BFloat(
                        bs->character, CHARACTERISTIC_CHAT_REPLY, 0, 1);
                    if (random() < 1.5 / (NumBots() + 1) &&
                        random() < chat_reply) {
                        // if bot replies with a chat message
                        if (trap_BotReplyChat(bs->cs, message, context,
                                              CONTEXT_REPLY, NULL, NULL, NULL,
                                              NULL, NULL, NULL, botname,
                                              netname)) {
                            // remove the console message
                            trap_BotRemoveConsoleMessage(bs->cs, handle);
                            bs->stand_time = FloatTime() + BotChatTime(bs);
                            AIEnter_Stand(
                                bs, "BotCheckConsoleMessages: reply chat");
...
void BotCheckForGrenades(bot_state_t *bs, entityState_t *state) {
    ...
    if (state->eType == ET_MISSILE && state->weapon == WP_DYNAMITE &&
        state->apos.trDelta[0]) {
        trap_BotAddAvoidSpot(bs->ms, state->pos.trBase, 600, AVOID_ALWAYS);
    } else if ((!Q_stricmp(g_entities[state->number].classname,
                           "whiskey_pool")) &&
               state->apos.trDelta[0] == 1) {
...
void BotCheckEvents(bot_state_t *bs, entityState_t *state) {
    ...
    case EV_OBITUARY: {
        ...
        if (target == bs->client) {
            ...
            //
            if (target == attacker || target == ENTITYNUM_NONE ||
    ...
    case EV_CHANGE_WEAPON:
    case EV_FIRE_WEAPON:
        // FIXME: either add to sound queue or mark player as someone making
        // noise
        break;
    case EV_USE_ITEM0:
    case EV_USE_ITEM1:
...
void BotCheckAir(bot_state_t *bs) {
    if (trap_AAS_PointContents(bs->eye) &
...
bot_goal_t *BotAlternateRoute(bot_state_t *bs, bot_goal_t *goal) {
    ...
    if (bs->altroutegoal.areanum) {
        ...
        t = trap_AAS_AreaTravelTimeToGoalArea(
            bs->areanum, bs->origin, bs->altroutegoal.areanum, bs->tfl);
        if (t && t < 20) {
...
void BotDeathmatchAI(bot_state_t *bs, float thinktime) {
    ...
    if (!BotIntermission(bs)) {
        ...
        // check for air
        BotCheckAir(bs);
        // check if bot should reload
        BotCheckReload(bs, bs->cur_ps.weapon, BFL_RELOAD);
        BotCheckReload(bs, bs->cur_ps.weapon2, BFL_RELOAD2);
...
==================
*/
void BotSetEntityNumForGoalWithModel(bot_goal_t *goal, int eType,
                                     char *modelname) {
    gentity_t *ent;
    int i, modelindex;
    vec3_t dir;
...
int BotGoalForBSPEntity(char *classname, bot_goal_t *goal) {
    ...
    for (ent = trap_AAS_NextBSPEntity(0); ent;
         ent = trap_AAS_NextBSPEntity(ent)) {
        if (!trap_AAS_ValueForBSPEpairKey(ent, "classname", value,
                                          sizeof(value)))
            continue;
