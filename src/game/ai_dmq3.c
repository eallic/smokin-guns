...
int BotBRCarryingMoneyBag(bot_state_t *bs) {
	if (gametype != GT_BR) return qfalse;

	if (bs->inventory[INVENTORY_MONEY] > 0) return qtrue;
	return qfalse;
}
#endif
	...
	for(i=WP_MOLOTOV; i>0; i--){
		...
		if(ps->stats[STAT_WEAPONS] & (1 << i) &&
			...
			if(bg_weaponlist[i].wp_sort == WPS_PISTOL &&
				...
				BG_CountTypeWeapons(WPS_PISTOL, ps->stats[STAT_WEAPONS]) >= 2)){
				return WP_AKIMBO;
			}
			return i;
		}
...
void BotSetupForMovement(bot_state_t *bs) {
	...
	/*if(g_gametype.integer == GT_BR &&
		...
		for(i=0; i<MAX_GENTITIES; i++){
			if(!Q_stricmp("trigger_escape", g_entities[i].classname)){
				...
				VectorAdd(g_entities[i].r.absmax, g_entities[i].r.absmin, origin);
				VectorScale(origin, 0.5f, origin);
				BotAI_FindNearestNode(qfalse, bs, origin, bs->escape_point, 0, NULL);
			}
		}
		bs->flags |= BFL_ESCAPE;
		bs->flags &= ~BFL_MONEY;
	...
	// Blame TheDoctor ;)
	if ((bs->cur_ps.torsoAnim==BOTH_LADDER || bs->cur_ps.torsoAnim==BOTH_LADDER_STAND) && (bs->cur_ps.pm_time > 0)) {
...
#endif
#else
	bs->inventory[INVENTORY_KNIFE] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_KNIFE))*bs->cur_ps.ammo[WP_KNIFE];
	bs->inventory[INVENTORY_PEACEMAKER] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_PEACEMAKER))*bs->cur_ps.ammo[WP_PEACEMAKER];
	bs->inventory[INVENTORY_REM58] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_REM58))*bs->cur_ps.ammo[WP_REM58];
	bs->inventory[INVENTORY_SCHOFIELD] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_SCHOFIELD))*bs->cur_ps.ammo[WP_SCHOFIELD];
	bs->inventory[INVENTORY_WINCHESTER66] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_WINCHESTER66))*bs->cur_ps.ammo[WP_WINCHESTER66];
	bs->inventory[INVENTORY_LIGHTNING] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_LIGHTNING))*bs->cur_ps.ammo[WP_LIGHTNING];
	bs->inventory[INVENTORY_WINCH97] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_WINCH97))*bs->cur_ps.ammo[WP_LIGHTNING];
	bs->inventory[INVENTORY_SHARPS] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_SHARPS))*bs->cur_ps.ammo[WP_SHARPS];
	bs->inventory[INVENTORY_REMGAUGE] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_REMINGTON_GAUGE))*bs->cur_ps.ammo[WP_REMINGTON_GAUGE];
	bs->inventory[INVENTORY_SAWEDOFF] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_KNIFE))*bs->cur_ps.ammo[WP_SAWEDOFF];
	bs->inventory[INVENTORY_GATLING] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_GATLING))*bs->cur_ps.ammo[WP_GATLING];
	bs->inventory[INVENTORY_DYNAMITE] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_DYNAMITE))*bs->cur_ps.ammo[WP_DYNAMITE];
	bs->inventory[INVENTORY_MOLOTOV] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_MOLOTOV))*bs->cur_ps.ammo[WP_MOLOTOV];
#endif
...
float BotAggression(bot_state_t *bs) {
	...
	if (bs->inventory[INVENTORY_SHARPS] > 0) return 100;

	//if the bot can use the lightning
	if (bs->inventory[INVENTORY_LIGHTNING] > 0) return 95;

	//if the bot can use the peacemaker
	if (bs->inventory[INVENTORY_PEACEMAKER] > 0) return 90;

	//if the bot can use the shotgun
	if (bs->inventory[INVENTORY_REMGAUGE] > 0) return 90;

	//if the bot can use the sawedoff
	if (bs->inventory[INVENTORY_SAWEDOFF] > 0) return 85;

	//if the bot can use the gatling
	if (bs->inventory[INVENTORY_GATLING] > 0) return 80;

	//if the bot can use the winchester 66
	if (bs->inventory[INVENTORY_WINCHESTER66] > 0) return 60;

	//if the bot can use the schofield
	if (bs->inventory[INVENTORY_SCHOFIELD] > 0) return 45;

	//if the bot can use the rem58
	if (bs->inventory[INVENTORY_REM58] > 0) return 40;

	//if the bot can use the dynamite
	if (bs->inventory[INVENTORY_DYNAMITE] > 0) return 45;

	if (bs->inventory[INVENTORY_MOLOTOV] > 0) return 40;

	//if the bot can use the dynamite
	if (bs->inventory[INVENTORY_KNIFE] > 1) return 45;
#endif
...
int BotWantsToRetreat(bot_state_t *bs) {
	...
	if (gametype == GT_BR) {
		if (BotBRCarryingMoneyBag(bs)) return qtrue;
	}
#endif
	if (BotAggression(bs) < 50)
		return qtrue;
...
int BotWantsToChase(bot_state_t *bs) {
	...
	if (gametype == GT_BR) {
		if (BotBRCarryingMoneyBag(bs)) return qfalse;
	}
#endif
	if (BotAggression(bs) > 50)
		return qtrue;
...
#else
	if ((!bs->inventory[INVENTORY_SHARPS] || bs->inventory[INVENTORY_SHARPS_AMMO] < 5) &&
		(bs->inventory[INVENTORY_LIGHTNING] < 8) &&
		(bs->inventory[INVENTORY_WINCHESTER66] < 8) &&
		(bs->inventory[INVENTORY_GATLING] <= 0 || !bs->inventory[INVENTORY_GATLING_AMMO])){
#endif
		return qfalse;
	}
...
//	if (attack_skill < 0.2) return moveresult;
	...
	if (bs->weaponnum == WP_KNIFE || (entinfo.weapon==WP_KNIFE && dist<200)
		|| ((entinfo.flags & BFL_RELOAD || bs->flags & BFL_RELOAD2) && dist<140) || dist<70) {
		// TheDoctor: attack!
		movetype = MOVE_WALK;
...
float BotEntityVisible(int viewer, vec3_t eye, vec3_t viewangles, float fov, int ent) {
...
#else
	if (!InFieldOfVision(viewangles, fov, entangles, ent)) return 0;
#endif
	//
	...
	for (i = 0; i < 3; i++) {
		...
		if (trace.fraction >= 1 || trace.ent == hitent) {
			...
			BotAI_Trace(&trace, start, NULL, NULL, end, passent, (contents_mask|CONTENTS_BODY));
			glassfactor=1.0;
			// Propagate from each BODY to the next until we hit
			// the target (at most 10 levels to catch infinite loops)
			for (j=0; j<10 && trace.fraction < 1 && trace.ent != hitent; j++) {
				// Only keep going if we hit glass (water is here only for completeness, looking
				// into lakes is something completely different !)
				if (trace.surface.value & (SURF_GLASS|SURF_WATER)) {
					...
					VectorMA(trace.endpos, fabs(1.0/DotProduct(tmpstart,trace.plane.normal)), tmpstart, tmpstart);
					// After we propageted through the glass, we try again
					BotAI_Trace(&trace, tmpstart, NULL, NULL, end, passent, (contents_mask|CONTENTS_BODY));
				} else {
...
#else
	if (aim_accuracy <= 0) aim_accuracy = 0.2f;
#endif
	//get the enemy entity information
	BotEntityInfo(bs->enemy, &entinfo);
...
#else
			if (wi.number == WP_SAWEDOFF||
				wi.number == WP_WINCH97 ||
				wi.number == WP_DYNAMITE ||
				wi.number == WP_MOLOTOV) {
#endif
				//create the chase goal
...
#else
	if (wi.number == WP_SHARPS ||
		wi.number == WP_WINCHESTER66 ||
		wi.number == WP_LIGHTNING ||
		wi.number == WP_PEACEMAKER) {
#endif
		//distance towards the enemy
		dist = VectorLength(dir);
...
void BotCheckReload(bot_state_t *bs, int weapon, int reloadbit) {
	...
	if(!bs->cur_ps.ammo[weapon] && bs->cur_ps.ammo[bg_weaponlist[weapon].clip]){
	...
	if(bs->cur_ps.ammo[bg_weaponlist[weapon].clip] &&
		...
		// go through all clients and look if there's noone in the near environment
		for(i = 0; i < MAX_CLIENTS; i++){
			...
			if( Distance(entinfo.origin, bs->eye) < 500 && BotIsARealEnemy(bs->client, i))
				return;
		}
...
*/
#ifdef SMOKINGUNS
int BotFuncBreakableGoal(bot_state_t *bs, int entitynum, bot_activategoal_t *activategoal) {
	int weapon;
	vec3_t origin;
	...
	activategoal->shoot = qtrue;
	//
	activategoal->goal.entitynum = entitynum; //NOTE: this is the entity number of the breakable
	activategoal->goal.number = 0;
	activategoal->goal.flags = 0;
	VectorCopy(bs->origin, activategoal->goal.origin);
	activategoal->goal.areanum = bs->areanum;
...
#ifdef SMOKINGUNS
static void GetAvoidDir(int num, vec3_t origin, vec3_t maxs, vec3_t mins, vec3_t result) {
	...
	for(i=0;i<MAX_GENTITIES;i++){
		...
		if(i != num &&
			!Q_stricmp(ent->classname, "func_breakable") &&
	...
	for(i=0; i<2; i++){
		...
		if(sec && (
			(maxs2[i]>origin[i] && mins2[i]<origin[i]) ||
	...
	for(i=0,mark=0 ; i<4; i++){
		if(Distance(pdir, dir[i]) <
			Distance(pdir, dir[mark]))
			mark = i;
	}
