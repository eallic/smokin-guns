...
// unlagged - backward reconciliation #3
    ...
    if (player->client->sess.sessionTeam < TEAM_SPECTATOR) {
        ...
        if (player->client->dontTelefrag) {
            ...
            G_Printf(
                S_COLOR_MAGENTA
                "TeleportPlayer: Telefrag case delayed at respawn for %s...\n",
...
static void Think_AnimationExternalmodel(gentity_t *ent) {
    ...
    if (ent->animationEnd > ent->animationStart) {
        ent->s.frame = (int)((float)level.time * 0.001f * ent->animationFPS) %
                       (ent->animationEnd - ent->animationStart);
...
misc_portal_camera, or a mirror view if untargeted. This must be within 64 world
...
noswing The target for a misc_portal_director.  You can set either angles or
target another entity to determine the direction of view. "roll" an angle
modifier to orient the camera around the target vector;
*/
...
void Use_Shooter(gentity_t *ent, gentity_t *other, gentity_t *activator) {
    ...
    switch (ent->s.weapon) {
    case WP_DYNAMITE:
        missile = fire_dynamite(ent, ent->s.origin, dir,
                                ent->speed ? (int)ent->speed : 700);
        ...
        break;
    case WP_MOLOTOV:
        missile = fire_molotov(ent, ent->s.origin, dir,
                               ent->speed ? (int)ent->speed : 700);
...
"wait" is the number of seconds before molotov explodes (0.0 default for
disabled)
*/
void SP_shooter_molotov(gentity_t *ent) { InitShooter(ent, WP_MOLOTOV); }
...
*/
void SP_shooter_dynamite(gentity_t *ent) { InitShooter(ent, WP_DYNAMITE); }
