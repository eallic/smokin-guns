...
void SP_info_player_intermission(gentity_t *ent) {
    ...
    // add this so that cgame also knows about this entity (needed for duel
    // camera moves)
...
gentity_t *SelectRandomFurthestSpawnPoint(vec3_t avoidPoint, vec3_t origin,
    ...
    while ((spot = G_Find(spot, FOFS(classname), "info_player_deathmatch")) !=
        ...
        if (spot->mappart == mappart && g_gametype.integer == GT_DUEL &&
            !spot->used) {
    ...
    ignoreTeam =
        (g_gametype.integer == GT_TEAM) ? client->ps.persistant[PERS_TEAM] : -1;
    ...
    for (i = rnd; i >= 0; i--) {
        ...
        if (!G_IsAnyClientWithinRadius(spot->s.origin, 700, ignoreTeam)) {
            // Check if the spawn point is appropriate for the bot or human
            // player. If not, try another one.
            if (((spot->flags & FL_NO_BOTS) && isbot) ||
                ((spot->flags & FL_NO_HUMANS) && !isbot))
                continue;
            perfectSpot = qtrue;
        ...
        if (numAvailSpots > 0) {
            ...
            if (developer) {
                G_Printf("Error: Invalid %s spawn point\n",
                         isbot ? "bot" : "human");
                G_Printf(
                    "Fix: a valid spawn point has been elected. Is any client "
                    "within radius ? %s\n",
                    G_IsAnyClientWithinRadius(spot->s.origin, 700, ignoreTeam)
                        ? "Yes"
                        : "No");
            }
        } else {
            // We *SHOULD NOT* be here, or we may experience a real spawn policy
            // problem ! Mostly happen, when there is no place to spawn,
            // typically because telefrag would happen.
            // -> Should we spawn in the place with the risk to telefrag
            // somebody else ?
            ...
            if (developer) {
                ...
                for (i = 0; i < numAllSpots; i++) {
                    spot = allSpot[i];
                    if ((spot->flags & FL_NO_BOTS) && isbot)
                        G_Printf("DumpInfo: spawnPoint[%i]: not allowed for "
                                 "you, bot, telefrag: %s\n",
                                 i, telefragSpot[i] ? "Yes" : "No");
                    else if ((spot->flags & FL_NO_HUMANS) && !isbot)
                        G_Printf("DumpInfo: spawnPoint[%i]: not allowed for "
                                 "you, human, telefrag: %s\n",
                                 i, telefragSpot[i] ? "Yes" : "No");
                    else
                        G_Printf(
                            "DumpInfo: spawnPoint[%i]: allowed, telefrag: %s\n",
                            i, telefragSpot[i] ? "Yes" : "No");
            ...
            if (numAvailSpots > 0) {
                // We get all possible spawn points, even if telefrag would
                // happen. Get a random one
                i = random() * (numAvailSpots - 1);
                rnd = availSpot[i];
                spot = allSpot[rnd];
                ...
                // maybe FL_NO_BOTS and FL_NO_HUMANS have to be removed on *ALL*
                // spawn points.
                spot = list_spot[rnd];

                if (developer)
                    G_Printf("Fatal: %s spawn problem ! Maybe FL_NO_BOTS and "
                             "FL_NO_HUMANS have to be removed on all spawn "
                             "points ??\n",
                             isbot ? "bot" : "human");
            }
        }
...
gentity_t *SelectSpawnPoint(vec3_t avoidPoint, vec3_t origin, vec3_t angles,
                            qboolean isbot, int mappart, gclient_t *client) {
...
gentity_t *SelectInitialSpawnPoint(vec3_t origin, vec3_t angles, qboolean isbot,
    ...
    if (!spot || SpotWouldTelefrag(spot))
        return SelectSpawnPoint(vec3_origin, origin, angles, isbot, mappart,
                                client);
...
team_t PickTeam(int ignoreClientNum) {
    ...
    counts[TEAM_BLUE] = TeamCount(ignoreClientNum, TEAM_BLUE) +
                        TeamCount(ignoreClientNum, TEAM_BLUE_SPECTATOR);
    counts[TEAM_RED] = TeamCount(ignoreClientNum, TEAM_RED) +
                       TeamCount(ignoreClientNum, TEAM_RED_SPECTATOR);
...
check_again:
    Q_strncpyz(level.clients[clientNum].pers.cleanname,
               level.clients[clientNum].pers.netname,
               sizeof(level.clients[clientNum].pers.cleanname));
    ...
    for (i = 0; i < level.maxclients; i++) {
        ...
        // We compare on the cleanname as this is a guarantee to avoid confusion
        // between players
        if (Q_stricmp(level.clients[i].pers.cleanname,
                      level.clients[clientNum].pers.cleanname)) {
            continue;
        }
        ...
        // Tag the name
        Com_sprintf(level.clients[clientNum].pers.netname,
                    sizeof(level.clients[clientNum].pers.netname), "%s %s",
                    name, tag);

        // Then we need to scan again all the names, eventually adding a new
        // star if this tagged name is still used
        star++;
        goto check_again;
    }
    return qtrue;
...
static void ClientCleanName(const char *in, char *out, int outSize) {
        ...
        if (*in == ' ') {
            ...
            if (Q_IsColorString(&out[outpos - 1])) {
                ...
                if (ColorIndex(*in) == 0) {
                    ...
                    // from getting advantage playing in front of black
                    // backgrounds
                    outpos--;
                    continue;
                }
    ...
    // Tequila: Discard trailing spaces and stars (stars are reserved at this
    // position)
    ...
    // Tequila: Don't allow leading numbers as it can confuse ClientForString()
    // in g_svcmds.c
    while (*out != '\0' && *out >= '0' && *out <= '9')
        Q_strncpyz(out, out + 1, outSize);

    // Tequila: Don't allow empty names and reserved word
    if (*out == '\0' || colorlessLen == 0 || !Q_stricmp(out, "all") ||
        !Q_stricmp(out, "allbots"))
...
// unlagged - client options
    ...
    if (!ForceUniqueName(clientNum)) {
        // We should only came here in extremely rare case and surely not with
        // fair cowboys...
        if (client->pers.connected == CON_CONNECTING)
            // When connecting, not setting cleanname will discard the client
            // with a suitable message about an invalid name
    ...
    if (client->pers.connected == CON_CONNECTED) {
        // Tequila: Handle case player has set an empty name (see
        // ClientCleanName)
        if (!strcmp("UnnamedPlayer", client->pers.netname) && oldname[0]) {
            ...
            Q_strncpyz(client->pers.netname, oldname,
                       sizeof(client->pers.netname));
            Q_strncpyz(client->pers.cleanname, oldname,
                       sizeof(client->pers.cleanname));
        ...
        if (level.voteTime &&
            !strcmp(level.voteString, va("sendaway \"%s\"", oldname))) {
            Q_strncpyz(client->pers.netname, oldname,
                       sizeof(client->pers.netname));
            trap_SendServerCommand(
                -1,
                va("print \"%s" S_COLOR_WHITE " tried to rename\n\"", oldname));
        ...
        if (strcmp(oldname, client->pers.netname)) {
             ...
             * 3. renameTime is older then the g_delayedRenaming specified time
             * in seconds
             * 4. g_splitChat is set, gametype is round base and player is a
             * spectator
             */
            if (!client->pers.renameTime || !g_delayedRenaming.integer ||
                (level.time - client->pers.renameTime >=
                 g_delayedRenaming.integer * 1000) ||
                !(g_splitChat.integer && g_gametype.integer >= GT_RTP &&
                  client->sess.sessionTeam >= TEAM_SPECTATOR)) {
                PushMinilogf("RENAME: %i > %s", clientNum,
                             client->pers.netname);
                trap_SendServerCommand(-1, va("print \"%s" S_COLOR_WHITE
                                              " renamed to %s\n\"",
                                              oldname, client->pers.netname));
                // Store when last renaming occured only if needed
                if (g_delayedRenaming.integer && g_splitChat.integer &&
                ...
                if (g_roundendtime > level.time &&
                    client->pers.renameTime > g_roundendtime)
                    client->pers.renameTime = g_roundendtime;

                // Keep requested name
                Q_strncpyz(client->pers.renameName, client->pers.netname,
                           sizeof(client->pers.renameName));
                ...
                Q_strncpyz(client->pers.netname, oldname,
                           sizeof(client->pers.netname));
                trap_SendServerCommand(
                    clientNum, va("print \"Renaming delayed to %i seconds, "
                                  "%s" S_COLOR_WHITE "...\n\"",
                                  (client->pers.renameTime - level.time) / 1000,
                                  client->pers.netname));
            }
        }

        // Tequila: Advertize client if we have changed significantly his player
        // name to a suitable one
        Q_CleanStr(s); // Here s is a requested name backup, clean it and
                       // compare it to the cleanname set
        if (Q_stricmp(s, client->pers.cleanname))
            trap_SendServerCommand(
                clientNum, va("cp \"Your name is %s\"", client->pers.netname));
    }
    ...
    if (g_gametype.integer >= GT_TEAM) {
        Q_strncpyz(model, Info_ValueForKey(userinfo, "team_model"),
                   sizeof(model));
        Q_strncpyz(headModel, Info_ValueForKey(userinfo, "team_headmodel"),
                   sizeof(headModel));
    } else {
        Q_strncpyz(model, Info_ValueForKey(userinfo, "model"), sizeof(model));
        Q_strncpyz(headModel, Info_ValueForKey(userinfo, "headmodel"),
                   sizeof(headModel));
    }
    ...
    //		ForceClientSkin(client, headModel, "blue");
            ...
            // don't ever use a default skin in teamplay, it would just waste
    memory
            // however bots will always join a team but they spawn in as
    spectator if ( g_gametype.integer >= GT_TEAM && team == TEAM_SPECTATOR) {
                    ForceClientSkin(client, model, "red");
    //		ForceClientSkin(client, headModel, "red");
            }
    */
    ...
    */
    ...
    if (ent->r.svFlags & SVF_BOT) {
        s = va("n\\%s\\t\\%i\\model\\%s\\hc\\%i\\w\\%i\\l\\%i\\skill\\%s\\tt\\%"
               "d\\tl\\%d",
               client->pers.netname, client->sess.sessionTeam, model,
               client->pers.maxHealth, client->sess.wins, client->sess.losses,
               Info_ValueForKey(userinfo, "skill"), teamTask, teamLeader);
    } else {
        s = va("\\n\\%s\\t\\%i\\model\\%s\\g_redteam\\%s\\g_blueteam\\%s\\hc\\%"
               "i\\w\\%i\\l\\%i\\tt\\%d\\tl\\%d\\v\\%s\\md5\\%s",
               client->pers.netname, client->sess.sessionTeam, model, redTeam,
               blueTeam, client->pers.maxHealth, client->sess.wins,
               client->sess.losses, teamTask, teamLeader, version, md5);
    }
...
char *ClientConnect(int clientNum, qboolean firstTime, qboolean isBot) {
    ...
    // recommanding PB based IP / GUID banning, the builtin system is pretty
    // limited check to see if they are on the banned IP list
    ...
    if (!isBot && g_checkClients.integer) {
        ...
        if (!*value)
            return "Please download " PRODUCT_NAME " " PRODUCT_VERSION
                   " from " PRODUCT_URL;
    }
    ...
    //	if ( !client->areabits )
    //		client->areabits = G_Alloc(
    //(trap_AAS_PointReachabilityAreaIndex( NULL ) + 7) / 8 );
    ...
    if (firstTime && !isBot) {
        ...
        trap_SendConsoleCommand(
            EXEC_APPEND, va("vstr onEvent_playerUpTo%i\n", g_humancount));
    }
...
static void G_RestorePlayerStats(gentity_t *ent, qboolean save) {
    ...
    // also save the predictable events otherwise we might get double or dropped
    // events
...
void ClientSpawn(gentity_t *ent) {
    ...
    if (g_gametype.integer != GT_DUEL)
        // hika comments: client->ps.pm_type is set to PM_DEAD in player_die()
        // at g_combat.c
    ...
    if (client->sess.sessionTeam == TEAM_SPECTATOR &&
        ...
        spawnPoint =
            SelectSpectatorSpawnPoint(spawn_origin, spawn_angles, ent->mappart);
    ...
    if ((g_gametype.integer < GT_RTP && g_gametype.integer != GT_DUEL) ||
        ...
        for (i = WP_REM58; i < WP_DYNAMITE; i++) {
            if (client->ps.stats[STAT_WEAPONS] & (1 << i)) {
                ...
                if (g_gametype.integer == GT_DUEL)
                    client->ps.ammo[bg_weaponlist[i].clip] =
                        bg_weaponlist[i].maxAmmo;

                if (client->ps.ammo[bg_weaponlist[i].clip] <
                        bg_weaponlist[i].maxAmmo / 2 &&
...
if (ent->client->sess.sessionTeam < TEAM_SPECTATOR) {
    if (g_gametype.integer == GT_DUEL) {
        ...
        if (g_moneyRespawn.integer == 1) {
            ...
            if ((g_gametype.integer == GT_BR) ||
                (g_gametype.integer == GT_RTP)) {
                min_money =
                    g_startingMoney.integer +
                    (client->ps.stats[STAT_MONEY] - m_teamlose.integer) / 4;
            } else {
                min_money =
                    g_startingMoney.integer + client->ps.stats[STAT_MONEY] / 4;
            }
        }
    ...
    if (g_gametype.integer != GT_DUEL) {
        ...
        if (client->dontTelefrag) {
            ...
            G_Printf(
                S_COLOR_MAGENTA
                "ClientSpawn: Telefrag case delayed at respawn for %s...\n",
                client->pers.netname);
#endif
            trap_SendServerCommand(
                index, va("print \"Go away %s\n\"", client->pers.netname));
...
if (!level.intermissiontime) {
    if (ent->client->sess.sessionTeam != TEAM_SPECTATOR) {
        ...
        // select the highest weapon number available, after any spawn given
        // items have fired
...
void ClientDisconnect(int clientNum) {
    ...
    // if we are playing in tourney mode and losing, give a win to the other
    // player
    ...
    // Joe Kari: exec content of the onEvent_playerDisconnect cvar
        ...
        trap_SendConsoleCommand(
            EXEC_APPEND, va("vstr onEvent_playerDownTo%i\n", g_humancount));
    }
