...
static void G_LoadArenasFromFile(char *filename) {
    ...
    g_numArenas +=
        G_ParseInfos(buf, MAX_ARENAS - g_numArenas, &g_arenaInfos[g_numArenas]);
}
...
void G_CheckMinimumPlayers(void) {
    ...
    if (g_gametype.integer >= GT_TEAM) {
        ...
        botplayers =
            G_CountBotPlayers(TEAM_RED) + G_CountBotPlayers(TEAM_RED_SPECTATOR);
        //
        if (humanplayers + botplayers < minplayers &&
            !(g_gametype.integer == GT_BR && noBR)) {
...
void G_CheckBotSpawn(void) {
    ...
    for (n = 0; n < BOT_SPAWN_QUEUE_DEPTH; n++) {
        ...
        if (g_gametype.integer == GT_SINGLE_PLAYER) {
            trap_GetUserinfo(botSpawnQueue[n].clientNum, userinfo,
                             sizeof(userinfo));
...
static void G_AddBot(const char *name, float skill, const char *team, int delay,
    ...
    if (clientNum == -1) {
        ...
        G_Printf(S_COLOR_RED "Start server with more 'open' slots (or check "
                             "setting of sv_maxclients cvar).\n");
        return;
    }
...
void Svcmd_AddBot_f(void) {
    ...
    if (!name[0]) {
        trap_Print("Usage: Addbot <botname> [skill 1-5] [team] [msec delay] "
                   "[altname]\n");
        return;
    }
...
static void G_SpawnBots(char *botList, int baseDelay) {
    ...
    while (*p) {
        ...
        // does "Bad Things"
        trap_SendConsoleCommand(
            EXEC_INSERT, va("addbot %s %f free %i\n", bot, skill, delay));
...
void G_InitBots(qboolean restart) {
    ...
    if ((g_gametype.integer == GT_BR && bot_noBR.integer) ||
