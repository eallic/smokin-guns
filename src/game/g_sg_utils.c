...
void G_ThrowWeapon(int weapon, gentity_t *ent) {
    ...
    if (drop) {
        trap_SendServerCommand(
            ent - g_entities, va("print \"%s dropped.\n\"", item->pickup_name));
    }
...
static void LetGoOfGatling(gclient_t *client, gentity_t *gatling) {
    ...
    G_AddEvent(&g_entities[gatling->s.eventParm], EV_CHANGE_TO_WEAPON,
               client->ps.stats[STAT_OLDWEAPON]);
...
static void Gatling_Think(gentity_t *self) {
    ...
    if (!self->s.time && self->s.pos.trType == TR_STATIONARY) {
        ...
        trap_Trace(&trace, self->r.currentOrigin, NULL, NULL, end,
                   self->s.eventParm, MASK_SHOT);
        ...
        if (trace.fraction == 1.0) { // the ground is gone
            ...
            if (ent != self) {
                // drop away from client, so he won't just pick it up in the
                // fall
                VectorClear(dir);
                AngleVectors(angles, dir, NULL, NULL);
                VectorScale(dir, 50, dir);
                VectorAdd(ent->s.pos.trBase, dir, dir);
                // snap to integer coordinates for more efficient network
                // bandwidth usage
                SnapVector(dir);
            } else {
    ...
    if (client && self->s.time2 < 0) {
        ...
        if (self->s.time2 - TRIPOD_TIME - 4 * STAGE_TIME +
                    G_AnimLength(WP_ANIM_CHANGE, WP_GATLING) >
                -level.time &&
            !(client->ps.stats[STAT_FLAGS] & SF_GAT_CHANGE)) {
    ...
    if (self->s.eventParm != -1) {
        ...
        if (client->buttons & BUTTON_ALT_ATTACK &&
            !(self->mapparttime & BUTTON_ALT_ATTACK)) {
            // don't dismantle if player already has a special weapon
            if (BG_FindPlayerWeapon(WP_WINCHESTER66, WP_GATLING, &client->ps) ||
                (client->ps.stats[STAT_FLAGS] & SF_GAT_CARRY)) {
                trap_SendServerCommand(
                    client->ps.clientNum,
                    va("print \"You already have a secondary weapon.\n\""));
            } else {
                int userNum =
                    self->s.eventParm; // save eventparm, because its
        ...
        if ((trace.allsolid || trace.startsolid) &&
            ...
            // Tequila comment: Don't stuck a client playing with another
            // gatling too next from this one
            self->r.contents = CONTENTS_CORPSE;
        } else {
...
gentity_t *LaunchGatling(gentity_t *ent) {
    ...
    gatling->r.contents =
        0; // will be set to MASK_SHOT if no player controls it
    // gatling->r.ownerNum = ent->s.number;
    // gatling->parent = ent;
    gatling->s.pos.trTime =
...
void G_GatlingBuildUp(gentity_t *ent) {
    ...
    // build it up
    // trap_SendServerCommand( ent-g_entities, va("print \"Gatling Gun building
    // up...\n\""));
    ...
    // the guy who built up the gatling: important for rtp-respawn system(with
    // weapons)
...
qboolean G_CanEntityBeActivated(gentity_t *player, gentity_t *target,
    ...
    if (!exact && Distance(player->client->ps.origin, target->r.currentOrigin) >
                      ACTIVATE_RANGE)
        return qfalse;
    ...
    trap_Trace(&tr, muzzle, NULL, NULL, end, player->client->ps.clientNum,
               MASK_SHOT);
    ...
    if (exact &&
        Distance(player->client->ps.origin, tr.endpos) > ACTIVATE_RANGE)
        return qfalse;
