    ...
    for (i = WP_KNIFE; i < WP_NUM_WEAPONS; i++) {
        ...
        if (i > WP_KNIFE && i < WP_WINCHESTER66) {
            if ((self->client->ps.stats[STAT_FLAGS] & SF_SEC_PISTOL)) {
                ...
                // Since the second pistol is dropped (the one on the left
                // holster), the correct ammo must be set in the dropped weapon
                if (self->client->ps.ammo[WP_AKIMBO] >
                    bg_weaponlist[i].clipAmmo)
                    sec_pistol_drop->count = bg_weaponlist[i].clipAmmo + 1;
                else
                    sec_pistol_drop->count =
                        self->client->ps.ammo[WP_AKIMBO] + 1;
        ...
        if (item->giType == IT_WEAPON) {
            if (item->giTag == WP_DYNAMITE || item->giTag == WP_KNIFE ||
                ...
                if (self->client->ps.ammo[i] >
                    bg_weaponlist[item->giTag].clipAmmo)
...
void LookAtKiller(gentity_t *self, gentity_t *attacker) {
    ...
    // small hack. grapplePoint isnt used when player dead, so we use it to tell
    // the client where to turn. A better solution would be if we could add a
    // new variable to playerstate which is communicated between the client and
    // the server.
...
char *modNames[] = {
    ...
    // pistols
    "MOD_REM58", "MOD_SCHOFIELD", "MOD_PEACEMAKER",

    // rifles
    "MOD_WINCHESTER66", "MOD_LIGHTNING", "MOD_SHARPS",

    // shotguns
    "MOD_REMINGTON_GAUGE", "MOD_SAWEDOFF", "MOD_WINCH97",

    // automatics
    "MOD_GATLING",

    // explosives
    "MOD_DYNAMITE", "MOD_MOLOTOV",

    "MOD_WATER", "MOD_SLIME", "MOD_LAVA", "MOD_CRUSH", "MOD_TELEFRAG",
    "MOD_FALLING", "MOD_SUICIDE", "MOD_WORLD_DAMAGE", "MOD_TRIGGER_HURT",
    "MOD_BOILER"
};
...
// unlagged - backward reconciliation #2
    ...
    if (attacker && attacker->client) {
        ...
        if (attacker == self) {
            ...
            ent->s.eFlags |=
                EF_SAME_TEAM; // Joe Kari: new flag to report teamkilling
            PushMinilogf("TEAMKILL: %i => %i [%s]", attacker->s.number,
                         self->s.number, obit);
            AddScore(attacker, self->r.currentOrigin, -1);
        } else {
            attacker->client->pers.kill++;
            PushMinilogf("KILL: %i => %i [%s]", attacker->s.number,
                         self->s.number, obit);
            AddScore(attacker, self->r.currentOrigin, 1);
        }
    ...
    if (self->client->ps.groundEntityNum != ENTITYNUM_NONE) {
        ...
        trap_Trace(&trace, pos, mins, maxs, pos, self->client->ps.clientNum,
                   MASK_SHOT);
        ...
        // at last check if we can get a direct line to the
        // falling-down-position
        trap_Trace(&trace, self->client->ps.origin, mins2, maxs2, pos, -1,
                   MASK_SHOT);
    ...
    if (self->s.number != ENTITYNUM_NONE && self->s.number != ENTITYNUM_WORLD &&
        ...
        // if he kill a mate
        if (victim->client->sess.sessionTeam ==
                assaulter->client->sess.sessionTeam &&
            ...
            if (assaulter->client->ps.stats[STAT_MONEY] < 0) {
                if (g_moneyRespawn.integer == 1) {
                    // Joe Kari: in the new money system, the teamkiller don't
                    // waste its money, what he loses is a gift to his mate
                    victim->client->ps.stats[STAT_MONEY] +=
                        2 * LOSE_MONEY +
                        assaulter->client->ps.stats[STAT_MONEY];
                }
                assaulter->client->ps.stats[STAT_MONEY] = 0;
            } else if (g_moneyRespawn.integer == 1) {
                // Joe Kari: in the new money system, the teamkiller don't waste
                // its money, what he loses is a gift to his mate
        ...
        for (i = 0; i < level.maxclients; i++) {
            ...
            if (level.clients[i].ps.persistant[PERS_SCORE] >
                victim->client->ps.persistant[PERS_SCORE]) {
                rank++;
            }
        }
        ...
        if (assaulter->client->ps.clientNum != victim->client->ps.clientNum) {
            const int players =
                TeamCount(-1, TEAM_FREE) + TeamCount(-1, TEAM_RED) +
                TeamCount(-1, TEAM_BLUE) + TeamCount(-1, TEAM_RED_SPECTATOR) +
                TeamCount(-1, TEAM_BLUE_SPECTATOR) +
                (g_gametype.integer == GT_DUEL ? TeamCount(-1, TEAM_SPECTATOR)
                                               : 0);
            ...
            f_amount =
                (float)(f_amount * f_amount * f_amount) *
                (float)(m_maxreward.integer * factor2); // here exponent is 3
...
============
*/
float G_LocationDamage(vec3_t point, gentity_t *targ, gentity_t *attacker,
                       float take, int *mod) {
    vec3_t bulletPath;
    vec3_t bulletAngle;
    ...
    case HIT_CHEST:
        if (targ->client->ps.stats[STAT_ARMOR] && !sharps) {
            ...
            if (g_debugDamage.integer) {
                G_Printf("%i: client:%i health:%i damage:%.1f "
                         "where:chest_boiler_plate(%i) from:%s by:%i\n",
                         level.time, targ->s.number, targ->health, take,
                         targ->client->ps.stats[STAT_ARMOR], modNames[*mod],
                         attacker->s.number);
            }
    ...
    case HIT_STOMACH:
        if (targ->client->ps.stats[STAT_ARMOR] && !sharps) {
            ...
            if (g_debugDamage.integer) {
                G_Printf("%i: client:%i health:%i damage:%.1f "
                         "where:stomack_boiler_plate(%i) from:%s by:%i\n",
                         level.time, targ->s.number, targ->health, take,
                         targ->client->ps.stats[STAT_ARMOR], modNames[*mod],
                         attacker->s.number);
            }
    ...
    case HIT_UPPER_LEG_R:
        ...
        if (targ->client->ps.speed == g_speed.integer) {
            ...
            // trap_SendServerCommand( targ-g_entities, va("print \"You were hit
            // in the leg!\n\""));
        }
        break;
    case HIT_FOOT_L:
...
static qboolean CheckTeamKills(gentity_t *attacker, gentity_t *targ, int mod) {
    ...
    // g_teamkillschecktime can be set to only activate the check at the round
    // beginning
    if (g_teamkillschecktime.integer && g_roundstarttime &&
        level.time > g_roundstarttime + g_teamkillschecktime.integer * 1000)
        return qfalse;
    ...
    if (attacker->client->pers.lastTeamKillTime &&
        level.time > attacker->client->pers.lastTeamKillTime +
                         g_teamkillsforgettime.integer * 1000) {
#ifdef DEBUG_CHECKTEAMKILL
        G_Printf(S_COLOR_MAGENTA
                 "CheckTeamKills: Forget TeamKillsCount (%i) from %s\n",
    ...
    if (attacker->client->pers.lastTeamKillTime)
        G_Printf(S_COLOR_MAGENTA
                 "CheckTeamKills: Last TeamKills for %s was %.2f seconds ago\n",
                 attacker->client->pers.netname,
                 (float)(level.time - attacker->client->pers.lastTeamKillTime) /
                     1000.0f);
#endif
    ...
    if (attacker->client->pers.TeamKillsCount >= g_maxteamkills.integer) {
        trap_DropClient(attacker - g_entities,
                        "Reached the limit of supported teammate kills.");
        return qtrue;
    } else if (attacker->client->pers.TeamKillsCount ==
               g_maxteamkills.integer - 1)
        trap_SendServerCommand(attacker - g_entities,
                               va("print \"%s" S_COLOR_YELLOW
                                  "... Be careful teammate !!! Next teammate "
                                  "kill could kick you from that server.\n\"",
                                  attacker->client->pers.netname));
#ifdef DEBUG_CHECKTEAMKILL
...
damage DAMAGE_NO_KNOCKBACK		do not affect velocity, just view angles
...
void G_Damage(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
    ...
    if (!(dflags & DAMAGE_NO_PROTECTION)) {
        ...
        if (targ != attacker && OnSameTeam(targ, attacker)) {
            ...
            // Can be used later in case of TK limit reached to protect the last
            // victim
            hsave = targ->health;
        }
        ...
        if (point && targ && targ->health > 0 && attacker && take) {
            if (targ->client) {
                if (targ->client->lasthurt_part == PART_LOWER) {
                    ...
                    targ->client->ps.stats[STAT_KNOCKTIME] = 1;
                    VectorScale(targ->client->ps.velocity, 0.6,
    ...
    if (take) {
        ...
        if (g_debugDamage.integer) {
            if (client) {
                G_LogPrintf("%i: client:%i health:%i damage:%.1f where:%s "
                            "from:%s by:%i\n",
                            level.time, targ->s.number, targ->health, take,
                            client->ps.weapon
                ...
                G_LogPrintf(
                    "%i: world:%i/%s health:%i damage:%.1f from:%s by:%i\n",
                    level.time, targ->s.number, targ->classname, targ->health,
                    take, modNames[mod], attacker->s.number);
            }
        }

        // Tequila fix: take is a float in SG so round it with a cast to got a
        // real damage
        ...
        if (targ->client) {
            if (attacker->client) {
                ...
                if (attacker == targ) {
                    PushMinilogf(
                        "SELFHIT: %i (%i) [%s] [%s]", targ->s.number, (int)take,
                        client->ps.weapon
                            ? hit_info[client->lasthurt_location].forename
                            : hit_info[client->lasthurt_location].backname,
                        modNames[mod]);
                    ...
                    PushMinilogf(
                        "TEAMHIT: %i => %i (%i) [%s] [%s]", attacker->s.number,
                        targ->s.number, (int)take,
                        client->ps.weapon
                            ? hit_info[client->lasthurt_location].forename
                            : hit_info[client->lasthurt_location].backname,
                        modNames[mod]);
                    ...
                    PushMinilogf(
                        "HIT: %i => %i (%i) [%s] [%s]", attacker->s.number,
                        targ->s.number, (int)take,
                        client->ps.weapon
                            ? hit_info[client->lasthurt_location].forename
                            : hit_info[client->lasthurt_location].backname,
                ...
                PushMinilogf("WORLDHIT: %i (%i) [%s]", targ->s.number,
                             (int)take, modNames[mod]);
                if (targ->health <= 0) {
                    PushMinilogf("WORLDKILL: %i [%s]", targ->s.number,
                                 modNames[mod]);
                }
            }
        }
        ...
        if (targ->health <= 0) {
            ...
            if (attacker->s.angles2[0] != -1 &&
                ...
                attacker->s.eFlags &= ~EF_HIT_MESSAGE;
                attacker->s.angles2[0] = attacker->s.angles2[1] =
                    attacker->s.angles2[2] = -1;
            }
...
void G_LookForBreakableType(gentity_t *ent) {
    ...
    for (j = 0; j < 6; j++) {
        ...
        shaderNum = trap_Trace_New2(&tr, origin2, NULL, NULL, origin, -1,
                                    MASK_SHOT | CONTENTS_JUMPPAD);
...
qboolean CanDamage(gentity_t *targ, vec3_t origin) {
    ...
    if (targ->s.eType == ET_BREAKABLE) {
        ...
        if (targ->count == -1) {
            ...
            shaderNum = trap_Trace_New2(&tr, origin, NULL, NULL, dest,
                                        targ->s.clientNum,
                                        CONTENTS_SOLID | CONTENTS_JUMPPAD);
