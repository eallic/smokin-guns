...
#define VectorShortCopy(a, b)                                                  \
    ((b)[0] = (float)(a)[0], (b)[1] = (float)(a)[1], (b)[2] = (float)(a)[2])
#define VectorShortAdd(a, b, c)                                                \
    ((c)[0] = (float)((a)[0] + (b)[0]), (c)[1] = (float)((a)[1] + (b)[1]),     \
     (c)[2] = (float)((a)[2] + (b)[2]))
...
================
*/
static void SetupLines(vec3_t offset_origin, vec3_t offset_angles, vec3_t start,
                       vec3_t end, hit_part_t *mesh, lerpFrame_t *lerp,
                       vec3_t trace_dir, vec3_t cyl_dir, vec3_t trace_start,
                       vec3_t cyl_start, vec3_t edge1, vec3_t edge2) {
    int frame, oldframe;
    vec3_t diff;
...
static qboolean LineHitsPlane(vec3_t plane_org, vec3_t plane_normal,
                              vec3_t line_org, vec3_t line_dir, vec3_t result) {
    float s;
    vec3_t w;
...
===================
*/
static void SmallestDistance(vec3_t trace_dir, vec3_t cyl_dir,
    ...
    // the two lines are: trace: x=trace_start + t*trace_dir;  cyl: x=cyl_start
    // + s*cyl_dir
...
======================
*/
static qboolean CheckIntersection(vec3_t offset_origin, vec3_t offset_angles,
    ...
    SetupLines(offset_origin, offset_angles, start, end, mesh, lerp, trace_dir,
               cyl_dir, trace_start, cyl_start, edge1, edge2);

    VectorMA(cyl_start, length, cyl_dir, cyl_end);

    // find the point on the trace-line with the smallest distance from the
    // cylinder
    SmallestDistance(trace_dir, cyl_dir, trace_start, cyl_start, trace_p,
                     cyl_p);
    ...
    if (Distance(p, trace_p) > length) {
        ...
        if (LineHitsPlane(cyl_start, cyl_dir, trace_start, trace_dir,
                          trace_p)) {
            VectorCopy(cyl_start, cyl_p);
        } else if (LineHitsPlane(cyl_end, cyl_dir, trace_start, trace_dir,
                                 trace_p)) {
...
static void TagOffset(vec3_t offset_origin, vec3_t offset_angles,
    ...
    for (i = 0; i < 3; i++) {
        tag[i] = ptag[oldFrame].origin[i] +
                 (ptag[frame].origin[i] - ptag[oldFrame].origin[i]) *
                     (1 - lerp->backlerp);

        angles[i] = ptag[oldFrame].angles[i] +
                    (ptag[frame].angles[i] - ptag[oldFrame].angles[i]) *
                        (1 - lerp->backlerp);
    }
...
===============
*/
static qboolean G_BuildHitModel(hit_data_t *data, vec3_t origin, vec3_t angles,
                                vec3_t torso_angles, vec3_t head_angles,
...
static void GetVectorOfPart(hit_part_t *mesh, vec3_t vec, lerpFrame_t *lerp,
                            vec3_t start, vec3_t end, vec3_t offset_origin,
...
=====================
*/
static int G_NearestPart(hit_data_t *data, vec3_t origin, vec3_t angles,
                         vec3_t torso_angles, vec3_t head_angles,
                         lerpFrame_t *torso, lerpFrame_t *legs, vec3_t start,
                         vec3_t end, float *mindist) {
    int i;
    float bestdist = 999;
    int bestpart = 0;
    vec3_t offset_origin, offset_angles;
    ...
    for (i = 0; i < NUM_HIT_LOCATIONS; i++) {
        ...
        if (hit_info[i].hit_part == PART_LOWER) {
            GetVectorOfPart(&data->meshes[i], vector, legs, start, end,
                            offset_origin, offset_angles);
        ...
        if (hit_info[i].hit_part == PART_UPPER) {
            GetVectorOfPart(&data->meshes[i], vector, torso, start, end,
                            offset_origin, offset_angles);
        ...
        if (hit_info[i].hit_part == PART_HEAD) {
            GetVectorOfPart(&data->meshes[i], vector, NULL, start, end,
                            offset_origin, offset_angles);
...
===============
*/
int G_HitModelCheck(hit_data_t *data, vec3_t origin, vec3_t angles,
                    vec3_t torso_angles, vec3_t head_angles, lerpFrame_t *torso,
                    lerpFrame_t *legs, vec3_t start, vec3_t end) {
    int i;
    float mindist = 0;
    int part;
...
static qboolean G_ParseAnimationFile(const char *filename, hit_data_t *data) {
    ...
    for (i = 0; i < MAX_ANIMATIONS; i++) {
        ...
        // G_Printf("%i. anim\nstart: %i\nnum: \n%i framelerp\n\n", i,
        // data->animations[i].firstFrame, 	data->animations[i].numFrames,
        //data->animations[i].frameLerp);
    }
    ...
    // sizeof(animation_t)); animations[LEGS_BACKCR].reversed = qtrue;
    //
    return qtrue;
}
...
qboolean G_OpenFileAiNode(const char *filename) {
    ...
    for (i = 0; i < MAX_AINODES; i++) {
        ...
        pos += sizeof(vec3_t);
        // G_Printf("%i. %f %f %f\n", i+1, ai_nodes[i][0], ai_nodes[i][1],
        // ai_nodes[i][2]);

        // check if they're in solid
        /*trap_Trace(&trace, ai_nodes[i], NULL, NULL, ai_nodes[i], -1,
        CONTENTS_SOLID); if(trace.allsolid || trace.startsolid) G_Printf("node
        %i is in solid\n", i); else G_Printf("node %i is ok\n", i);*/
    }
...
qboolean G_ParseHitFile(hit_data_t *hit_data, int part) {
    ...
    if (header.ident != HIT_IDENT) {
        ...
        G_Printf(
            ...
            "no valid hitfile: %s\nident: %i\nframes: %i\nmeshes: %i\n%i %i %i "
            "%i %i",
            name, (int)header.ident, (int)header.numFrames,
            (int)header.numMeshes, sizeof(header.ident),
            sizeof(header.numFrames), sizeof(header.numMeshes), sizeof(short),
            sizeof(int));
        return qfalse;
    }
    ...
    if (part != PART_HEAD) {
        for (f = 0; f < header.numFrames; f++) {
            ...
            for (j = 0; j < 3; j++) {     // fix endianness
                ptag->angles[j] =
                    LittleFloat(((const float *)&ptag->angles)[j]);
                ptag->origin[j] =
                    LittleFloat(((const float *)&ptag->origin)[j]);
            }
#endif
    ...
    for (i = 0; i < header.numMeshes; i++) {
        ...
        /*if(pos >= len && filecount < 2 && part != PART_HEAD){
                ...
                if ( len >= MAX_HITFILE ) {
                        G_Printf(  "hit file too large: %s is %i, max allowed is
        %i\n", name, len, MAX_HITFILE  ); trap_FS_FCloseFile( file ); return
        qfalse;
                }
                trap_FS_Read( buf, len, file );
                buf[len] = 0;
        ...
        Com_Memcpy(&hit_data->meshes[hit_num].header, &m_header,
                   sizeof(short) * 9 + sizeof(m_header.name));

        // now parse the frames
        for (f = 0; f < header.numFrames; f++) {
            Com_Memcpy(&hit_data->meshes[hit_num].pos[f], buf + pos,
                       sizeof(hit_mesh_t));
#if defined Q3_VM || !defined LittleShort // QVM or SO on a big endian processor
...
qboolean G_LoadHitFiles(hit_data_t *hit_data) {
    ...
    // load animations
    if (!G_ParseAnimationFile("models/wq3_players/wq_male1/animation.cfg",
                              hit_data)) {
        G_Printf(
            "models/wq3_players/wq_male1/animation.cfg could not be loaded\n");
        return qfalse;
    }
...
void G_RunLerpFrame(lerpFrame_t *lf, int newAnimation, float speedScale) {
    ...
    if (lf->frameTime == lf->oldFrameTime) {
        ...
        lf->backlerp = 1.0 - (float)(level.time - lf->oldFrameTime) /
...
==================
*/
static void G_SwingAngles(float destination, float swingTolerance,
...
void G_PlayerAngles(gentity_t *ent, vec3_t legs[3], vec3_t torso[3]) {
    ...
    // torso
    G_SwingAngles(torsoAngles[YAW], 25, 90, 0.3f, &client->torso.yawAngle,
                  &client->torso.yawing);
    G_SwingAngles(legsAngles[YAW], 40, 90, 0.3f, &client->legs.yawAngle,
                  &client->legs.yawing);
    ...
    G_SwingAngles(dest, 15, 30, 0.1f, &client->torso.pitchAngle,
                  &client->torso.pitching);
