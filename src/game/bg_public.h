...
-----------------------
*/
// #define MAX_MONEY				200.00	// now, it is stored in
// the cvar g_maxMoney
#define MIN_MONEY "20"
...
#define MINS_Z -24
// Tequila: maxs bbox will be added on Z axis with this for the hit detection
// system
...
#define CS_INTERMISSION                                                        \
    22 // when 1, fraglimit/timelimit has been hit and intermission will start
       // in a second or two
...
typedef enum {
    ...
    FARCLIP_BOX_Y,
    FARCLIP_BOX_Z,
    FARCLIP_CONE_X, // distance along X-axis is substracted from YZ-axis
                    // distance calculation
    FARCLIP_CONE_Y,
    FARCLIP_CONE_Z,    // could be usefull for clipping grass brush/models
    FARCLIP_PYRAMID_X, // distance along X-axis is substracted from YZ-axis
                       // bounding box calculation
    FARCLIP_PYRAMID_Y,
    FARCLIP_PYRAMID_Z,
    FARCLIP_CIRCLE_INFINITE_X, // X-axis is not used while calculating distance
                               // (cylinder with infinite length)
    FARCLIP_CIRCLE_INFINITE_Y,
    FARCLIP_CIRCLE_INFINITE_Z,
    FARCLIP_SQUARE_INFINITE_X, // X-axis is not used while calculating bounding
                               // box (box with infinite length on X-axis)
    FARCLIP_SQUARE_INFINITE_Y,
    FARCLIP_SQUARE_INFINITE_Z
} farclip_t;

#define CLOSECLIP_BINARY_MASK                                                  \
    0x80 // bit on => this is a close-clip rather than a far-clip
...
-----------------------
*/
#define MAPLOD_BINARY_MASK                                                     \
    0x400 // bit on => LOD definition, bit off => no LOD definition: always
          // displayed
#define MAPLOD_GTE_BINARY_MASK                                                 \
    0x800 // bit on => object is displayed if LOD is GreaTer or Equal (GTE) to
          // the value bit off => object is displayed if LOD is less or equal to
          // the value
...
#define KNOCKTIME                                                              \
    500 // time player can't move with normal speed after being hit
...
typedef struct {
    ...
    qboolean
        gauntletHit; // true if a gauntlet attack would actually hit something
...
#define SF_SEC_PISTOL                                                          \
    0x00002 // has player a second pistol, equal to the first one?
#define SF_GAT_CHANGE                                                          \
    0x00004 // player has taken a built-up gatling -> play WP_ANIM_CHANGE
#define SF_RELOAD                                                              \
    0x00008 // reload animation of weapon1 must still be started (akimbos)
#define SF_RELOAD2                                                             \
    0x00010 // reload animation of weapon2 must still be started (akimbos)
#define SF_DU_INTRO                                                            \
    0x00020 // player is in intro mode, don't update the viewangles
#define SF_DU_WON 0x00040 // player has won in duel and is now able to spectate
#define SF_WP_CHOOSE                                                           \
    0x00080 // this player is in weapon choose mode don't react to BUTTON_ATTACK
...
typedef enum {
    ...
    PERS_HITS,  // total points damage inflicted so damage beeps can sound on
                // change
...
#define EF_SAME_TEAM                                                           \
    0x00000008 // Joe Kari: EV_OBITUARY + EF_SAME_TEAM = TeamKilling
               // Note that it is the same than EF_AWARD_EXCELLENT that is not
               // use in SG
#define EF_BOUNCE 0x00000010         // for missiles
#define EF_BOUNCE_HALF 0x00000020    // for missiles
#define EF_AWARD_GAUNTLET 0x00000040 // draw a gauntlet sprite
#define EF_NODRAW                                                              \
    0x00000080 // may have an event, but no model (unspawned items)
...
#define EF_HIT_MESSAGE                                                         \
    0x00100000 // a hit message of several locations is being sent

// Spoon flags
#define EF_RELOAD 0x00100000  // if reloading
#define EF_RELOAD2 0x00200000 // if weapon2 is reloading
#define EF_BROKEN                                                              \
    0x00400000 // broken func_breakable just too respawn on roundstart
...
typedef enum {
    ...
    // explosives
    WP_DYNAMITE, // this always should be the last weapon after the special
                 // weapons
    WP_MOLOTOV,  // has to be the last weapon in here, because of the
                 // +prevweap-cmd
...
// means of death
typedef enum {
    // means of death of the weapons have to be in order of the weaponnums,
    // important!
    MOD_UNKNOWN,
    // melee
    MOD_KNIFE,
...
#define MASK_SHOT                                                              \
    (CONTENTS_SOLID | CONTENTS_EXPLOSIVE | CONTENTS_BODY | CONTENTS_CORPSE)
...
typedef enum { TYPE_CYLINDER, TYPE_SPHERE, NUM_TYPES } hit_types;
...
#else
// Because bg_* are also shared with UI VM... but we don't care about it, let's
// give it the default maxMoney value
