...
void QDECL BotAI_BotInitialChat(bot_state_t *bs, char *type, ...) {
    ...
    trap_BotInitialChat(bs->cs, type, mcontext, vars[0], vars[1], vars[2],
                        vars[3], vars[4], vars[5], vars[6], vars[7]);
}
...
void BotTestAAS(vec3_t origin) {
    ...
    if (bot_testsolid.integer) {
        ...
        if (!areanum)
            ...
            BotAI_Print(PRT_MESSAGE, "\rarea %d, cluster %d       ", areanum,
                        info.cluster);
        }
    }
}
...
void BotInterbreedBots(void) {
    ...
    if (trap_GeneticParentsAndChildSelection(MAX_CLIENTS, ranks, &parent1,
                                             &parent2, &child)) {
        trap_BotInterbreedGoalFuzzyLogic(botstates[parent1]->gs,
                                         botstates[parent2]->gs,
                                         botstates[child]->gs);
...
==============
*/
int NumBots(void) { return numbots; }
...
void BotChangeViewAngles(bot_state_t *bs, float thinktime) {
    ...
    if (bs->enemy >= 0) {
        factor = trap_Characteristic_BFloat(
            bs->character, CHARACTERISTIC_VIEW_FACTOR, 0.01f, 1);
        maxchange = trap_Characteristic_BFloat(
            bs->character, CHARACTERISTIC_VIEW_MAXCHANGE, 1, 1800);
    } else {
        factor = 0.05f;
        maxchange = 360;
    }
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (bot_challenge.integer) {
            ...
            diff = abs(
                AngleDifference(bs->viewangles[i], bs->ideal_viewangles[i]));
            anglespeed = diff * factor;
            if (anglespeed > maxchange)
                anglespeed = maxchange;
            bs->viewangles[i] = BotChangeViewAngle(
                bs->viewangles[i], bs->ideal_viewangles[i], anglespeed);
        } else {
        ...
        // bs->ideal_viewangles[2]);` bs->viewangles[i] =
...
==============
*/
void BotInputToUserCommand(bot_state_t *bs, bot_input_t *bi, usercmd_t *ucmd,
                           int delta_angles[3], int time) {
    ...
    if (bi->actionflags & ACTION_ATTACK)
        ucmd->buttons |= BUTTON_ATTACK;
    if (bs->flags & BFL_ATTACK)
        ucmd->buttons |= BUTTON_ATTACK;
    if (bs->flags & BFL_ATTACK2)
    ...
    if (bs->flags & BFL_RELOAD) {
        ...
        if (bs->cur_ps.weapon2)
            ucmd->buttons |= BUTTON_ATTACK;
    }

    if (bs->flags & BFL_RELOAD2) {
        ucmd->buttons |= BUTTON_RELOAD;
...
void BotUpdateInput(bot_state_t *bs, int time, int elapsed_time) {
    ...
    BotInputToUserCommand(bs, &bi, &bs->lastucmd, bs->cur_ps.delta_angles,
                          time);
    // subtract the delta angles
    for (j = 0; j < 3; j++) {
        bs->viewangles[j] = AngleMod(bs->viewangles[j] -
...
void BotScheduleBotThink(void) {
    ...
    for (i = 0; i < MAX_CLIENTS; i++) {
        ...
        botstates[i]->botthink_residual =
            bot_thinktime.integer * botnum / numbots;
        botnum++;
    }
}
...
void BotWriteSessionData(bot_state_t *bs) {
    ...
    s = va("%i %i %i %i %i %i %i %i"
           ...
           " %f %f %f"
           " %f %f %f",
           bs->lastgoal_decisionmaker, bs->lastgoal_ltgtype,
           bs->lastgoal_teammate, bs->lastgoal_teamgoal.areanum,
           bs->lastgoal_teamgoal.entitynum, bs->lastgoal_teamgoal.flags,
           bs->lastgoal_teamgoal.iteminfo, bs->lastgoal_teamgoal.number,
           bs->lastgoal_teamgoal.origin[0], bs->lastgoal_teamgoal.origin[1],
           bs->lastgoal_teamgoal.origin[2], bs->lastgoal_teamgoal.mins[0],
           bs->lastgoal_teamgoal.mins[1], bs->lastgoal_teamgoal.mins[2],
           bs->lastgoal_teamgoal.maxs[0], bs->lastgoal_teamgoal.maxs[1],
...
int BotAISetupClient(int client, struct bot_settings_s *settings,
    ...
    bs->walker =
        trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_WALKER, 0, 1);
    numbots++;
...
int BotAIShutdownClient(int client, qboolean restart) {
    ...
    bs = botstates[client];
    if (!bs || !bs->inuse) {
        // BotAI_Print(PRT_ERROR, "BotAIShutdownClient: client %d already
        // shutdown\n", client);
        return qfalse;
    }
...
#define BOT_AUTOPAUSABLE 0
    ...
    if (bot_pause.integer && trap_AAS_Initialized())
        || (bot_pause.integer == 2 && g_humancount == 0) {
#else
    if (bot_pause.integer && trap_AAS_Initialized()) {
#endif
...
int BotInitLibrary(void) {
    ...
    // set the maxclients and maxentities library variables before calling
    // BotSetupLibrary
