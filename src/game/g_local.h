...
#define FL_BUY_ITEM                                                            \
    0x00020000 // item that is bought to prevent startsolid
               // problems
#define FL_INITIALIZED                                                         \
    0x00040000 // is entity already initialized? (doors, etc.)
...
struct gentity_s {
    ...
    entityShared_t
        r; // shared by both the server system and game (game/g_public.h)
...
typedef struct {
    ...
    int lastVoteTime;
    // Tequila: Keep a cleanname as unique name, players shouldn't be able to
    // have same cleanname
...
struct gclient_s {
    ...
    qboolean
        inactivityWarning; // qtrue if the five seoond warning has been given
    ...
    qboolean specwatch; // if yes the player has joines the specs because he's a
                        // winner
    int specmappart;
...
#define SOCIAL_HELP_COUNT 3
    ...
    // when using a deployed gatling, and at the same time having one in the
    // inventory, the ammo count for the latter one is saved here
    int carriedGatlingAmmo;

    // Tequila: Flag to avoid telefrag at respawn when not enough spawnpoint are
    // available
...
int G_InvulnerabilityEffect(gentity_t *targ, vec3_t dir, vec3_t point,
                            vec3_t impactpoint, vec3_t bouncedir);
...
#define DAMAGE_NO_KNOCKBACK                                                    \
    0x00000004 // do not affect velocity, just view angles
#define DAMAGE_NO_PROTECTION                                                   \
    0x00000008 // armor, shields, invulnerability, and godmode have no effect
...
qboolean G_CanEntityBeActivated(gentity_t *player, gentity_t *target,
                                qboolean exact);
...
// new trap-functions
void trap_Trace_New(trace_t *results, const vec3_t start, const vec3_t mins,
                    const vec3_t maxs, const vec3_t end, int passEntityNum,
                    int contentmask);
int trap_Trace_New2(trace_t *results, const vec3_t start, const vec3_t mins,
                    const vec3_t maxs, const vec3_t end, int passEntityNum,
                    int contentmask);
