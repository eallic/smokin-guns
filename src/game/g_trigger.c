...
void Touch_Escape(gentity_t *self, gentity_t *other, trace_t *trace) {
    ...
    if (!other->client->ps.powerups[PW_GOLD])
        return;
    ...
    if (reward < MIN_POINT_ROBBER_REWARD)
        reward = MIN_POINT_ROBBER_REWARD;
    else if (reward > MAX_POINT_ROBBER_REWARD)
        reward = MAX_POINT_ROBBER_REWARD;
...
void SP_trigger_escape(gentity_t *ent) {
    ...
    ent->touch = Touch_Escape;

    InitTrigger(ent);

    ent->s.eType = ET_ESCAPE;
    ent->r.contents = 0;
    ent->r.svFlags = 0; // SVF_BROADCAST; // send to client

    // save the midpoint
    VectorAdd(ent->r.absmax, ent->r.absmin, p);
    VectorScale(p, 0.5f, ent->s.angles2);
