...
==================
*/
void BotResetNodeSwitches(void) { numnodeswitches = 0; }
    ...
    if (bs->lastair_time < FloatTime() - 6) {
       ...
       // if we can find an air goal
        if (BotGetAirGoal(bs, &goal)) {
            ...
            while (trap_BotChooseNBGItem(bs->gs, bs->origin, bs->inventory, tfl,
                                         ltg, range)) {
...
int BotReachedGoal(bot_state_t *bs, bot_goal_t *goal) {
    if (goal->flags & GFL_ITEM) {
        ...
        if (trap_BotItemGoalInVisButNotVisible(bs->entitynum, bs->eye,
                                               bs->viewangles, goal)) {
            /*
            float avoidtime;
            int t;
            ...
            if (avoidtime > 0) {
                    t = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum,
            bs->origin, goal->areanum, bs->tfl); if ((float) t * 0.009 <
            avoidtime) return qtrue;
            }
            */
            return qtrue;
        }
        ...
        if ((bs->flags & BFL_SEEK) &&
            Distance(bs->origin, goal->origin) < (200 + (rand() % 100 - 50)))
            return qtrue;
    ...
    for (i = 0, mark = 0; i < ai_nodecount; i++) {
        ...
        if (avoid != NULL) {
            ...
            if (Distance(avoid, dir1) < Distance(avoid, dir2) &&
                Distance(org, ai_nodes[i]) < 4000 &&
                Distance(org, ai_nodes[i]) >= mindist) {
                mark = i;
            }
            // valid = qtrue;
        } else if (Distance(org, ai_nodes[i]) < Distance(org, ai_nodes[mark]) &&
                   Distance(org, ai_nodes[i]) >= mindist) {
            mark = i;
        }
    }
...
static void BotAI_SetRoute(bot_state_t *bs) {
    ...
    if (Distance(bs->teamgoal.origin, bs->orientation.endpos) < 200 ||
        ...
        for (i = 0, node = 0, alt_node = -1; i < ai_nodecount; i++) {
            ...
            // if we're playing br, they shouldn't move too far away from the
            // moneybags
            /*if(g_gametype.integer == GT_BR &&
            !bs->cur_ps.persistant[PERS_ROBBER]){ vec3_t org;

                    if(VectorLength(g_entities[vec_money[0]].r.currentOrigin)){
                            VectorCopy(g_entities[vec_money[0]].r.currentOrigin,
            org); if(Distance(ai_nodes[i], org) > MONEYBAG_DIST) continue;
                    }
                    if(VectorLength(g_entities[vec_money[1]].r.currentOrigin)){
                            VectorCopy(g_entities[vec_money[1]].r.currentOrigin,
            org); if(Distance(ai_nodes[i], org) > MONEYBAG_DIST) continue;
                    }
            }*/
            ...
            for (j = 0; j < 2; j++) {
                ...
                VectorSubtract(bs->orientation.midpoint[j], ai_nodes[node],
                               dir3);
                vectoangles(dir3, angles3);
            ...
            if (fabs(diff1) > fabs(diff2) &&
                (Distance(bs->origin, ai_nodes[i]) >
                     Distance(bs->origin, ai_nodes[node]) ||
        ...
        VectorCopy(bs->orientation.startpos,
                   bs->orientation.midpoint[bs->orientation.bit]);
...
static void BotAI_SetMoneyRoute(bot_state_t *bs, int money1, int money2) {
    ...
    if (*pmoney == 0) {
        ...
        if (Distance(g_entities[money1].r.currentOrigin, bs->origin) < 1000)
            *pmoney = money1;
        else if (money2 && Distance(g_entities[money2].r.currentOrigin,
                                    bs->origin) < 1000)
            *pmoney = money2;
        else if (rand() % 2 || !money2)
            *pmoney = money1;
        else
            *pmoney = money2;
        *pmoney2 = 0;
...
static qboolean BotAI_FindNode(bot_state_t *bs, vec3_t waypoint) {
    ...
    for (i = 0, count = 0; i < ai_nodecount; i++) {
        ...
        time2 = TravelTime(bs->origin, ai_nodes[i]) +
                TravelTime(ai_nodes[i], bs->orientation.endpos);
    ...
    // add to the midpoint
    bs->orientation.count++;
    VectorAdd(bs->orientation.midpoint[bs->orientation.bit], waypoint,
              bs->orientation.midpoint[bs->orientation.bit]);

    // G_Printf("waypoint %i: %f %f %f\n", bs->orientation.count, waypoint[0],
    // waypoint[1], 		waypoint[2]);

    return qtrue;
}

static void BotAI_LookForNodes(bot_state_t *bs, bot_goal_t *goal, int money1,
                               int money2) {
    ...
    if (bs->ltg_time < FloatTime()) {
        ...
        do {
...
#ifdef DEBUG
                    BotAI_Print(PRT_MESSAGE, "wait buy %d>%d.", bs->buytime,
                                level.time);
#endif
                    return qfalse;
                }
...
#endif
            } else { // the bot gets sorta stuck with all the avoid timings,
            ...
            if (valid) {
...
#endif
                /*if(g_gametype.integer == GT_BR &&
                bs->cur_ps.persistant[PERS_ROBBER] &&
                        !Q_stricmp(g_entities[goal->entitynum].classname,
                "item_money") &&
                        !(g_entities[goal->entitynum].r.svFlags &
                SVF_NOCLIENT)){ int *pmoney;
                        ...
                        if( (bs->flags & BFL_MONEY) && *pmoney ==
                goal->entitynum){ break; } else if( Distance(goal->origin,
                bs->origin) < 1000 && bs->waypointtime + 5000 < level.time){
                ...
                // chase him
                /*if(!Q_stricmp(g_entities[goal->entitynum].classname,
                "player")){

                        if(Distance(level.clients[goal->entitynum].ps.origin,
                bs->eye) < 800 ||
                                (Distance(level.clients[goal->entitynum].ps.origin,
                bs->eye) < 6000 &&
...
#ifdef DEBUG
            BotAI_Print(PRT_MESSAGE, "grabbable %d.",
                        BG_CanItemBeGrabbed(g_gametype.integer,
                                            &g_entities[goal->entitynum].s,
                                            &bs->cur_ps));
#endif
        } while (
            !valid || (bs->flags & BFL_AVOID) ||
            ...
            (g_entities[goal->entitynum].s.eType == ET_ITEM &&
             !BG_CanItemBeGrabbed(g_gametype.integer,
                                  &g_entities[goal->entitynum].s,
                                  &bs->cur_ps)));

        /*Com_Printf("yes %i %i %s %i return: %i\n", goal->iteminfo,
           goal->entitynum, g_entities[goal->entitynum].classname,
...
==================
*/
int BotGetLongTermGoal(bot_state_t *bs, int tfl, int retreat,
                       bot_goal_t *goal) {
    ...
    if (bs->ltgtype == LTG_TEAMHELP && !retreat) {
        ...
        if (BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360,
                             bs->teammate)) {
        ...
        if (BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360,
            ...
            if (VectorLengthSquared(dir) < Square(bs->formation_dist)) {
                ...
                if (botinfo.origin[2] + botinfo.maxs[2] >
                    entinfo.origin[2] + entinfo.mins[2]) {
                    ...
                    if (botinfo.origin[0] + botinfo.maxs[0] >
                        ...
                        if (botinfo.origin[1] + botinfo.maxs[1] >
                            ...
                            if (botinfo.origin[2] + botinfo.maxs[2] >
                                ...
                                // if the followed client looks in the direction
                                // of this bot
                                AngleVectors(entinfo.angles, dir, NULL, NULL);
                                dir[2] = 0;
                                VectorNormalize(dir);
                                // VectorSubtract(entinfo.origin,
                                // entinfo.lastvisorigin, dir);
                                VectorSubtract(bs->origin, entinfo.origin,
                                               dir2);
                                VectorNormalize(dir2);
                                if (DotProduct(dir, dir2) > 0.7) {
                                    // back up
                                    BotSetupForMovement(bs);
                                    // trap_BotMoveInDirection(bs->ms, dir2,
                                    // 400, MOVE_WALK);
                                }
                            }
                        }
                ...
                if (bs->attackcrouch_time < FloatTime() - 5) {
                    croucher = trap_Characteristic_BFloat(
                        bs->character, CHARACTERISTIC_CROUCHER, 0, 1);
    ...
    if (bs->ltgtype == LTG_DEFENDKEYAREA) {
        if (trap_AAS_AreaTravelTimeToGoalArea(
                bs->areanum, bs->origin, bs->teamgoal.areanum, TFL_DEFAULT) >
            bs->defendaway_range) {
            ...
            if (!bs->arrive_time) {
                if (bs->ltgtype == LTG_CAMPORDER) {
                    BotAI_BotInitialChat(
                        ...
                        NULL);
                    trap_BotEnterChat(bs->cs, bs->decisionmaker, CHAT_TELL);
                    BotVoiceChatOnly(bs, bs->decisionmaker,
                                     VOICECHAT_INPOSITION);
                }
                bs->arrive_time = FloatTime();
            }
    ...
    if (bs->lead_time > 0 && !retreat) {
        ...
        if (bs->leadbackup_time > FloatTime()) {
            ...
            if (squaredist > Square(500)) {
                if (bs->leadmessage_time < FloatTime() - 20) {
                    BotAI_BotInitialChat(bs, "followme",
                                         EasyClientName(bs->lead_teammate,
                                                        teammate,
                                                        sizeof(teammate)),
                                         NULL);
                    trap_BotEnterChat(bs->cs, bs->teammate, CHAT_TELL);
...
#define BOX 300
    {
        ...
        for (i = 0; i < num; i++) {
            ...
            // if there's a grenade or a molotov in the near environment->avoid
            // it
            if (state->eType == ET_MISSILE && state->weapon == WP_DYNAMITE &&
                state->apos.trDelta[0]) {
            } else if ((state->eType == ET_MISSILE ||
                        !Q_stricmp(g_entities[state->number].classname,
                                   "sprotz")) &&
    ...
    // if blocked by an avoid spot and the view angles and weapon are used for
    // movement
    if (bs->blockedbyavoidspot_time > level.time && !(bs->flags & BFL_AVOID)) {
        ...
        if (mark != -1) {
            ...
            if (bs->flags & BFL_ACTIVATE)
                BotAI_FindNearestNode(qtrue, bs, state.pos.trBase,
                                      bs->escape_point, 1000, bs->avoid_point);
            else
                BotAI_FindNearestNode(qtrue, bs, state.pos.trBase,
                                      bs->escape_point, 1000, NULL);
...
int AINode_Seek_ActivateEntity(bot_state_t *bs) {
    ...
    if (bs->activatestack->shoot) {
        //
        BotAI_Trace(&bsptrace, bs->eye, NULL, NULL, bs->activatestack->target,
    ...
    if (targetvisible) {
        ...
        if (!VectorCompare(bs->activatestack->origin, entinfo.origin)) {
            ...
            if (g_entities[bs->activatestack->goal.entitynum].health <= 0 ||
                Q_stricmp(
                    g_entities[bs->activatestack->goal.entitynum].classname,
                    "func_breakable"))
    ...
    // Tequila: Avoid using a NULL pointer as activatestack may have been clear
    // in modified BotClearPath() API
    ...
    if (moveresult.flags & (MOVERESULT_MOVEMENTVIEWSET |
                            MOVERESULT_MOVEMENTVIEW | MOVERESULT_SWIMVIEW)) {
...
int AINode_Seek_NBG(bot_state_t *bs) {
    ...
    if (bs->nbg_time < FloatTime() && !override) {
        ...
        // NOTE: we canNOT reset the check_time to zero because it would create
        // an endless loop of node switches
...
int AINode_Seek_LTG(bot_state_t *bs) {
    ...
    BotAI_Print(PRT_MESSAGE, "movement %d %d %d\n", moveresult.blockentity,
                moveresult.blocked, moveresult.flags);
#endif
    return qtrue;
}
...
int AINode_Battle_Chase(bot_state_t *bs) {
    ...
    if (moveresult.flags & (MOVERESULT_MOVEMENTVIEWSET |
        ...
        if (bs->chase_time > FloatTime() - 2) {
            ...
            if (trap_BotMovementViewTarget(bs->ms, &goal, bs->tfl, 300,
                                           target)) {
                VectorSubtract(target, bs->origin, dir);
                vectoangles(dir, bs->ideal_viewangles);
            } else {
    ...
    if (bs->areanum == bs->lastenemyareanum)
        bs->chase_time = 0;
    // if the bot wants to retreat (the bot could have been damage during the
    // chase)
    return qtrue;
}
...
int AINode_Battle_Retreat(bot_state_t *bs) {
    ...
    // update the attack inventory values
    BotUpdateBattleInventory(bs, bs->enemy);
    // if the bot doesn't want to retreat anymore... probably picked up some
    // nice items
    if (BotWantsToChase(bs)) {
...
int AINode_Battle_NBG(bot_state_t *bs) {
    ...
    if (moveresult.flags & (MOVERESULT_MOVEMENTVIEW | MOVERESULT_SWIMVIEW)) {
        ...
        if (attack_skill > 0.3) {
            //&& BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360,
            //bs->enemy)
            BotAimAtEnemy(bs);
        } else {
            if (trap_BotMovementViewTarget(bs->ms, &goal, bs->tfl, 300,
