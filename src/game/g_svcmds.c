...
Joe Kari: same than Svcmd_ForceTeam_f() except it accepts a client ID rather
than its name. Usefull for RCON script bots. Also, it prevent user from setting
it to anything. Old Svcmd_ForceTeam_f() can lead to some bug.
*
...
void Svcmd_ForceTeamNum_f(void) {
    ...
    if (!Q_stricmp(arg, "lawmen") || !Q_stricmp(arg, "red") ||
        !Q_stricmp(arg, "r")) {
        SetTeam(&g_entities[cl - level.clients], "rs");
    } else if (!Q_stricmp(arg, "outlaws") || !Q_stricmp(arg, "blue") ||
               !Q_stricmp(arg, "b")) {
        SetTeam(&g_entities[cl - level.clients], "bs");
    } else if (!Q_stricmp(arg, "spectator") || !Q_stricmp(arg, "spec") ||
               !Q_stricmp(arg, "s")) {
...
==================
Svcmd_GiveMoney_f
Joe Kari: Give money to a client, usefull for third party RCON script (I planned
to write a poker RCON script).
*
...
void Svcmd_GiveMoney_f() {
    ...
    if (cl->ps.stats[STAT_MONEY] > g_maxMoney.integer)
        cl->ps.stats[STAT_MONEY] = g_maxMoney.integer;
    else if (cl->ps.stats[STAT_MONEY] < 0)
        cl->ps.stats[STAT_MONEY] = 0;

    amount = cl->ps.stats[STAT_MONEY] - saved_money;

    // Joe Kari: it's important to advertise here, to prevent abuser (yes,
    // admins can be cheaters too)
    if (amount > 0) {
...
void Svcmd_GiveItem_f() {
    ...
    case IT_WEAPON:
        ...
        if (!clip) {
            ...
            if (item->weapon_sort != WS_PISTOL)
                return;
            if (cl->ps.stats[STAT_FLAGS] & SF_SEC_PISTOL)
                return;
            ...
            // give ammo
            if (cl->ps.ammo[clip] < maxAmmo)
                cl->ps.ammo[clip] = maxAmmo;
        } else {
            // add weapon
    ...
    case IT_AMMO:
        if (item->giTag == WP_BULLETS_CLIP) {
            ...
            if (cl->ps.stats[STAT_GATLING_MODE])
                weapon = WP_GATLING;
            else
                weapon =
                    BG_FindPlayerWeapon(WP_WINCHESTER66, WP_DYNAMITE, &cl->ps);

            if (weapon)
                item = BG_FindItemForAmmo(bg_weaponlist[weapon].clip);
            else
                return;
        }
    ...
    case IT_POWERUP:
        if (item->giTag == PW_BELT) {
            ...
            cl->ps.ammo[WP_SHELLS_CLIP] =
                bg_weaponlist[WP_REMINGTON_GAUGE].maxAmmo * 2;
            cl->ps.ammo[WP_CART_CLIP] =
    ...
    case IT_ARMOR:
        if (/* cl->ps.powerups[item->giTag] && */ cl->ps.stats[STAT_ARMOR] >=
            BOILER_PLATE)
            return;
        // cl->ps.powerups[item->giTag] = 1 ;
...
===================
*/
void Svcmd_PushMinilog_f(void) { PushMinilog(ConcatArgs(1)); }
...
==================
Svcmd_KickNum_f
Joe Kari: there was in server/sv_ccmds.c a note that mention "FIXME: move to
game", so I move it to game... Additionnaly, you can now provide a reason for
kicking someone.
*
...
void Svcmd_SendAway_f(void) {
    ...
    if (cl)
        ...
        trap_SendServerCommand(-1,
                               va("print \"Can't kick %s\"", ConcatArgs(1)));
}
...
static void Svcmd_PlayerStatus_f(void) {
    ...
    for (i = 0, cl = level.clients; i < level.maxclients; i++, cl++) {
        ...
        // Health/Max (handicap)
        Com_Printf("^2HP^7:%i/%i ", cl->ps.stats[STAT_HEALTH],
                   cl->pers.maxHealth);
        // Money
        Com_Printf("^2$^7:%i ", cl->ps.stats[STAT_MONEY]);
        // Team and robber
        switch (cl->ps.persistant[PERS_TEAM]) {
        // use only generic team name here (not real team name, it would confuse
        // admin-bots)
        case 1:
            s = "lawmen";
            break;
        ...
        Com_Printf("^2TM^7:%s%s ", s,
                   cl->ps.persistant[PERS_ROBBER] ? "[R]" : "");
        // Time
...
qboolean ConsoleCommand(void) {
    ...
    // It is probably not a good idea to allow an user to push the minilog, it
    // would interfer with game log
    ...
    // Tequila: Replacement for kick command from server engine with sendaway
    // one
