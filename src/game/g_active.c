    ...
    if (waterlevel == 3) {
        ...
        if (ent->client->airOutTime < level.time) {
            ...
            if (ent->health > 0) {
                ...
                G_Damage(ent, NULL, NULL, NULL, NULL, ent->damage,
        ...
        if (ent->health > 0 && ent->pain_debounce_time <= level.time) {
            ...
            if (envirosuit) {
                ...
                if (ent->watertype & CONTENTS_LAVA) {
                    G_Damage(ent, NULL, NULL, NULL, NULL, 30 * waterlevel, 0,
                             MOD_LAVA);
                }

                if (ent->watertype & CONTENTS_SLIME) {
                    G_Damage(ent, NULL, NULL, NULL, NULL, 10 * waterlevel, 0,
                             MOD_SLIME);
                }
            }
        }
...
void G_TouchTriggers(gentity_t *ent) {
    ...
    for (i = 0; i < num; i++) {
        ...
        if (!(hit->r.contents & CONTENTS_TRIGGER) &&
            !(hit->r.contents & CONTENTS_EXPLOSIVE) &&
            !(hit->s.eType == ET_MOVER && hit->s.angles2[0] == -1000) &&
        ...
        if (hit->s.eType == ET_ITEM) {
            if (hit->item->giType == IT_POWERUP &&
                ...
                trap_Trace(&tr, muzzle, NULL, NULL, hit->r.currentOrigin,
                           ent->client->ps.clientNum, MASK_SHOT);
            ...
            if ((!(ent->client->buttons & BUTTON_ACTIVATE) ||
                ...
                (!(ent->r.svFlags & SVF_BOT) ||
                 (hit->moverState != MOVER_POS1 &&
                  hit->moverState !=
                      MOVER_POS2))) //  bots dont need to press down activate,
                                    //  but sometimes they need to close doors
                                    //  in order to re-open them outwards
...
void G_TouchStartpoint(gentity_t *ent) {
    ...
    for (i = 0; i < MAX_GENTITIES; i++) {
        ...
        if (!Q_stricmp(hit->classname, classname)) {
            ...
            VectorSubtract(ent->client->ps.origin, hit->r.currentOrigin,
                           distance);
...
void SpectatorThink(gentity_t *ent, usercmd_t *ucmd) {
    ...
    if (ent->client->sess.spectatorState == SPECTATOR_CHASECAM ||
        ...
        if (ent->client->sess.spectatorState != SPECTATOR_FOLLOW) {
            ...
            client->ps.stats[STAT_HEALTH] =
                level.clients[client->sess.spectatorClient]
                    .ps.stats[STAT_HEALTH];
            client->ps.stats[STAT_FLAGS] &= ~SF_DU_INTRO;

            if (ent->client->sess.spectatorState == SPECTATOR_FIXEDCAM) {
                VectorCopy(
                    level.clients[client->sess.spectatorClient].ps.viewangles,
                    client->ps.viewangles);
                VectorCopy(
                    level.clients[client->sess.spectatorClient].ps.delta_angles,
                    client->ps.delta_angles);
            }
        }

        client->ps.pm_flags |= PMF_FOLLOW;
        VectorCopy(level.clients[client->sess.spectatorClient].ps.origin,
                   client->ps.origin);

        if (g_gametype.integer == GT_DUEL && client->realspec &&
            ent->client->specmappart !=
                g_entities[client->sess.spectatorClient].mappart) {
            ent->client->specmappart =
                g_entities[client->sess.spectatorClient].mappart;
            ent->mappart = ent->client->specmappart;
        }

        if (level.clients[client->sess.spectatorClient].sess.sessionTeam >=
            TEAM_SPECTATOR) {
            if (TeamCount(-1, ent->client->sess.sessionTeam - 3) &&
                g_gametype.integer >= GT_RTP && g_chaseonly.integer &&
    ...
    if (client->sess.spectatorState != SPECTATOR_FOLLOW) {
        ...
        pm.tracemask = MASK_PLAYERSOLID &
                       ~CONTENTS_BODY; // spectators can fly through bodies
        ...
        // We need to update max box as it is used for the hit computing. This
        // prevent to use some dirty hack at bullet fire checks which involves
        // very bad side effects on not linked entities... We do that after the
        // move and after touching triggers like trap doors from underneath

        // The fix is to rise up the Bbox to include the head model
        pm.maxs[2] += MAXS_Z_BBOX_UPDATE;
        // We don't modify other bbox sizes as it usually involves bad loop in
        // bullet trace computing when shooting at 2 players too next to each
        // other. This may involves some missed hit but not so often than people
        // will believe :P
    ...
    if (!g_specsareflies.integer ||
        ...
        if (g_smoothClients.integer) {
            BG_PlayerStateToEntityStateExtraPolate(
                &ent->client->ps, &ent->s, ent->client->ps.commandTime, qtrue);
        } else {
    ...
    if (g_gametype.integer >= GT_RTP && g_chaseonly.integer) {
        ...
        if ((client->buttons & BUTTON_ALT_ATTACK) &&
            !(client->oldbuttons & BUTTON_ALT_ATTACK)) {
            ChaseCam_Start(ent, -1);
        }
        ...
        if ((client->buttons & BUTTON_ALT_ATTACK) &&
            ...
            if (g_gametype.integer == GT_DUEL) {
                ...
                // has to be done, otherwise no "info_player_intermission" would
                // be used
...
qboolean ClientInactivityTimer(gclient_t *client) {
    if (!g_inactivity.integer) {
        ...
        if (level.time > client->inactivityTime &&
            g_humancount) { // TheDoctor: energy save
...
void ClientEvents(gentity_t *ent, int oldEventSequence) {
    ...
    for (i = oldEventSequence; i < client->ps.eventSequence; i++) {
        ...
        case EV_FALL_FAR:
            ...
            // This event is throwed from bg_pmove.c by the PM_CrashLand()
            // function
            damage = eventParm;
...
void ClientThink_real(gentity_t *ent) {
    ...
    client->pers.pingsamples[client->pers.samplehead] =
        level.previousTime + client->frameOffset - ucmd->serverTime;
    ...
    //  if the client is adding latency to received snapshots (server-to-client
    //  latency)
    if (client->pers.latentSnaps) {
        // adjust the real ping
        client->pers.realPing +=
            client->pers.latentSnaps * (1000 / sv_fps.integer);
    ...
    if (client->ps.pm_type == PM_DEAD && g_deathcam.integer &&
        !(g_entities[client->ps.clientNum].r.svFlags & SVF_BOT) &&
    ...
    if (ent->client->ps.weapon == WP_KNIFE && (ucmd->buttons & BUTTON_ATTACK) &&
        ...
        ent->client->ps.weaponTime -
                (ucmd->serverTime - ent->client->ps.commandTime) <=
            0) {
    ...
    if (client->ps.stats[STAT_KNOCKTIME]) {
        ...
        if (fabs(client->ps.stats[STAT_KNOCKTIME]) >= KNOCKTIME ||
    ...
    if ((!du_nextroundstart || du_nextroundstart <= level.time) &&
        ...
        VectorClear(ent->s.pos.trDelta);
               ...
               (level.time < g_roundstarttime + level.roundNoMoveTime) &&
               (ent->client->sess.sessionTeam < TEAM_SPECTATOR) &&
               !(client->dontTelefrag)) {
        // added by Joe Kari: delete all movement and cmd stats until the end of
        // the countdown in RTP and BR gametype Tequila: But authorize to move
        // if we are in the Telefrag case, so player can move to a safe place
        pm.ps->speed = 0;
        pm.cmd.forwardmove = 0;
        pm.cmd.rightmove = 0;
        pm.cmd.upmove = 0;
    ...
    // if player is able to change to the spectators in duel mode remove the
    // attack button
    ...
    // use the snapped origin for linking so it matches client predicted
    // versions
    ...
    // Tequila comment: When client should have telefrag another player when
    // respawning, he's in the Telefrag case. But we must limit that case in the
    // time as it is not suitable for playing to being able to not interact with
    // the world
#define TELEFRAG_CASE_TIME                                                     \
    15000 // Max seconds a client can be kept in Telefrag case
#define TELEFRAG_CASE_CHECK_DELAY                                              \
    500 // Delay between each check if we still are in that case
    if (client->dontTelefrag) {
        ...
        while (client->timeResidual >= TELEFRAG_CASE_CHECK_DELAY) {
            ...
            if (level.time < client->respawnTime + TELEFRAG_CASE_TIME)
                client->ps.pm_flags |=
                    PMF_RESPAWNED; // simulate a respawn check
            else {
                client->ps.pm_flags &= ~PMF_RESPAWNED;
#ifdef DEBUG_TELEFRAG_CASE
                G_Printf(S_COLOR_YELLOW
                         "Telefrag case workaround disabled on %s...\n",
                         client->pers.netname);
#endif
            }
            trap_UnlinkEntity(ent);
        ...
        if (clientNum >= 0) {
            ...
            if (cl->pers.connected == CON_CONNECTED &&
                ...
                if (g_gametype.integer != GT_FFA) {
                    savedScore = ent->client->ps
                                     .persistant[PERS_SCORE]; // keep own score
                    ent->client->ps = cl->ps;
                ...
                // drop them to free spectators unless they are dedicated camera
                // followers
                if (ent->client->sess.spectatorClient >= 0) {
                    ...
                    if (g_gametype.integer >= GT_RTP) {
                        ...
                        /*trap_SendServerCommand(-1,
                                va("print \"DEBUG: %s " S_COLOR_WHITE "'s score
