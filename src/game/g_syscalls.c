...
void trap_Print(const char *text) { syscall(G_PRINT, text); }
...
int trap_Milliseconds(void) { return syscall(G_MILLISECONDS); }
int trap_Argc(void) { return syscall(G_ARGC); }
...
void trap_FS_FCloseFile(fileHandle_t f) { syscall(G_FS_FCLOSE_FILE, f); }
...
void trap_Cvar_Update(vmCvar_t *cvar) { syscall(G_CVAR_UPDATE, cvar); }
...
// Joe Kari: It is needed to change type of "reason" from const char * to char *
// if you want custom user reason
...
void trap_DropClient(int clientNum, char *reason) {
    ...
    trap_DropClient_string[MAX_TOKEN_CHARS - 1] =
        0; // to prevent some nasty bugs
...
void trap_LinkEntity(gentity_t *ent) { syscall(G_LINKENTITY, ent); }

void trap_UnlinkEntity(gentity_t *ent) { syscall(G_UNLINKENTITY, ent); }
...
int trap_RealTime(qtime_t *qtime) { return syscall(G_REAL_TIME, qtime); }
...
int trap_BotLibShutdown(void) { return syscall(BOTLIB_SHUTDOWN); }
...
int trap_AAS_Initialized(void) { return syscall(BOTLIB_AAS_INITIALIZED); }
...
int trap_AAS_PredictRoute(void /*struct aas_predictroute_s*/ *route,
                          int areanum, vec3_t origin, int goalareanum,
...
int trap_AAS_AlternativeRouteGoals(
    ...
    int travelflags, void /*struct aas_altroutegoal_s*/ *altroutegoals,
    int maxaltroutegoals, int type) {
    return syscall(BOTLIB_AAS_ALTERNATIVE_ROUTE_GOAL, start, startareanum, goal,
                   goalareanum, travelflags, altroutegoals, maxaltroutegoals,
                   type);
}
...
void trap_EA_Say(int client, char *str) { syscall(BOTLIB_EA_SAY, client, str); }
...
void trap_EA_Gesture(int client) { syscall(BOTLIB_EA_GESTURE, client); }

void trap_EA_Talk(int client) { syscall(BOTLIB_EA_TALK, client); }

void trap_EA_Attack(int client) { syscall(BOTLIB_EA_ATTACK, client); }

void trap_EA_Use(int client) { syscall(BOTLIB_EA_USE, client); }

void trap_EA_Respawn(int client) { syscall(BOTLIB_EA_RESPAWN, client); }

void trap_EA_Crouch(int client) { syscall(BOTLIB_EA_CROUCH, client); }

void trap_EA_MoveUp(int client) { syscall(BOTLIB_EA_MOVE_UP, client); }

void trap_EA_MoveDown(int client) { syscall(BOTLIB_EA_MOVE_DOWN, client); }

void trap_EA_MoveForward(int client) {
    syscall(BOTLIB_EA_MOVE_FORWARD, client);
}

void trap_EA_MoveBack(int client) { syscall(BOTLIB_EA_MOVE_BACK, client); }

void trap_EA_MoveLeft(int client) { syscall(BOTLIB_EA_MOVE_LEFT, client); }

void trap_EA_MoveRight(int client) { syscall(BOTLIB_EA_MOVE_RIGHT, client); }

void trap_EA_SelectWeapon(int client, int weapon) {
    syscall(BOTLIB_EA_SELECT_WEAPON, client, weapon);
}

void trap_EA_Jump(int client) { syscall(BOTLIB_EA_JUMP, client); }
...
void trap_EA_ResetInput(int client) { syscall(BOTLIB_EA_RESET_INPUT, client); }
...
float trap_Characteristic_BFloat(int character, int index, float min,
                                 float max) {
    floatint_t fi;
    fi.i = syscall(BOTLIB_AI_CHARACTERISTIC_BFLOAT, character, index,
...
int trap_BotAllocChatState(void) { return syscall(BOTLIB_AI_ALLOC_CHAT_STATE); }
...
void trap_BotInitialChat(int chatstate, char *type, int mcontext, char *var0,
    ...
    syscall(BOTLIB_AI_INITIAL_CHAT, chatstate, type, mcontext, var0, var1, var2,
...
int trap_BotReplyChat(int chatstate, char *message, int mcontext, int vcontext,
    ...
    return syscall(BOTLIB_AI_REPLY_CHAT, chatstate, message, mcontext, vcontext,
...
void trap_BotPopGoal(int goalstate) { syscall(BOTLIB_AI_POP_GOAL, goalstate); }
...
void trap_BotInitLevelItems(void) { syscall(BOTLIB_AI_INIT_LEVEL_ITEMS); }

void trap_BotUpdateEntityItems(void) { syscall(BOTLIB_AI_UPDATE_ENTITY_ITEMS); }
...
void trap_BotAddAvoidSpot(int movestate, vec3_t origin, float radius,
                          int type) {
    syscall(BOTLIB_AI_ADD_AVOID_SPOT, movestate, origin, PASSFLOAT(radius),
            type);
}
...
int trap_BotMovementViewTarget(int movestate,
                               ...
                               vec3_t target) {
    return syscall(BOTLIB_AI_MOVEMENT_VIEW_TARGET, movestate, goal, travelflags,
...
int trap_BotAllocMoveState(void) { return syscall(BOTLIB_AI_ALLOC_MOVE_STATE); }
