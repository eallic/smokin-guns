...
void G_KillBox(gentity_t *ent) {
    ...
    for (i = 0; i < num; i++) {
        ...
        // G_Printf(S_COLOR_YELLOW "G_KillBox: Found %s in %s's
        // killbox\n",hit->client->pers.netname,ent->client->pers.netname);

        // Tequila comment: If the client is being respawned, we would prefer
        // authorize the client to share the spawnpoint for a short time... So
        // just notice we are in a Telefrag case
...
/*
================
G_IsAnyClientWithinRadius

Is any client within the given radius from 'org'?

ignoreTeam can be TEAM_RED or TEAM_BLUE for team games that use
spawnpoints that are spread out (unlike CTF), or -1 otherwise.
================
*/
qboolean G_IsAnyClientWithinRadius(const vec3_t org, float rad,
                                   int ignoreTeam) {
    int length, i, j;
    float radSqr;
    vec_t diff;
    ...
    for (i = 0; i < level.numPlayingClients; i++) {
        ...
        if (ps->stats[STAT_HEALTH] <= 0 ||
            ps->persistant[PERS_TEAM] == ignoreTeam)
            continue;
...
// Joe Kari: New minilog feature. A small stack of FIFO logs. When it is full,
// older entries are removed. Please restrict its use to admin-bots oriented
// stuff. Please respect the log format: <event_name>: <subject_id> [=>
// <object_id>] ... Example: client 1 killed client 3 with remington '58 will
// be: KILL: 1 => 3 [MOD_REM58]
...
=================
PushMinilogf
Joe Kari: push a formated string into of the minilog (similar than printf family
function)
=================
*/
