...
void CL_MouseMove(usercmd_t *cmd) {
    ...
    if (cl_mouseAccel->value != 0.0f) {
        if (cl_mouseAccelStyle->integer == 0) {
            ...
            // cl_mouseAccelOffset is the rate for which the acceleration will
            // have doubled the non accelerated amplification NOTE: decouple the
            // config cvars for independent acceleration setup along X and Y?
            ...
            mx = cl_sensitivity->value *
                 (mx + ((mx < 0) ? -power[0] : power[0]) *
                           cl_mouseAccelOffset->value);
            my = cl_sensitivity->value *
                 (my + ((my < 0) ? -power[1] : power[1]) *
                           cl_mouseAccelOffset->value);
...
void CL_WritePacket(void) {
    ...
    if (clc.voipOutgoingDataSize > 0) {
        if ((clc.voipFlags & VOIP_SPATIAL) ||
            ...
            //  back to us, and we might as well eliminate concerns about
            //  dropped and misordered packets here.
