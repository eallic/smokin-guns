...
void SCR_DrawStringExt(int x, int y, float size, const char *string,
    ...
    while (*s) {
        if (Q_IsColorString(s)) {
            if (!forceColor) {
                Com_Memcpy(color, g_color_table[ColorIndex(*(s + 1))],
                           sizeof(color));
                color[3] = setColor[3];
                re.SetColor(color);
            }
...
void SCR_DrawDemoRecording(void) {
    ...
    SCR_DrawStringExt(320 - strlen(string) * 4, 20, 8, string, g_color_table[7],
                      qtrue, qfalse);
}
...
void SCR_DrawVoipMeter(void) {
    ...
    SCR_DrawStringExt(320 - strlen(string) * 4, 10, 8, string, g_color_table[7],
                      qtrue, qfalse);
}
...
void SCR_DrawVoipSender(void) {
    ...
    switch (atoi(Info_ValueForKey(
        cl.gameState.stringData +
            cl.gameState.stringOffsets[CS_PLAYERS + cls.voipSender],
        "t"))) {
    case TEAM_FREE:
        teamColor = '3';
        break;
...
void SCR_DrawDebugGraph(void) {
    ...
    re.DrawStretchPic(x, y - cl_graphheight->integer, w,
                      cl_graphheight->integer, 0, 0, 0, 0, cls.whiteShader);
    re.SetColor(NULL);
...
void SCR_DrawScreenField(stereoFrame_t stereoFrame) {
    ...
    if (uivm && !uiFullscreen) {
        ...
        case CA_PRIMED:
            ...
            break;
        case CA_ACTIVE:
            // always supply STEREO_CENTER as vieworg offset is now done by the
            // engine.
            CL_CGameRendering(stereoFrame);
            SCR_DrawDemoRecording();
#ifdef USE_VOIP
