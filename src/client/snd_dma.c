...
void S_Base_SoundList(void) {
    ...
    for (sfx = s_knownSfx, i = 0; i < s_numSfx; i++, sfx++) {
        ...
        total += size;
        Com_Printf("%6i[%s] : %s[%s]\n", size,
                   type[sfx->soundCompressionMethod], sfx->soundName,
                   mem[sfx->inMemory]);
    }
...
void S_Base_BeginRegistration(void) {
    ...
    if (s_numSfx == 0) {
        ...
        S_Base_RegisterSound("sound/feedback/hit.wav",
                             qfalse); // changed to a sound in baseq3
    }
}
...
void S_memoryLoad(sfx_t *sfx) {
    // load the sound file
    if (!S_LoadSound(sfx)) {
        //		Com_Printf( S_COLOR_YELLOW "WARNING: couldn't load
        //sound: %s\n", sfx->soundName );
...
====================
*/
static void S_Base_StartSoundEx(vec3_t origin, int entityNum, int entchannel,
                                sfxHandle_t sfxHandle, qboolean localSound) {
    channel_t *ch;
    sfx_t *sfx;
    ...
    for (i = 0; i < MAX_CHANNELS; i++, ch++) {
        if (ch->entnum == entityNum && ch->thesfx == sfx) {
            if (time - ch->allocTime < 50) {
                ...
                //"cg_showmiss" )) { 					Com_Printf("double sound start\n");
                //				}
                return;
            }
            inplay++;
...
====================
*/
void S_Base_StartSound(vec3_t origin, int entityNum, int entchannel,
...
void S_Base_ClearLoopingSounds(qboolean killall) {
    ...
    for (i = 0; i < MAX_GENTITIES; i++) {
        if (killall || loopSounds[i].kill == qtrue ||
            (loopSounds[i].sfx && loopSounds[i].sfx->soundLength == 0)) {
...
==================
*/
void S_Base_AddLoopingSound(int entityNum, const vec3_t origin,
    ...
    if (s_doppler->integer && VectorLengthSquared(velocity) > 0.0) {
        ...
        if (loopSounds[entityNum].dopplerScale <= 1.0) {
            loopSounds[entityNum].doppler =
                qfalse; // don't bother doing the math
...
==================
*/
void S_Base_AddRealLoopingSound(int entityNum, const vec3_t origin,
                                const vec3_t velocity, sfxHandle_t sfxHandle) {
    sfx_t *sfx;
...
=================
*/
void S_ByteSwapRawSamples(int samples, int width, int s_channels,
                          const byte *data) {
    int i;
...
============
*/
void S_Base_RawSamples(int stream, int samples, int rate, int width,
                       int s_channels, const byte *data, float volume,
                       int entityNum) {
    int i;
    int src, dst;
    float scale;
    ...
    if (s_muted->integer) {
        ...
        if (entityNum >= 0 && entityNum < MAX_GENTITIES) {
            ...
            S_SpatializeOrigin(loopSounds[entityNum].origin, 256, &leftvol,
                               &rightvol);
        } else {
    ...
    if (s_channels == 2 && width == 2) {
        if (scale == 1.0) { // optimized case
            for (i = 0; i < samples; i++) {
                ...
                rawsamples[dst].right =
                    ((short *)data)[i * 2 + 1] * intVolumeRight;
            }
        } else {
            for (i = 0;; i++) {
                src = i * scale;
                ...
                rawsamples[dst].right =
                    ((short *)data)[src * 2 + 1] * intVolumeRight;
            }
        }
        ...
        for (i = 0;; i++) {
            ...
            rawsamples[dst].right =
                ((char *)data)[src * 2 + 1] * intVolumeRight;
        }
        ...
        for (i = 0;; i++) {
            ...
            rawsamples[dst].right =
                (((byte *)data)[src] - 128) * intVolumeRight;
        }
    }
...
============
*/
void S_Base_Respatialize(int entityNum, const vec3_t head, vec3_t axis[3],
                         int inwater) {
    int i;
    channel_t *ch;
    vec3_t origin;
...
void S_GetSoundtime(void) {
    ...
    if (CL_VideoRecording()) {
        ...
        float frameDuration =
            MAX(dma.speed / fps, 1.0f) + clc.aviSoundFrameRemainder;
...
void S_Update_(void) {
    ...
    endtime =
        (endtime + dma.submission_chunk - 1) & ~(dma.submission_chunk - 1);
    ...
    while (s_rawend[0] < s_soundtime + MAX_RAW_SAMPLES) {
        ...
        if (r > 0) {
            ...
            S_Base_RawSamples(0, fileSamples, s_backgroundStream->info.rate,
                              ...
                              s_musicVolume->value * s_musicFading->value, -1);
        } else {
            // loop
