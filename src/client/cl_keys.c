...
===================
*/
void Field_VariableSizeDraw(field_t *edit, int x, int y, int width, int size,
    ...
    drawLen =
        edit->widthInChars - 1; // - 1 so there is always a space for the cursor
...
void Console_Key(int key) {
    ...
    if (key == K_ENTER || key == K_KP_ENTER) {
        ...
        if (clc.state != CA_ACTIVE && g_consoleField.buffer[0] &&
            g_consoleField.buffer[0] != '\\' &&
            g_consoleField.buffer[0] != '/') {
            char temp[MAX_EDIT_LINE - 1];

            Q_strncpyz(temp, g_consoleField.buffer, sizeof(temp));
            Com_sprintf(g_consoleField.buffer, sizeof(g_consoleField.buffer),
                        "\\%s", temp);
        ...
        if (g_consoleField.buffer[0] == '\\' ||
            ...
            if (!g_consoleField.buffer[0]) {
                return; // empty lines just scroll the console without adding to
                        // history
            } else {
    ...
    if (key == K_MWHEELDOWN) { //----(SA)	added some mousewheel
...
void CL_KeyDownEvent(int key, unsigned time) {
    keys[key].down = qtrue;
    keys[key].repeats++;
    if (keys[key].repeats == 1 && key != K_SCROLLOCK && key != K_KP_NUMLOCK &&
        key != K_CAPSLOCK)
        anykeydown++;
...
void CL_LoadConsoleHistory(void) {
    ...
    if (consoleSaveBufferSize <= MAX_CONSOLE_SAVE_BUFFER &&
        FS_Read(consoleSaveBuffer, consoleSaveBufferSize, f) ==
        ...
        for (i = COMMAND_HISTORY - 1; i >= 0; i--) {
            ...
            text_p++;
            if (numChars >
                (strlen(consoleSaveBuffer) - (text_p - consoleSaveBuffer))) {
...
void CL_SaveConsoleHistory(void) {
        ...
        if (historyEditLines[i].buffer[0]) {
            ...
            if (saveBufferLength + additionalLength < MAX_CONSOLE_SAVE_BUFFER) {
                Q_strcat(consoleSaveBuffer, MAX_CONSOLE_SAVE_BUFFER,
                         va("%d %d %d %s ", historyEditLines[i].cursor,
    ...
    if (FS_Write(consoleSaveBuffer, consoleSaveBufferSize, f) <
        consoleSaveBufferSize)
        Com_Printf("Couldn't write %s.\n", CONSOLE_HISTORY_FILE);
