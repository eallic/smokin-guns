...
#if idppc_altivec
static void S_PaintChannelFrom16_altivec(channel_t *ch, const sfx_t *sc,
                                         int count, int sampleOffset,
                                         int bufferOffset) {
    int data, aoff, boff;
    ...
    if (!ch->doppler || ch->dopplerScale == 1.0f) {
        ...
        while (i < count) {
            ...
            while (i < count &&
                   (((unsigned long)&samp[i] & 0x1f) || ((count - i) < 8) ||
                    ((SND_CHUNK_SIZE - sampleOffset) < 8))) {
            ...
            if (vectorCount) {
                ...
                vector unsigned char samplePermute0 = VECCONST_UINT8(
                    0, 1, 4, 5, 0, 1, 4, 5, 2, 3, 6, 7, 2, 3, 6, 7);
                vector unsigned char samplePermute1 = VECCONST_UINT8(
                    8, 9, 12, 13, 8, 9, 12, 13, 10, 11, 14, 15, 10, 11, 14, 15);
                vector unsigned char loadPermute0, loadPermute1;

                // Rather than permute the vectors after we load them to do the
                // sample replication and rearrangement, we permute the
                // alignment vector so we do everything in one step below and
                ...
                while (vectorCount) {
                    ...
                    merge0 = vec_sra(
                        merge0, volume_shift); /* Shift down to proper range */
...
static void S_PaintChannelFrom16_scalar(channel_t *ch, const sfx_t *sc,
                                        int count, int sampleOffset,
                                        int bufferOffset) {
    int data, aoff, boff;
    int leftvol, rightvol;
...
void S_PaintChannelFromWavelet(channel_t *ch, sfx_t *sc, int count,
                               int sampleOffset, int bufferOffset) {
    int data;
    int leftvol, rightvol;
    int i;
...
void S_PaintChannelFromADPCM(channel_t *ch, sfx_t *sc, int count,
                             int sampleOffset, int bufferOffset) {
    int data;
    int leftvol, rightvol;
    int i;
...
void S_PaintChannelFromMuLaw(channel_t *ch, sfx_t *sc, int count,
                             int sampleOffset, int bufferOffset) {
    int data;
    int leftvol, rightvol;
    int i;
    ...
    if (!ch->doppler) {
        ...
        for (i = 0; i < count; i++) {
            ...
            samples++;
            if (chunk != NULL &&
                samples == (byte *)chunk->sndChunk + (SND_CHUNK_SIZE * 2)) {
                chunk = chunk->next;
    ...
    while (s_paintedtime < endtime) {
        ...
        for (stream = 0; stream < MAX_RAW_STREAMS; stream++) {
            if (s_rawend[stream] >= s_paintedtime) {
                ...
                const int stop =
                    (end < s_rawend[stream]) ? end : s_rawend[stream];
