...
void CL_ConfigstringModified(void) {
    ...
    for (i = 0; i < MAX_CONFIGSTRINGS; i++) {
        ...
        Com_Memcpy(cl.gameState.stringData + cl.gameState.dataCount, dup,
                   len + 1);
...
qboolean CL_GetServerCommand(int serverCommandNumber) {
    ...
    if (serverCommandNumber <=
        clc.serverCommandSequence - MAX_RELIABLE_COMMANDS) {
        // when a demo record was started after the client got a whole bunch of
        // reliable commands then the client never got those first reliable
        // commands
        if (clc.demoplaying)
            return qfalse;
        Com_Error(ERR_DROP,
...
rescan:
    ...
    if (!strcmp(cmd, "bcs0")) {
        Com_sprintf(bigConfigString, BIG_INFO_STRING, "cs %s \"%s", Cmd_Argv(1),
                    Cmd_Argv(2));
        return qfalse;
    }
...
#define min(a, b) (a) < (b) ? a : b
            ...
            if ((Q_stricmp(badWords[i], wordBuf) == 0) &&
                (at == str || (at[-1] == ' ') ||
                 (at > (str + 1) && at[-2] == '^')))
...
intptr_t CL_CgameSystemCalls(intptr_t *args) {
    ...
    case CG_UPDATESCREEN:
        ...
        //		Com_EventLoop();	// FIXME: if a server restarts
        //here, BAD THINGS HAPPEN!
        // We can't call Com_EventLoop here, a restart will crash and this
        // _does_ happen if there is a map change while we are downloading at
        // pk3. ZOID
        SCR_UpdateScreen();
        return 0;
    case CG_CM_LOADMAP:
    ...
    case CG_CM_MARKFRAGMENTS:
        return re.MarkFragments(args[1], VMA(2), VMA(3), args[4], VMA(5),
                                args[6], VMA(7));
    ...
    case CG_R_DRAWSTRETCHPIC:
        re.DrawStretchPic(VMF(1), VMF(2), VMF(3), VMF(4), VMF(5), VMF(6),
                          VMF(7), VMF(8), args[9]);
        return 0;
    ...
    case CG_CIN_PLAYCINEMATIC:
        return CIN_PlayCinematic(VMA(1), args[2], args[3], args[4], args[5],
                                 args[6]);
    ...
    case CG_R_CULL_SPHERE:
        return re.CullPointAndRadius(VMA(1), VMF(2));
...
void CL_InitCGame(void) {
    ...
    if (!badWords) {
        ...
        if (strlen(buf) > 0) {
            ...
            at = buf;
            while ((at = strchr(buf, ','))) {
                if ((l - 1) > capacity) // ( l-1 ) because we want to leave room
                                        // for the last word
                {
...
void CL_AdjustTimeDelta(void) {
    ...
    if (deltaDelta > RESET_TIME) {
        ...
        // the granularity of +1 / -2 is too high for timescale modified
        // frametimes
        if (com_timescale->value == 0 || com_timescale->value == 1) {
            if (cl.extrapolatedSnapshot) {
                ...
                // otherwise, move our sense of time forward to minimize total
                // latency
                cl.serverTimeDelta++;
            }
        }
...
void CL_FirstSnapshot(void) {
    ...
    if (!clc.speexInitialized) {
        ...
        i = 1;
        speex_preprocess_ctl(clc.speexPreprocessor, SPEEX_PREPROCESS_SET_AGC,
                             &i);
...
void CL_SetCGameTime(void) {
    ...
    if (clc.demoplaying && cl_freezeDemo->integer) {
        // cl_freezeDemo is used to lock a demo in place for single frame
        // advances
