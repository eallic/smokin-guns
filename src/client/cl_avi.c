...
===============
*/
static ID_INLINE void SafeFS_Write(const void *buffer, int len,
                                   fileHandle_t f) {
    ...
    START_CHUNK("RIFF");
    {
        WRITE_STRING("AVI ");
        {
            START_CHUNK("LIST");
            {
                ...
                WRITE_4BYTES(0x110);         // dwFlags bits HAS_INDEX and
                                     // IS_INTERLEAVED
...
qboolean CL_OpenAVIForWriting(const char *fileName) {
    ...
    if ((afd.idxF =
             FS_FOpenFileWrite(va("%s" INDEX_FILE_EXTENSION, fileName))) <= 0) {
        FS_FCloseFile(afd.f);
        return qfalse;
    }
...
#define MAX_PACK_LEN 16
    afd.cBuffer = Z_Malloc((afd.width * 3 + MAX_PACK_LEN - 1) * afd.height +
                           MAX_PACK_LEN - 1);
...
===============
*/
qboolean CL_VideoRecording(void) { return afd.fileOpen; }
