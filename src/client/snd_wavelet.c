...
void daub4(float b[], unsigned long n, int isign) {
    ...
    if (isign >= 0) {
        for (i = 1, j = 1; j <= n - 3; j += 2, i++) {
            ...
            wksp[i + nh] =
                C3 * a[j] - C2 * a[j + 1] + C1 * a[j + 2] - C0 * a[j + 3];
        }
        ...
        for (i = 1, j = 3; i < nh; i++) {
            wksp[j++] =
                C2 * a[i] + C1 * a[i + nh] + C0 * a[i + 1] + C3 * a[i + nh1];
            wksp[j++] =
                C3 * a[i] - C0 * a[i + nh] + C1 * a[i + 1] - C2 * a[i + nh1];
        }
    }
    for (i = 1; i <= n; i++) {
        a[i] = wksp[i];
...
void NXPutc(NXStream *stream, char out) { stream[NXStreamCount++] = out; }
