...
typedef struct {
    ...
    qboolean extrapolatedSnapshot; // set if any cgame frame has been forced to
                                   // extrapolate cleared when
                                   // CL_AdjustTimeDelta looks at it
...
typedef struct {
    ...
    char servername[MAX_OSPATH]; // name of server from original connect (used
                                 // by reconnect)
    ...
    qboolean downloadRestart; // if true, we need to do another FS_Restart
                              // because we downloaded a pak
    ...
    unsigned char
        timeDemoDurations[MAX_TIMEDEMO_DURATIONS]; // log of frame durations
...
void SCR_DrawBigString(
    int x, int y, const char *s, float alpha,
    qboolean noColorEscape); // draws a string with embedded color control
                             // characters with fade
void SCR_DrawBigStringColor(
    int x, int y, const char *s, vec4_t color,
    qboolean noColorEscape); // ignores embedded color control characters
