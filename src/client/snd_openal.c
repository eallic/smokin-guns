...
typedef struct alSfx_s {
    ...
    int masterLoopSrc; // All other sources looping this buffer are synced to
                       // this master src
} alSfx_t;
...
static sfxHandle_t S_AL_BufferFind(const char *filename) {
    ...
    if (filename[0] == '*') {
        Com_Printf(S_COLOR_YELLOW
                   "WARNING: Tried to load player sound directly: %s\n",
                   filename);
        return 0;
    }
...
=================
*/
static qboolean S_AL_GenBuffers(ALsizei numBuffers, ALuint *buffers,
                                const char *name) {
    ALenum error;
...
static void S_AL_BufferLoad(sfxHandle_t sfx, qboolean cache) {
    ...
    if (info.size == 0) {
        ...
        qalBufferData(curSfx->buffer, AL_FORMAT_MONO16, (void *)dummyData, 2,
                      22050);
    } else
...
=================
*/
static ALuint S_AL_BufferGet(sfxHandle_t sfx) { return knownSfx[sfx].buffer; }
...
=================
S_AL_ScaleGain
Adapt the gain if necessary to get a quicker fadeout when the source is too far
away.
=================
*/
...
static qboolean S_AL_HearingThroughEntity(int entityNum) {
    ...
    if (lastListenerNumber == entityNum) {
        ...
        // whether or not the player is rendering in third person or not. We
        // can't ask the renderer because the renderer has no notion of entities
        // and we can't ask cgame since that would involve changing the API and
        // hence mod compatibility. I don't think there is any way around this,
        // but I'll leave the FIXME just in case anyone has a bright idea.
        distanceSq =
            DistanceSquared(entityList[entityNum].origin, lastListenerOrigin);
...
=================
*/
static void S_AL_SrcSetup(srcHandle_t src, sfxHandle_t sfx,
                          alSrcPriority_t priority, int entity, int channel,
                          qboolean local) {
    src_t *curSource;
    ...
    qalSourcef(curSource->alSource, AL_REFERENCE_DISTANCE,
...
=================
S_AL_NewLoopMaster
Remove given source as loop master if it is the master and hand off master
status to another source in this case.
=================
*/
    ...
    if (curSfx->loopCnt) {
        if (rmSource->priority == SRCPRI_ENTITY) {
            if (!iskilled && rmSource->isPlaying) {
                ...
                // It makes more sense to have sounds for weapons/projectiles
                // unsynced
            ...
            // Only if rmSource was the master and if there are still playing
            // loops for this sound will we need to find a new master.
            ...
            if (iskilled || curSfx->loopActiveCnt) {
                for (index = 0; index < srcCount; index++) {
                    ...
                    if (curSource->sfx == rmSource->sfx &&
                        curSource != rmSource && curSource->isActive &&
                        curSource->isLooping &&
            ...
            if (!curSfx->loopActiveCnt) {
                ...
                if (rmSource->isPlaying) {
                    // this was the last not stopped source, save last sample
                    // position + time
                    S_AL_SaveLoopPos(curSource, rmSource->alSource);
                } else {
                    // second case: all loops using this sound have stopped due
                    // to listener being of of range, and now the inactive
                    // master gets deleted. Just move over the soundpos settings
                    // to the new master.
...
=================
*/
static srcHandle_t S_AL_SrcAlloc(alSrcPriority_t priority, int entnum,
                                 int channel) {
    int i;
    int empty = -1;
    int weakest = -1;
    ...
    for (i = 0; i < srcCount; i++) {
        ...
        if (curSource->isPlaying) {
            if (weakest_isplaying && curSource->priority < priority &&
                ...
                // If it has lower priority, is fainter or older, flag it as
                // weak the last two values are only compared if it's not a
                // looping sound, because we want to prevent two loops (loops
                // are added EVERY frame) fighting for a slot
            ...
            if (weakest < 0 ||
                ...
                // also try to always keep at least one loop master for every
                // loop sound
                weakest_pri = curSource->priority;
                weakest_time = curSource->lastUsedTime;
        ...
        // implemented in snd_dma.c, so while the following is strictly correct,
        // it causes incorrect behaviour versus defacto baseq3
#if 0
...
=================
*/
static void S_AL_SrcLock(srcHandle_t src) { srcList[src].isLocked = qtrue; }
...
=================
*/
static void S_AL_SrcUnlock(srcHandle_t src) { srcList[src].isLocked = qfalse; }
...
=================
*/
static void S_AL_StartSound(vec3_t origin, int entnum, int entchannel,
                            sfxHandle_t sfx) {
    vec3_t sorigin;
    srcHandle_t src;
    src_t *curSource;
    ...
    if ((srcActiveCnt > 5 * srcCount / 3) &&
        (DistanceSquared(sorigin, lastListenerOrigin) >=
         (s_alMaxDistance->value + s_alGraceDistance->value) *
             (s_alMaxDistance->value + s_alGraceDistance->value))) {
        // We're getting tight on sources and source is not within hearing
        // distance so don't add it
        return;
    }
...
static void S_AL_SrcUpdate(void) {
    ...
    for (i = 0; i < srcCount; i++) {
        ...
        if ((s_alRolloff->modified) && (!curSource->local))
            qalSourcef(curSource->alSource, AL_ROLLOFF_FACTOR,
                       s_alRolloff->value);
        ...
        if (curSource->isLooping) {
            ...
            // If a looping effect hasn't been touched this frame, pause or kill
            // it
            if (sent->loopAddedThisFrame) {
                ...
                if (!curSource->scaleGain) {
                    if (curSource->isPlaying) {
                        // Sound is mute, stop playback until we are in range
                        // again
                        S_AL_NewLoopMaster(curSource, qfalse);
                        qalSourceStop(curSource->alSource);
                        curSource->isPlaying = qfalse;
                    } else if (!curSfx->loopActiveCnt &&
                               curSfx->masterLoopSrc < 0)
                ...
                if (!curSource->isPlaying) {
                    ...
                    if (curSource->priority == SRCPRI_AMBIENT) {
                        // If there are other ambient looping sources with the
                        // same sound, make sure the sound of these sources are
                        // in sync.

                        if (curSfx->loopActiveCnt) {
                            int offset, error;

                            // we already have a master loop playing, get buffer
                            // position.
                            S_AL_ClearError(qfalse);
                            qalGetSourcei(
                                srcList[curSfx->masterLoopSrc].alSource,
                                AL_SAMPLE_OFFSET, &offset);
                            if ((error = qalGetError()) != AL_NO_ERROR) {
                                if (error != AL_INVALID_ENUM) {
                                    Com_Printf(S_COLOR_YELLOW
                                               "WARNING: Cannot get sample "
                                               "offset from source %d: "
                                               "%s\n",
                                               i, S_AL_ErrorMsg(error));
                                }
                            } else
                                qalSourcei(curSource->alSource,
                                           AL_SAMPLE_OFFSET, offset);
                        } else if (curSfx->loopCnt &&
                                   curSfx->masterLoopSrc >= 0) {
                            float secofs;

                            src_t *master = &srcList[curSfx->masterLoopSrc];
                            // This loop sound used to be played, but all
                            // sources are stopped. Use last sample
                            // position/time to calculate offset so the player
                            // thinks the sources continued playing while they
                            // were inaudible.
                                ...
                                secofs = master->lastTimePos +
                                         (Sys_Milliseconds() -
                                          master->lastSampleTime) /
                                             1000.0f;
                                secofs =
                                    fmodf(secofs, (float)curSfx->info.samples /
                                                      curSfx->info.rate);

                                qalSourcef(curSource->alSource, AL_SEC_OFFSET,
                                           secofs);
                            }
                        ...
                        // For unsynced loops (SRCPRI_ENTITY) just carry on
                        // playing as if the sound was never stopped

                        secofs =
                            curSource->lastTimePos +
                            (Sys_Milliseconds() - curSource->lastSampleTime) /
                                1000.0f;
                        secofs = fmodf(secofs, (float)curSfx->info.samples /
                                                   curSfx->info.rate);
                ...
                // Update locality
                if (curSource->local) {
                    qalSourcei(curSource->alSource, AL_SOURCE_RELATIVE,
                               AL_TRUE);
                    qalSourcef(curSource->alSource, AL_ROLLOFF_FACTOR, 0.0f);
                } else {
                    qalSourcei(curSource->alSource, AL_SOURCE_RELATIVE,
        ...
        if (curSource->isTracking && !state) {
            qalSourcefv(curSource->alSource, AL_POSITION,
...
=================
*/
static ALuint S_AL_SrcGet(srcHandle_t src) { return srcList[src].alSource; }
...
static void S_AL_AllocateStreamChannel(int stream, int entityNum) {
    ...
    if (entityNum >= 0) {
        ...
        // Lock the streamSource so nobody else can use it, and get the raw
        // streamSource
        S_AL_SrcLock(cursrc);

        // make sure that after unmuting the S_AL_Gain in S_Update() does not
        // turn volume up prematurely for this source
...
=================
*/
static void S_AL_RawSamples(int stream, int samples, int rate, int width,
    ...
    if (numBuffers == MAX_STREAM_BUFFERS) {
        Com_DPrintf(S_COLOR_RED "WARNING: Steam dropping raw samples, reached "
                                "MAX_STREAM_BUFFERS\n");
        return;
    }
    ...
    if (numBuffers == streamNumBuffers[stream]) {
        ...
        for (i = 0; i < streamNumBuffers[stream]; ++i)
            streamBuffers[stream][i] = oldBuffers[(streamBufIndex[stream] + i) %
    ...
    streamBufIndex[stream] =
        (streamBufIndex[stream] + 1) % streamNumBuffers[stream];

    // Fill buffer
    qalBufferData(buffer, format, (ALvoid *)data, (samples * width * channels),
                  rate);
    ...
    if (entityNum < 0) {
        // Volume
        S_AL_Gain(streamSources[stream],
...
static void S_AL_MusicSourceGet(void) {
    ...
    // Lock the musicSource so nobody else can use it, and get the raw
    // musicSource
...
static void S_AL_StartBackgroundTrack(const char *intro, const char *loop) {
    ...
    S_AL_Gain(musicSource,
              s_alGain->value * s_musicVolume->value * s_musicFading->value);
...
=================
*/
static void S_AL_DisableSounds(void) { S_AL_StopAllSounds(); }
...
static int S_AL_AvailableCaptureSamples(void) {
    ...
    if (alCaptureDevice != NULL) {
        ALint samples = 0;
        qalcGetIntegerv(alCaptureDevice, ALC_CAPTURE_SAMPLES, sizeof(samples),
                        &samples);
        retval = (int)samples;
    }
    return retval;
...
void S_AL_MasterGain(float gain) { qalListenerf(AL_GAIN, gain); }
#endif
...
qboolean S_AL_Init(soundInterface_t *si) {
    ...
    s_alDriver =
        Cvar_Get("s_alDriver", ALDRIVER_DEFAULT, CVAR_ARCHIVE | CVAR_LATCH);

    s_alInputDevice =
        Cvar_Get("s_alInputDevice", "", CVAR_ARCHIVE | CVAR_LATCH);
...
#ifdef _WIN32
            defaultdevice =
                qalcGetString(NULL, ALC_DEFAULT_ALL_DEVICES_SPECIFIER);
#endif
        } else {
...
#ifdef _WIN32
        // check whether the default device is generic hardware. If it is,
        // change to Generic Software as that one works more reliably with
        // various sound systems. If it's not, use OpenAL's default selection as
        // we don't want to ignore native hardware acceleration.
        if (!device && defaultdevice &&
            !strcmp(defaultdevice, "Generic Hardware"))
    ...
    if (!s_alCapture->integer) {
        Com_Printf("OpenAL capture support disabled by user ('+set s_alCapture "
                   "1' to enable)\n");
    }
#if USE_MUMBLE
...
#endif
        {
            ...
            s_alAvailableInputDevices =
                Cvar_Get("s_alAvailableInputDevices", inputdevicenames,
                         CVAR_ROM | CVAR_NORESTART);
            ...
            alCaptureDevice = qalcCaptureOpenDevice(inputdevice, 8000,
                                                    AL_FORMAT_MONO16, 4096);
            if (!alCaptureDevice && inputdevice) {
                Com_Printf("Failed to open OpenAL Input device '%s', trying "
                           "default.\n",
                           inputdevice);
                alCaptureDevice =
                    qalcCaptureOpenDevice(NULL, 8000, AL_FORMAT_MONO16, 4096);
            }
