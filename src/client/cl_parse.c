...
char *svc_strings[256] = {
    ...
    "svc_nop",      "svc_gamestate",     "svc_configstring",
    "svc_baseline", "svc_serverCommand", "svc_download",
    "svc_snapshot", "svc_EOF",           "svc_voip",
};
...
void CL_ParsePacketEntities(msg_t *msg, clSnapshot_t *oldframe,
    ...
    if (!oldframe) {
        ...
        if (oldindex >= oldframe->numEntities) {
            ...
            oldstate =
                &cl.parseEntities[(oldframe->parseEntitiesNum + oldindex) &
    ...
    while (1) {
        ...
        if (oldnum > newnum) {
            ...
            CL_DeltaEntity(msg, newframe, newnum, &cl.entityBaselines[newnum],
                           qfalse);
            continue;
        }
    }
    ...
    if (newSnap.deltaNum <= 0) {
        ...
        if (!old->valid) {
            ...
            Com_Printf("Delta frame too old.\n");
                   ...
                   MAX_PARSE_ENTITIES - MAX_SNAPSHOT_ENTITIES) {
...
void CL_SystemInfoChanged(void) {
    ...
    // in some cases, outdated cp commands might get sent with this news
    // serverId
...
static void CL_ParseServerInfo(void) {
    ...
    clc.sv_allowDownload =
        atoi(Info_ValueForKey(serverInfo, "sv_allowDownload"));
    Q_strncpyz(clc.sv_dlURL, Info_ValueForKey(serverInfo, "sv_dlURL"),
               sizeof(clc.sv_dlURL));
}
...
void CL_ParseGamestate(msg_t *msg) {
    ...
    while (1) {
        ...
        if (cmd == svc_configstring) {
            ...
            Com_Memcpy(cl.gameState.stringData + cl.gameState.dataCount, s,
                       len + 1);
    ...
    // stop recording now so the demo won't have an unnecessary level load at
    // the end.
...
void CL_ParseDownload(msg_t *msg) {
    ...
    if (!size) { // A zero length block means EOF
        ...
        // send intentions now
        // We need this because without it, we would hold the last nextdl and
        // then start loading right away.  If we take a while to load, the
        // server is happily trying to send us that last block over and over.
...
static void CL_PlayVoip(int sender, int samplecnt, const byte *data,
                        int flags) {
    if (flags & VOIP_DIRECT) {
        S_RawSamples(sender + 1, samplecnt, clc.speexSampleRate, 2, 1,
                     data, clc.voipGain[sender] + cl_voipDefaultGain->value,
                     -1);
    }

    if (flags & VOIP_SPATIAL) {
        S_RawSamples(sender + MAX_CLIENTS + 1, samplecnt, clc.speexSampleRate,
                     2, 1, data, 1.0f, sender);
    }
}
...
static void CL_ParseVoip(msg_t *msg) {
    ...
    if (generation != clc.voipIncomingGeneration[sender]) {
        ...
        Com_DPrintf("VoIP: misordered sequence! %d < %d!\n", sequence,
                    clc.voipIncomingSequence[sender]);
        // reset the bits just in case.
        speex_bits_reset(&clc.speexDecoderBits[sender]);
        seqdiff = 0;
    } else if (seqdiff * clc.speexFrameSize * 2 >=
               sizeof(decoded)) { // dropped more than we can handle?
        // just start over.
...
void CL_ParseServerMessage(msg_t *msg) {
    ...
    if (clc.reliableAcknowledge <
        clc.reliableSequence - MAX_RELIABLE_COMMANDS) {
