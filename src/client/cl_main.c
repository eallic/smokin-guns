...
static void CL_UpdateMumble(void) {
    ...
    if (cl_useMumble->integer > 1) {
        fprintf(stderr, "%f %f %f, %f %f %f, %f %f %f\n", pos[0], pos[1],
                pos[2], forward[0], forward[1], forward[2], up[0], up[1],
                up[2]);
    }
    ...
    if ((*idstr >= '0') && (*idstr <= '9')) {
        ...
        if ((id >= 0) && (id < MAX_CLIENTS)) {
            ...
            CL_AddReliableCommand(
                va("voip %s %d", ignore ? "ignore" : "unignore", id), qfalse);
...
static void CL_CaptureVoip(void) {
    ...
    if (cl_voip->modified || cl_rate->modified) {
        if ((cl_voip->integer) && (cl_rate->integer < 25000)) {
            ...
            Com_Printf("Set 'Data Rate' to 'LAN/Cable/xDSL' in "
    ...
    if ((cl_voipSend->integer) ||
        (finalFrame)) { // user wants to capture audio?
        ...
        if (samples >= (clc.speexFrameSize * mult)) {
            ...
            while (samples > 0) {
                ...
                bytes =
                    speex_bits_write(&clc.speexEncoderBits,
...
void CL_Record_f(void) {
    ...
    if (NET_IsLocalAddress(clc.serverAddress) &&
        ...
        Com_Printf(S_COLOR_YELLOW
                   "WARNING: You should set 'g_synchronousClients 1' for "
                   "smoother demo recording\n");
    }
    ...
    if (Cmd_Argc() == 2) {
        ...
        if (clc.compat)
            Com_sprintf(name, sizeof(name), "demos/%s.%s%d", demoName, DEMOEXT,
...
#ifdef LEGACY_PROTOCOL
            if (clc.compat)
                Com_sprintf(name, sizeof(name), "demos/%s.%s%d", demoName,
                            DEMOEXT, com_legacyprotocol->integer);
            else
#endif
                Com_sprintf(name, sizeof(name), "demos/%s.%s%d", demoName,
                            DEMOEXT, com_protocol->integer);
    ...
    if (Cvar_VariableValue("ui_recordSPDemo")) {
        ...
        // Tequila: Handle case we don't want to see the demo filename over the
        // screen when cl_autoRecordDemo is enabled
    ...
    if (cl_timedemo && cl_timedemo->integer) {
        ...
        if (time > 0) {
            ...
            Com_sprintf(
                ...
                clc.timeDemoFrames * 1000.0 / time, clc.timeDemoMinDuration,
                time / (float)clc.timeDemoFrames, clc.timeDemoMaxDuration,
                CL_DemoFrameDurationSDev());
            Com_Printf("%s", buffer);
...
static int CL_WalkDemoExt(char *arg, char *name, int *demofile) {
    ...
    *demofile = 0;
    ...
    if (com_legacyprotocol->integer > 0) {
        Com_sprintf(name, MAX_OSPATH, "demos/%s.%s%d", arg, DEMOEXT,
                    com_legacyprotocol->integer);
...
only ways a client gets into a game Also called by Com_Error
=================
*/
...
static void CL_UpdateGUID(const char *prefix, int prefix_len) {
    ...
    if (len != QKEY_SIZE)
        ...
        Cvar_Set("cl_guid",
                 Com_MD5File(QKEY_FILE, QKEY_SIZE, prefix, prefix_len));
}
...
void CL_RequestMotd(void) {
    ...
    Com_sprintf(cls.updateChallenge, sizeof(cls.updateChallenge), "%i",
    ...
    if (!comment->integer) {
        ...
        for (i = 0; i < *sdk_shared_size && sdk_shared_textarray[i]; i++) {
            NET_OutOfBandPrint(NS_CLIENT, cls.updateServer,
                               "comment \"\\index\\%i\\diff\\%s\"\n", i,
                               sdk_shared_textarray[i]);
        }
        NET_OutOfBandPrint(NS_CLIENT, cls.updateServer,
                           "comment \"\\total\\%i\"\n", i);
    }
...
#OLD
#OLD A client will be accepted if the cdkey is valid and it has not been used by
any other IP #OLD address in the last 15 minutes.
...
A client will be accepted if a valid cdkey was sent by that ip (only) in the
last 15 minutes. If no response is received from the authorize server after two
tries, the client will be let in anyway.
===================
*/
void CL_RequestAuthorization(void) {
    ...
    NET_OutOfBandPrint(NS_CLIENT, cls.authorizeServer,
                       "getClientAuthorize %i \"%s\"", fs->integer,
...
void CL_Connect_f(void) {
    ...
    if (NET_IsLocalAddress(clc.serverAddress))
        ...
        // Set a client challenge number that ideally is mirrored back by the
        // server.
...
=================
*/
void CL_ResetPureClientAtServer(void) { CL_AddReliableCommand("vdr", qfalse); }
...
void CL_Vid_Restart_f(void) {
    ...
    if (!FS_ConditionalRestart(clc.checksumFeed, qtrue)) {
        ...
        // reinitialize the filesystem if the game directory or checksum has
        // changed
...
void CL_DownloadsComplete(void) {
    ...
    if (clc.downloadRestart) {
        ...
        FS_Restart(clc.checksumFeed); // We possibly downloaded a pak, restart
                                      // the file system to load it
...
void CL_BeginDownload(const char *localName, const char *remoteName) {
    ...
    Com_sprintf(clc.downloadTempName, sizeof(clc.downloadTempName), "%s.tmp",
                localName);
...
void CL_NextDownload(void) {
    ...
    if (*clc.downloadName) {
        char *zippath = FS_BuildOSPath(Cvar_VariableString("fs_homepath"),
                                       clc.downloadName, "");
        zippath[strlen(zippath) - 1] = '\0';

        if (!FS_CompareZipChecksum(zippath))
            Com_Error(ERR_DROP, "Incorrect checksum for file: %s",
                      clc.downloadName);
    }
        ...
        if (!(cl_allowDownload->integer & DLF_NO_REDIRECT)) {
            if (clc.sv_allowDownload & DLF_NO_REDIRECT) {
                ...
                CL_cURL_BeginDownload(localName,
                                      va("%s/%s", clc.sv_dlURL, remoteName));
                useCURL = qtrue;
            }
...
void CL_InitDownloads(void) {
    ...
    if (!(cl_allowDownload->integer & DLF_ENABLE)) {
        ...
        // but it's possible that some referenced files on the server are
        // missing
        if (FS_ComparePaks(missingfiles, sizeof(missingfiles), qfalse)) {
            // NOTE TTimo I would rather have that printed as a modal message
            // box
            //   but at this point while joining the game we don't know wether
            //   we will successfully join or not
            Com_Printf("\nWARNING: You are missing some files referenced by "
                       "the server:\n%s"
                       "You might not be able to join the game\n"
                       "Go to the setting menu to turn on autodownload, or get "
                       "the file elsewhere\n\n",
                       missingfiles);
        }
...
void CL_CheckForResend(void) {
    ...
    switch (clc.state) {
    case CA_CONNECTING:
        // requesting a challenge .. IPv6 users always get in as authorize
        // server supports no ipv6.
        if (clc.serverAddress.type == NA_IP &&
            !Sys_IsLANAddress(clc.serverAddress)) {
            CL_RequestAuthorization();
        }

        // The challenge request shall be followed by a client challenge so no
        // malicious server can hijack this connection. Add the gamename so the
        // server knows we're running the correct game or can reject the client
    ...
    case CA_CHALLENGING:
        ...
        NET_OutOfBandData(NS_CLIENT, clc.serverAddress, (byte *)&data[0],
                          i + 10);
...
void CL_ServersResponsePacket(const netadr_t *from, msg_t *msg,
    ...
    for (i = 0; i < numservers && count < MAX_GLOBAL_SERVERS; i++) {
        ...
        // We just avoid to add a server if it is still in the global servers
        // list.
        for (j = 0; j < count; j++) {
    ...
    if (!Q_stricmp(c, "challengeResponse")) {
        ...
        if (clc.compat) {
            if (!NET_CompareAdr(from, clc.serverAddress)) {
                // This challenge response is not coming from the expected
                // address. Check whether we have a matching client challenge to
                // prevent connection hi-jacking.

                if (!*c || challenge != clc.challenge) {
                    Com_DPrintf("Challenge response received from unexpected "
                                "source. Ignored.\n");
                    return;
                }
            }
...
#else
        Netchan_Setup(NS_CLIENT, &clc.netchan, from,
                      Cvar_VariableValue("net_qport"), clc.challenge, qfalse);
#endif
...
void CL_CheckUserinfo(void) {
    ...
    if (cvar_modifiedFlags & CVAR_USERINFO) {
        cvar_modifiedFlags &= ~CVAR_USERINFO;
        CL_AddReliableCommand(
            va("userinfo \"%s\"", Cvar_InfoString(CVAR_USERINFO)), qfalse);
    }
}
...
void CL_Frame(int msec) {
    ...
    if (cls.cddialog) {
        ...
        VM_Call(uivm, UI_SET_ACTIVE_MENU, UIMENU_NEED_CD);
               ...
               !(Key_GetCatcher() & KEYCATCH_UI) && !com_sv_running->integer &&
               uivm) {
    ...
    if (CL_VideoRecording() && cl_aviFrameRate->integer && msec) {
        ...
        if (clc.state == CA_ACTIVE || cl_forceavidemo->integer) {
            float fps =
                MIN(cl_aviFrameRate->value * com_timescale->value, 1000.0f);
            float frameDuration =
                MAX(1000.0f / fps, 1.0f) + clc.aviVideoFrameRemainder;
    ...
    if (cl_autoRecordDemo->integer) {
        if (clc.state == CA_ACTIVE && !clc.demorecording && !clc.demoplaying) {
            ...
            Com_RealTime(&now);
            nowString = va("%04d%02d%02d%02d%02d%02d", 1900 + now.tm_year,
            ...
            Cbuf_ExecuteText(EXEC_NOW, va("record %s-%s-%s", nowString,
                                          serverName, mapName));
...
============
*/
void *CL_RefMalloc(int size) { return Z_TagMalloc(size, TAG_RENDERER); }
...
void CL_InitRef(void) {
    ...
    Com_sprintf(dllName, sizeof(dllName), "renderer_%s_" ARCH_STRING DLL_EXT,
                cl_renderer->string);
...
void CL_Video_f(void) {
    ...
    if (Cmd_Argc() == 2) {
        ...
        for (i = 0; i <= 9999; i++) {
            ...
            Com_sprintf(filename, MAX_OSPATH, "videos/video%d%d%d%d.avi", a, b,
                        c, d);
...
void CL_Init(void) {
    ...
    cl_consoleColor[3] =
        Cvar_Get("cl_consoleTransparency", "0.8", CVAR_ARCHIVE);
    ...
    Cvar_Get("g_blueteam", DEFAULT_BLUETEAM_NAME,
             CVAR_SERVERINFO | CVAR_ARCHIVE);
    Cvar_Get("handicap", "100", CVAR_USERINFO | CVAR_ARCHIVE);
    Cvar_Get("teamtask", "0", CVAR_USERINFO);
    Cvar_Get("sex", "male", CVAR_USERINFO | CVAR_ARCHIVE);
    Cvar_Get("cl_version", PRODUCT_VERSION " " PRODUCT_RELEASE,
    ...
    // Make sure cg_stereoSeparation is zero as that variable is deprecated and
...
static void CL_SetServerInfoByAddress(netadr_t from, const char *info,
                                      int ping) {
    int i;
...
void CL_ServerInfoPacket(netadr_t from, msg_t *msg) {
...
#endif
        gameMismatch =
            !*gamename || strcmp(gamename, com_gamename->string) != 0;
    ...
    for (i = 0; i < MAX_PINGREQUESTS; i++) {
        if (cl_pinglist[i].adr.port && !cl_pinglist[i].time &&
            ...
            // NOTE: make sure these types are in sync with the netnames strings
            // in the UI
            switch (from.type) {
            case NA_BROADCAST:
            case NA_IP:
...
void CL_ServerStatusResponse(netadr_t from, msg_t *msg) {
    ...
    for (i = 0, s = MSG_ReadStringLine(msg); *s;
        ...
        len = strlen(serverStatus->string);
        Com_sprintf(&serverStatus->string[len],
...
void CL_GlobalServers_f(void) {
    ...
    if (!i) {
        Com_Printf("CL_GlobalServers_f: Error: could not resolve address of "
                   "master %s\n",
                   masteraddress);
        return;
    } else if (i == 2)
...
Joe Kari: attempt to have a command that get a list form *ALL* master server
available.
==================
*/
        ...
        if (to.type == NA_IP6 || to.type == NA_MULTICAST6) {
            ...
            if (v4enabled) {
                Com_sprintf(command, sizeof(command),
                            "getserversExt %s %s ipv6", com_gamename->string,
                            Cmd_Argv(1));
            } else {
                Com_sprintf(command, sizeof(command), "getserversExt %s %s",
            ...
            //       request IPv6 servers only by appending " ipv6" to the
            //       command
        } else
            ...
            if (server[i].visible) {
                if (server[i].ping == -1) {
                    ...
                    if (j >= MAX_PINGREQUESTS) {
                        ...
                        memcpy(&cl_pinglist[j].adr, &server[i].adr,
                               sizeof(netadr_t));
                ...
                // the ping packet got lost
                    ...
                    if (source == AS_GLOBAL) {
                        ...
                        if (cls.numGlobalServerAddresses > 0) {
                            // overwrite this server with one from the
                            // additional global servers
                            cls.numGlobalServerAddresses--;
                            CL_InitServerInfo(
                                &server[i],
                                &cls.globalServerAddresses
                                     [cls.numGlobalServerAddresses]);
