...
char *cg_customSoundNames[MAX_CUSTOM_SOUNDS] = {
    "*jump1.wav",    "*pain25_1.wav",  "*pain50_1.wav",
    "*pain75_1.wav", "*pain100_1.wav", "*falling1.wav",
    "*gasp.wav",     "*drown.wav",     "*fall1.wav",
};
...
qboolean CG_ParseMusicFile(void) {
    ...
    if (len >= MAX_TEXINFOFILE) {
        trap_Print(va(
            S_COLOR_RED
            "music file too large: music/music.cfg is %i, max allowed is %i\n",
            len, MAX_TEXINFOFILE));
        trap_FS_FCloseFile(f);
        return qfalse;
    }
    ...
    if (trackInfoNum > MAX_TRACKS) {
        trap_Print(va(S_COLOR_RED
                      "music-files: %i tracks found; max allowed %i tracks\n",
    ...
    for (i = 0; i < trackInfoNum; i++) {
        ...
        // trackInfo[i].path, trackInfo[i].length  ) );
    }
    return qtrue;
}
...
qboolean CG_ParseTexFile(const char *filename) {
    ...
    if (len >= MAX_TEXINFOFILE) {
        trap_Print(va(S_COLOR_RED
                      "tex file too large: %s is %i, max allowed is %i\n",
    ...
    if(texInfoNum > MAX_BRUSHSIDES){
            trap_Print( va( S_COLOR_RED "Tex-file: %i brushsides max allowed %i
    brushsides\n", shaderInfoNum, MAX_BRUSHSIDES ) ); return qfalse;
    }
    ...
    for(i=0;i<texInfoNum;i++) {
            ...
            while(Q_stricmp("}", token)){
                    for(j=0;j<3;j++){
                            texInfo[i].points[texInfo[i].numpoints][j] =
    atof(token); token = COM_ParseExt(p,qtrue); trap_Print( va( S_COLOR_RED "%f
    ", texInfo[i].points[texInfo[i].numpoints][j] ) );
                    }
                    texInfo[i].numpoints++;
                    trap_Print( "\n" );
    ...
    if (shaderInfoNum > MAX_BRUSHSIDES) {
        trap_Print(va(S_COLOR_RED
                      "Tex-file: %ishaders max allowed %i shaders\n",
...
static qboolean CG_ParseAnimationFile(const char *filename, clientInfo_t *ci) {
    ...
    memcpy(&animations[LEGS_RUN_SLIDE], &animations[LEGS_RUN],
           sizeof(animation_t));
...
static qboolean CG_RegisterClientSkin(clientInfo_t *ci, const char *modelName,
    ...
    Com_sprintf(filename, sizeof(filename),
                "models/wq3_players/%s/holster_%s.skin", modelName, skinName);
    ci->holsterSkin = trap_R_RegisterSkin(filename);
    /*
    Com_sprintf( filename, sizeof( filename ),
    "models/players/%s/%slower_%s.skin", modelName, teamName, skinName );
    ci->legsSkin = trap_R_RegisterSkin( filename );
    if (!ci->legsSkin) {
            Com_sprintf( filename, sizeof( filename ),
    "models/players/characters/%s/%slower_%s.skin", modelName, teamName,
    skinName ); ci->legsSkin = trap_R_RegisterSkin( filename ); if
    (!ci->legsSkin) { Com_Printf( "Leg skin load failure: %s\n", filename );
            }
    }


    Com_sprintf( filename, sizeof( filename ),
    "models/players/%s/%supper_%s.skin", modelName, teamName, skinName );
    ci->torsoSkin = trap_R_RegisterSkin( filename );
    if (!ci->torsoSkin) {
            Com_sprintf( filename, sizeof( filename ),
    "models/players/characters/%s/%supper_%s.skin", modelName, teamName,
    skinName ); ci->torsoSkin = trap_R_RegisterSkin( filename ); if
    (!ci->torsoSkin) { Com_Printf( "Torso skin load failure: %s\n", filename );
            }
    }
    */
    Com_sprintf(filename, sizeof(filename),
                "models/wq3_players/%s/lower_%s.skin", modelName, skinName);
    ci->legsSkin = trap_R_RegisterSkin(filename);
    ...
    Com_sprintf(filename, sizeof(filename),
                "models/wq3_players/%s/upper_%s.skin", modelName, skinName);
    ci->torsoSkin = trap_R_RegisterSkin(filename);
    ...
    // head
    Com_sprintf(filename, sizeof(filename),
                "models/wq3_players/%s/head_%s_damage_1.skin", modelName,
                skinName);
    ...
    Com_sprintf(filename, sizeof(filename),
                "models/wq3_players/%s/head_%s_damage_2.skin", modelName,
                skinName);
    ...
    // upper
    Com_sprintf(filename, sizeof(filename),
                "models/wq3_players/%s/upper_%s_damage_1.skin", modelName,
                skinName);
    ...
    Com_sprintf(filename, sizeof(filename),
                "models/wq3_players/%s/upper_%s_damage_2.skin", modelName,
                skinName);
    ...
    // lower
    Com_sprintf(filename, sizeof(filename),
                "models/wq3_players/%s/lower_%s_damage_1.skin", modelName,
                skinName);
    ...
    Com_sprintf(filename, sizeof(filename),
                "models/wq3_players/%s/lower_%s_damage_2.skin", modelName,
                skinName);
    ...
    /*if( headModelName[0] == '*' ) {
            Com_sprintf( filename, sizeof( filename ),
    "models/wq3_players/heads/%s/head_%s.skin", &headModelName[1], headSkinName
    ); } else {*/
    Com_sprintf(filename, sizeof(filename),
                "models/wq3_players/%s/head_%s.skin", modelName,
                skinName /*headModelName,headSkinName*/);
    //}
    ...
    /*if ( !ci->headSkin && headModelName[0] != '*' ) {
            Com_sprintf( filename, sizeof( filename ),
    "models/wq3_players/heads/%s/head_%s.skin", headModelName, headSkinName );
            ci->headSkin = trap_R_RegisterSkin( filename );
    }*/
    ...
    if (!ci->legsSkin || !ci->torsoSkin || !ci->headSkin ||
        !ci->legsSkin_damage_1 || !ci->legsSkin_damage_1 ||
        !ci->headSkin_damage_1 || !ci->headSkin_damage_2 ||
...
static qboolean CG_RegisterClientModelname(clientInfo_t *ci,
                                           ...
                                           const char *skinName) {
    char filename[MAX_QPATH];
    Com_sprintf(filename, sizeof(filename), "models/wq3_players/%s/lower.md3",
                modelName);
    ...
    Com_sprintf(filename, sizeof(filename), "models/wq3_players/%s/upper.md3",
                modelName);
    ...
    Com_sprintf(filename, sizeof(filename), "models/wq3_players/%s/head.md3",
                modelName);
    ...
    // load the animations
    Com_sprintf(filename, sizeof(filename),
                "models/wq3_players/%s/animation.cfg", modelName);
    ...
    Com_sprintf(filename, sizeof(filename), "models/wq3_players/%s/icon_%s.tga",
                modelName, skinName);
    ...
    if (!CG_RegisterClientModelname(ci, ci->modelName, ci->skinName)) {
        if (cg_buildScript.integer) {
            CG_Error("CG_RegisterClientModelname( %s, %s, %s, %s %s ) failed",
                     ci->modelName, ci->skinName, ci->headModelName,
                     ci->headSkinName, teamname);
        }
    ...
    for (i = 0; i < MAX_CUSTOM_SOUNDS; i++) {
        ...
        if (!ci->sounds[i]) {
            ci->sounds[i] = trap_S_RegisterSound(
                va("sound/player/%s/%s", gender, s + 1), qfalse);
        }
    }
...
void CG_NewClientInfo(int clientNum) {
    ...
    if (cg_forceModel.integer) {
        ...
        if (cgs.gametype >= GT_TEAM) {
            Q_strncpyz(newInfo.modelName, DEFAULT_MODEL,
    ...
    if (cg_forceModel.integer) {
        ...
        if (cgs.gametype >= GT_TEAM) {
            Q_strncpyz(newInfo.headModelName, DEFAULT_MODEL,
                       sizeof(newInfo.headModelName));
            Q_strncpyz(newInfo.headSkinName, "red",
            ...
            Q_strncpyz(newInfo.headSkinName, skin,
                       sizeof(newInfo.headSkinName));
            Q_strncpyz(newInfo.headModelName, modelStr,
                       sizeof(newInfo.headModelName));
        }
...
===============
*/
void CG_RunLerpFrame(clientInfo_t *ci, lerpFrame_t *lf, int newAnimation,
                     float speedScale, int weapon) {
    int f, numFrames;
    animation_t *anim;
...
static void CG_PlayerAnimation(centity_t *cent, int *legsOld, int *legs,
    ...
    if (cent->pe.legs.yawing &&
        ...
        CG_RunLerpFrame(ci, &cent->pe.legs, cent->currentState.legsAnim,
                        speedScale, 0);
    }
    ...
    *torsoBackLerp = cent->pe.torso.backlerp;
    ...
    // cent->pe.torso.oldFrame, cent->pe.torso.backlerp,
...
static qboolean CG_PlayerShadow(centity_t *cent, float *shadowPlane,
    ...
    *shadowPlane = trace.endpos[2] + 1;
    ...
    CG_ImpactMark(cgs.media.shadowMarkShader, trace.endpos, trace.plane.normal,
                  cent->pe.legs.yawAngle, alpha, alpha, alpha, 1, qfalse, 24,
                  qtrue, -1);
...
static void CG_PlayerSplash(centity_t *cent) {
    ...
    trap_CM_BoxTrace_New(&trace, start, end, NULL, NULL, 0,
...
void CG_Turret(centity_t *cent) {
    ...
    if (cent->currentState.eventParm >= 0 &&
        cent->currentState.eventParm < cgs.maxclients) {
    ...
    if (cent->currentState.time2 < 0) {
        ...
        if (player && time <= cent->currentState.time2 -
                                  BG_AnimLength(WP_ANIM_CHANGE, WP_GATLING) -
    ...
    */
    ...
    VectorCopy(tripod.origin,
               tripod.oldorigin); // don't positionally lerp at all
...
#define TRIPOD_FRAMES 21
    ...
    if (cent->currentState.time2 &&
        ...
        if (time + 4 * STAGE_TIME + TRIPOD_TIME < cent->currentState.time2) {
            ...
            const int delta =
                time - cent->currentState.time2 + 4 * STAGE_TIME + TRIPOD_TIME;
            // Convert it to step frame, but first frame is the last in the
            // model
            ...
            if (tripod.frame < 0)
                tripod.frame = 0;
            tripod.backlerp =
                (tripod.oldframe > tripod.frame + 1) ? 1.0f : 0.0f;
            tripod.oldframe =
                tripod.frame + (tripod.frame < TRIPOD_FRAMES) ? 1 : 0;
            // Com_Printf("tripod: time %d ; built time: %d ; frame %d ; delta
            // %d ; pos %.2f\n", time, cent->currentState.time2, tripod.frame,
            // delta, pos);
        }
    }
    ...
    */
    ...
    CG_PositionRotatedEntityOnTag(&middle, &tripod, tripod.hModel,
                                  "tag_tripod");
    ...
    */
    ...
    CG_PositionRotatedEntityOnTag(&gatling, &middle, middle.hModel,
                                  "tag_gatling");
    ...
    */
    ...
    CG_PositionRotatedEntityOnTag(&barrel, &gatling, gatling.hModel,
                                  "tag_barrel");
    ...
    */
    ...
    CG_PositionRotatedEntityOnTag(&crank, &gatling, gatling.hModel,
                                  "tag_crank");
...
#define MAG_FRAMES 41
    ...
    if (cent->currentState.eventParm == cg.snap->ps.clientNum &&
        ...
        cg.snap->ps.stats[STAT_GATLING_MODE] && !cg_thirdPerson.integer) {
        ...
        if (cent->currentState.time2 &&
            ...
            mag.backlerp = 0.0f;
            // Com_Printf("g_mag_v: time %d ; built time: %d ; frame %d ; delta
            // %d ; pos %.2f\n", time, cent->currentState.time2, mag.frame,
            // delta, pos);
        } else if ((player->currentState.torsoAnim & ~ANIM_TOGGLEBIT) ==
                   TORSO_GATLING_RELOAD) {
            // Reloading animation
            CG_RunLerpFrame(ci, &cent->pe.weapon, WP_ANIM_RELOAD, 1.0,
                            WP_GATLING);
            ...
            CG_RunLerpFrame(ci, &cent->pe.weapon, WP_ANIM_IDLE, 1.0,
                            WP_GATLING);
        }

    } else {
        if (player && (player->currentState.torsoAnim & ~ANIM_TOGGLEBIT) ==
                          TORSO_GATLING_RELOAD)
            reload = qtrue;
        mag.hModel = cgs.media.g_mag;
    }
    ...
    */
    ...
    if (eFlags & EF_FIRING) {
        ...
        CG_PositionRotatedEntityOnTag(&flash, &gatling, gatling.hModel,
                                      "tag_flash");
        ...
        if (weapon->flashDlightColor[0] || weapon->flashDlightColor[1] ||
            ...
            trap_R_AddLightToScene(
                flash.origin, 50 + (rand() & 31), weapon->flashDlightColor[0],
...
#define LIFE_TIME 4000
    ...
    */
    ...
    if (cg.time >= cg.predictedPlayerEntity.lastSmokeTime &&
        cg.predictedPlayerEntity.lastSmokeTime && cg_gunsmoke.integer &&
        thisplayer) {
        refEntity_t re;

        if (cg.predictedPlayerEntity.lastSmokeTime -
                cg.predictedPlayerEntity.smokeTime >
            SMOKE_TIME) {
        ...
        CG_PositionRotatedEntityOnTag(&re, &gatling, gatling.hModel,
                                      "tag_flash");
...
===============
*/
static int CG_AddHolster(refEntity_t *holster, refEntity_t *legs,
                         centity_t *cent, clientInfo_t *ci, int h_pistol,
                         int renderfx, float shadowPlane, qboolean left) {
    int i;
    ...
    if (left)
        CG_PositionRotatedEntityOnTag(holster, legs, ci->legsModel,
                                      "tag_holsterl");
    else
        CG_PositionRotatedEntityOnTag(holster, legs, ci->legsModel,
                                      "tag_holsterr");
    ...
    for (i = 0; i < WP_NUM_WEAPONS; i++) {
        ...
        if (cent->currentState.time2 & (1 << WP_AKIMBO)) {
            ...
            // and BG_PlayerStateToEntityStateExtraPolate() about the "time2"
            // trick
            ...
            // The player holds only one pistol and has already this kind of
            // pistol in an holster.
            ...
            // currentState.frame contains the pistol on the right hand, on
            // akimbo mode. On akimbo mode, the one on the left hand is
            // currentState.weapon
            if (cent->currentState.frame == i)
                continue;
        }
        /*
                        hika comment: it seems that currentState.weapon is never
           set to WP_AKIMBO if(cent->currentState.weapon == WP_AKIMBO) continue;
        */
        if (cent->currentState.time2 & (1 << i)) {
            ...
            CG_PositionRotatedEntityOnTag(&pistol, holster, holster->hModel,
                                          "tag_weapon");
...
===============
*/
static void AddBackWeapon(refEntity_t *torso, centity_t *cent, clientInfo_t *ci,
                          int renderfx, float shadowPlane) {
    int i;
    ...
    for (i = 0; i < WP_NUM_WEAPONS; i++) {
        ...
        if (cent->currentState.time2 & (1 << i)) {
            ...
            CG_PositionRotatedEntityOnTag(&weapon, torso, torso->hModel,
                                          "tag_back");
...
void CG_Fly(centity_t *cent) {
    ...
    time1 = fabs((float)((cg.time - cent->currentState.clientNum * 33) %
                             deltatime1 -
                         (deltatime1 / 2)) /
                 (deltatime1 / 20)) -
            5;
    time2 = fabs((float)((cg.time - cent->currentState.clientNum * 55) %
                             deltatime2 -
                         (deltatime2 / 2)) /
                 (deltatime2 / 20)) -
            5;
    ...
    angles[ROLL] +=
        fabs((float)((cg.time - cent->currentState.clientNum * 75) % 5000 -
                     2500) /
             83) -
        15;
...
void CG_Player(centity_t *cent) {
    ...
    if (hit_model.integer &&
        ...
        CG_Printf("cg %f %f %f  %f %f %f\n", legsAngles[0], legsAngles[1],
                  legsAngles[2], torsoAngles[0], torsoAngles[1],
                  torsoAngles[2]);

        BuildHitModel(&hit_data, cent->lerpOrigin, legsAngles, torsoAngles,
                      cent->lerpAngles, torso.frame, torso.oldframe,
                      torso.backlerp, legs.frame, legs.oldframe, legs.backlerp);
    }
#endif
...
#define ACCEL_LENGTH 1500.0
        ...
        if (cent->fallTime == 0) {
            ...
            if (anim == BOTH_DEAD_LAND) {
                ...
                VectorCopy(cg.deathpos[cent->currentState.clientNum],
                           cent->fallLastPos);

                // get fallTimes
                cent->fallTimeReal =
                    cg.realFallTimes[cent->currentState.clientNum];
        ...
        if (cent->landing) {
            ...
            factor = (float)deltaTime / ((float)anim_d->frameLerp *
                                         (float)(anim_d->numFrames - 1));
            ...
            legs.backlerp = ((float)legs.oldframe - (float)anim_d->firstFrame) *
                            (float)anim_d->frameLerp;
        ...
        if (!cent->fixed) {
            ...
            if (deltaTime > 0) {
                ...
                VectorMA(legs.origin, factor * FALLOFF_RANGE, legs.axis[0],
                         legs.origin);
            ...
            if (trace.fraction != 1.0 || trace.allsolid || trace.startsolid) {
                ...
                if (trace.plane.normal[2] < 0.75f &&
                    (trace.plane.normal[0] > 0 || trace.plane.normal[1] > 0 ||
    ...
    if (cgs.media.holster_l && addguns) {
        ...
        h_pistol = CG_AddHolster(&holster_l, &legs, cent, ci, h_pistol,
                                 renderfx, shadowPlane, qtrue);
    }
    if (cgs.media.holster_r && addguns) {
        ...
        CG_AddHolster(&holster_r, &legs, cent, ci, h_pistol, renderfx,
                      shadowPlane, qfalse);
    }
...
#define FIRE_SHRINK 200
    ...
    if (cent->currentState.powerups & (1 << PW_BURNBIT) &&
        ...
        refEntity_t fire;
        float factor =
            (float)(cent->currentState.time + FIRE_DURATION - cg.time);
        factor /= FIRE_SHRINK;
        ...
        CG_PositionRotatedEntityOnTag(&fire, &legs, legs.hModel,
                                      "tag_holsterr");
    ...
    if (cg.snap->ps.clientNum == cent->currentState.number) {
        ...
        // origin checks
        CG_Trace(&trace, head.origin, mins, maxs, cg.anim_vieworigin, -1,
                 MASK_SOLID);
        ...
        if (trace.fraction == 1.0) {
            ...
            CG_Trace(&trace, origin, mins, maxs, cg.anim_vieworigin, -1,
                     MASK_SOLID);
        ...
        // CG_Printf("%f, %f, %f\n", cg.anim_vieworigin[0],
        // cg.anim_vieworigin[1], cg.anim_vieworigin[2]);
    }
    ...
    if (cent->currentState.powerups & (1 << PW_GOLD)) {
        ...
        CG_PositionRotatedEntityOnTag(&money, &torso, ci->torsoModel,
                                      "tag_back");
    ...
    if (cent->currentState.weapon == WP_GATLING &&
        ...
        for (i = 0; i < MAX_GENTITIES; i++) {
            ...
            if (t->currentState.eType == ET_TURRET &&
                ...
                CG_PositionRotatedEntityOnTag(&mag, &torso, torso.hModel,
                                              "tag_weapon2");
                ...
                VectorCopy(mag.origin, mag.oldorigin);
                CG_AddRefEntityWithPowerups(&mag, &cent->currentState,
                                            ci->team);
                break;
            }
        }
...
void CG_ResetPlayerEntity(centity_t *cent) {
    ...
    if (cg.snap->ps.clientNum == cent->currentState.clientNum) {
        CG_ClearLerpFrame(&cgs.clientinfo[cent->currentState.clientNum],
                          &cent->pe.weapon, cent->pe.weapon.weaponAnim);
        CG_ClearLerpFrame(&cgs.clientinfo[cent->currentState.clientNum],
                          &cent->pe.weapon2, cent->pe.weapon2.weaponAnim);
    }
