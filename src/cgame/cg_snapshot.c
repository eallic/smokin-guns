...
static void CG_TransitionSnapshot(void) {
    ...
    BG_PlayerStateToEntityState(
        &cg.snap->ps, &cg_entities[cg.snap->ps.clientNum].currentState, qfalse);
...
static void CG_SetNextSnap(snapshot_t *snap) {
    ...
    BG_PlayerStateToEntityState(
        &snap->ps, &cg_entities[snap->ps.clientNum].nextState, qfalse);
...
static snapshot_t *CG_ReadNextSnapshot(void) {
    ...
    while (cgs.processedSnapshotNum < cg.latestSnapshotNum) {
        ...
        if (cg_latentSnaps.integer && r) {
            ...
            while (dest->serverTime >
                   time - cg_latentSnaps.integer * (1000 / sv_fps.integer)) {
                if (!(r = trap_GetSnapshot(cgs.processedSnapshotNum - i,
                                           dest))) {
    ...
    do {
        ...
        if (!cg.nextSnap) {
            ...
            if (cg.nextSnap->serverTime < cg.snap->serverTime) {
                ...
                //  this may actually happen with lag simulation going on
                if (cg_latentSnaps.integer) {
                    CG_Printf("WARNING: CG_ProcessSnapshots: Server time went "
                              "backwards\n");
                } else {
