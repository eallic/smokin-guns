...
static void CG_DeathCamView(void) {
    ...
    // trace to walls
    CG_Trace(&trace, tracestart, mins, maxs, origin,
    ...
    if (prevclientframe == 0 || focusAngles[YAW] != prevangles[YAW] ||
        focusAngles[PITCH] != prevangles[PITCH]) {
        if (prevclientframe != 0)
            diff = cg.clientFrame - prevclientframe;
...
static void CG_OffsetThirdPersonView(void) {
    ...
    if (cgs.deathcam && (cg.snap->ps.stats[STAT_HEALTH] <= 0) &&
        (cgs.gametype != GT_DUEL) && !(cg.snap->ps.pm_flags & PMF_SUICIDE) &&
        !(cg.snap->ps.pm_flags & PMF_FOLLOW) &&
        (cg.snap->ps.pm_type != PM_SPECTATOR)) {
        // if dead & player didnt commit suicide & not specting sy, cs-style
        // deathcam.
        CG_DeathCamView();
        return;
    }
...
static void CG_DuelIntroView(void) {
    float factor = (float)(cg.time - cg.introstart - WAIT_TIME) /
                   (DU_INTRO_CAM - WAIT_TIME);
...
static void CG_OffsetFirstPersonView(void) {
    ...
    if (cg.snap->ps.persistant[PERS_TEAM] < TEAM_SPECTATOR) {
        BG_ModifyEyeAngles(origin, angles, CG_Trace, cg.legOffset, qfalse);
    }
    ...
    /*
            if (cg.cameraMode) {
                    ...
                    if (trap_getCameraInfo(cg.time, &origin, &angles)) {
                            ...
                            AnglesToAxis( cg.refdefViewAngles,
       cg.refdef.viewaxis ); return CG_CalcFov(); } else { cg.cameraMode =
       qfalse;
                    }
            }
    */
    ...
    // link to view to head of ppm if dying - edited, only do this if the player
    // killed himself or specting someone else
    if ((((ps->pm_flags & PMF_SUICIDE) || !cgs.deathcam ||
...
// unlagged - lag simulation #1
    ...
    if (cg.roundstarttime && cg.roundstarttime <= cg.time &&
        cgs.gametype == GT_DUEL) {
        ...
        trap_S_StartLocalSound(cgs.media.duelstart[cg.duelstartsound],
                               CHAN_ANNOUNCER);
        cg.duelstartsound++;
    ...
    cg.renderingThirdPerson =
        cg_thirdPerson.integer || cg.introstart ||
        (cgs.deathcam && (cg.snap->ps.stats[STAT_HEALTH] <= 0) &&
         !(cg.snap->ps.pm_flags & PMF_SUICIDE) &&
         !(cg.snap->ps.pm_flags & PMF_FOLLOW) &&
    ...
    if (!cg.hyperspace) {
        CG_AddPacketEntities(); // adter calcViewValues, so predicted player
                                // state is correct
        CG_AddMarks();
        CG_AddLocalEntities();
    }
    ...
    // make sure the lagometerSample and frame timing isn't done twice when in
    // stereo
