...
static void CG_MachineGunEjectBrass(centity_t *cent) {
    ...
    xoffset[0] =
        offset[0] * v[0][0] + offset[1] * v[1][0] + offset[2] * v[2][0];
    xoffset[1] =
        offset[0] * v[0][1] + offset[1] * v[1][1] + offset[2] * v[2][1];
    xoffset[2] =
        offset[0] * v[0][2] + offset[1] * v[1][2] + offset[2] * v[2][2];
    ...
    xvelocity[0] =
        velocity[0] * v[0][0] + velocity[1] * v[1][0] + velocity[2] * v[2][0];
    xvelocity[1] =
        velocity[0] * v[0][1] + velocity[1] * v[1][1] + velocity[2] * v[2][1];
    xvelocity[2] =
        velocity[0] * v[0][2] + velocity[1] * v[1][2] + velocity[2] * v[2][2];
...
void CG_RegisterWeapon(int weaponNum) {
    ...
    if (!weaponInfo->handsModel) {
        weaponInfo->handsModel =
            trap_R_RegisterModel("models/weapons2/shotgun/shotgun_hand.md3");
    }
    ...
    if (bg_weaponlist[weaponNum].snd_fire && weaponNum >= WP_REM58 &&
        ...
        for (i = 0; i < 3; i++) {
            ...
            Com_sprintf(name, sizeof(name), "%s%i.wav",
                        bg_weaponlist[weaponNum].snd_fire, i + 1);
        ...
        weaponInfo->flashSound[0] =
            trap_S_RegisterSound(bg_weaponlist[weaponNum].snd_fire, qfalse);
    }

    if (bg_weaponlist[weaponNum].snd_reload)
        weaponInfo->reloadSound =
            trap_S_RegisterSound(bg_weaponlist[weaponNum].snd_reload, qfalse);

    switch (weaponNum) {
    case WP_KNIFE:
        weaponInfo->missileModel =
            trap_R_RegisterModel("models/weapons2/knife/e_knife.md3");
        break;
    ...
    case WP_SCHOFIELD:
        ...
        cgs.media.bulletExplosionShader =
            trap_R_RegisterShader("waterExplosion");
    ...
    case WP_DYNAMITE:
        weaponInfo->missileModel =
            trap_R_RegisterModel("models/weapons2/dyn/e_dynamite.md3"); // Spoon
        cgs.media.dynamiteExplosionShader =
            trap_R_RegisterShader("wqfx/dynexp");
    ...
    case WP_MOLOTOV:
        weaponInfo->missileModel = trap_R_RegisterModel(
            "models/weapons2/molotov/e_molotov_missile.md3"); // Spoon
        weaponInfo->missileDlight = 30;
        VectorSet(weaponInfo->missileDlightColor, 1, 0.75f, 0.4f);
        break;
...
static void CG_CalculateWeaponPosition(vec3_t origin, vec3_t angles) {
    ...
    VectorCopy(cg_entities[cg.snap->ps.clientNum].currentState.pos.trDelta,
               forward);
...
float CG_MachinegunSpinAngle(centity_t *cent) {
    ...
    float speed;
    qboolean firing =
        ((cent->currentState.eFlags & EF_FIRING) /*&& !reloading*/);
    ...
    if (cent->pe.barrelSpinning) {
        ...
        trap_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
                               vec3_origin, cgs.media.sfx_gatling_loop);
    } else {
        if (delta > COAST_TIME) {
            delta = COAST_TIME;
        }
...
=============
*/
void CG_InitWeaponAnim(centity_t *cent, int weapon, int weaponAnim,
                       lerpFrame_t *anim, qboolean weapon2) {

    int realanim = weaponAnim;
    realanim &= ~ANIM_TOGGLEBIT;

    if (!cg.snap->ps.ammo[WP_REMINGTON_GAUGE])
        bg_weaponlist[WP_REMINGTON_GAUGE].animations[WP_ANIM_CHANGE].numFrames =
            20;
    else
        bg_weaponlist[WP_REMINGTON_GAUGE].animations[WP_ANIM_CHANGE].numFrames =
            49;
    ...
    if (realanim == WP_ANIM_SPECIAL && weapon == WP_WINCHESTER66) {
        if (cg.snap->ps.powerups[PW_SCOPE] == 2)
            bg_weaponlist[WP_WINCHESTER66]
                .animations[WP_ANIM_SPECIAL]
                .reversed = qtrue;
        else
            bg_weaponlist[WP_WINCHESTER66]
                .animations[WP_ANIM_SPECIAL]
...
models (ps is NULL) The main player will have this called for BOTH cases, so
effects like light and sound should only be done on the world model case.
=============
*/
void CG_AddPlayerWeapon(refEntity_t *parent, playerState_t *ps, centity_t *cent,
    ...
    if (ps) {
        ...
        if (ps->weapon2) {
            ...
            CG_InitWeaponAnim(cent, weapon2Num, cg.weapon2Anim,
                              &cent->pe.weapon2, qtrue);
            CG_RunLerpFrame(ci, &cent->pe.weapon2, cent->pe.weapon2.weaponAnim,
                            1.0, weapon2Num);
        }

        // 1st weapon
        CG_InitWeaponAnim(cent, weaponNum, cg.weaponAnim, &cent->pe.weapon,
                          qfalse);
        CG_RunLerpFrame(ci, &cent->pe.weapon, cent->pe.weapon.weaponAnim, 1.0,
                        weaponNum);
    }
    ...
    if (weaponNum ==
        WP_GATLING) { // doesn't have a weapon, "tagged" on the player
        ...
        if (ps && cg.gatlingmodeOld && !cg.gatlingmode &&
            ...
            if (anim != WP_ANIM_DROP && anim != WP_ANIM_CHANGE) {
                return;
            }
        }
        ...
        if (ps && ps->stats[STAT_GATLING_MODE]) {
            ...
            if (cent->pe.weapon.frame ==
                bg_weaponlist[WP_GATLING].animations[anim].numFrames) {
                return;
            }
    ...
    if ((cent->currentState.powerups & (1 << PW_BURN)) &&
        ...
        if (weaponNum == WP_DYNAMITE) {
            ...
            CG_PositionRotatedEntityOnTag(&sparks, &gun, gun.hModel,
                                          "tag_sparks");
            ...
            trap_R_AddLightToScene(
                origin, weapon->missileDlight, weapon->missileDlightColor[0],
...
#define LIFE_TIME 4000
    ...
    if (cg.time >= cent->lastSmokeTime2 && cent->lastSmokeTime2 &&
        cg_gunsmoke.integer) {
        refEntity_t re;
        int contents;
...
muzzle:
    ...
    for (i = 0; i < 2; i++) {
        ...
        } else if (!(cg.time - cent->muzzleFlashTime2 > MUZZLE_FLASH_TIME) &&
                   cent->currentState.frame) {
        ...
        if (i == 2) {
            CG_PositionRotatedEntityOnTag(&flash, &gun2, gun2.hModel,
                                          "tag_flash");
        } else {
            CG_PositionRotatedEntityOnTag(&flash, &gun, gun.hModel,
                                          "tag_flash");
        }
        ...
        if (ps || cg.renderingThirdPerson ||
            ...
            if (i == 2) {
                if (weapon2->flashDlightColor[0] ||
                    weapon2->flashDlightColor[1] ||
                    weapon2->flashDlightColor[2]) {
                    trap_R_AddLightToScene(flash.origin, 100 + (rand() & 31),
                                           weapon2->flashDlightColor[0],
                                           weapon2->flashDlightColor[1],
                                           weapon2->flashDlightColor[2]);
                }
...
void CG_DrawWeaponSelect(void) {
    ...
    if (cgs.gametype == GT_DUEL && cg.introend &&
    ...
    for (i = 1; i < WPS_NUM_ITEMS; i++) {
        CG_DrawPic((i - 1) * FIELD_WIDTH, 0, FIELD_WIDTH, TOP_HEIGHT,
                   cgs.media.hud_wp_top);
        // draw the number
        trap_R_SetColor(color2[0]);
        CG_DrawPic((i - 1) * FIELD_WIDTH + FIELD_WIDTH / 2 - 15 / 2,
                   (TOP_HEIGHT - 15) / 2, 15, 15,
                   cgs.media.numbermoneyShaders[i]);
    ...
    akimbo = (wp_sort == WPS_PISTOL &&
              (BG_CountTypeWeapons(WPS_PISTOL,
                                   cg.snap->ps.stats[STAT_WEAPONS]) >= 2 ||
    ...
    for (i = WP_KNIFE; i < WP_NUM_WEAPONS; i++) {
        ...
        if (item->xyrelation == 1) {
            ...
            if (i == cg.markedweapon) {
                CG_FillRect(x + 2, y + 2, FIELD_WIDTH - 4, FIELD_WIDTH - 4,
                            color2[2]);
            } else {
                CG_FillRect(x + 2, y + 2, FIELD_WIDTH - 4, FIELD_WIDTH - 4,
                            color2[1]);
            ...
            // draw field
            CG_DrawPic(x, y, FIELD_WIDTH, FIELD_WIDTH,
                       cgs.media.hud_wp_box_quad);
            ...
            if (weapon && cg.snap->ps.ammo[weapon] > 0) {
                for (ammo = 0; ammo < cg.snap->ps.ammo[weapon]; ammo++) {
                    CG_DrawPic(x + 4 + FIELD_WIDTH,
                               y + 4 + (ammo * (AMMO_HEIGHT + 1)), ammowidth,
                               AMMO_HEIGHT, clip);
                }
            }
            ...
            if (i == cg.markedweapon &&
                ...
                CG_FillRect(x + 2, y + 2, FIELD_WIDTH - 4, FIELD_HEIGHT - 4,
                            color2[2]);
            } else {
                CG_FillRect(x + 2, y + 2, FIELD_WIDTH - 4, FIELD_HEIGHT - 4,
                            color2[1]);
            ...
            case WP_BULLETS_CLIP:
                ...
                if (cg.snap->ps.stats[STAT_FLAGS] & SF_SEC_PISTOL)
                    // The top pistol is the left one when the player has two
                    // pistols
                    weapon = WP_AKIMBO;
                break;
            case WP_CART_CLIP:
            ...
            if (weapon && cg.snap->ps.ammo[weapon] > 0) {
                ...
                CG_DrawSmallStringColor(
                    x + 4 + FIELD_WIDTH, y + 6 + FIELD_HEIGHT / 2,
                    va("x%i", cg.snap->ps.ammo[i]), color2[0]);
            }

            // if player has pistol: give option to combine it with another
            // weapon
            /*if(bg_weaponlist[cg.snap->ps.weapon].wp_sort == WPS_PISTOL &&
                    bg_weaponlist[i].wp_sort == WPS_PISTOL &&
                    i != cg.snap->ps.weapon && !cg.snap->ps.weapon2) {
                    CG_DrawStringExt( x+66, y+24, "FIRE2: Add", color2[3],
            qtrue, qfalse, 6, 15, -1);
            }*/
    ...
    if (akimbo) {
        ...
        weapon1 =
            BG_SearchTypeWeapon(WPS_PISTOL, cg.snap->ps.stats[STAT_WEAPONS], 0);
        if (weapon1 < WP_PEACEMAKER && cg.weaponSelect == WP_PEACEMAKER &&
            ...
            weapon2 = BG_SearchTypeWeapon(
                WPS_PISTOL, cg.snap->ps.stats[STAT_WEAPONS], weapon1);
        ...
        if (cg.snap->ps.stats[STAT_FLAGS] & SF_SEC_PISTOL) {
            ...
            if (cg.weaponSelect == weapon1) {
                // Player holds the weakest pistol, so the left one is the
                // strongest
                leftweapon = weapon2;
                rightweapon = weapon1;
            } else if (cg.weaponSelect == weapon2) {
                // Player holds the strongest pistol, so the left one is the
                // weakest
                leftweapon = weapon1;
                rightweapon = weapon2;
        ...
        if (cg.snap->ps.stats[STAT_FLAGS] & SF_SEC_PISTOL &&
            ...
            // draw weapon icon
            CG_DrawPic(x + 2, y + 4, FIELD_WIDTH - 4, FIELD_HEIGHT - 8,
                       cg_weapons[weapon1].weaponIcon);
            ...
            if (cg.snap->ps.ammo[weapon1] > 0) {
                for (ammo = 0; ammo < cg.snap->ps.ammo[weapon1]; ammo++) {
                    CG_DrawPic(x + 4 + FIELD_WIDTH,
                               y + 4 + (ammo * (AMMO_HEIGHT + 1)),
                               AMMO_BULLET_WIDTH, AMMO_HEIGHT,
                               cgs.media.hud_ammo_bullet);
                }
            }
        ...
        if (cg.snap->ps.stats[STAT_FLAGS] & SF_SEC_PISTOL) {
            ...
            CG_DrawPic(x + 2, y + 4, FIELD_WIDTH - 4, FIELD_HEIGHT - 8,
                       cg_weapons[leftweapon].weaponIcon);
        }
        // Draw the weapon icon on the right side
        CG_DrawPic(x - 2 + FIELD_WIDTH, y - 4 + FIELD_HEIGHT,
                   -(FIELD_WIDTH - 4), -(FIELD_HEIGHT - 8),
                   cg_weapons[rightweapon].weaponIcon);
        ...
        if (cg.snap->ps.ammo[leftweapon] > 0) {
            for (ammo = 0; ammo < cg.snap->ps.ammo[leftweapon]; ammo++) {
                CG_DrawPic(x - 4 - AMMO_BULLET_WIDTH,
                           y + 4 + (ammo * (AMMO_HEIGHT + 1)),
                           AMMO_BULLET_WIDTH, AMMO_HEIGHT,
                           cgs.media.hud_ammo_bullet);
            }
        ...
        if (cg.snap->ps.ammo[rightweapon] > 0) {
            for (ammo = 0; ammo < cg.snap->ps.ammo[rightweapon]; ammo++) {
                CG_DrawPic(
                    x + 4 + FIELD_WIDTH, y + 4 + (ammo * (AMMO_HEIGHT + 1)),
                    AMMO_BULLET_WIDTH, AMMO_HEIGHT, cgs.media.hud_ammo_bullet);
            }
        }
...
void CG_NextWeapon_f(void) {
    ...
    if (first_time) { // signal BUTTON_CHOOSE_MODE to the server
        // Tequila comment: By torhu, see
        // http://www.quake3world.com/forum/viewtopic.php?f=16&t=17815
    ...
    for (j = 0; j < WPS_NUM_ITEMS; j++) {
        ...
        for (i = 1, weapon = 0; i <= WP_NUM_WEAPONS; i++) {
            ...
            if (num == WPS_PISTOL && weapon + 1 == WP_NUM_WEAPONS &&
                ...
                // 2) Player has two equal pistols and the second pistol is
                // selected. 3) Player has two equal pistols and has already got
                // one pistol in hand.
                ...
                // This must happen *ONLY* if the player is not already in
                // single pistol mode
                ...
                cg.markedweapon = BG_SearchTypeWeapon(
                    num, cg.snap->ps.stats[STAT_WEAPONS], 0);
...
void CG_Weapon_f(void) {
    ...
    if ((cg.snap->ps.stats[STAT_FLAGS] & SF_SEC_PISTOL) && num == WPS_PISTOL &&
        startnum < WP_NUM_WEAPONS &&
        bg_weaponlist[startnum].wp_sort != WPS_PISTOL) {
        cg.markedfirstpistol = qtrue;
    }
    ...
    for (i = 1, weapon = 0; i <= WP_NUM_WEAPONS; i++) {
        ...
        if (num == WPS_PISTOL && weapon + 1 == WP_NUM_WEAPONS &&
            ...
            // 1) Player has two different pistols.
            // 2) Player has two equal pistols and the second pistol is
            // selected. 3) Player has two equal pistols and has already got one
            // pistol in hand.
            ...
            // This must happen *ONLY* if the player is not already in single
            // pistol mode
            cg.markedfirstpistol = qfalse;
            if (!cg.markedweapon)
            ...
            cg.markedweapon =
                BG_SearchTypeWeapon(num, cg.snap->ps.stats[STAT_WEAPONS], 0);
...
void CG_LastUsedWeapon_f(void) {
    ...
    if (cg.lastusedweapon == WP_SEC_PISTOL &&
        (BG_CountTypeWeapons(WPS_PISTOL, cg.snap->ps.stats[STAT_WEAPONS]) !=
             1 ||
         !(cg.snap->ps.stats[STAT_FLAGS] & SF_SEC_PISTOL))) {
        return;
    }
    // return if player doesn't have the previously used weapon
    if (cg.lastusedweapon != WP_AKIMBO && cg.lastusedweapon != WP_SEC_PISTOL &&
    ...
    // is the other single same type pistol, because it will become the last
    // used weapon as the secondary same type pistol.
    if (honeymug == WP_SEC_PISTOL && cg.weaponSelect != WP_AKIMBO &&
        bg_weaponlist[cg.weaponSelect].wp_sort == WPS_PISTOL) {
...
==============
*/
    void CG_BottleBreak(vec3_t org, vec3_t bottledir, qboolean fire,
        ...
        for (i = 0; i < 4; i++) {
            ...
            CG_LaunchParticle(origin, dir, cgs.media.molotovgib[i], LEBS_NONE,
                              BOUNCE_LIGHT, 4000 + rand() % 2000,
                              /*LEF_BREAKS*/ 0);
        }
        ...
        for (i = 0; i < 8; i++) {
            ...
            le = CG_LaunchSpriteParticle(origin, dir, BOUNCE_LIGHT, 4000,
                                         (LEF_PARTICLE | LEF_BLOOD));
...
void CG_MissileHitWall(int weapon, int clientNum, vec3_t origin, vec3_t dir,
                       impactSound_t soundType, int surfaceFlags, int shaderNum,
    ...
    if (sfx && weapon != WP_SAWEDOFF && weapon != WP_REMINGTON_GAUGE) {
        ...
        //
    if (mod || shader) {
            le = CG_MakeExplosion(origin, dir, mod, shader,
                              duration, isSprite, radius_exp,
                              (weapon == WP_DYNAMITE));
            le->light = light;
    ...
    if (mark) {
        ...
        if ((surfaceFlags & SURF_BREAKABLE) &&
            ...
            CG_ImpactMark(mark, origin, dir, random() * 360, red, green, blue,
                          1, alphaFade, radius_mrk, qfalse, -1);
        }
    }

    if (surfaceFlags != -1) {
        CG_LaunchImpactParticle(origin, dir, surfaceFlags, shaderNum, weapon,
                                qfalse);
    }
    ...
    */
    void CG_MissileHitPlayer(int weapon, vec3_t origin, vec3_t dir,
        ...
        switch (weapon) {
    case WP_DYNAMITE:
        CG_MissileHitWall(weapon, 0, origin, dir, IMPACTSOUND_FLESH, -1, -1,
                          qfalse, NULL, -1);
        break;
    case WP_MOLOTOV:
        CG_MissileHitWall(weapon, 0, origin, dir, IMPACTSOUND_FLESH, -1, -1,
...
================
*/
static void CG_ShotgunPellet(vec3_t start, vec3_t origin2, vec3_t end,
                             int skipNum, int playerhit, vec3_t player, float r,
                             float u, int weapon) {
    trace_t tr;
...
cg_shotgunfire:
    ...
    if (playerhit != -1) {
        ...
        shaderNum = CG_Trace_New(&tr, tr_start, NULL, NULL, end, ENTITYNUM_NONE,
                                 MASK_SHOT);
        ...
        while (tr.entityNum < MAX_CLIENTS) {
            ...
            shaderNum = CG_Trace_New(&tr, tr.endpos, NULL, NULL, end,
                                     tr.entityNum, MASK_SHOT);
        }
    }
    ...
    if (sourceContentType == destContentType) {
        ...
        trap_CM_BoxTrace_New(&trace, end, tr_start, NULL, NULL, 0,
                             CONTENTS_WATER);
        CG_BubbleTrail(tr_start, trace.endpos, 32);
    } else if (destContentType & CONTENTS_WATER) {
        trace_t trace;

        shaderNum = trap_CM_BoxTrace_New(&trace, tr_start, end, NULL, NULL, 0,
                                         CONTENTS_WATER);
        CG_BubbleTrail(tr.endpos, trace.endpos, 32);

        // water particles
        CG_LaunchImpactParticle(trace.endpos, trace.plane.normal, SURF_WATER,
                                shaderNum, WP_SAWEDOFF, qfalse);
    }
    ...
    if (cg_entities[tr.entityNum].currentState.eType == ET_PLAYER) {
        ...
        VectorAdd(cg_entities[tr.entityNum].currentState.pos.trDelta,
                  tr.plane.normal, dir);
        VectorNormalize(dir);

        CG_LaunchImpactParticle(tr.endpos, dir, -1, -1, -1, qtrue);
        // CG_MissileHitPlayer( WP_REMINGTON_GAUGE, tr.endpos, tr.plane.normal,
        // tr.entityNum );
        ...
        if (tr.surfaceFlags & SURF_METAL) {
            CG_MissileHitWall(WP_REMINGTON_GAUGE, 0, tr.endpos, tr.plane.normal,
                              IMPACTSOUND_METAL, tr.surfaceFlags, shaderNum,
                              qfalse, NULL, tr.entityNum);
        } else {
            CG_MissileHitWall(WP_REMINGTON_GAUGE, 0, tr.endpos, tr.plane.normal,
                              IMPACTSOUND_DEFAULT, tr.surfaceFlags, shaderNum,
                              qfalse, NULL, tr.entityNum);
        }

        // check if bullets are gone thru
#define DAM_FACTOR 0.5f
        // modify damage for a short time, cause shotguns have particles with
        // low damage
        damage *= DAM_FACTOR;

        // look if the weapon is able to shoot through the wall
        shootthru = BG_ShootThruWall(&damage, tr.endpos, tr_start,
                                     tr.surfaceFlags, endpos, CG_Trace);
        ...
        if (shootthru) {
            ...
            shaderNum =
                CG_Trace_New(&tr, endpos, NULL, NULL, tr_start, ENTITYNUM_NONE,
            ...
            if (tr.surfaceFlags & SURF_METAL) {
                CG_MissileHitWall(WP_REMINGTON_GAUGE, 0, tr.endpos,
                                  tr.plane.normal, IMPACTSOUND_METAL,
                                  tr.surfaceFlags, shaderNum, qfalse, NULL,
                                  tr.entityNum);
            } else {
                CG_MissileHitWall(WP_REMINGTON_GAUGE, 0, tr.endpos,
                                  tr.plane.normal, IMPACTSOUND_DEFAULT,
                                  tr.surfaceFlags, shaderNum, qfalse, NULL,
                                  tr.entityNum);
            }
    ...
    if (cgs.newShotgunPattern) {
        ...
        for (i = -extra_center_pellet; i < count - extra_center_pellet; i++) {
            ...
            spread_dist = (current_spread_circle + Q_random(&seed)) /
                          max_spread_circle * bg_weaponlist[es->weapon].spread *
                          16;
            ...
            if ((i + extra_center_pellet + 1) < 16 &&
                ((int)es->angles[1] & (1 << (i + extra_center_pellet + 1)))) {
                vec3_t player;

                VectorCopy(cg_entities[es->clientNum].lerpOrigin, player);

                CG_ShotgunPellet(origin, origin2, end, otherEntNum,
                                 es->clientNum, player, r, u, es->weapon);
            } else {
                CG_ShotgunPellet(origin, origin2, end, otherEntNum, -1, origin,
                                 r, u, es->weapon);
            }
        }
    ...
    */
    void CG_ShotgunFire(entityState_t * es) {
        ...
        if (cgs.glconfig.hardwareType != GLHW_RAGEPRO) {
            ...
            if (!(contents & CONTENTS_WATER) &&
                (cg.snap->ps.clientNum != es->otherEntityNum ||
...
void CG_Bullet(vec3_t end, int sourceEntityNum, vec3_t normal, qboolean flesh,
               int fleshEntityNum, int weapon, int surfaceFlags, int shaderNum,
               int entityNum) {
        trace_t trace;
        int sourceContentType, destContentType;
        vec3_t start;
        ...
        if (sourceEntityNum >= 0 && cg_tracerChance.value > 0) {
            if (CG_CalcMuzzlePoint(sourceEntityNum, start)) {
                ...
                shaderNum = trap_CM_BoxTrace_New(&trace, end, start, NULL, NULL,
                                                 0, CONTENTS_WATER);
                    CG_BubbleTrail(start, trace.endpos, 32);
                }
                // bubble trail from air into water
                else if ((destContentType & CONTENTS_WATER)) {
                shaderNum = trap_CM_BoxTrace_New(&trace, start, end, NULL, NULL,
                                                 0, CONTENTS_WATER);
                    CG_BubbleTrail(trace.endpos, end, 32);
                    // water particles
                    CG_LaunchImpactParticle(trace.endpos, trace.plane.normal,
                                            SURF_WATER, shaderNum, weapon,
                                            qfalse);
                }
        ...
        if (weapon)
            CG_MissileHitWall(weapon, 0, end, normal, IMPACTSOUND_DEFAULT,
                              surfaceFlags, shaderNum, qfalse, NULL, entityNum);
        else
            CG_MissileHitWall(WP_PEACEMAKER, 0, end, normal,
                              IMPACTSOUND_DEFAULT, surfaceFlags, shaderNum,
                              qfalse, NULL, entityNum);
        }
    }
    ...
    void CG_MakeSmokePuff(entityState_t * ent) {
        ...
        if (cgs.glconfig.hardwareType != GLHW_RAGEPRO) {
            ...
            if (!(contents & CONTENTS_WATER)) {
                VectorSet(up, 0, 0, 8);
                CG_SmokePuff(v, up, 24, 1, 1, 0.8f, 0.40f, 400, cg.time, 0,
                             LEF_PUFF_DONT_SCALE,
                             cgs.media.shotgunSmokePuffShader);
            }
        }
