...
typedef enum {
        ...
        RT_RAIL_RINGS,
        RT_LIGHTNING,
        RT_PORTALSURFACE,		// doesn't draw anything, just info for
portals
...
localEntity_t *CG_CreateBulletHole(vec3_t origin, vec3_t dir, int surfaceFlags,
    ...
    VectorScale(le->refEntity.axis[0], le->refEntity.radius,
                le->refEntity.axis[0]);
    VectorScale(le->refEntity.axis[1], le->refEntity.radius,
                le->refEntity.axis[1]);
    VectorScale(le->refEntity.axis[2], le->refEntity.radius,
                le->refEntity.axis[2]);
...
=========================
*/
localEntity_t *CG_LaunchParticle(vec3_t origin, vec3_t dir, qhandle_t hModel,
                                 int snd, int bouncefactor, int duration,
                                 int leFlags) {
...
void CG_LaunchGlass(vec3_t mins, vec3_t maxs, int surfaceFlags) {
    ...
    // columns
    if ((length[side] / quad_size) > ((int)length[side] / quad_size))
        cols = (int)length[side] / quad_size + 1;
    else
        cols = (int)length[side] / quad_size + 1;

    // rows
    if ((length[2] / quad_size) > ((int)length[2] / quad_size))
    ...
    for (c = cols - 1; c >= 0; c--) {
        ...
        for (r = rows - 1; r >= 0; r--) {
            ...
            for (i = 0; i < num_gibs; i++) {
                ...
                gib = CG_LaunchParticle(origin, dir, model, snd, BOUNCE_LIGHT,
                                        6000 + random() * 4000, LEF_BREAKS);
                ...
                VectorScale(gib->refEntity.axis[0], factor[0],
                            gib->refEntity.axis[0]);
                VectorScale(gib->refEntity.axis[1], factor[1],
                            gib->refEntity.axis[1]);
                VectorScale(gib->refEntity.axis[2], factor[2],
                            gib->refEntity.axis[2]);
            }
        }
    }
}
...
void CG_LaunchDefault(vec3_t mins, vec3_t maxs, int surfaceFlags,
                      int shaderNum) {
    ...
    for (i = 0; i < border || !i; i++) {
        ...
        *ypos = miny + rand() % width;
        ...
        if (surfaceFlags & SURF_WOOD) {
            ...
            if ((surfaceFlags & SURF_GRASS) || (surfaceFlags & SURF_SNOW) ||
                (surfaceFlags & SURF_SAND) || (surfaceFlags & SURF_PLANT)) {
                qhandle_t shader;
                int surfaceNum = 0;
                ...
                le = CG_LaunchSpriteParticle(origin, newdir, BOUNCE_MEDIUM,
                                             2000 * 1 / weight, LEF_PARTICLE);
...
void CG_LaunchFuncBreakable(centity_t *cent) {
    ...
    if ((surfaceFlags & SURF_GLASS) || (surfaceFlags & SURF_CLOTH) ||
        (surfaceFlags & SURF_METAL))
        CG_LaunchGlass(cent->currentState.origin2, cent->currentState.origin,
                       surfaceFlags);
    else
        CG_LaunchDefault(cent->currentState.origin2, cent->currentState.origin,
                         surfaceFlags, shaderNum);

    if (surfaceFlags & SURF_GLASS) {
        trap_S_StartSound(cent->currentState.origin, ENTITYNUM_WORLD, CHAN_AUTO,
                          cgs.media.sfxglassgib[rand() % 2]);
    } else if ((surfaceFlags & SURF_STONE) || (surfaceFlags & SURF_METAL) ||
               (surfaceFlags & SURF_DIRT) || (surfaceFlags & SURF_SAND)) {
        trap_S_StartSound(cent->currentState.origin, ENTITYNUM_WORLD, CHAN_AUTO,
...
void CG_CheckSparkGen(localEntity_t *le) {
    ...
    while ((le2[i - 1]->startTime + le2[i - 1]->refEntity.frame * SPARK_TIME) <
               cg.time &&
           le2[i - 1]->refEntity.frame) {
        const int deltaTime =
            cg.time -
            (le2[i - 1]->startTime + le2[i - 1]->refEntity.frame * SPARK_TIME);

        CG_Printf(
            "%i (%i, %i)\n", deltaTime, cg.time,
            (le2[i - 1]->startTime + le2[i - 1]->refEntity.frame * SPARK_TIME));

        le2[i] = CG_CreateSparkTrail(le2[i - 1]->refEntity.oldorigin,
                                     le2[i - 1]->refEntity.lightingOrigin);
        ...
        VectorCopy(le2[i - 1]->refEntity.oldorigin,
                   le2[i]->refEntity.oldorigin);
        VectorCopy(le2[i - 1]->refEntity.lightingOrigin,
                   le2[i]->refEntity.lightingOrigin);
...
void CG_LaunchImpactParticle(vec3_t origin, vec3_t dir, int surfaceFlags,
    ...
    if (flesh) {
        for (i = 0; i < 5; i++) {
            ...
            le = CG_LaunchSpriteParticle(origin, newdir, BOUNCE_LIGHT, 2000,
    ...
    for (i = 0; i < num; i++) {
        ...
        le = CG_LaunchSpriteParticle(spawnpoint, newdir, BOUNCE_MEDIUM,
                                     2000 * 1 / weight, LEF_PARTICLE);
    ...
    if (prefixInfo[surfaceNum].surfaceFlags == SURF_WATER) {
        localEntity_t *le;
        vec4_t light, color;
        int i;
        vec3_t normal;

        le = CG_MakeExplosion(origin, dir, cgs.media.bulletFlashModel,
                              cgs.media.bulletExplosionShader, 375, qfalse, 8,
                              qfalse);
...
==============================
*/
int trap_CM_TransformedBoxTrace_New(trace_t *results, const vec3_t start,
    ...
    trap_CM_TransformedBoxTrace(results, start, end, mins, maxs, model,
...
qboolean CG_CheckPlayerVisible(vec3_t view, vec3_t player) {
    ...
    for (i = 0; i <= STEP; i++) {
        ...
        // left/right
        VectorMA(view, (SCREEN_SIZE / STEP) * (i) - (SCREEN_SIZE / 2),
                 cg.refdef.viewaxis[1], start);

        CG_Trace(&trace, start, vec3_origin, vec3_origin, player, -1,
        ...
        for (j = 0; j <= STEP; j++) {
            // up/down
            VectorMA(temp, (SCREEN_SIZE / STEP) * (j) - (SCREEN_SIZE / 2),
                     cg.refdef.viewaxis[2], start);
...
localEntity_t *CG_AddSmokeParticle(vec3_t origin, int radius, int randomradius,
                                   int lifetime, int boost, int wind,
                                   vec3_t winddir) {
    localEntity_t *le;
    refEntity_t *re;
...
void CG_SetupFrustum(void) {
    ...
    VectorMA(cg_frustum[0].normal, xc, cg.refdef.viewaxis[1],
             cg_frustum[0].normal);

    VectorScale(cg.refdef.viewaxis[0], xs, cg_frustum[1].normal);
    VectorMA(cg_frustum[1].normal, -xc, cg.refdef.viewaxis[1],
             cg_frustum[1].normal);
    ...
    VectorMA(cg_frustum[2].normal, xc, cg.refdef.viewaxis[2],
             cg_frustum[2].normal);

    VectorScale(cg.refdef.viewaxis[0], xs, cg_frustum[3].normal);
    VectorMA(cg_frustum[3].normal, -xc, cg.refdef.viewaxis[2],
             cg_frustum[3].normal);

    for (i = 0; i < 4; i++) {
        cg_frustum[i].type = PLANE_NON_AXIAL;
        cg_frustum[i].dist =
            DotProduct(cg.refdef.vieworg, cg_frustum[i].normal);
...
qboolean CG_IsEntityVisible(centity_t *cent, vec_t max_sight) {
    ...
    // But, when we want to compare distances and check what entities are
    // farther than the given distance, it works in most of the case.
    ...
    if (cent->currentState.number == cg.predictedPlayerState.groundEntityNum)
        // Keep current player ground entity visible to avoid player fall
        // prediction even if it is not really visible as player ground entity
        // can only be well predicted if it is visible (traces will only detect
        // visible entities when cg_boostfps is set)
        visible = qtrue;
...
=================
*/
qboolean CG_EntityVectors(centity_t *cent, vec3_t origin,
                          vec3_t box_vertex[8]) {
    ...
    entityState_t *ent;
    clipHandle_t cmodel;
    vec3_t angles, r, vertex, rotate_origin = {0.0f, 0.0f, 0.0f}, median,
                              ext_xyz;
    ...
    if (rotate) {
        ...
        // Transpose matrix as it is the entity axis point of view, which
        // rotate. Consider that it is not each bounds that rotate, but the
        // entity axis. So, if the entity axis rotate in one direction, the
        // bounds will be rotate in the opposite, from the entity axis point of
        // view.
        // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        TransposeMatrix(matrix, transpose);
        for (i = 0; i < 8; i++) {
            // Rotate should be done from the rotation origin, not the entity
            // origin
...
qboolean CG_Farclip_None(vec3_t entite_vec3, vec3_t camera_vec3,
                         float farclip_dist, float farclip_alt_dist) {
    return qfalse;
}

qboolean CG_Farclip_Always(vec3_t entite_vec3, vec3_t camera_vec3,
                           float farclip_dist, float farclip_alt_dist) {
    return qtrue;
}

qboolean CG_Farclip_Sphere(vec3_t entite_vec3, vec3_t camera_vec3,
                           float farclip_dist, float farclip_alt_dist) {
    float dx, dy, dz;
...
qboolean CG_Farclip_Cube(vec3_t entite_vec3, vec3_t camera_vec3,
                         float farclip_dist, float farclip_alt_dist) {
    if (abs(entite_vec3[0] - camera_vec3[0]) > farclip_dist)
        return qtrue;
    if (abs(entite_vec3[1] - camera_vec3[1]) > farclip_dist)
        return qtrue;
    if (abs(entite_vec3[2] - camera_vec3[2]) > farclip_dist)
        return qtrue;
    return qfalse;
}

qboolean CG_Farclip_Ellipse_Z(vec3_t entite_vec3, vec3_t camera_vec3,
                              float farclip_dist, float farclip_alt_dist) {
    float dx, dy, dz;
    if ((farclip_alt_dist < 1) || (farclip_dist < 1))
        return qtrue;
    dx = entite_vec3[0] - camera_vec3[0];
    dy = entite_vec3[1] - camera_vec3[1];
...
qboolean CG_Farclip_Cylinder_Z(vec3_t entite_vec3, vec3_t camera_vec3,
                               float farclip_dist, float farclip_alt_dist) {
    float dx, dy;
    if (abs(entite_vec3[2] - camera_vec3[2]) > farclip_alt_dist)
        return qtrue;
    dx = entite_vec3[0] - camera_vec3[0];
    dy = entite_vec3[1] - camera_vec3[1];
...
qboolean CG_Farclip_Box_Z(vec3_t entite_vec3, vec3_t camera_vec3,
                          float farclip_dist, float farclip_alt_dist) {
    if (abs(entite_vec3[0] - camera_vec3[0]) > farclip_dist)
        return qtrue;
    if (abs(entite_vec3[1] - camera_vec3[1]) > farclip_dist)
...
qboolean CG_Farclip_Circle_Infinite_Z(vec3_t entite_vec3, vec3_t camera_vec3,
                                      float farclip_dist,
                                      float farclip_alt_dist) {
    float dx, dy;
    dx = entite_vec3[0] - camera_vec3[0];
    dy = entite_vec3[1] - camera_vec3[1];
    return (dx * dx + dy * dy) > (farclip_dist * farclip_dist);
...
qboolean CG_Farclip_Square_Infinite_Z(vec3_t entite_vec3, vec3_t camera_vec3,
                                      float farclip_dist,
                                      float farclip_alt_dist) {
    if (abs(entite_vec3[0] - camera_vec3[0]) > farclip_dist)
        return qtrue;
...
qboolean CG_Farclip_Cone_Z(vec3_t entite_vec3, vec3_t camera_vec3,
                           float farclip_dist, float farclip_alt_dist) {
    float dx, dy, dz;
    dx = entite_vec3[0] - camera_vec3[0];
    dy = entite_vec3[1] - camera_vec3[1];
    // not usefull to test if farclip_dist > 0, cause it have been tested before
    // calling this function
    dz =
        abs(entite_vec3[2] - camera_vec3[2]) * farclip_alt_dist / farclip_dist +
        farclip_dist;
    return (dx * dx + dy * dy) > (dz * dz);
}

qboolean CG_Farclip_Pyramid_Z(vec3_t entite_vec3, vec3_t camera_vec3,
                              float farclip_dist, float farclip_alt_dist) {
    float dz;
    // not usefull to test if farclip_dist > 0, cause it have been tested before
    // calling this function
    dz =
        abs(entite_vec3[2] - camera_vec3[2]) * farclip_alt_dist / farclip_dist +
        farclip_dist;
    if (abs(entite_vec3[0] - camera_vec3[0]) > dz)
        return qtrue;
    if (abs(entite_vec3[1] - camera_vec3[1]) > dz)
        return qtrue;
    return qfalse;
}

qboolean CG_Farclip_Ellipse_X(vec3_t entite_vec3, vec3_t camera_vec3,
                              float farclip_dist, float farclip_alt_dist) {
    float dx, dy, dz;
    if ((farclip_alt_dist < 1) || (farclip_dist < 1))
        return qtrue;
...
qboolean CG_Farclip_Cylinder_X(vec3_t entite_vec3, vec3_t camera_vec3,
                               float farclip_dist, float farclip_alt_dist) {
    float dy, dz;
    if (abs(entite_vec3[0] - camera_vec3[0]) > farclip_alt_dist)
        return qtrue;
    dy = entite_vec3[1] - camera_vec3[1];
    dz = entite_vec3[2] - camera_vec3[2];
...
qboolean CG_Farclip_Box_X(vec3_t entite_vec3, vec3_t camera_vec3,
                          float farclip_dist, float farclip_alt_dist) {
    if (abs(entite_vec3[0] - camera_vec3[0]) > farclip_alt_dist)
        return qtrue;
    if (abs(entite_vec3[1] - camera_vec3[1]) > farclip_dist)
...
qboolean CG_Farclip_Circle_Infinite_X(vec3_t entite_vec3, vec3_t camera_vec3,
                                      float farclip_dist,
                                      float farclip_alt_dist) {
    float dy, dz;
    dy = entite_vec3[1] - camera_vec3[1];
...
qboolean CG_Farclip_Square_Infinite_X(vec3_t entite_vec3, vec3_t camera_vec3,
                                      float farclip_dist,
                                      float farclip_alt_dist) {
    if (abs(entite_vec3[1] - camera_vec3[1]) > farclip_dist)
        return qtrue;
...
qboolean CG_Farclip_Cone_X(vec3_t entite_vec3, vec3_t camera_vec3,
                           float farclip_dist, float farclip_alt_dist) {
    float dx, dy, dz;
    // not usefull to test if farclip_dist > 0, cause it have been tested before
    // calling this function
    dx =
        abs(entite_vec3[0] - camera_vec3[0]) * farclip_alt_dist / farclip_dist +
        farclip_dist;
    dy = entite_vec3[1] - camera_vec3[1];
    dz = entite_vec3[2] - camera_vec3[2];
...
qboolean CG_Farclip_Pyramid_X(vec3_t entite_vec3, vec3_t camera_vec3,
                              float farclip_dist, float farclip_alt_dist) {
    float dx;
    // not usefull to test if farclip_dist > 0, cause it have been tested before
    // calling this function
    dx =
        abs(entite_vec3[0] - camera_vec3[0]) * farclip_alt_dist / farclip_dist +
        farclip_dist;
    if (abs(entite_vec3[1] - camera_vec3[1]) > dx)
        return qtrue;
    if (abs(entite_vec3[2] - camera_vec3[2]) > dx)
        return qtrue;
    return qfalse;
}

qboolean CG_Farclip_Ellipse_Y(vec3_t entite_vec3, vec3_t camera_vec3,
                              float farclip_dist, float farclip_alt_dist) {
    float dx, dy, dz;
    if ((farclip_alt_dist < 1) || (farclip_dist < 1))
        return qtrue;
...
qboolean CG_Farclip_Cylinder_Y(vec3_t entite_vec3, vec3_t camera_vec3,
                               float farclip_dist, float farclip_alt_dist) {
    float dx, dz;
    if (abs(entite_vec3[1] - camera_vec3[1]) > farclip_alt_dist)
        return qtrue;
    dx = entite_vec3[0] - camera_vec3[0];
    dz = entite_vec3[2] - camera_vec3[2];
...
qboolean CG_Farclip_Box_Y(vec3_t entite_vec3, vec3_t camera_vec3,
                          float farclip_dist, float farclip_alt_dist) {
    if (abs(entite_vec3[0] - camera_vec3[0]) > farclip_dist)
        return qtrue;
    if (abs(entite_vec3[1] - camera_vec3[1]) > farclip_alt_dist)
...
qboolean CG_Farclip_Circle_Infinite_Y(vec3_t entite_vec3, vec3_t camera_vec3,
                                      float farclip_dist,
                                      float farclip_alt_dist) {
    float dx, dz;
    dx = entite_vec3[0] - camera_vec3[0];
...
qboolean CG_Farclip_Square_Infinite_Y(vec3_t entite_vec3, vec3_t camera_vec3,
                                      float farclip_dist,
                                      float farclip_alt_dist) {
    if (abs(entite_vec3[0] - camera_vec3[0]) > farclip_dist)
        return qtrue;
...
qboolean CG_Farclip_Cone_Y(vec3_t entite_vec3, vec3_t camera_vec3,
                           float farclip_dist, float farclip_alt_dist) {
    float dx, dy, dz;
    dx = entite_vec3[0] - camera_vec3[0];
    // not usefull to test if farclip_dist > 0, cause it have been tested before
    // calling this function
    dy =
        abs(entite_vec3[1] - camera_vec3[1]) * farclip_alt_dist / farclip_dist +
        farclip_dist;
    dz = entite_vec3[2] - camera_vec3[2];
    return (dx * dx + dz * dz) > (dy * dy);
}

qboolean CG_Farclip_Pyramid_Y(vec3_t entite_vec3, vec3_t camera_vec3,
                              float farclip_dist, float farclip_alt_dist) {
    float dy;
    // not usefull to test if farclip_dist > 0, cause it have been tested before
    // calling this function
    dy =
        abs(entite_vec3[1] - camera_vec3[1]) * farclip_alt_dist / farclip_dist +
        farclip_dist;
    if (abs(entite_vec3[0] - camera_vec3[0]) > dy)
        return qtrue;
    if (abs(entite_vec3[2] - camera_vec3[2]) > dy)
        return qtrue;
    return qfalse;
}
...
qboolean (*CG_Farclip_Tester[])(vec3_t, vec3_t, float,
                                float) = {CG_Farclip_None,
                                          CG_Farclip_Always,
                                          CG_Farclip_Sphere,
