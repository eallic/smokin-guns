...
static void CG_Speaker(centity_t *cent) {
    if (!cent->currentState
             .clientNum) { // FIXME: use something other than clientNum...
...
static void CG_Missile(centity_t *cent) {
    ...
    /*
       ...
       weapon->missileDlight, weapon->missileDlightColor[col][0],
       weapon->missileDlightColor[col][1], weapon->missileDlightColor[col][2] );
            }
    */
    ...
    // add dynamic light
    if (weapon->missileDlight && s1->weapon != WP_DYNAMITE &&
        s1->weapon != WP_MOLOTOV) {
    ...
    if (s1->weapon == WP_DYNAMITE && (int)s1->apos.trDelta[0]) {
        ...
        const int num =
            bg_weaponlist[WP_DYNAMITE].animations[WP_ANIM_SPECIAL].numFrames;
...
static void CG_Smoke(centity_t *cent) {
    ...
    for (x = 0; x < x_num; x++) {
        ...
        for (y = 0; y < y_num; y++) {
            ...
            le = CG_AddSmokeParticle(origin, size, size / 4, lifetime, boost,
                                     wind, winddir);
...
static void CG_Flare(centity_t *cent) {
    ...
    if (cgs.media.flare) {
        ...
        if (cg_boostfps.integer) {
            ...
            CG_Trace_Flare(&trace, cg.refdef.vieworg, vec3_origin, vec3_origin,
                           s1->origin, cg.snap->ps.clientNum, MASK_SHOT,
                           cent->distFromCamera);
            // FPS Optimization ?
        } else {
            ...
            CG_Trace(&trace, cg.refdef.vieworg, vec3_origin, vec3_origin,
                     cent->currentState.origin, cg.snap->ps.clientNum,
                     MASK_SHOT);
...
static void CG_Mover(centity_t *cent) {
    ...
    int farclip_type =
        ...
        127; // only 7 lower bits of powerups is used for farclipping
    int invert_clip =
        cent->currentState.powerups & 128; // if it's a close clipping
    // fast multiply by 64:
    ...
    if (!cg_farclip.integer || farclip_dist < 1) {
        ...
        if (cg.zoomed)
            farclip_factor = cg_farclipZoomValue.value;
        else
            farclip_factor = cg_farclipValue.value;
        if (farclip_factor < 1)
            farclip_factor = 1;
    ...
    if (cent->currentState.powerups & MAPLOD_BINARY_MASK) {
        ...
        if (cent->currentState.powerups & MAPLOD_GTE_BINARY_MASK) {
            if (cg_mapLOD.integer < objectLOD)
                return;
        } else if (cg_mapLOD.integer > objectLOD)
            return;
    }
    ...
    if ((farclip_type + invert_clip > FARCLIP_NONE) &&
        (farclip_type < CG_Farclip_Tester_Table_Size)) {
        if (!invert_clip &&
            CG_Farclip_Tester[farclip_type](s1->origin, cg.refdef.vieworg,
                                            farclip_dist, farclip_alt_dist))
            return;
        else if (invert_clip && !CG_Farclip_Tester[farclip_type](
                                    s1->origin, cg.refdef.vieworg, farclip_dist,
                                    farclip_alt_dist))
            return;
    }
...
void CG_AdjustPositionForMover(const vec3_t in, int moverNum, int fromTime,
                               int toTime, vec3_t out, vec3_t angles_in,
                               vec3_t angles_out) {
    centity_t *cent;
...
//  players will always be TR_INTERPOLATE
    ...
    if (cent->currentState.eType ==
        ET_MISSILE /*&& cent->currentState.weapon != WP_GRAPPLING_HOOK*/) {
        // if it's one of ours
        if (cent->currentState.otherEntityNum == cg.clientNum) {
            // extrapolate one server frame's worth - this will correct for tiny
            // visual inconsistencies introduced by backward-reconciling all
            // players one server frame before running projectiles
        ...
        // else if ( cent->currentState.weapon != WP_GRENADE_LAUNCHER ) {
                 ...
                 cent->currentState.weapon != WP_MOLOTOV) {
    ...
    BG_EvaluateTrajectory(&cent->currentState.pos, cg.time + timeshift,
                          cent->lerpOrigin);
    BG_EvaluateTrajectory(&cent->currentState.apos, cg.time + timeshift,
                          cent->lerpAngles);
    ...
    if (timeshift != 0) {
        ...
        if (tr.fraction < 1.0f) {
            cent->lerpOrigin[0] =
                lastOrigin[0] +
                tr.fraction * (cent->lerpOrigin[0] - lastOrigin[0]);
            cent->lerpOrigin[1] =
                lastOrigin[1] +
                tr.fraction * (cent->lerpOrigin[1] - lastOrigin[1]);
            cent->lerpOrigin[2] =
                lastOrigin[2] +
                tr.fraction * (cent->lerpOrigin[2] - lastOrigin[2]);
        }
    }
// unlagged - projectile nudge
    ...
    if (cent != &cg.predictedPlayerEntity) {
        CG_AdjustPositionForMover(
            cent->lerpOrigin, cent->currentState.groundEntityNum,
...
static void CG_Intermission(centity_t *cent) {
    ...
    // CG_Printf("%f %f %f\n", cg_entities[i].lerpOrigin[0],
    // cg_entities[i].lerpOrigin[1], 	cg_entities[i].lerpOrigin[2]);
}
...
void CG_AddPacketEntities(void) {
    ...
    if (cg.nextSnap) {
        ...
        cg.frameInterpolation =
            0; // actually, it should never be used, because
