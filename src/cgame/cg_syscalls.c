...
void trap_Print(const char *fmt) { syscall(CG_PRINT, fmt); }
...
int trap_Argc(void) { return syscall(CG_ARGC); }
...
void trap_FS_FCloseFile(fileHandle_t f) { syscall(CG_FS_FCLOSEFILE, f); }
...
void trap_UpdateScreen(void) { syscall(CG_UPDATESCREEN); }
...
void trap_CM_TransformedBoxTrace(trace_t *results, const vec3_t start,
                                 ...
                                 const vec3_t angles) {
    syscall(CG_CM_TRANSFORMEDBOXTRACE, results, start, end, mins, maxs, model,
...
void trap_R_ClearScene(void) { syscall(CG_R_CLEARSCENE); }
...
int trap_R_CullPointAndRadius(vec3_t pt, float radius) {
    return syscall(CG_R_CULL_SPHERE, pt, PASSFLOAT(radius));
}
