...
// void CG_ShotgunPattern( vec3_t origin, vec3_t origin2, int seed, int
// otherEntNum ); void CG_Bullet( vec3_t end, int sourceEntityNum, vec3_t
// normal, qboolean flesh, int fleshEntityNum );
...
SG comment by torhu:
This has not been adapted for sg, since the hit calculations are much more
complicated than in vanilla q3, due to the locational damage etc.  So this
function is not being used. I doesn't matter that much, since it doesn't affect
the actual hitscan delagging, only weapon impact effects (bullet holes, possibly
player anims and blood sprays).
=======================
*/
#if 0
...
================
*/
qboolean CG_Cvar_ClampInt(const char *name, vmCvar_t *vmCvar, int min,
                          int max) {
