...
typedef enum { MENU_NONE, MENU_BUY, MENU_DUEL_FINISHED } menutype_t;
...
typedef struct weaponInfo_s {
    ...
    qhandle_t handsModel; // the hands don't actually draw, they just position
                          // the weapon
    qhandle_t
        l_weaponModel; // "real" hands-model, first person view, left handed
...
typedef struct {
    ...
    qboolean intermissionStarted; // don't play voice rewards, because game will
                                  // end shortly
    ...
    float
        frameInterpolation; // (float)( cg.time - cg.frame->serverTime ) /
...
void UI_DrawProportionalString2(int x, int y, const char *str, vec4_t color,
                                float sizeScale, qhandle_t charset);
...
int CG_Trace_New(trace_t *result, const vec3_t start, const vec3_t mins,
                 const vec3_t maxs, const vec3_t end, int skipNumber, int mask);
void CG_Trace2(trace_t *result, const vec3_t start, const vec3_t mins,
               const vec3_t maxs, const vec3_t end, int skipNumber, int mask);
...
void CG_MissileHitWall(int weapon, int clientNum, vec3_t origin, vec3_t dir,
                       ...
                       qboolean fire, vec3_t dirs[ALC_COUNT], int entityNum);
void CG_MissileHitPlayer(int weapon, vec3_t origin, vec3_t dir, int entityNum);
void CG_ShotgunFire(entityState_t *es);
void CG_Bullet(vec3_t origin, int sourceEntityNum, vec3_t normal,
               qboolean flesh, int fleshEntityNum, int weapon, int surfaceFlags,
               int shaderNum, int entityNum);
...
localEntity_t *CG_MakeExplosion(vec3_t origin, vec3_t dir, qhandle_t hModel,
                                qhandle_t shader, int msec,
                                qboolean isSprite, float radius,
                                qboolean smokeexp);
...
void trap_S_StartBackgroundTrack(const char *intro,
                                 const char *loop); // empty name stops music
...
localEntity_t *CG_LaunchSpriteParticle(vec3_t origin, vec3_t dir,
                                       /*int sound,*/ int bouncefactor,
                                       int duration, int leFlags);

void BuildHitModel(hit_data_t *data, vec3_t origin, vec3_t angles,
                   vec3_t torso_angles, vec3_t head_angles, int frame_upper,
                   int oldframe_upper, float backlerp_upper, int frame_lower,
                   int oldframe_lower, float backlerp_lower);
...
typedef enum { SYSTEM_PRINT, CHAT_PRINT, TEAMCHAT_PRINT } q3print_t;
