...
void CG_DrawLoadingStage(void) {
    ...
    loaded = loadingstage + loadingPlayerIconCount * 1.5 +
             loadingItemIconCount * 1.5;
    ...
    CG_FillRect(DISTANCE + 2 * LINE, 480 - DISTANCE - 2 * LINE - LOADING_HEIGHT,
                progress, LOADING_HEIGHT, color);

    // lines

    // bottom
    CG_FillRect(DISTANCE, 480 - DISTANCE - LINE, 640 - 2 * DISTANCE, LINE,
                color);
    // top
    CG_FillRect(DISTANCE, 480 - DISTANCE - 4 * LINE - LOADING_HEIGHT,
                640 - 2 * DISTANCE, LINE, color);
    // left
    CG_FillRect(DISTANCE, 480 - DISTANCE - 3 * LINE - LOADING_HEIGHT, LINE,
                LOADING_HEIGHT + 2 * LINE, color);
    // right
    CG_FillRect(640 - DISTANCE - LINE,
                480 - DISTANCE - 3 * LINE - LOADING_HEIGHT, LINE,
                LOADING_HEIGHT + 2 * LINE, color);
}
...
void CG_LoadingClient(int clientNum) {
    ...
    if (loadingPlayerIconCount < MAX_LOADING_PLAYER_ICONS) {
        ...
        Com_sprintf(iconName, MAX_QPATH, "models/wq3_players/%s/icon_%s.tga",
                    model, skin);
        ...
        if (!loadingPlayerIcons[loadingPlayerIconCount]) {
            Com_sprintf(iconName, MAX_QPATH,
                        "models/wq3_players/%s/icon_%s.tga", DEFAULT_MODEL,
                        "red");
            loadingPlayerIcons[loadingPlayerIconCount] =
                trap_R_RegisterShaderNoMip(iconName);
        }
    ...
    CG_LoadingString(personality); // patch colored loading screen: draw names
...
void CG_DrawInformation(void) {
    ...
    if (cg.infoScreenText[0]) {
        UI_DrawProportionalString(320, 454,
                                  va("Loading... %s", cg.infoScreenText),
    ...
    if (!atoi(buf)) {
        ...
        // Q_CleanStr(buf);
        UI_DrawProportionalString(320, y, buf, UI_CENTER | UI_DROPSHADOW,
                                  colorWhite);
        y += PROP_HEIGHT + 10;
    ...
    if (s[0]) {
        UI_DrawProportionalString(
            320, y, s, UI_CENTER | UI_SMALLFONT | UI_DROPSHADOW, colorWhite);
        y += PROP_HEIGHT;
    }
    ...
    if (s[0] == '1') {
        ...
        UI_DrawProportionalString(
            320, y,
            S_COLOR_RED "CHEATS ARE ENABLED!", // patch colored loading screen
