...
typedef struct {
    ...
    entityState_t
        entities[MAX_ENTITIES_IN_SNAPSHOT]; // at the time of this snapshot
...
typedef enum {
    ...
    // time. If demoPlayback is set, local movement prediction will not be
    // enabled
...
typedef struct trackInfo_s {
    char path[64];
    int length;
    int replays; // how many times should it be played?
    qboolean played;
} trackInfo_t;
