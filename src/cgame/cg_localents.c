...
A fragment localentity interacts with the environment in some way (hitting
walls), or generates more localentities along a trail.
...
void CG_BloodTrail(localEntity_t *le) {
    ...
    if (t <= cg.time && t > cg.oldTime) {
        ...
        blood = CG_LaunchSpriteParticle(newOrigin, newDelta, 0, 2000,
                                        (LEF_PARTICLE | LEF_TRAIL));

        if (!whiskey) {
            blood->refEntity.customShader =
                cgs.media.blood_particles[rand() % 6];
...
void CG_ReflectVelocity(localEntity_t *le, trace_t *trace) {
    ...
    // check for stop, making sure that even on low FPS systems it doesn't
    // bobble
    if (trace->allsolid ||
...
void CG_CreateFire(vec3_t origin, vec3_t normal) {
    ...
    CG_ImpactMark(cgs.media.burnMarkShader, origin, normal, random() * 360, red,
                  green, blue, 1, qtrue, (rand() % 8) + 25, qfalse,
                  WHISKEY_SICKERTIME);
}
...
void CG_AddFragment(localEntity_t *le) {
    ...
    if (cg_boostfps.integer) {
        ...
        CG_Trace_Fragment(&trace, le->refEntity.origin, NULL, NULL, newOrigin,
                          -1, CONTENTS_SOLID);
        // FPS Optimization ?
    } else
    ...
    if (trace.fraction == 1.0 || (le->leFlags & LEF_SPARKS)) {
        ...
        if (le->leFlags & LEF_TUMBLE) {
            ...
            if ((le->leFlags & LEF_BREAKS) || (le->leFlags & LEF_BREAKS_DEF)) {
                VectorScale(le->refEntity.axis[0], le->vector[0],
                            le->refEntity.axis[0]);
                VectorScale(le->refEntity.axis[1], le->vector[1],
                            le->refEntity.axis[1]);
                VectorScale(le->refEntity.axis[2], le->vector[2],
                            le->refEntity.axis[2]);
            }
        ...
        if (le->leFlags & LEF_FIRE) {
            ...
            if (le->lightColor[0] == 1) {
                ...
                trap_R_AddLightToScene(
                    origin, 50, cg_weapons[WP_MOLOTOV].missileDlightColor[0],
                    cg_weapons[WP_MOLOTOV].missileDlightColor[1],
                    cg_weapons[WP_MOLOTOV].missileDlightColor[2]);
            }
        }
    ...
    if (le->leFlags & LEF_BLOOD) {
        ...
        if ((le->leFlags & LEF_WHISKEY) && (le->leFlags & LEF_FIRE)) {
            ...
            // male a black burn shader
            CG_ImpactMark(cgs.media.burnMarkShader, trace.endpos,
                          trace.plane.normal, random() * 360, red, green, blue,
                          1, qtrue, (rand() % 8) + 25, qfalse, time);
        } else {
            CG_ImpactMark(shader, trace.endpos, trace.plane.normal,
                          random() * 360, red, green, blue, 1, qtrue, radius,
                          qfalse, time);
            CG_FreeLocalEntity(le);
        }
        return;
...
void CG_AddSmoke(localEntity_t *le) {
    ...
    // if there's wind
    if ((le->vector[0] || le->vector[1] || le->vector[2]) && le->wind) {
...
static void CG_AddMoveScaleFade(localEntity_t *le) {
    ...
    if (le->fadeInTime > le->startTime && cg.time < le->fadeInTime) {
        // fade / grow time
        c = 1.0 - (float)(le->fadeInTime - cg.time) /
                      (le->fadeInTime - le->startTime);
    } else {
        // fade / grow time
        c = (le->endTime - cg.time) * le->lifeRate;
    }
...
static void CG_AddExplosion(localEntity_t *ex) {
    ...
    if (ex->light) {
        ...
        light =
            (float)(cg.time - ex->startTime) / (ex->endTime - ex->startTime);
        if (light < 0.5) {
            light = 1.0;
        } else {
        ...
        trap_R_AddLightToScene(ent->origin, light, ex->lightColor[0],
...
static void CG_AddSpriteExplosion(localEntity_t *le) {
    ...
    if (le->light) {
        ...
        light =
            (float)(cg.time - le->startTime) / (le->endTime - le->startTime);
        if (light < 0.5) {
            light = 1.0;
        } else {
            light = 1.0 - (light - 0.5) * 2;
        ...
        trap_R_AddLightToScene(re.origin, light, le->lightColor[0],
