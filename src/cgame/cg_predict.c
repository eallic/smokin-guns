...
====================
*/
// if light is set to 1, movers will be taken into recognition when being a
// spectator
void CG_BuildSolidList(qboolean light) {
    ...
    for (i = 0; i < snap->numEntities; i++) {
        ...
        if (ent->eType == ET_ITEM || ent->eType == ET_PUSH_TRIGGER ||
            ent->eType == ET_TELEPORT_TRIGGER || ent->eType == ET_ESCAPE ||
...
void CG_BuildVisibleSolidList(qboolean light) {
    ...
    for (i = 0; i < snap->numEntities; i++) {
        ...
        if (cent->nextState.solid && CG_IsEntityVisible(cent, 0)) {
...
====================
*/
static void CG_ClipMoveEntity(centity_t *cent, const vec3_t start,
                              const vec3_t mins, const vec3_t maxs,
                              const vec3_t end, int mask, trace_t *tr) {
    int x, zd, zu;
    entityState_t *ent;
    clipHandle_t cmodel;
    vec3_t bmins, bmaxs;
    ...
    trap_CM_TransformedBoxTrace_New(tr, start, end, mins, maxs, cmodel, mask,
                                    origin, angles);
}
...
====================
*/
static void CG_ClipMoveToVisibleEntities(const vec3_t start, const vec3_t mins,
...
Ignore solid entities if their bounding sphere does not contain "start" and
"end".
====================
*/
static void CG_ClipMoveToNearEntities(const vec3_t start, const vec3_t mins,
                                      const vec3_t maxs, const vec3_t end,
                                      int skipNumber, int mask, trace_t *tr) {
    int i;
    trace_t trace;
...
static void CG_ClipMoveToEntities(const vec3_t start, const vec3_t mins,
    ...
    if (cg_boostfps.integer) {
        // FPS Optimization ?
        CG_ClipMoveToVisibleEntities(start, mins, maxs, end, skipNumber, mask,
                                     tr, 0.0);
        return;
        // FPS Optimization ?
    }
    ...
    for (i = 0; i < cg_numSolidEntities; i++) {
        ...
        if (ent->solid == SOLID_BMODEL) {
            ...
            cmodel = trap_CM_InlineModel(ent->modelindex);
            VectorCopy(cent->lerpAngles, angles);
            BG_EvaluateTrajectory(&cent->currentState.pos, cg.physicsTime,
                                  origin);
        } else {
            // encoded bbox
            x = (ent->solid & 255);
        ...
        trap_CM_TransformedBoxTrace_New(
            &trace, start, end,
...
================
*/
int CG_Trace_New(trace_t *result, const vec3_t start, const vec3_t mins,
...
================
*/
int CG_Trace_Visible(trace_t *result, const vec3_t start, const vec3_t mins,
                     const vec3_t maxs, const vec3_t end, int skipNumber,
                     int mask, vec_t max_distance) {
    trace_t t;
    int shaderNum;
    ...
    CG_ClipMoveToVisibleEntities(start, mins, maxs, end, skipNumber, mask, &t,
                                 max_distance);
...
================
*/
int CG_Trace_Flare(trace_t *result, const vec3_t start, const vec3_t mins,
                   const vec3_t maxs, const vec3_t end, int skipNumber,
                   int mask, vec_t max_distance) {
    trace_t t;
    int shaderNum;
...
Same as CG_Trace_Visible.
If vec3_t "start" and vec3_t "end" are both outside a solid entity bounding
sphere, then ignore this entity.

We can do this kind of test, because "start" and "end" are (should be ?) very
closed, when this function is called.
================
*/
int CG_Trace_Fragment(trace_t *result, const vec3_t start, const vec3_t mins,
                      const vec3_t maxs, const vec3_t end, int skipNumber,
                      int mask) {
    trace_t t;
    int shaderNum;
...
int CG_PointContentsVisible(const vec3_t point, int passEntityNum) {
    ...
    for (i = 0; i < cg_numVisibleSolidEntities; i++) {
        ...
        contents |= trap_CM_TransformedPointContents(point, cmodel, ent->origin,
                                                     ent->angles);
    }
...
static void CG_TouchItem(centity_t *cent) {
    ...
    if (item->giType == IT_POWERUP && item->giTag == PW_GOLD) {
        ...
        CG_Trace_New(&tr, muzzle, NULL, NULL, cent->lerpOrigin,
    ...
    if (Q_stricmp(item->classname, "pickup_money")) {
        ...
        BG_AddPredictableEventToPlayerstate(EV_MONEY_PICKUP,
    ...
    // if it's a weapon, give them some predicted ammo so the autoswitch will
    // work
...
static void CG_TouchTriggerPrediction(void) {
    ...
    for (i = 0; i < cg_numTriggerEntities; i++) {
        ...
        trap_CM_BoxTrace_New(&trace, cg.predictedPlayerState.origin,
...
static int IsUnacceptableError(playerState_t *ps, playerState_t *pps) {
    ...
    for (i = 0; i < MAX_STATS; i++) {
        if (pps->stats[i] != ps->stats[i]) {
            // Tequila comment:
            // IDLE_TIMER is only used to handle WP_ANIM_IDLE, so that's
            // acceptable
            if (i != IDLE_TIMER)
                return 15;
        }
    ...
    if (cg_optimizePrediction.integer && !cg_latentCmds.integer) {
        ...
        // if it's the same as the last one
            ...
            for (i = cg.stateHead; i != cg.stateTail;
                ...
                // if we find a predicted state whose commandTime matches the
                // snapshot player state's commandTime
                if (cg.savedPmoveStates[i].commandTime ==
                    cg.predictedPlayerState.commandTime) {
                    // make sure the state differences are acceptable
                    int errorcode = IsUnacceptableError(
                        &cg.predictedPlayerState, &cg.savedPmoveStates[i]);
                    ...
                    // this one is almost exact, so we'll copy it in as the
                    // starting point
    ...
    for (cmdNum = current - CMD_BACKUP + 1; cmdNum <= current; cmdNum++) {
        ...
        if (cg.predictedPlayerState.commandTime == oldPlayerState.commandTime) {
            ...
            if (cg.thisFrameTeleport) {
                ...
                // If ground entity has changed and cg_boostfps is set, be sure
                // it is in visible entity list
                if (cg_boostfps.integer &&
                    oldPlayerState.groundEntityNum !=
                        cg.predictedPlayerState.groundEntityNum) {
                    centity_t *cent =
                        &cg_entities[cg.predictedPlayerState.groundEntityNum];
                    if (!cent->visible) {
                        cg_VisibleSolidEntities[cg_numVisibleSolidEntities] =
                            cent;
                        cg_numVisibleSolidEntities++;
                    }
                }
        ...
        if (cg.introend >= cg.time && cgs.gametype == GT_DUEL &&
            ...
            cg_pmove.ps->stats[STAT_OLDWEAPON] = 0;
                   ...
                   (cg.time < cg.roundstarttime + cgs.roundNoMoveTime) &&
                   (cg.predictedPlayerState.persistant[PERS_TEAM] <
                    TEAM_SPECTATOR)) {
            // added by Joe Kari: delete all movement and cmd stats until the
            // end of the countdown in RTP and BR gametype
            cg_pmove.ps->speed = 0;
            cg_pmove.cmd.forwardmove = 0;
            cg_pmove.cmd.rightmove = 0;
            cg_pmove.cmd.upmove = 0;
        ...
        if (cgs.gametype == GT_DUEL &&
            cg_pmove.ps->stats[STAT_FLAGS] & SF_DU_WON)
        ...
        if (cg.predictedPlayerState.persistant[PERS_SPAWN_COUNT] !=
            oldPlayerState.persistant[PERS_SPAWN_COUNT]) {
        ...
        //  we check for cg_latentCmds because it'll mess up the optimization
        if (cg_optimizePrediction.integer && !cg_latentCmds.integer) {
            // if we need to predict this command, or we've run out of space in
            // the saved states queue
            if (cmdNum >= predictCmd ||
                ...
                if ((stateIndex + 1) % NUM_SAVED_STATES != cg.stateHead) {
                    // save the state for the false case (of cmdNum >=
                    // predictCmd) in later calls to this function
                ...
                if (cg_showmiss.integer &&
                    cg.savedPmoveStates[stateIndex].commandTime !=
                        cg_pmove.cmd.serverTime) {
...
// unlagged - optimized prediction
    ...
    if (cg_showmiss.integer) {
        if (cg.predictedPlayerState.eventSequence >
            oldPlayerState.eventSequence + MAX_PS_EVENTS) {
