...
void CG_BuyMenu(void) {
    ...
    /* as suggested by The Doctor, this test shouldn't be done in cgame, it is
    already done in game, it removes a bug
    ...
    */
    ...
    /*if(cg.snap->ps.weapon == WP_GATLING &&
       cg.snap->ps.stats[STAT_GATLING_MODE]) return;*/ // weapon change isn't activated anymore when buying and having a gatling
...
static void CG_NextTeamMember_f(void) { CG_SelectNextPlayer(); }

static void CG_PrevTeamMember_f(void) { CG_SelectPrevPlayer(); }

// ASS U ME's enumeration order as far as task specific orders, OFFENSE is zero,
// CAMP is last
//
...
static void CG_EditHud_f( void ) {
  ...
  //VM_Call (cgvm, CG_EVENT_HANDLING, (cls.keyCatchers & KEYCATCH_CGAME) ?
CGAME_EVENT_EDITHUD : CGAME_EVENT_NONE);
}
*/
...
void CG_InitConsoleCommands(void) {
    ...
    //
    // the game server will interpret these commands, which will be
    // automatically forwarded to the server after they are not recognized
    // locally
    //
