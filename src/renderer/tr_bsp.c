...
static void R_LoadLightmaps(lump_t *l) {
    ...
    if (tr.numLightmaps == 1) {
        ...
        // reason. this avoids this, but isn't the correct solution.
        tr.numLightmaps++;
    }
    ...
    for (i = 0; i < tr.numLightmaps; i++) {
        ...
        if (r_lightmap->integer ==
            2) { // color code by intensity as development tool	(FIXME: check
                 // range)
        ...
        tr.lightmaps[i] = R_CreateImage(
            va("*lightmap%d", i), image, LIGHTMAP_SIZE, LIGHTMAP_SIZE,
            IMGTYPE_COLORALPHA,
...
=================
*/
void RE_SetWorldVisData(const byte *vis) { tr.externalVisData = vis; }
...
===============
*/
static void ParseFace(dsurface_t *ds, drawVert_t *verts, msurface_t *surf,
                      int *indexes) {
    int i, j;
    srfSurfaceFace_t *cv;
...
===============
*/
static void ParseTriSurf(dsurface_t *ds, drawVert_t *verts, msurface_t *surf,
                         int *indexes) {
    srfTriangles_t *tri;
    int i, j;
            ...
            for (k = 1; k < grid1->width - 1; k++) {
                for (m = 0; m < 2; m++) {
                    ...
                    for (l = 1; l < grid2->width - 1; l++) {
                        ...
                        if (fabs(grid1->verts[k + offset1].xyz[2] -
                            ...
                            continue;
                        // ok the points are equal and should have the same lod
                        // error
                ...
                for (m = 0; m < 2; m++) {
                    ...
                    for (l = 1; l < grid2->height - 1; l++) {
                        //
                        if (fabs(grid1->verts[k + offset1].xyz[0] -
                                 grid2->verts[grid2->width * l + offset2]
                                     .xyz[0]) > .1)
                            continue;
                        if (fabs(grid1->verts[k + offset1].xyz[1] -
                                 grid2->verts[grid2->width * l + offset2]
        ...
        for (n = 0; n < 2; n++) {
            ...
            for (k = 1; k < grid1->height - 1; k++) {
                for (m = 0; m < 2; m++) {
                    ...
                    for (l = 1; l < grid2->width - 1; l++) {
                        //
                        if (fabs(grid1->verts[grid1->width * k + offset1]
                                     .xyz[0] -
                                 grid2->verts[l + offset2].xyz[0]) > .1)
                            continue;
                        if (fabs(grid1->verts[grid1->width * k + offset1]
...
for the highest LoD. If this is not the case this function will still do its job
but won't fix the highest LoD cracks.
=================
*/
            ...
            for (m = 0; m < 2; m++) {
                ...
                for (l = 0; l < grid2->width - 1; l++) {
                    ...
                    if (fabs(v1[0] - v2[0]) < .01 &&
                        ...
                        continue;
                    //
                    // ri.Printf( PRINT_ALL, "found highest LoD crack between
                    // two patches\n" );
                    ...
                    grid2 = R_GridInsertColumn(
                        grid2, l + 1, row, grid1->verts[k + 1 + offset1].xyz,
...
side of the patch (on the same row or column) the vertices will not be joined
and cracks might still appear at that side.
===============
*/
...
void R_MovePatchSurfacesToHunk(void) {
    ...
    for (i = 0; i < s_worldData.numsurfaces; i++) {
        ...
        if (grid->surfaceType != SF_GRID)
            continue;
        //
        size = (grid->width * grid->height - 1) * sizeof(drawVert_t) +
               sizeof(*grid);
        hunkgrid = ri.Hunk_Alloc(size, h_low);
        Com_Memcpy(hunkgrid, grid, size);

        hunkgrid->widthLodError = ri.Hunk_Alloc(grid->width * 4, h_low);
        Com_Memcpy(hunkgrid->widthLodError, grid->widthLodError,
                   grid->width * 4);

        hunkgrid->heightLodError = ri.Hunk_Alloc(grid->height * 4, h_low);
        Com_Memcpy(hunkgrid->heightLodError, grid->heightLodError,
                   grid->height * 4);
...
static void R_LoadNodesAndLeafs(lump_t *nodeLump, lump_t *leafLump) {
    ...
    for (i = 0; i < numLeafs; i++, inLeaf++, out++) {
        ...
        out->firstmarksurface =
            s_worldData.marksurfaces + LittleLong(inLeaf->firstLeafSurface);
...
static void R_LoadFogs(lump_t *l, lump_t *brushesLump, lump_t *sidesLump) {
    ...
    for (i = 0; i < count; i++, fogs++) {
        ...
        out->colorInt =
            ColorBytes4(shader->fogParms.color[0] * tr.identityLight,
                        shader->fogParms.color[1] * tr.identityLight,
                        shader->fogParms.color[2] * tr.identityLight, 1.0);

        d = shader->fogParms.depthForOpaque < 1
                ? 1
...
void R_LoadEntities(lump_t *l) {
    ...
    while (1) {
        ...
        if (!Q_stricmp(keyname, "gridsize")) {
            sscanf(value, "%f %f %f", &w->lightGridSize[0],
                   &w->lightGridSize[1], &w->lightGridSize[2]);
            continue;
        }
    }
}
