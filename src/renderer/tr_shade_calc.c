...
void RB_CalcDeformNormals(deformStage_t *ds) {
    ...
    for (i = 0; i < tess.numVertexes; i++, xyz += 4, normal += 4) {
        ...
        scale = 0.98f;
        scale =
            R_NoiseGet4f(100 + xyz[0] * scale, xyz[1] * scale, xyz[2] * scale,
                         tess.shaderTime * ds->deformationWave.frequency);
        normal[1] += ds->deformationWave.amplitude * scale;

        scale = 0.98f;
        scale =
            R_NoiseGet4f(200 + xyz[0] * scale, xyz[1] * scale, xyz[2] * scale,
                         tess.shaderTime * ds->deformationWave.frequency);
...
void RB_CalcBulgeVertexes(deformStage_t *ds) {
    ...
    for (i = 0; i < tess.numVertexes; i++, xyz += 4, st += 4, normal += 4) {
        ...
        off = (float)(FUNCTABLE_SIZE / (M_PI * 2)) *
              (st[0] * ds->bulgeWidth + now);
...
void RB_CalcMoveVertexes(deformStage_t *ds) {
    ...
    scale = WAVEVALUE(table, ds->deformationWave.base,
                      ds->deformationWave.amplitude, ds->deformationWave.phase,
...
void DeformText(const char *text) {
    ...
    for (i = 0; i < len; i++) {
        ...
        if (ch != ' ') {
            ...
            RB_AddQuadStampExt(origin, width, height, color, fcol, frow,
                               fcol + size, frow + size);
        }
        ...
        for (j = 0; j < 2; j++) {
            ...
            for (k = 0; k < 5; k++) {
                if (tess.indexes[indexes + k] == i + edgeVerts[nums[j]][0] &&
                    tess.indexes[indexes + k + 1] ==
                        i + edgeVerts[nums[j]][1]) {
                    break;
                }
            }
...
void RB_CalcColorFromOneMinusEntity(unsigned char *dstColors) {
    ...
    invModulate[3] =
        255 -
        backEnd.currentEntity->e
            .shaderRGBA[3]; // this trashes alpha, but the AGEN block fixes it
...
void RB_CalcWaveColor(const waveForm_t *wf, unsigned char *dstColors) {
    ...
    if (wf->func == GF_NOISE) {
        glow = wf->base +
               R_NoiseGet4f(0, 0, 0,
                            (tess.shaderTime + wf->phase) * wf->frequency) *
                   wf->amplitude;
    } else {
...
void RB_CalcFogTexCoords(float *st) {
    ...
    if (fog->hasSurface) {
        ...
        eyeT = DotProduct(backEnd.or.viewOrigin, fogDepthVector) +
               fogDepthVector[3];
    } else {
    ...
    for (i = 0, v = tess.xyz[0]; i < tess.numVertexes; i++, v += 4) {
        ...
        if (eyeOutside) {
            if (t < 1.0) {
                ...
                t = 1.0 / 32 +
                    30.0 / 32 * t /
                        (t - eyeT); // cut the distance at the fog plane
            }
        } else {
            if (t < 0) {
                t = 1.0 / 32; // point is outside, so no fogging
...
// Joe Kari: This new function allow custom projection axis for tcgen
// environment
...
void RB_CalcCustomEnvironmentTexCoords(float *st, vec3_t dir_vector,
                                       vec3_t normal_weight) {
    int i;
    float *v, *normal;
    vec3_t viewer, reflected;
    float d;
    ...
    for (i = 0; i < tess.numVertexes; i++, v += 4, normal += 4, st += 2) {
        ...
        st[0] =
            0.5 + (reflected[0] * xy_ratio - reflected[1] * yx_ratio) * 0.5f;
        st[1] = 0.5 + (reflected[0] * yx_ratio * inv_z_ratio -
                       reflected[1] * xy_ratio * inv_z_ratio -
                       reflected[2] * z_ratio) *
                          0.5f;
    }
}

void RB_CalcEnvironment360TexCoords(float *st, vec3_t dir_vector,
                                    vec3_t normal_weight) {
    int i;
    float *v, *normal;
    vec3_t viewer, reflected;
...
void RB_CalcTurbulentTexCoords(const waveForm_t *wf, float *st) {
    ...
    for (i = 0; i < tess.numVertexes; i++, st += 2) {
        ...
        st[0] = s + tr.sinTable[((int)(((tess.xyz[i][0] + tess.xyz[i][2]) *
                                            1.0 / 128 * 0.125 +
                                        now) *
                                       FUNCTABLE_SIZE)) &
                                (FUNCTABLE_MASK)] *
                        wf->amplitude;
        st[1] =
            t + tr.sinTable[((int)((tess.xyz[i][1] * 1.0 / 128 * 0.125 + now) *
                                   FUNCTABLE_SIZE)) &
                            (FUNCTABLE_MASK)] *
                    wf->amplitude;
    }
...
void RB_CalcTransformTexCoords(const texModInfo_t *tmi, float *st) {
    ...
    for (i = 0; i < tess.numVertexes; i++, st += 2) {
        ...
        st[0] =
            s * tmi->matrix[0][0] + t * tmi->matrix[1][0] + tmi->translate[0];
        st[1] =
            s * tmi->matrix[0][1] + t * tmi->matrix[1][1] + tmi->translate[1];
    }
}
...
static void RB_CalcDiffuseColor_altivec(unsigned char *colors) {
    ...
    for (i = 0; i < numVertexes; i++, v += 4, normal += 4) {
        ...
        jVecChar = vec_sel(jVecChar, vSel,
                           vSel); // RGBARGBARGBARGBA replace alpha with 255
        vec_ste((vector unsigned int)jVecChar, 0,
                (unsigned int *)&colors[i * 4]); // store color
    }
}
#endif
