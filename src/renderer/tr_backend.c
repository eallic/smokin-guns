...
void RB_BeginDrawingView(void) {
    ...
    if (r_fastsky->integer && !(backEnd.refdef.rdflags & RDF_NOWORLDMODEL)) {
        clearBits |=
            GL_COLOR_BUFFER_BIT; // FIXME: only if sky shaders have been used
#ifdef _DEBUG
...
void RB_RenderDrawSurfList(drawSurf_t *drawSurfs, int numDrawSurfs) {
    ...
    for (i = 0, drawSurf = drawSurfs; i < numDrawSurfs; i++, drawSurf++) {
        ...
        // puff sprites
        if (shader != NULL &&
            (shader != oldShader || fogNum != oldFogNum ||
        ...
        if (entityNum != oldEntityNum) {
            ...
            if (entityNum != REFENTITYNUM_WORLD) {
                ...
                backEnd.refdef.floatTime =
                    originalTime - backEnd.currentEntity->e.shaderTime;
                // we have to reset the shaderTime as well otherwise image
                // animations start from the wrong frame
                tess.shaderTime =
                    backEnd.refdef.floatTime - tess.shader->timeOffset;

                // set up the transformation matrix
                R_RotateForEntity(backEnd.currentEntity, &backEnd.viewParms,
                                  &backEnd.or);

                // set up the dynamic lighting if needed
                if (backEnd.currentEntity->needDlights) {
                    R_TransformDlights(backEnd.refdef.num_dlights,
                                       backEnd.refdef.dlights, &backEnd.or);
                }

                if (backEnd.currentEntity->e.renderfx & RF_DEPTHHACK) {
                    // hack the depth range to prevent view model from poking
                    // into walls
                    depthRange = qtrue;
                ...
                // we have to reset the shaderTime as well otherwise image
                // animations on the world (like water) continue with the wrong
                // frame
                tess.shaderTime =
                    backEnd.refdef.floatTime - tess.shader->timeOffset;
                R_TransformDlights(backEnd.refdef.num_dlights,
            ...
            // weapon does not look like coming out of the screen.
            //
            if (oldDepthRange != depthRange || wasCrosshair != isCrosshair) {
                if (depthRange) {
                    if (backEnd.viewParms.stereoFrame != STEREO_CENTER) {
                        if (isCrosshair) {
                            if (oldDepthRange) {
                                // was not a crosshair but now is, change back
                                // proj matrix
                                qglMatrixMode(GL_PROJECTION);
                                qglLoadMatrixf(
...
void RE_StretchRaw(int x, int y, int w, int h, int cols, int rows,
    ...
    if (cols != tr.scratchImage[client]->width ||
        ...
        qglTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, cols, rows, 0, GL_RGBA,
        ...
        if (dirty) {
            // otherwise, just subimage upload it so that drivers can tell we
            // are going to be changing it and don't try and do a texture
            // compression
    ...
    *(int *)tess.vertexColors[numVerts] =
        *(int *)tess.vertexColors[numVerts + 1] =
            *(int *)tess.vertexColors[numVerts + 2] =
                *(int *)tess.vertexColors[numVerts + 3] =
                    *(int *)backEnd.color2D;
...
#ifdef FRAMEBUFFER_AND_GLSL_SUPPORT
            data =
                useFrameBuffer ? RB_DrawFrameBuffer(data) : RB_DrawBuffer(data);
#else
