...
we use statics to store a count and start writing the first
screenshot/screenshot????.tga (.jpg) available (with FS_FileExists /
...
byte *RB_ReadPixels(int x, int y, int width, int height, size_t *offset,
                    int *padlen) {
    ...
    buffer =
        ri.Hunk_AllocateTempMemory(padwidth * height + *offset + packAlign - 1);
...
const void *RB_TakeScreenshotCmd(const void *data) {
    ...
    if (cmd->jpeg)
        ...
        RB_TakeScreenshot(cmd->x, cmd->y, cmd->width, cmd->height,
                          cmd->fileName);
...
void R_ScreenshotFilename(int lastNumber, char *fileName) {
    ...
    Com_sprintf(fileName, MAX_OSPATH, "screenshots/shot%i%i%i%i.tga", a, b, c,
                d);
}
...
void R_ScreenshotFilenameJPEG(int lastNumber, char *fileName) {
    ...
    Com_sprintf(fileName, MAX_OSPATH, "screenshots/shot%i%i%i%i.jpg", a, b, c,
                d);
}
...
void R_LevelShot(void) {
    ...
    Com_sprintf(checkname, sizeof(checkname), "levelshots/%s.tga",
                tr.world->baseName);
...
void R_ScreenShot_f(void) {
    ...
    if (ri.Cmd_Argc() == 2 && !silent) {
        // explicit filename
        Com_sprintf(checkname, MAX_OSPATH, "screenshots/%s.tga",
                    ri.Cmd_Argv(1));
    } else {
...
void R_ScreenShotJPEG_f(void) {
    ...
    if (ri.Cmd_Argc() == 2 && !silent) {
        // explicit filename
        Com_sprintf(checkname, MAX_OSPATH, "screenshots/%s.jpg",
                    ri.Cmd_Argv(1));
    } else {
        // scan for a free filename
...
const void *RB_TakeVideoFrameCmd(const void *data) {
    ...
    qglReadPixels(0, 0, cmd->width, cmd->height, GL_RGB, GL_UNSIGNED_BYTE,
                  cBuf);
...
void R_Register(void) {
    ...
    r_ext_compressed_textures = ri.Cvar_Get("r_ext_compressed_textures", "0",
                                            CVAR_ARCHIVE | CVAR_LATCH);
    r_ext_multitexture =
        ri.Cvar_Get("r_ext_multitexture", "1", CVAR_ARCHIVE | CVAR_LATCH);
    r_ext_compiled_vertex_array = ri.Cvar_Get("r_ext_compiled_vertex_array",
    ...
    r_ext_max_anisotropy =
        ri.Cvar_Get("r_ext_max_anisotropy", "2", CVAR_ARCHIVE | CVAR_LATCH);
    ...
    r_texturebits =
        ri.Cvar_Get("r_texturebits", "0", CVAR_ARCHIVE | CVAR_LATCH);
    r_colorbits = ri.Cvar_Get("r_colorbits", "0", CVAR_ARCHIVE | CVAR_LATCH);
    r_stencilbits =
        ri.Cvar_Get("r_stencilbits", "8", CVAR_ARCHIVE | CVAR_LATCH);
    r_depthbits = ri.Cvar_Get("r_depthbits", "0", CVAR_ARCHIVE | CVAR_LATCH);
    r_ext_multisample =
        ri.Cvar_Get("r_ext_multisample", "0", CVAR_ARCHIVE | CVAR_LATCH);
    ri.Cvar_CheckRange(r_ext_multisample, 0, 4, qtrue);
    r_overBrightBits =
        ri.Cvar_Get("r_overBrightBits", "1", CVAR_ARCHIVE | CVAR_LATCH);
    r_ignorehwgamma =
        ri.Cvar_Get("r_ignorehwgamma", "0", CVAR_ARCHIVE | CVAR_LATCH);
    ...
    r_customPixelAspect =
        ri.Cvar_Get("r_customPixelAspect", "1", CVAR_ARCHIVE | CVAR_LATCH);
    r_simpleMipMaps =
    ...
    r_stereoEnabled =
        ri.Cvar_Get("r_stereoEnabled", "0", CVAR_ARCHIVE | CVAR_LATCH);
    r_ignoreFastPath =
    ...
    //
    r_lodCurveError =
        ri.Cvar_Get("r_lodCurveError", "250", CVAR_ARCHIVE | CVAR_CHEAT);
    ...
    r_swapInterval =
        ri.Cvar_Get("r_swapInterval", "0", CVAR_ARCHIVE | CVAR_LATCH);
    ...
    r_marksOnTriangleMeshes =
        ri.Cvar_Get("r_marksOnTriangleMeshes", "0", CVAR_ARCHIVE);

    r_aviMotionJpegQuality =
        ri.Cvar_Get("r_aviMotionJpegQuality", "90", CVAR_ARCHIVE);
    r_screenshotJpegQuality =
        ri.Cvar_Get("r_screenshotJpegQuality", "90", CVAR_ARCHIVE);
    ...
    r_ext_framebuffer_bloom =
        ri.Cvar_Get("r_ext_framebuffer_bloom", "0", CVAR_ARCHIVE | CVAR_LATCH);
    r_ext_framebuffer_blur_size = ri.Cvar_Get("r_ext_framebuffer_blur_size",
                                              "256", CVAR_ARCHIVE | CVAR_LATCH);
    r_ext_framebuffer_blur_amount =
        ri.Cvar_Get("r_ext_framebuffer_blur_amount", "7", CVAR_ARCHIVE);
    r_ext_framebuffer_blur_samples = ri.Cvar_Get(
        "r_ext_framebuffer_blur_samples", "9", CVAR_ARCHIVE | CVAR_LATCH);
    r_ext_framebuffer_bloom_sharpness =
        ri.Cvar_Get("r_ext_framebuffer_bloom_sharpness", "0.75", CVAR_ARCHIVE);
    r_ext_framebuffer_bloom_brightness =
        ri.Cvar_Get("r_ext_framebuffer_bloom_brightness", "0.85", CVAR_ARCHIVE);
    r_ext_framebuffer_rotoscope = ri.Cvar_Get("r_ext_framebuffer_rotoscope",
                                              "0", CVAR_ARCHIVE | CVAR_LATCH);
    r_ext_framebuffer_rotoscope_zedge = ri.Cvar_Get(
        "r_ext_framebuffer_rotoscope_zedge", "0", CVAR_ARCHIVE | CVAR_LATCH);
    r_ext_framebuffer_multisample = ri.Cvar_Get("r_ext_framebuffer_multisample",
                                                "0", CVAR_ARCHIVE | CVAR_LATCH);
#endif
...
void R_Init(void) {
    ...
    for (i = 0; i < FUNCTABLE_SIZE; i++) {
        ...
        if (i < FUNCTABLE_SIZE / 2) {
            if (i < FUNCTABLE_SIZE / 4) {
                ...
                tr.triangleTable[i] =
                    1.0f - tr.triangleTable[i - FUNCTABLE_SIZE / 4];
            }
        } else {
    ...
    ptr = ri.Hunk_Alloc(sizeof(*backEndData) + sizeof(srfPoly_t) * max_polys +
                            sizeof(polyVert_t) * max_polyverts,
                        h_low);
    backEndData = (backEndData_t *)ptr;
    backEndData->polys = (srfPoly_t *)((char *)ptr + sizeof(*backEndData));
    backEndData->polyVerts = (polyVert_t *)((char *)ptr + sizeof(*backEndData) +
...
refexport_t *GetRefAPI(int apiVersion, refimport_t *rimp) {
    ...
    r_debugRenderer =
        ri.Cvar_Get("r_debugRenderer", va("%li", (chk_ref)&re), CVAR_ROM);
