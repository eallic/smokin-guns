...
static qboolean R_LoadMD3(model_t *mod, int lod, void *buffer,
                          const char *name);
static qboolean R_LoadMDR(model_t *mod, void *buffer, int filesize,
                          const char *name);
...
qhandle_t R_RegisterMD3(const char *name, model_t *mod) {
    ...
    for (lod = MD3_MAX_LODS - 1; lod >= 0; lod--) {
        if (lod)
            Com_sprintf(namebuf, sizeof(namebuf), "%s_%d.%s", filename, lod,
                        fext);
        else
...
static qboolean R_LoadMD3(model_t *mod, int lod, void *buffer,
    ...
    for (i = 0; i < mod->md3[lod]->numTags * mod->md3[lod]->numFrames;
         i++, tag++) {
        for (j = 0; j < 3; j++) {
...
static qboolean R_LoadMDR(model_t *mod, void *buffer, int filesize,
    ...
    if (pinmodel->ofsFrames < 0) {
        ...
        size += pinmodel->numFrames * pinmodel->numBones *
                ((sizeof(mdrBone_t) - sizeof(mdrCompBone_t)));
    }

    // simple bounds check
    if (pinmodel->numBones < 0 ||
        sizeof(*mdr) + pinmodel->numFrames *
                           (sizeof(*frame) +
                            (pinmodel->numBones - 1) * sizeof(*frame->bones)) >
            size) {
    ...
    // We don't care about the other offset values, we'll generate them
    ...
    if (pinmodel->ofsFrames < 0) {
        ...
        for (i = 0; i < mdr->numFrames; i++) {
            ...
            frame->name[0] =
                '\0'; // No name supplied in the compressed version.

            for (j = 0; j < mdr->numBones; j++) {
                for (k = 0; k < (sizeof(cframe->bones[j].Comp) / 2); k++) {
                    // Do swapping for the uncompressing functions. They seem to
                    // use shorts values only, so I assume this will work. Never
                    // tested it on other platforms, though.
        ...
        for (i = 0; i < mdr->numFrames; i++) {
            ...
            for (j = 0; j < (int)(mdr->numBones * sizeof(mdrBone_t) / 4); j++) {
                ((float *)frame->bones)[j] =
                    LittleFloat(((float *)curframe->bones)[j]);
            }
    ...
    for (l = 0; l < mdr->numLODs; l++) {
        ...
        cursurf =
            (mdrSurface_t *)((byte *)curlod + LittleLong(curlod->ofsSurfaces));
        ...
        for (i = 0; i < lod->numSurfaces; i++) {
            ...
            curv = (mdrVertex_t *)((byte *)cursurf +
                                   LittleLong(cursurf->ofsVerts));
            ...
            // find the next surface.
            cursurf =
                (mdrSurface_t *)((byte *)cursurf + LittleLong(cursurf->ofsEnd));
        }
...
static md3Tag_t *R_GetTag(md3Header_t *mod, int frame, const char *tagName) {
    ...
    if (frame >= mod->numFrames) {
        // it is possible to have a bad frame while changing models, so don't
        // error
...
void R_GetAnimTag(mdrHeader_t *mod, int framenum, const char *tagName,
    ...
    for (i = 0; i < mod->numTags; i++, tag++) {
        if (!strcmp(tag->name, tagName)) {
            ...
            frame = (mdrFrame_t *)((byte *)mod + mod->ofsFrames +
                                   framenum * frameSize);

            for (j = 0; j < 3; j++) {
                for (k = 0; k < 3; k++)
                    dest->axis[j][k] =
                        frame->bones[tag->boneIndex].matrix[k][j];
            }
...
================
*/
int R_LerpTag(orientation_t *tag, qhandle_t handle, int startFrame,
    ...
    if (!model->md3[0]) {
        if (model->type == MOD_MDR) {
            ...
            R_GetAnimTag((mdrHeader_t *)model->modelData, startFrame, tagName,
                         start);
            R_GetAnimTag((mdrHeader_t *)model->modelData, endFrame, tagName,
                         end);
        } else if (model->type == MOD_IQM) {
            return R_IQMLerpTag(tag, model->modelData, startFrame, endFrame,
                                frac, tagName);
        } else {
    ...
    for (i = 0; i < 3; i++) {
        tag->origin[i] =
            start->origin[i] * backLerp + end->origin[i] * frontLerp;
        tag->axis[0][i] =
