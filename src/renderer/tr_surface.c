...
void RB_CheckOverflow(int verts, int indexes) {
    ...
    if (verts >= SHADER_MAX_VERTEXES) {
        ri.Error(ERR_DROP, "RB_CheckOverflow: verts > MAX (%d > %d)", verts,
                 SHADER_MAX_VERTEXES);
    }
    if (indexes >= SHADER_MAX_INDEXES) {
        ri.Error(ERR_DROP, "RB_CheckOverflow: indices > MAX (%d > %d)", indexes,
                 SHADER_MAX_INDEXES);
    }
...
static void RB_SurfaceTriangles(srfTriangles_t *srf) {
    ...
    for (i = 0; i < srf->numIndexes; i += 3) {
        tess.indexes[tess.numIndexes + i + 0] =
            tess.numVertexes + srf->indexes[i + 0];
        tess.indexes[tess.numIndexes + i + 1] =
            tess.numVertexes + srf->indexes[i + 1];
        tess.indexes[tess.numIndexes + i + 2] =
            tess.numVertexes + srf->indexes[i + 2];
    }
    ...
    for (i = 0; i < srf->numVerts;
         i++, dv++, xyz += 4, normal += 4, texCoords += 4, color += 4) {
        xyz[0] = dv->xyz[0];
        xyz[1] = dv->xyz[1];
        xyz[2] = dv->xyz[2];
...
static void DoRailCore(const vec3_t start, const vec3_t end, const vec3_t up,
    ...
    tess.vertexColors[tess.numVertexes][0] =
        backEnd.currentEntity->e.shaderRGBA[0] * 0.25;
    tess.vertexColors[tess.numVertexes][1] =
        backEnd.currentEntity->e.shaderRGBA[1] * 0.25;
    tess.vertexColors[tess.numVertexes][2] =
        backEnd.currentEntity->e.shaderRGBA[2] * 0.25;
    tess.numVertexes++;
...
static void DoRailDiscs(int numSegs, const vec3_t start, const vec3_t dir,
...
*
* The inputs to this routing seem to always be close to length = 1.0 (about 0.6
* to 2.0) This means that we don't have to worry about zero length or enormously
* long vectors.
*/
static void VectorArrayNormalize(vec4_t *normals, unsigned int count) {
...
#if idppc
    {
        ...
        // instruction, runs *much* faster than calling sqrt().  We'll use a
        // single Newton-Raphson refinement step to get a little more precision.
        // This seems to yeild results that are correct to 3 decimal places and
        // usually correct to at least 4 (sometimes 5). (That is, for the given
        // input range of about 0.6 to 2.0).
        do {
            float x, y, z;
            float B, y0, y1;
...
static void LerpMeshVertexes_altivec(md3Surface_t *surf, float backlerp) {
    ...
    if (backlerp == 0) {
        ...
        for (vertNum = 0; vertNum < numVerts; vertNum++, newXyz += 4,
            newNormals += 4, outXyz += 4, outNormal += 4) {
            newNormalsLoadPermute = vec_lvsl(0, newXyz);
            newNormalsStorePermute = vec_lvsr(0, outXyz);
            ...
            newNormalsFloatVec = vec_perm(
                newNormalsFloatVec, newNormalsFloatVec, newNormalsStorePermute);
            ...
            outNormal[0] =
                tr.sinTable[(lat + (FUNCTABLE_SIZE / 4)) & FUNCTABLE_MASK] *
                tr.sinTable[lng];
        ...
        for (vertNum = 0; vertNum < numVerts; vertNum++, oldXyz += 4,
            newXyz += 4, oldNormals += 4, newNormals += 4, outXyz += 4,
            outNormal += 4) {
            ...
            outNormal[0] = uncompressedOldNormal[0] * oldNormalScale +
                           uncompressedNewNormal[0] * newNormalScale;
            outNormal[1] = uncompressedOldNormal[1] * oldNormalScale +
...
static void RB_SurfaceFace(srfSurfaceFace_t *surf) {
    ...
    for (i = 0, v = surf->points[0], ndx = tess.numVertexes; i < numPoints;
         i++, v += VERTEXSIZE, ndx++) {
...
static void RB_SurfaceGrid(srfGridMesh_t *cv) {
    ...
    while (used < lodHeight - 1) {
        // see how many rows of both verts and indexes we can add without
        // overflowing
        do {
            vrows = (SHADER_MAX_VERTEXES - tess.numVertexes) / lodWidth;
            irows = (SHADER_MAX_INDEXES - tess.numIndexes) / (lodWidth * 6);

            // if we don't have enough space for at least one strip, flush the
            // buffer
            if (vrows < 2 || irows < 1) {
                RB_EndSurface();
                RB_BeginSurface(tess.shader, tess.fogNum);
