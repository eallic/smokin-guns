...
void R_TransformModelToClip(const vec3_t src, const float *modelMatrix,
                            const float *projectionMatrix, vec4_t eye,
                            vec4_t dst) {
    int i;
    ...
    for (i = 0; i < 4; i++) {
        eye[i] = src[0] * modelMatrix[i + 0 * 4] +
                 ...
                 src[2] * modelMatrix[i + 2 * 4] + 1 * modelMatrix[i + 3 * 4];
    }
...
static void R_SetFarClip(void) {
    ...
    for (i = 0; i < 8; i++) {
        ...
        distance =
            vecTo[0] * vecTo[0] + vecTo[1] * vecTo[1] + vecTo[2] * vecTo[2];
...
Set up the culling frustum planes for the current view using the results we got
from computing the first two rows of the projection matrix.
=================
*/
void R_SetupFrustum(viewParms_t *dest, float xmin, float xmax, float ymax,
    ...
    if (stereoSep == 0 && xmin == -xmax) {
        ...
        VectorMA(dest->frustum[0].normal, adjleg, dest->or.axis[1],
                 dest->frustum[0].normal);

        VectorScale(dest->or.axis[0], oppleg, dest->frustum[1].normal);
        VectorMA(dest->frustum[1].normal, -adjleg, dest->or.axis[1],
                 dest->frustum[1].normal);
    } else {
        // In stereo rendering, due to the modification of the projection
        // matrix, dest->or.origin is not the actual origin that we're rendering
        // so offset the tip of the view pyramid.
        VectorMA(dest->or.origin, stereoSep, dest->or.axis[1], ofsorigin);

        oppleg = xmax + stereoSep;
        length = sqrt(oppleg * oppleg + zProj * zProj);
        VectorScale(dest->or.axis[0], oppleg / length, dest->frustum[0].normal);
        VectorMA(dest->frustum[0].normal, zProj / length, dest->or.axis[1],
                 dest->frustum[0].normal);

        oppleg = xmin + stereoSep;
        length = sqrt(oppleg * oppleg + zProj * zProj);
        VectorScale(dest->or.axis[0], -oppleg / length,
                    dest->frustum[1].normal);
        VectorMA(dest->frustum[1].normal, -zProj / length, dest->or.axis[1],
                 dest->frustum[1].normal);
    }
    ...
    VectorMA(dest->frustum[2].normal, adjleg, dest->or.axis[2],
             dest->frustum[2].normal);

    VectorScale(dest->or.axis[0], oppleg, dest->frustum[3].normal);
    VectorMA(dest->frustum[3].normal, -adjleg, dest->or.axis[2],
             dest->frustum[3].normal);
...
void R_SetupProjection(viewParms_t *dest, float zProj,
    ...
    // Now that we have all the data for the projection matrix we can also setup
    // the view frustum.
    if (computeFrustum)
...
=================
*/
void R_MirrorPoint(vec3_t in, orientation_t *surface, orientation_t *camera,
                   vec3_t out) {
    int i;
    vec3_t local;
    vec3_t transformed;
...
void R_MirrorVector(vec3_t in, orientation_t *surface, orientation_t *camera,
                    vec3_t out) {
    int i;
    float d;
...
void R_PlaneForSurface(surfaceType_t *surfType, cplane_t *plane) {
    ...
    case SF_POLY:
        ...
        PlaneFromPoints(plane4, poly->verts[0].xyz, poly->verts[1].xyz,
                        poly->verts[2].xyz);
        VectorCopy(plane4, plane->normal);
        plane->dist = plane4[3];
        return;
...
qboolean R_GetPortalOrientations(drawSurf_t *drawSurf, int entityNum,
    ...
    if (entityNum != REFENTITYNUM_WORLD) {
        ...
        plane.dist =
            originalPlane.dist + DotProduct(plane.normal, tr.or.origin);
...
static qboolean SurfIsOffscreen(const drawSurf_t *drawSurf,
    ...
    for (i = 0; i < tess.numVertexes; i++) {
        ...
        R_TransformModelToClip(tess.xyz[i], tr.or.modelMatrix,
    ...
    for (i = 0; i < tess.numIndexes; i += 3) {
        ...
        VectorSubtract(tess.xyz[tess.indexes[i]], tr.viewParms.or.origin,
                       normal);
    ...
    // mirrors can early out at this point, since we don't do a fade over
    // distance with them (although we could)
...
===============
*/
static ID_INLINE void R_Radix(int byte, int size, drawSurf_t *source,
                              drawSurf_t *dest) {
    int count[256] = {0};
    int index[256];
    int i;
...
void R_AddDrawSurf(surfaceType_t *surface, shader_t *shader, int fogIndex,
    ...
    tr.refdef.drawSurfs[index].sort =
        (shader->sortedIndex << QSORT_SHADERNUM_SHIFT) | tr.shiftedEntityNum |
...
void R_DecomposeSort(unsigned sort, int *entityNum, shader_t **shader,
    ...
    *shader =
        tr.sortedShaders[(sort >> QSORT_SHADERNUM_SHIFT) & (MAX_SHADERS - 1)];
...
void R_AddEntitySurfaces(void) {
    ...
    for (tr.currentEntityNum = 0; tr.currentEntityNum < tr.refdef.num_entities;
        ...
        case RT_RAIL_RINGS:
            ...
            // primary view.  We can't just do this check for all entities,
            // because md3 entities may still want to cast shadows from them
...
void R_GenerateDrawSurfs(void) {
    ...
    // we know the size of the clipping volume. Now set the rest of the
    // projection matrix.
...
void R_DebugPolygon(int color, int numPoints, float *points) {
    ...
    GL_State(GLS_POLYMODE_LINE | GLS_DEPTHMASK_TRUE | GLS_SRCBLEND_ONE |
             GLS_DSTBLEND_ONE);
    qglDepthRange(0, 0);
    qglColor3f(1, 1, 1);
    qglBegin(GL_POLYGON);
