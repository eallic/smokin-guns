...
static uint32_t DecompressIDATs(struct BufferedFile *BF, uint8_t **Buffer) {
    ...
    *Buffer = DecompressedData;
    ...
    puffSrcLen =
        CompressedDataLength - PNG_ZlibHeader_Size - PNG_ZlibCheckValue_Size;
...
static qboolean UnfilterImage(uint8_t *DecompressedData, uint32_t ImageHeight,
    ...
    for (h = 0; h < ImageHeight; h++) {
        ...
        /*
         *  Set PixelUp to previous line only if we are on the second line or
         * above.
         *
        ...
        /*
         * The pixel left to the first pixel of the previous scanline is zero
         * too.
         */
        ...
        for (w = 0; w < (BytesPerScanline / BytesPerPixel); w++) {
            ...
            for (p = 0; p < BytesPerPixel; p++) {
                ...
                case PNG_FilterType_Average: {
                    DecompPtr[p] += ((uint8_t)((((uint16_t)PixelLeft[p]) +
                                                ((uint16_t)PixelUp[p])) /
                                               2));
                ...
                case PNG_FilterType_Paeth: {
                    DecompPtr[p] +=
                        PredictPaeth(PixelLeft[p], PixelUp[p], PixelUpLeft[p]);
            ...
            /*
             *  We only have an upleft pixel if we are on the second line or
             * above.
             */
            ...
            /*
             *  We only have a previous line if we are on the second line and
             * above.
             */
...
static qboolean ConvertPixel(struct PNG_Chunk_IHDR *IHDR, byte *OutPtr,
                             uint8_t *DecompPtr, qboolean HasTransparentColour,
                             uint8_t *TransparentColour, uint8_t *OutPal) {
    /*
     *  input verification
     */
...
static qboolean DecodeImageNonInterlaced(
    struct PNG_Chunk_IHDR *IHDR, byte *OutBuffer, uint8_t *DecompressedData,
    uint32_t DecompressedDataLength, qboolean HasTransparentColour,
    uint8_t *TransparentColour, uint8_t *OutPal) {
    uint32_t IHDR_Width;
    ...
    case PNG_ColourType_GreyAlpha: {
        ...
        case PNG_BitDepth_16: {
            BytesPerPixel =
                (IHDR->BitDepth / 8) * PNG_NumColourComponents_GreyAlpha;
            PixelsPerByte = 1;
    ...
    case PNG_ColourType_TrueAlpha: {
        ...
        case PNG_BitDepth_16: {
            BytesPerPixel =
                (IHDR->BitDepth / 8) * PNG_NumColourComponents_TrueAlpha;
            PixelsPerByte = 1;
    ...
    BytesPerScanline =
        (IHDR_Width * BytesPerPixel + (PixelsPerByte - 1)) / PixelsPerByte;
            ...
            if (PixelsPerByte > 1) {
                ...
                for (p = 0; p < PixelsPerByte; p++) {
                    if (CurrPixel < IHDR_Width) {
                        ...
                        SinglePixel =
                            ((DecompPtr[0] & (Mask << Shift)) >> Shift);

                        if (!ConvertPixel(IHDR, OutPtr, &SinglePixel,
...
static qboolean DecodeImageInterlaced(
    struct PNG_Chunk_IHDR *IHDR, byte *OutBuffer, uint8_t *DecompressedData,
    uint32_t DecompressedDataLength, qboolean HasTransparentColour,
    uint8_t *TransparentColour, uint8_t *OutPal) {
    uint32_t IHDR_Width;
    uint32_t IHDR_Height;
    uint32_t BytesPerScanline[PNG_Adam7_NumPasses], BytesPerPixel,
        PixelsPerByte;
    ...
    for (a = 0; a < PNG_Adam7_NumPasses; a++) {
        BytesPerScanline[a] =
            (PassWidth[a] * BytesPerPixel + (PixelsPerByte - 1)) /
            PixelsPerByte;
    }
        ...
        for (h = 0; h < PassHeight[a]; h++) {
            ...
            for (w = 0; w < (BytesPerScanline[a] / BytesPerPixel); w++) {
                if (PixelsPerByte > 1) {
                    ...
                    if (!ConvertPixel(IHDR, OutPtr, DecompPtr,
                                      HasTransparentColour, TransparentColour,
                                      OutPal)) {
                        return (qfalse);
                    }
                }
    ...
    *pic = NULL;
    ...
    case PNG_InterlaceMethod_NonInterlaced: {
        if (!DecodeImageNonInterlaced(
                IHDR, OutBuffer, DecompressedData, DecompressedDataLength,
    ...
    case PNG_InterlaceMethod_Interlaced: {
        if (!DecodeImageInterlaced(IHDR, OutBuffer, DecompressedData,
                                   DecompressedDataLength, HasTransparentColour,
                                   TransparentColour, OutPal)) {
            ri.Free(OutBuffer);
            ri.Free(DecompressedData);
            CloseBufferedFile(ThePNG);
