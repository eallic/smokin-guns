...
typedef struct shader_s {
    ...
    int lightmapIndex;    // for a shader to match, both name and lightmapIndex
                          // must match
    ...
    qboolean
        polygonOffset;  // set for decals and other items that must be offset
...
// Joe Kari: Attempt to allow lowering of r_subdivisions without exceeding the
// MAX_GRID_SIZE limit
...
*/
typedef struct {
    int c_sphere_cull_patch_in, c_sphere_cull_patch_clip,
...
typedef struct {
    ...
    qboolean
        projection2D; // if qtrue, drawstretchpic doesn't need to change modes
    byte color2D[4];
...
typedef struct {
    ...
    trRefEntity_t *currentEntity;
    trRefEntity_t
        worldEntity; // point currentEntity at this when rendering world
    ...
    int overbrightBits;    // r_overbrightBits->integer, but set to 0 if no hw
                           // gamma
...
extern cvar_t *r_shadows; // controls shadows: 0 = none, 1 = blur, 2 = stencil,
                          // 3 = black planar projection
...
srfGridMesh_t *R_GridInsertColumn(srfGridMesh_t *grid, int column, int row,
                                  vec3_t point, float loderror);
srfGridMesh_t *R_GridInsertRow(srfGridMesh_t *grid, int row, int column,
                               vec3_t point, float loderror);
...
void RB_CalcCustomEnvironmentTexCoords(float *dstTexCoords, vec3_t dir_vector,
                                       vec3_t normal_weight);
void RB_CalcEnvironment360TexCoords(float *dstTexCoords, vec3_t dir_vector,
                                    vec3_t normal_weight);
