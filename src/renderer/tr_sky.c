    ...
    for (i = 0; i < input->numIndexes; i += 3) {
        for (j = 0; j < 3; j++) {
            VectorSubtract(input->xyz[input->indexes[i + j]],
                           backEnd.viewParms.or.origin, p[j]);
        }
...
*/
static void MakeSkyVec(float s, float t, int axis, float outSt[2],
                       vec3_t outXYZ) {
    // 1 = s, 2 = t, 3 = 2048
...
static void DrawSkySide(struct image_s *image, const int mins[2],
                        const int maxs[2]) {
    int s, t;
    ...
    for (i = 0; i < 6; i++) {
        ...
        for (t = sky_mins_subd[1] + HALF_SKY_SUBDIVISIONS;
            ...
            for (s = sky_mins_subd[0] + HALF_SKY_SUBDIVISIONS;
                ...
                MakeSkyVec(
                    (s - HALF_SKY_SUBDIVISIONS) / (float)HALF_SKY_SUBDIVISIONS,
...
static void FillCloudySkySide(const int mins[2], const int maxs[2],
    ...
    for (t = mins[1] + HALF_SKY_SUBDIVISIONS;
        ...
        for (s = mins[0] + HALF_SKY_SUBDIVISIONS;
             s <= maxs[0] + HALF_SKY_SUBDIVISIONS; s++) {
            VectorAdd(s_skyPoints[t][s], backEnd.viewParms.or.origin,
...
void RB_DrawSun(float scale, shader_t *shader) {
    ...
    qglTranslatef(backEnd.viewParms.or.origin[0],
