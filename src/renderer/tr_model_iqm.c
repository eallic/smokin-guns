...
static qboolean IQM_CheckRange(iqmHeader_t *header, int offset, int count,
    ...
    return (count <= 0 || offset < 0 || offset > header->filesize ||
            offset + count * size < 0 ||
...
=================
*/
qboolean R_LoadIQM(model_t *mod, void *buffer, int filesize,
                   const char *mod_name) {
    iqmHeader_t *header;
    ...
    if (header->version != IQM_VERSION) {
        ri.Printf(PRINT_WARNING,
                  "R_LoadIQM: %s is a unsupported IQM version (%d), only "
    ...
    if (header->num_joints > IQM_MAX_JOINTS) {
        ri.Printf(PRINT_WARNING,
                  ...
                  IQM_MAX_JOINTS, header->num_joints);
        return qfalse;
    }
    ...
    if (IQM_CheckRange(header, header->ofs_triangles, header->num_triangles,
                       sizeof(iqmTriangle_t))) {
        return qfalse;
    }
    ...
    if (IQM_CheckRange(header, header->ofs_meshes, header->num_meshes,
                       sizeof(iqmMesh_t))) {
        return qfalse;
    }
    ...
    for (i = 0; i < header->num_meshes; i++, mesh++) {
        ...
        if (mesh->name < header->num_text) {
            Q_strncpyz(meshName, (char *)header + header->ofs_text + mesh->name,
                       sizeof(meshName));
        } else {
            meshName[0] = '\0';
        }
    ...
    if (header->num_poses != header->num_joints && header->num_poses != 0) {
        ri.Printf(PRINT_WARNING,
                  "R_LoadIQM: %s has %d poses and %d joints, must have the "
                  "same number or 0 poses\n",
    ...
    if (header->num_joints) {
        ...
        if (IQM_CheckRange(header, header->ofs_joints, header->num_joints,
                           sizeof(iqmJoint_t))) {
            return qfalse;
        }
        ...
        for (i = 0; i < header->num_joints; i++, joint++) {
            ...
            joint_names +=
                strlen((char *)header + header->ofs_text + joint->name) + 1;
        }
    }

    if (header->num_poses) {
        // check and swap poses
        if (IQM_CheckRange(header, header->ofs_poses, header->num_poses,
                           sizeof(iqmPose_t))) {
            return qfalse;
        }
    ...
    if (header->ofs_bounds) {
        ...
        if (IQM_CheckRange(header, header->ofs_bounds, header->num_frames,
                           sizeof(*bounds))) {
            return qfalse;
        }
    ...
    size += header->num_poses * header->num_frames * 12 *
            sizeof(float); // pose mats
    if (header->ofs_bounds)
    ...
    if (header->ofs_bounds) {
        iqmData->bounds =
            iqmData->poseMats + 12 * header->num_poses * header->num_frames;
    ...
    iqmData->blendIndexes =
        (byte *)(iqmData->tangents + 4 * header->num_vertexes);

    if (blendWeightsType == IQM_FLOAT) {
        iqmData->blendWeights.f =
            (float *)(iqmData->blendIndexes + 4 * header->num_vertexes);
        iqmData->colors =
            (byte *)(iqmData->blendWeights.f + 4 * header->num_vertexes);
    } else {
        iqmData->blendWeights.b =
            iqmData->blendIndexes + 4 * header->num_vertexes;
    ...
    for (i = 0; i < header->num_joints; i++, joint++) {
        ...
        if (joint->parent >= 0) {
            Matrix34Multiply(iqmData->jointMats + 12 * joint->parent, baseFrame,
                             mat);
            mat += 12;
            Matrix34Multiply(invBaseFrame, jointInvMats + 12 * joint->parent,
                             matInv);
            matInv += 12;
        } else {
    ...
    for (i = 0; i < header->num_frames; i++) {
        ...
        for (j = 0; j < header->num_poses; j++, pose++) {
            ...
            if (pose->parent >= 0) {
                Matrix34Multiply(iqmData->jointMats + 12 * pose->parent, mat1,
                                 mat2);
            } else {
    ...
    for (i = 0; i < header->num_meshes; i++, mesh++, surface++) {
        ...
        Q_strlwr(surface->name); // lowercase the surface name so skin compares
                                 // are faster
        surface->shader =
            R_FindShader(str + mesh->material, LIGHTMAP_NONE, qtrue);
    ...
    for (i = 0; i < header->num_vertexarrays; i++, vertexarray++) {
        ...
        switch (vertexarray->type) {
        case IQM_POSITION:
            Com_Memcpy(iqmData->positions, (byte *)header + vertexarray->offset,
                       n * sizeof(float));
            break;
        case IQM_NORMAL:
            Com_Memcpy(iqmData->normals, (byte *)header + vertexarray->offset,
                       n * sizeof(float));
            break;
        case IQM_TANGENT:
            Com_Memcpy(iqmData->tangents, (byte *)header + vertexarray->offset,
                       n * sizeof(float));
            break;
        case IQM_TEXCOORD:
            Com_Memcpy(iqmData->texcoords, (byte *)header + vertexarray->offset,
                       n * sizeof(float));
            break;
        case IQM_BLENDINDEXES:
        ...
        case IQM_BLENDWEIGHTS:
            ...
            break;
        case IQM_COLOR:
            Com_Memcpy(iqmData->colors, (byte *)header + vertexarray->offset,
                       n * sizeof(byte));
            break;
        }
    }
...
static int R_CullIQM(iqmData_t *data, trRefEntity_t *ent) {
    ...
    for (i = 0; i < 3; i++) {
        bounds[0][i] =
            oldBounds[i] < newBounds[i] ? oldBounds[i] : newBounds[i];
        bounds[1][i] = oldBounds[i + 3] > newBounds[i + 3] ? oldBounds[i + 3]
                                                           : newBounds[i + 3];
    }
...
void R_AddIQMSurfaces(trRefEntity_t *ent) {
    ...
    personalModel =
        (ent->e.renderfx & RF_THIRD_PERSON) && !tr.viewParms.isPortal;
...
static void ComputePoseMats(iqmData_t *data, int frame, int oldframe,
    ...
    if (data->num_poses == 0) {
        for (i = 0; i < data->num_joints; i++, joint++) {
            if (*joint >= 0) {
                Matrix34Multiply(mat + 12 * *joint, identityMatrix,
                                 mat + 12 * i);
            } else {
    ...
    if (oldframe == frame) {
        ...
        for (i = 0; i < data->num_poses; i++, joint++) {
            if (*joint >= 0) {
                Matrix34Multiply(mat + 12 * *joint, mat1 + 12 * i,
                                 mat + 12 * i);
            } else {
        ...
        for (i = 0; i < data->num_poses; i++, joint++) {
            if (*joint >= 0) {
                float tmpMat[12];
                InterpolateMatrix(mat1 + 12 * i, mat2 + 12 * i, backlerp,
                                  tmpMat);
...
void RB_IQMSurfaceAnim(surfaceType_t *surface) {
    ...
    int frame = data->num_frames
                    ...
                    : 0;
    int oldframe = data->num_frames
                       ? backEnd.currentEntity->e.oldframe % data->num_frames
                       : 0;
    ...
    for (i = 0; i < surf->num_vertexes;
        ...
        for (numWeights = 0; numWeights < 4; numWeights++) {
            if (data->blendWeightsType == IQM_FLOAT)
                blendWeights[numWeights] =
                    data->blendWeights.f[4 * vtx + numWeights];
            else
                blendWeights[numWeights] =
                    (float)data->blendWeights.b[4 * vtx + numWeights] / 255.0f;
        ...
        (*outXYZ)[0] = vtxMat[0] * data->positions[3 * vtx + 0] +
                       vtxMat[1] * data->positions[3 * vtx + 1] +
                       vtxMat[2] * data->positions[3 * vtx + 2] + vtxMat[3];
        (*outXYZ)[1] = vtxMat[4] * data->positions[3 * vtx + 0] +
                       vtxMat[5] * data->positions[3 * vtx + 1] +
                       vtxMat[6] * data->positions[3 * vtx + 2] + vtxMat[7];
        (*outXYZ)[2] = vtxMat[8] * data->positions[3 * vtx + 0] +
                       vtxMat[9] * data->positions[3 * vtx + 1] +
                       vtxMat[10] * data->positions[3 * vtx + 2] + vtxMat[11];
        (*outXYZ)[3] = 1.0f;

        (*outNormal)[0] = nrmMat[0] * data->normals[3 * vtx + 0] +
                          nrmMat[1] * data->normals[3 * vtx + 1] +
                          nrmMat[2] * data->normals[3 * vtx + 2];
        (*outNormal)[1] = nrmMat[3] * data->normals[3 * vtx + 0] +
                          nrmMat[4] * data->normals[3 * vtx + 1] +
                          nrmMat[5] * data->normals[3 * vtx + 2];
        (*outNormal)[2] = nrmMat[6] * data->normals[3 * vtx + 0] +
...
int R_IQMLerpTag(orientation_t *tag, iqmData_t *data, int startFrame,
