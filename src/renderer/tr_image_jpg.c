...
void R_LoadJPG(const char *filename, unsigned char **pic, int *width,
               int *height) {
    ...
    if (!cinfo.output_width || !cinfo.output_height ||
        ...
        ri.Error(ERR_DROP,
                 "LoadJPG: %s has an invalid image format: %dx%d*4=%d, "
                 "components: %d",
...
static boolean empty_output_buffer(j_compress_ptr cinfo) {
    ...
    ri.Error(ERR_FATAL,
             "Output buffer for encoded JPEG image has insufficient size of %d "
             "bytes",
             dest->size);
...
size_t RE_SaveJPGToBuffer(byte *buffer, size_t bufSize, int quality,
    ...
    row_stride = image_width * cinfo.input_components +
                 padding; /* JSAMPLEs per row in image_buffer */
