...
gslsProg_t glsl_progs[] = {{"glslBase_vert", NULL, "\n\
void main() {\n\
...
void R_GLSLProgs_Init(void) {
    ...
    while (i--) {
        ...
        if (len < 0) {
            len = ri.FS_ReadFile(va("scripts/%s.glsl", glsl_progs[i].name),
                                 &buffer);
            if (len > 0)
                ri.Printf(
                    PRINT_DEVELOPER,
                    "Loading scripts/%s.glsl GLSL program file (%i bytes)\n",
                    glsl_progs[i].name, len);
        } else {
            ri.Printf(PRINT_DEVELOPER,
                      "Loading %s.cfg GLSL program file (%i bytes)\n",
                      glsl_progs[i].name, len);
        }
        ...
        if (len > 0) {
            if (strlen(buffer) != len) {
                ...
                ri.Printf(PRINT_WARNING,
                          "Can't load %s GLSL program from file\n",
                          glsl_progs[i].name);
            } else {
                byte *program;
...
const char *R_GLSLGetProgByName(const char *name) {
    ...
    ri.Printf(PRINT_DEVELOPER,
              "Bug!!! You're asking for unknown '%s' GLSL program\n", name);
    return NULL;
}
