...
static int R_MDRCullModel(mdrHeader_t *header, trRefEntity_t *ent) {
    ...
    newFrame = (mdrFrame_t *)((byte *)header + header->ofsFrames +
                              frameSize * ent->e.frame);
    oldFrame = (mdrFrame_t *)((byte *)header + header->ofsFrames +
                              frameSize * ent->e.oldframe);
    ...
    if (!ent->e.nonNormalizedAxes) {
        if (ent->e.frame == ent->e.oldframe) {
            switch (R_CullLocalPointAndRadius(newFrame->localOrigin,
                                              newFrame->radius)) {
                // Ummm... yeah yeah I know we don't really have an md3 here..
                // but we pretend we do. After all, the purpose of mdrs are not
...
int R_MDRComputeFogNum(mdrHeader_t *header, trRefEntity_t *ent) {
    ...
    mdrFrame = (mdrFrame_t *)((byte *)header + header->ofsFrames +
                              frameSize * ent->e.frame);
...
void R_MDRAddAnimSurfaces(trRefEntity_t *ent) {
    ...
    if ((ent->e.frame >= header->numFrames) || (ent->e.frame < 0) ||
        ...
        ri.Printf(PRINT_DEVELOPER,
                  "R_MDRAddAnimSurfaces: no such frame %d to %d for '%s'\n",
    ...
    // figure out the current LOD of the model we're rendering, and set the lod
    // pointer respectively.
    lodnum = R_ComputeLOD(ent);
    // check whether this model has as that many LODs at all. If not, try the
...
void RB_MDRSurfaceAnim(mdrSurface_t *surface) {
    ...
    // don't lerp if lerping off, or this is the only frame, or the last
    // frame...
    //
    ...
    for (j = 0; j < numVerts; j++) {
        ...
        for (k = 0; k < v->numWeights; k++, w++) {
            ...
            tempNormal[0] +=
                w->boneWeight * DotProduct(bone->matrix[0], v->normal);
            tempNormal[1] +=
                w->boneWeight * DotProduct(bone->matrix[1], v->normal);
            tempNormal[2] +=
                w->boneWeight * DotProduct(bone->matrix[2], v->normal);
        }
...
#define MC_SHIFT_V12                                                           \
    ((((MC_BITS_X + MC_BITS_Y + MC_BITS_Z + MC_BITS_VECT) % 8)))

#define MC_POS_V13                                                             \
    ((((MC_BITS_X + MC_BITS_Y + MC_BITS_Z + MC_BITS_VECT * 2)) / 8))
#define MC_SHIFT_V13                                                           \
    ((((MC_BITS_X + MC_BITS_Y + MC_BITS_Z + MC_BITS_VECT * 2) % 8)))

#define MC_POS_V21                                                             \
    ((((MC_BITS_X + MC_BITS_Y + MC_BITS_Z + MC_BITS_VECT * 3)) / 8))
#define MC_SHIFT_V21                                                           \
    ((((MC_BITS_X + MC_BITS_Y + MC_BITS_Z + MC_BITS_VECT * 3) % 8)))

#define MC_POS_V22                                                             \
    ((((MC_BITS_X + MC_BITS_Y + MC_BITS_Z + MC_BITS_VECT * 4)) / 8))
#define MC_SHIFT_V22                                                           \
    ((((MC_BITS_X + MC_BITS_Y + MC_BITS_Z + MC_BITS_VECT * 4) % 8)))

#define MC_POS_V23                                                             \
    ((((MC_BITS_X + MC_BITS_Y + MC_BITS_Z + MC_BITS_VECT * 5)) / 8))
#define MC_SHIFT_V23                                                           \
    ((((MC_BITS_X + MC_BITS_Y + MC_BITS_Z + MC_BITS_VECT * 5) % 8)))

#define MC_POS_V31                                                             \
    ((((MC_BITS_X + MC_BITS_Y + MC_BITS_Z + MC_BITS_VECT * 6)) / 8))
#define MC_SHIFT_V31                                                           \
    ((((MC_BITS_X + MC_BITS_Y + MC_BITS_Z + MC_BITS_VECT * 6) % 8)))

#define MC_POS_V32                                                             \
    ((((MC_BITS_X + MC_BITS_Y + MC_BITS_Z + MC_BITS_VECT * 7)) / 8))
#define MC_SHIFT_V32                                                           \
    ((((MC_BITS_X + MC_BITS_Y + MC_BITS_Z + MC_BITS_VECT * 7) % 8)))

#define MC_POS_V33                                                             \
    ((((MC_BITS_X + MC_BITS_Y + MC_BITS_Z + MC_BITS_VECT * 8)) / 8))
#define MC_SHIFT_V33                                                           \
    ((((MC_BITS_X + MC_BITS_Y + MC_BITS_Z + MC_BITS_VECT * 8) % 8)))
