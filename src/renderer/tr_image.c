...
void R_ImageList_f(void) {
    ...
    for (i = 0; i < tr.numImages; i++) {
        ...
        ri.Printf(PRINT_ALL, "%4i: %4ix%4i %s %4i%s %s\n", i,
                  image->uploadWidth, image->uploadHeight, format, displaySize,
                  sizeSuffix, image->imgName);
...
================
*/
static void ResampleTexture(unsigned *in, int inwidth, int inheight,
...
static void R_MipMap2(unsigned *in, int inWidth, int inHeight) {
    ...
    for (i = 0; i < outHeight; i++) {
        for (j = 0; j < outWidth; j++) {
            ...
            for (k = 0; k < 4; k++) {
                total =
                    1 * ((byte *)&in[((i * 2 - 1) & inHeightMask) * inWidth +
                                     ((j * 2 - 1) & inWidthMask)])[k] +
                    2 * ((byte *)&in[((i * 2 - 1) & inHeightMask) * inWidth +
                                     ((j * 2) & inWidthMask)])[k] +
                    2 * ((byte *)&in[((i * 2 - 1) & inHeightMask) * inWidth +
                                     ((j * 2 + 1) & inWidthMask)])[k] +
                    1 * ((byte *)&in[((i * 2 - 1) & inHeightMask) * inWidth +
                    ...
                    2 * ((byte *)&in[((i * 2) & inHeightMask) * inWidth +
                                     ((j * 2 - 1) & inWidthMask)])[k] +
                    4 * ((byte *)&in[((i * 2) & inHeightMask) * inWidth +
                                     ((j * 2) & inWidthMask)])[k] +
                    4 * ((byte *)&in[((i * 2) & inHeightMask) * inWidth +
                                     ((j * 2 + 1) & inWidthMask)])[k] +
                    2 * ((byte *)&in[((i * 2) & inHeightMask) * inWidth +
                    ...
                    2 * ((byte *)&in[((i * 2 + 1) & inHeightMask) * inWidth +
                                     ((j * 2 - 1) & inWidthMask)])[k] +
                    4 * ((byte *)&in[((i * 2 + 1) & inHeightMask) * inWidth +
                                     ((j * 2) & inWidthMask)])[k] +
                    4 * ((byte *)&in[((i * 2 + 1) & inHeightMask) * inWidth +
                                     ((j * 2 + 1) & inWidthMask)])[k] +
                    2 * ((byte *)&in[((i * 2 + 1) & inHeightMask) * inWidth +
                    ...
                    1 * ((byte *)&in[((i * 2 + 2) & inHeightMask) * inWidth +
                                     ((j * 2 - 1) & inWidthMask)])[k] +
                    2 * ((byte *)&in[((i * 2 + 2) & inHeightMask) * inWidth +
                                     ((j * 2) & inWidthMask)])[k] +
                    2 * ((byte *)&in[((i * 2 + 2) & inHeightMask) * inWidth +
                                     ((j * 2 + 1) & inWidthMask)])[k] +
                    1 * ((byte *)&in[((i * 2 + 2) & inHeightMask) * inWidth +
...
===============
*/
static void Upload32(unsigned *data, int width, int height, qboolean mipmap,
                     qboolean picmip, qboolean lightMap, int *format,
    ...
    if (scaled_width != width || scaled_height != height) {
        resampledBuffer =
            ri.Hunk_AllocateTempMemory(scaled_width * scaled_height * 4);
    ...
    scaledBuffer = ri.Hunk_AllocateTempMemory(sizeof(unsigned) * scaled_width *
                                              scaled_height);
    ...
    if ((scaled_width == width) && (scaled_height == height)) {
        if (!mipmap) {
            qglTexImage2D(GL_TEXTURE_2D, 0, internalFormat, scaled_width,
    ...
    *format = internalFormat;
    ...
    qglTexImage2D(GL_TEXTURE_2D, 0, internalFormat, scaled_width, scaled_height,
    ...
    if (mipmap) {
        ...
        while (scaled_width > 1 || scaled_height > 1) {
            ...
            if (r_colorMipLevels->integer) {
                R_BlendOverTexture((byte *)scaledBuffer,
                                   scaled_width * scaled_height,
                                   mipBlendColors[miplevel]);
            }

            qglTexImage2D(GL_TEXTURE_2D, miplevel, internalFormat, scaled_width,
...
image_t *R_CreateImage(const char *name, byte *pic, int width, int height,
    ...
    Upload32((unsigned *)pic, image->width, image->height,
             image->flags & IMGFLAG_MIPMAP, image->flags & IMGFLAG_PICMIP,
        ...
        if (!strcmp(name, image->imgName)) {
            ...
            if (strcmp(name, "*white")) {
                if (image->flags != flags) {
                    ri.Printf(PRINT_DEVELOPER,
                              "WARNING: reused image %s with mixed flags (%i "
                              "vs %i)\n",
...
static void R_CreateDlightImage(void) {
    ...
    tr.dlightImage =
        R_CreateImage("*dlight", (byte *)data, DLIGHT_SIZE, DLIGHT_SIZE,
...
static void R_CreateFogImage(void) {
    ...
    for (x = 0; x < FOG_S; x++) {
        for (y = 0; y < FOG_T; y++) {
            ...
            data[(y * FOG_S + x) * 4 + 0] = data[(y * FOG_S + x) * 4 + 1] =
...
static void R_CreateDefaultImage(void) {
    ...
    tr.defaultImage =
        R_CreateImage("*default", (byte *)data, DEFAULT_SIZE, DEFAULT_SIZE,
...
void R_CreateBuiltinImages(void) {
    ...
    tr.whiteImage = R_CreateImage("*white", (byte *)data, 8, 8,
                                  IMGTYPE_COLORALPHA, IMGFLAG_NONE, 0);
    ...
    tr.identityLightImage = R_CreateImage("*identityLight", (byte *)data, 8, 8,
                                          IMGTYPE_COLORALPHA, IMGFLAG_NONE, 0);
...
qhandle_t RE_RegisterSkin(const char *name) {
    ...
    while (text_p && *text_p) {
        ...
        surf = skin->surfaces[skin->numSurfaces] =
            ri.Hunk_Alloc(sizeof(*skin->surfaces[0]), h_low);
