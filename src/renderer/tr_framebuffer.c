...
//		- Motion blur
//			- generate a speed vector based on how the camera has
//moved since 			  the last frame and use that to compute per pixel blur vectors
//		- These would require mods to use some sort of framebuffer qvm
//api
...
qboolean useMultisample =
    qfalse; // is set after multisample support is initialized
qboolean useBloomEffect = qfalse; // is set after bloom effect is initialized
qboolean useRotoscopeEffect =
    qfalse; // is set after rotoscope effect is initialized
qboolean useRotoZEdgeEffect =
    qfalse;                 // is set after rotoscope zedge effect is selected
...
// FBO error reporting inspired by
...
qboolean R_CheckFramebufferStatus(const char *tag) {
    ...
    case GL_FRAMEBUFFER_UNSUPPORTED_EXT:
        message = "Framebuffer object format is unsupported by the video "
                  "hardware. (GL_FRAMEBUFFER_UNSUPPORTED_EXT)(FBO - 820)";
        break;
    ...
    case GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE_EXT:
        message = "Incomplete multisample buffer. "
                  "(GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE_EXT)(FBO - 820)";
        break;
    default:
        // Programming error; will fail on all hardware
        message = "Some video driver error or programming error occured. "
...
void R_DrawQuadMT(GLuint tex1, GLuint tex2, int width, int height) {
    ...
    qglBegin(GL_QUADS);
    qglMultiTexCoord2fARB(GL_TEXTURE0, 0.0, 1.0);
    qglMultiTexCoord2fARB(GL_TEXTURE1, 0.0, 1.0);
    qglVertex2f(0.0, 0.0);
    qglMultiTexCoord2fARB(GL_TEXTURE0, 1.0, 1.0);
    qglMultiTexCoord2fARB(GL_TEXTURE1, 1.0, 1.0);
    qglVertex2f(width, 0.0);
    qglMultiTexCoord2fARB(GL_TEXTURE0, 1.0, 0.0);
    qglMultiTexCoord2fARB(GL_TEXTURE1, 1.0, 0.0);
    qglVertex2f(width, height);
    qglMultiTexCoord2fARB(GL_TEXTURE0, 0.0, 0.0);
    qglMultiTexCoord2fARB(GL_TEXTURE1, 0.0, 0.0);
    qglVertex2f(0.0, height);
    qglEnd();
...
GLuint *R_CreateTexbuffer(GLuint *store, int width, int height, qboolean smooth,
                          GLenum bindSize, GLenum bindType) {
    ...
    qglTexImage2D(GL_TEXTURE_2D, 0, bindType, width, height, 0, bindType,
                  bindSize, NULL);
...
int R_CreateFBuffer(struct r_fbuffer *buffer, int width, int height,
                    int flags) {
    ...
    if (flags & FB_STENCIL) {
        if (flags & FB_PACKED) {
            if (flags & FB_ZTEXTURE) {
                ...
                R_CreateRenderbuffer(&buffer->depth, width, height,
                                     GL_DEPTH_STENCIL_EXT,
                                     flags & FB_MULTISAMPLE);
                qglFramebufferRenderbufferEXT(
                    GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT,
                    GL_RENDERBUFFER_EXT, buffer->depth);

                qglFramebufferRenderbufferEXT(
                    GL_FRAMEBUFFER_EXT, GL_STENCIL_ATTACHMENT_EXT,
        ...
        if (flags & FB_SEPARATEZS) {
            if (flags & FB_ZTEXTURE) {
                ...
                R_CreateRenderbuffer(&buffer->stencil, width, height,
        ...
        if (flags & FB_ZTEXTURE) {
            ...
            R_CreateRenderbuffer(&buffer->depth, width, height,
                                 GL_DEPTH_STENCIL_EXT, flags & FB_MULTISAMPLE);
    ...
    R_CreateTexbuffer(&buffer->front, width, height, filter, GL_UNSIGNED_BYTE,
                      GL_RGBA);
...
qboolean R_TestFbuffer_SeparateDS(void) {
    ...
    R_CreateTexbuffer(&buffers[0], width, height, qfalse, GL_UNSIGNED_BYTE,
                      GL_RGBA);
...
qboolean R_TestFbuffer_texD(void) {
    ...
    R_CreateTexbuffer(&buffers[0], width, height, qfalse, GL_UNSIGNED_INT,
                      GL_DEPTH_COMPONENT);
...
void R_Build_glsl(struct glslobj *obj) {
    ...
    qglShaderSource(frag_shader, obj->frag_numSources, obj->fragment_glsl,
                    NULL);
...
void R_FrameBuffer_BlurInit(void) {
    ...
    blur_size = (r_ext_framebuffer_blur_size->integer < 2)
                    ? 2
                    : r_ext_framebuffer_blur_size->integer;

    R_CreateFBuffer(&gaussblurBuffer, blur_size, blur_size,
                    FB_BACKBUFFER | FB_SMOOTH);
    R_FrameBufferUnBind();
    ...
    glslBlur.fragment_glsl =
        ri.Malloc(sizeof(char *) * glslBlur.frag_numSources);
...
void R_FrameBuffer_RotoInit(struct r_fbuffer *src) {
    ...
    glslRoto.fragment_glsl =
        ri.Malloc(sizeof(char *) * glslRoto.frag_numSources);
...
void R_FrameBuffer_RotoDraw(struct r_fbuffer *src, GLuint *srcTex) {
    ...
    if (useRotoZEdgeEffect) {
        R_DrawQuadMT(*srcTex, src->depth, glConfig.vidWidth,
                     glConfig.vidHeight);
    } else {
...
void R_FrameBuffer_BloomInit(void) {
    ...
    glslBloom.vertex_glsl =
        ri.Malloc(sizeof(char *) * glslBloom.frag_numSources);
    glslBloom.vertex_glsl[0] = R_GLSLGetProgByName("glslBase_vert");
    glslBloom.fragment_glsl =
        ri.Malloc(sizeof(char *) * glslBloom.frag_numSources);
...
void R_FrameBuffer_BloomDraw(GLuint *srcTex) {
    ...
    R_DrawQuadMT(*srcTex, gaussblurBuffer.front, glConfig.vidWidth,
                 glConfig.vidHeight);
    qglUseProgram(0);
    // quick test to just see the blur
    // R_DrawQuad(*gaussblurBuffer.front, glConfig.vidWidth,
    // glConfig.vidHeight);
}
...
void R_FrameBuffer_Init(void) {
    ...
    if (R_TestFbuffer_SeparateDS()) {
        ...
        ri.Printf(PRINT_WARNING,
                  "WARNING: separate depth stencil not supported\n");
    }
    ...
    if (maxbuffers < 2) {
        ...
        ri.Printf(PRINT_WARNING, "ERROR: Framebuffer rendering path disabled: "
                                 "Not enough color buffers available\n");
        ri.Cmd_ExecuteText(EXEC_APPEND,
                           "vid_restart"); // Reset the video without FBO
        return;
    }
    ...
    if ((depthTextureSupported) &&
        (r_ext_framebuffer_rotoscope_zedge->integer)) {
