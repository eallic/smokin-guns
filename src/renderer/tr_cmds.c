...
void R_PerformanceCounters(void) {
    ...
    if (r_speeds->integer == 1) {
        ri.Printf(PRINT_ALL,
                  "%i/%i shaders/surfs %i leafs %i verts %i/%i tris %.2f mtex "
                  "%.2f dc\n",
                  backEnd.pc.c_shaders, backEnd.pc.c_surfaces, tr.pc.c_leafs,
        ...
        ri.Printf(PRINT_ALL,
                  ...
                  tr.pc.c_sphere_cull_md3_out, tr.pc.c_box_cull_md3_in,
                  tr.pc.c_box_cull_md3_clip, tr.pc.c_box_cull_md3_out);
    } else if (r_speeds->integer == 3) {
        ri.Printf(PRINT_ALL, "viewcluster: %i\n", tr.viewCluster);
    } else if (r_speeds->integer == 4) {
        if (backEnd.pc.c_dlightVertexes) {
            ri.Printf(
                PRINT_ALL, "dlight srf:%i  culled:%i  verts:%i  tris:%i\n",
...
void RE_BeginFrame(stereoFrame_t stereoFrame) {
    ...
    if (r_measureOverdraw->integer) {
        if (glConfig.stencilBits < 4) {
            ...
            ri.Printf(PRINT_ALL, "Warning: stencil shadows and overdraw "
