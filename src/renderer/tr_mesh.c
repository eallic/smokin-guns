...
static int R_CullModel(md3Header_t *header, trRefEntity_t *ent) {
    ...
    newFrame =
        (md3Frame_t *)((byte *)header + header->ofsFrames) + ent->e.frame;
    oldFrame =
        (md3Frame_t *)((byte *)header + header->ofsFrames) + ent->e.oldframe;
...
int R_ComputeLOD(trRefEntity_t *ent) {
    ...
    if (tr.currentModel->numLods < 2) {
        ...
        if (tr.currentModel->type == MOD_MDR) {
            ...
            mdrframe = (mdrFrame_t *)((byte *)mdr + mdr->ofsFrames +
                                      frameSize * ent->e.frame);

            radius = RadiusFromBounds(mdrframe->bounds[0], mdrframe->bounds[1]);
        } else {
            frame = (md3Frame_t *)(((unsigned char *)tr.currentModel->md3[0]) +
