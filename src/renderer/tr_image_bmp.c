...
void R_LoadBMP(const char *name, byte **pic, int *width, int *height) {
    ...
    *pic = NULL;
    ...
    if (bmpHeader.id[0] != 'B' && bmpHeader.id[1] != 'M') {
        ri.Error(ERR_DROP,
                 "LoadBMP: only Windows-style BMP files supported (%s)", name);
    }
    if (bmpHeader.fileSize != length) {
        ri.Error(
            ERR_DROP,
            "LoadBMP: header size does not match file size (%u vs. %u) (%s)",
            bmpHeader.fileSize, length, name);
    }
    if (bmpHeader.compression != 0) {
        ri.Error(ERR_DROP,
                 "LoadBMP: only uncompressed BMP files supported (%s)", name);
    }
    if (bmpHeader.bitsPerPixel < 8) {
        ri.Error(ERR_DROP,
    ...
    if (columns <= 0 || !rows ||
        numPixels > 0x1FFFFFFF // 4*1FFFFFFF == 0x7FFFFFFC < 0x7FFFFFFF
