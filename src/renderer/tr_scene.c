...
void RE_AddPolyToScene(qhandle_t hShader, int numVerts, const polyVert_t *verts,
    ...
    for (j = 0; j < numPolys; j++) {
        if (r_numpolyverts + numVerts > max_polyverts ||
            ...
            since we don't plan on changing the const and making for room for
            those effects simply cut this message to developer only
            */
            ri.Printf(PRINT_DEVELOPER,
                      "WARNING: RE_AddPolyToScene: r_max_polys or "
        ...
        Com_Memcpy(poly->verts, &verts[numVerts * j],
                   numVerts * sizeof(*verts));
...
void RE_AddRefEntityToScene(const refEntity_t *ent) {
    ...
    if (r_numentities >= MAX_REFENTITIES) {
        ri.Printf(PRINT_DEVELOPER, "RE_AddRefEntityToScene: Dropping "
                                   "refEntity, reached MAX_REFENTITIES\n");
        return;
    }
    if (Q_isnan(ent->origin[0]) || Q_isnan(ent->origin[1]) ||
        ...
        if (firstTime) {
            firstTime = qfalse;
            ri.Printf(PRINT_WARNING,
                      "RE_AddRefEntityToScene passed a refEntity which has an "
                      "origin with a NaN component\n");
        }
        return;
    }
