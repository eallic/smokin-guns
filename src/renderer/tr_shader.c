...
static int NameToSrcBlendMode(const char *name) {
    ...
    ri.Printf(PRINT_WARNING,
              "WARNING: unknown blend mode '%s' in shader '%s', substituting "
              "GL_ONE\n",
              name, shader.name);
    return GLS_SRCBLEND_ONE;
}
...
static qboolean ParseStage(shaderStage_t *stage, char **text) {
    ...
    while (1) {
        ...
        //
            ...
            if (!token[0]) {
                ri.Printf(PRINT_WARNING,
                          "WARNING: missing parameter for 'map' keyword in "
                          "shader '%s'\n",
                          shader.name);
                return qfalse;
            }
            ...
            if (!Q_stricmp(token, "$whiteimage")) {
                ...
                if (!stage->bundle[0].image[0]) {
                    ri.Printf(PRINT_WARNING,
                              "WARNING: R_FindImageFile could not find '%s' in "
                              "shader '%s'\n",
                              token, shader.name);
                    return qfalse;
                }
        ...
        //
            ...
            if (!token[0]) {
                ri.Printf(PRINT_WARNING,
                          "WARNING: missing parameter for 'clampmap' keyword "
                          "in shader '%s'\n",
                          shader.name);
                return qfalse;
            }
        ...
        //
            ...
            if (!token[0]) {
                ri.Printf(PRINT_WARNING,
                          "WARNING: missing parameter for 'animMap' keyword in "
                          "shader '%s'\n",
                          shader.name);
                return qfalse;
            }
            ...
            while (1) {
                ...
                if (num < MAX_IMAGE_ANIMATIONS) {
                    ...
                    stage->bundle[0].image[num] =
                        R_FindImageFile(token, IMGTYPE_COLORALPHA, flags);
            ...
            if (!token[0]) {
                ri.Printf(PRINT_WARNING,
                          "WARNING: missing parameter for 'videoMap' keyword "
                          "in shader '%s'\n",
                          shader.name);
                return qfalse;
            }
        ...
        //
            ...
            if (!token[0]) {
                ri.Printf(PRINT_WARNING,
                          "WARNING: missing parameter for 'alphaFunc' keyword "
                          "in shader '%s'\n",
                          shader.name);
                return qfalse;
            }
        ...
        //
            ...
            if (!token[0]) {
                ri.Printf(PRINT_WARNING,
                          "WARNING: missing parameter for 'depthfunc' keyword "
                          "in shader '%s'\n",
                          shader.name);
                return qfalse;
            }
        ...
        //
            ...
            if (!Q_stricmp(token, "wave")) {
                ...
                if (token[0] == 0) {
                    ...
                    ri.Printf(PRINT_WARNING,
                              "WARNING: missing range parameter for alphaGen "
                              "portal in shader '%s', defaulting to 256\n",
                              shader.name);
                } else {
    ...
    //
    // if cgen isn't explicitly specified, use either identity or
    // identitylighting
    //
...
static void ParseDeform(char **text) {
    ...
    if (!Q_stricmp(token, "wave")) {
        ...
        if (atof(token) != 0) {
            ...
            ri.Printf(PRINT_WARNING,
                      "WARNING: illegal div value of 0 in deformVertexes "
                      "command for shader '%s'\n",
                      shader.name);
        }
    ...
    if (strcmp(token, "-")) {
        for (i = 0; i < 6; i++) {
            ...
            shader.sky.outerbox[i] =
                R_FindImageFile((char *)pathname, IMGTYPE_COLORALPHA,
                                imgFlags | IMGFLAG_CLAMPTOEDGE);
    ...
    if (strcmp(token, "-")) {
        for (i = 0; i < 6; i++) {
            ...
            shader.sky.innerbox[i] =
                R_FindImageFile((char *)pathname, IMGTYPE_COLORALPHA, imgFlags);
...
static qboolean ParseShader(char **text) {
    ...
    while (1) {
        ...
        // fogParms
            ...
            if (!token[0]) {
                ri.Printf(PRINT_WARNING,
                          "WARNING: missing parm for 'fogParms' keyword in "
                          "shader '%s'\n",
                          shader.name);
                continue;
            }
...
static void ComputeStageIteratorFunc(void) {
    ...
    //
    if (shader.numUnfoggedPasses == 1) {
        if ((stages[0].rgbGen == CGEN_IDENTITY) &&
...
static qboolean CollapseMultitexture(void) {
    ...
    if ((abits &
         ~(GLS_DSTBLEND_BITS | GLS_SRCBLEND_BITS | GLS_DEPTHMASK_TRUE)) !=
        (bbits &
         ~(GLS_DSTBLEND_BITS | GLS_SRCBLEND_BITS | GLS_DEPTHMASK_TRUE))) {
        return qfalse;
    }
    ...
    //
    memmove(&stages[1], &stages[2],
            sizeof(stages[0]) * (MAX_SHADER_STAGES - 2));
...
frame, cause drawsurfaces to be rendered with bad shaders. To fix this, need to
go through all render commands and fix sortedIndex.
==============
*/
static void FixRenderCommandList(int newShader) {
    ...
    if (cmdList) {
        ...
        while (1) {
            ...
            case RC_SET_COLOR: {
                const setColorCommand_t *sc_cmd =
                    (const setColorCommand_t *)curCmd;
            ...
            case RC_DRAW_SURFS: {
                ...
                const drawSurfsCommand_t *ds_cmd =
                    (const drawSurfsCommand_t *)curCmd;

                for (i = 0, drawSurf = ds_cmd->drawSurfs;
                     i < ds_cmd->numDrawSurfs; i++, drawSurf++) {
                    R_DecomposeSort(drawSurf->sort, &entityNum, &shader,
                                    &fogNum, &dlightMap);
                    sortedIndex = ((drawSurf->sort >> QSORT_SHADERNUM_SHIFT) &
                                   (MAX_SHADERS - 1));
                    if (sortedIndex >= newShader) {
                        sortedIndex++;
                        drawSurf->sort =
                            (sortedIndex << QSORT_SHADERNUM_SHIFT) | entityNum |
                            (fogNum << QSORT_FOGNUM_SHIFT) | (int)dlightMap;
                    }
                }
    ...
    for (i = 0; i < newShader->numUnfoggedPasses; i++) {
        ...
        *newShader->stages[i] = stages[i];
        ...
        for (b = 0; b < NUM_TEXTURE_BUNDLES; b++) {
            size = newShader->stages[i]->bundle[b].numTexMods *
                   sizeof(texModInfo_t);
            newShader->stages[i]->bundle[b].texMods =
                ri.Hunk_Alloc(size, h_low);
        ...
        if (pStage->isDetail && !r_detailTextures->integer) {
            ...
            if (index < MAX_SHADER_STAGES)
                ...
                if (stage + 1 < MAX_SHADER_STAGES)
                    memmove(pStage, pStage + 1,
                            sizeof(*pStage) * (index - stage - 1));
...
shader_t *R_FindShaderByName(const char *name) {
    ...
    for (sh = hashTable[hash]; sh; sh = sh->next) {
        ...
        // strippedName then a default shader is created with lightmapIndex ==
        // LIGHTMAP_NONE, so we have to check all default shaders otherwise for
        // every call to R_FindShader with that same strippedName a new default
        // shader is created.
        if (Q_stricmp(sh->name, strippedName) == 0) {
            // match found
            return sh;
...
===============
*/
shader_t *R_FindShader(const char *name, int lightmapIndex,
    ...
    if (lightmapIndex >= 0 && lightmapIndex >= tr.numLightmaps) {
        ...
        // negative lightmap indexes cause stray pointers (think
        // tr.lightmaps[lightmapIndex])
        ri.Printf(PRINT_WARNING,
...
qhandle_t RE_RegisterShaderFromImage(const char *name, int lightmapIndex,
                                     image_t *image, qboolean mipRawImage) {
    int i, hash;
    shader_t *sh;
...
void R_ShaderList_f(void) {
    ...
    for (i = 0; i < tr.numShaders; i++) {
        ...
        } else if (shader->optimalStageIteratorFunc ==
                   RB_StageIteratorLightmappedMultitexture) {
...
static void ScanAndLoadShaderFiles(void) {
    ...
    for (i = 0; i < numShaderFiles; i++) {
        ...
        // Do a simple check on the shader structure in that file to make sure
        // one bad shader file cannot fuck up all other shaders.
        p = buffers[i];
        ...
        while (1) {
            ...
            if (token[0] != '{' || token[1] != '\0') {
                ri.Printf(PRINT_WARNING,
                          "WARNING: Ignoring shader file %s. Shader \"%s\" on "
                          "line %d missing opening brace",
                          filename, shaderName, shaderLine);
                if (token[0]) {
                    ri.Printf(PRINT_WARNING, " (found \"%s\" on line %d)",
    ...
    for (i = 0; i < MAX_SHADERTEXT_HASH; i++) {
        ...
        hashMem = ((char *)hashMem) +
                  ((shaderTextHashTableSizes[i] + 1) * sizeof(char *));
    }
