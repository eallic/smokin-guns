...
/*
 ...
 days. However, qsort(3) already exists, and I'm really lazy. -PH
*/
...
/*
 Problem:
        BYTE values are specified as signed decimal string.  A properly
 functional atoip() will cap large signed values at 0x7FFFFFFF.  Negative word
 values are often specified as very large decimal values by lcc.  Therefore,
 values that should be between 0x7FFFFFFF and 0xFFFFFFFF come out as 0x7FFFFFFF
 when using atoi().  Bad.
...
static char *ExtractLine(char *data) {
    /* Goal:
             Given a string `data', extract one text line into buffer
     `lineBuffer' that is no longer than MAX_LINE_LENGTH characters long. Return
     value is remainder of `data' that isn't part of `lineBuffer'. -PH
    */
...
static int ParseExpression(void) {
    ...
    switch (*sym) { /* Resolve depending on first character. */
        /* Optimizing compilers can convert cases into "calculated jumps".  I
         * think these are faster.  -PH */
    case '-':
    case '0':
    case '1':
    case '2':
...
static void WriteVmFile(void) {
    ...
    if (!options.vanillaQ3Compatibility) {
        ...
        // Don't write the VM_MAGIC_VER2 bits when maintaining 1.32b
        // compatibility. (I know this isn't strictly correct due to padding,
        // but then platforms that pad wouldn't be able to write a correct
        // header anyway).  Note: if vmHeader_t changes, this needs to be
        // adjusted too.
...
int main(int argc, char **argv) {
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (!strcmp(argv[i], "-v")) {
            ...
            errors. Verbosity turns on all messages, error or not. Motivation:
            not wanting to scrollback for pages to find asm error.
            */
