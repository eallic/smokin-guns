...
Tree expr0(int tok) { return root(expr(tok)); }
Tree expr1(int tok) {
    static char stop[] = {IF, ID, 0};
    Tree p = expr2();
...
static Tree primary(void) {
    ...
    case SCON:
        if (ischar(tsym->type->type))
            ...
            tsym->u.c.v.p = memcpy(allocate(tsym->type->size, PERM),
                                   tsym->u.c.v.p, tsym->type->size);
        ...
        if (tsym == NULL) {
            ...
            if (getchr() == '(') {
                ...
                if (q && !eqtype(q->type, p->type, 1))
                    warning("implicit declaration of `%s' does not match "
                            "previous declaration at %w\n",
                            q->name, &q->src);
            ...
            if (src->op != dst->op)
                ...
                case FLOAT:
                    if (isunsigned(dst)) {
                        ...
                        Tree c = cast(
                            cnsttree(longdouble,
                                     (double)sdst->u.sym->u.limits.max.i + 1),
                            src);
                        p = condtree(
                            simplify(GE, src, p, c),
                            (*optree['+'])(
            ...
            p = tree(RIGHT, p->type,
                     tree(OR, voidtype, cond(asgn(t1, cast(p, voidptype))),
                          vcall(YYnull, voidtype,
                                (file && *file
                                     ? pointer(idtree(mkstr(file)->u.c.loc))
...
Tree nullcall(Type pty, Symbol f, Tree p, Tree e) {
    ...
    return vcall(f, pty, p, e, cnsttree(inttype, (long)ty->size),
