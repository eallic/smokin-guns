...
#define xfoldcnst(TYPE, VAR, OP, FUNC)                                         \
    if (l->op == CNST + TYPE && r->op == CNST + TYPE &&                        \
        FUNC(l->u.v.VAR, r->u.v.VAR, ty->u.sym->u.limits.min.VAR,              \
             ty->u.sym->u.limits.max.VAR, needconst))                          \
    return cnsttree(ty, l->u.v.VAR OP r->u.v.VAR)
#define xcvtcnst(FTYPE, SRC, DST, VAR, EXPR)                                   \
    if (l->op == CNST + FTYPE)                                                 \
        do {                                                                   \
            if (!explicitCast && ((SRC) < DST->u.sym->u.limits.min.VAR ||      \
                                  (SRC) > DST->u.sym->u.limits.max.VAR))       \
                warning("overflow in converting constant expression from "     \
                        "`%t' to `%t'\n",                                      \
                        l->type, DST);                                         \
            if (needconst || !((SRC) < DST->u.sym->u.limits.min.VAR ||         \
                               (SRC) > DST->u.sym->u.limits.max.VAR))          \
...
#define ufoldcnst(TYPE, EXP)                                                   \
    ...
    return EXP
#define sfoldcnst(OP)                                                          \
    if (l->op == CNST + U && r->op == CNST + I && r->u.v.i >= 0 &&             \
...
#define geu(L, R, V)                                                           \
    if (R->op == CNST + U && R->u.v.u == 0)                                    \
        do {                                                                   \
            ...
            return tree(RIGHT, inttype, root(L),                               \
                        cnsttree(inttype, (long)(V)));                         \
    } while (0)
...
static int addi(long x, long y, long min, long max, int needconst) {
    int cond = x == 0 || y == 0 || (x < 0 && y < 0 && x >= min - y) ||
               (x < 0 && y > 0) || (x > 0 && y < 0) ||
               (x > 0 && y > 0 && x <= max - y);
    if (!cond && needconst) {
...
Tree simplify(int op, Type ty, Tree l, Tree r) {
    ...
    case CVU + I:
        if (l->op == CNST + U) {
            if (!explicitCast && l->u.v.u > ty->u.sym->u.limits.max.i)
                warning("overflow in converting constant expression from `%t' "
                        "to `%t'\n",
                        l->type, ty);
    ...
    case CVP + P:
        ...
        break;
    case CVI + U:
        xcvtcnst(I, l->u.v.i, ty, u,
                 ((unsigned long)l->u.v.i) & ones(8 * ty->size));
        break;
    case CVU + U:
    ...
    case ADD + P:
        ...
        if (isaddrop(l->op) && ((r->op == CNST + I &&
                                 r->u.v.i <= longtype->u.sym->u.limits.max.i &&
                                 r->u.v.i >= longtype->u.sym->u.limits.min.i) ||
                                (r->op == CNST + U &&
                                 r->u.v.u <= longtype->u.sym->u.limits.max.i)))
        ...
        if (l->op == ADD + I && generic(l->kids[1]->op) == CNST &&
            ...
            return simplify(ADD + P, ty, l->kids[0],
                            simplify(ADD + P, ty, r->kids[0],
                                     simplify(ADD, r->kids[1]->type, l->kids[1],
                                              r->kids[1])));
    ...
    case OR + I:
        ...
        break;
    case BCOM + I:
        ufoldcnst(I, cnsttree(ty, (long)extend((~l->u.v.i) & ones(8 * ty->size),
                                               ty)));
        idempotent(BCOM + U);
        break;
    case BCOM + U:
        ufoldcnst(
            U, cnsttree(ty, (unsigned long)((~l->u.v.u) & ones(8 * ty->size))));
        idempotent(BCOM + U);
        break;
    case BOR + U:
        foldcnst(U, u, |);
    ...
    case LSH + I:
        ...
        if (l->op == CNST + I && r->op == CNST + I && r->u.v.i >= 0 &&
            r->u.v.i < 8 * l->type->size &&
            muli(l->u.v.i, 1 << r->u.v.i, ty->u.sym->u.limits.min.i,
                 ty->u.sym->u.limits.max.i, needconst))
    ...
    case SUB + P:
        ...
        if (r->op == CNST + I || r->op == CNST + U)
            return simplify(ADD, ty, l,
                            cnsttree(inttype, r->op == CNST + I
                                                  ? -r->u.v.i
                                                  : -(long)r->u.v.u));
        if (isaddrop(l->op) && r->op == ADD + I && r->kids[1]->op == CNST + I)
            /* l - (x + c) => l-c - x */
            return simplify(SUB, ty, simplify(SUB, ty, l, r->kids[1]),
                            r->kids[0]);
        break;
    default:
        assert(0);
