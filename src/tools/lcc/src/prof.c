...
static void bbcall(Symbol yycounts, Coordinate *cp, Tree *e) {
    ...
    t = tree(t->op, t->type,
             tree(RIGHT, t->kids[0]->type, t->kids[0],
                  tree(RIGHT, t->kids[0]->type, asgn(caller, idtree(p)),
                       t->kids[0])),
             t->kids[1]);
    if (generic((*e)->op) != CALL)
...
static void bbentry(Symbol yylink, Symbol f) {
    ...
    walk(vcall(prologue, voidtype, pointer(idtree(afunc)),
               pointer(idtree(yylink)), NULL),
         0, 0);
}
...
static void bbincr(Symbol yycounts, Coordinate *cp, Tree *e) {
    ...
    t = incr('+',
             rvalue((*optree['+'])(ADD, pointer(idtree(yycounts)),
                                   consttree(npoints++, inttype))),
             consttree(1, inttype));
    if (*e)
...
static void bbvars(Symbol yylink) {
    ...
    for (i = n, mp = ltov(&maplist, PERM); *mp; i -= (*mp)->size, mp++)
        for (j = 0; j < (*mp)->size; j++)
            (*IR->defconst)(U, unsignedtype->size,
                            (v.u = (*mp)->u[j].coord, v));
    if (i > 0)
        ...
        for (i = 1; i < argc; i++)
            if (strncmp(argv[i], "-a", 2) == 0) {
                ...
                if (strcmp(argv[i], "-b") == 0) {
                    YYcounts =
                        genident(STATIC, array(unsignedtype, 0, 0), GLOBAL);
                    maplist =
                        append(allocate(sizeof(struct map), PERM), maplist);
