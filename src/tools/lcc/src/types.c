...
#define xx(name)                                                               \
    else if (sscanf(argv[i], "-" #name "=%d,%d,%d", &size, &align,             \
                    &outofline) == 3) {                                        \
        IR->name.size = size;                                                  \
        IR->name.align = align;                                                \
        IR->name.outofline = outofline;                                        \
    }
        xx(charmetric) xx(shortmetric) xx(intmetric) xx(longmetric)
            xx(longlongmetric) xx(floatmetric) xx(doublemetric)
                xx(longdoublemetric) xx(ptrmetric) xx(structmetric)
#undef xx
    }
...
#define xx(v, t)                                                               \
    if (v == NULL && t->size == voidptype->size &&                             \
        t->align == voidptype->align)                                          \
    v = t
    ...
    if (unsignedptr == NULL)
        unsignedptr = type(UNSIGNED, NULL, voidptype->size, voidptype->align,
                           voidptype->u.sym);
    ...
    if (signedptr == NULL)
        signedptr = type(INT, NULL, voidptype->size, voidptype->align,
                         voidptype->u.sym);
#undef xx
...
#define xx(name, type)                                                         \
    if (strcmp(argv[i], "-wchar_t=" #name) == 0)                               \
        widechar = type;
        xx(unsigned_char, unsignedchar) xx(unsigned_int, unsignedtype)
...
Type ptr(Type ty) {
    return type(POINTER, ty, IR->ptrmetric.size, IR->ptrmetric.align,
                pointersym);
}
Type deref(Type ty) {
    if (isptr(ty))
...
Field fieldlist(Type ty) { return ty->u.sym->u.s.flist; }
...
static void prtype(Type ty, FILE *f, int indent, unsigned mark) {
    ...
    case UNSIGNED:
    case VOID:
        fprint(f, "(%k %d %d [\"%s\"])", ty->op, ty->size, ty->align,
               ty->u.sym->name);
        break;
    case CONST + VOLATILE:
    case CONST:
    ...
    case UNION:
        ...
        if (ty->x.marked != mark) {
            ...
            for (p = ty->u.sym->u.s.flist; p; p = p->link) {
                ...
                if (p->lsb)
                    fprint(f, ":%d..%d", fieldsize(p) + fieldright(p),
                           fieldright(p));
            }
            fprint(f, "\n%I", indent);
        }
...
char *typestring(Type ty, char *str) {
    for (; ty; ty = ty->type) {
        ...
        case INT:
        case UNSIGNED:
            return *str ? stringf("%s %s", ty->u.sym->name, str)
                        : ty->u.sym->name;
        case POINTER:
            if (!ischar(ty->type) && (p = findtype(ty)) != NULL)
                return *str ? stringf("%s %s", p->name, str) : p->name;
            str = stringf(
                isarray(ty->type) || isfunc(ty->type) ? "(*%s)" : "*%s", str);
            break;
        case FUNCTION:
            if ((p = findtype(ty)) != NULL)
                return *str ? stringf("%s %s", p->name, str) : p->name;
