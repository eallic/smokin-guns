...
Node listnodes(Tree tp, int tlab, int flab) {
    ...
    case LT: {
        ...
        if (tlab)
            assert(flab == 0), list(newnode(generic(tp->op) + opkind(l->op), l,
                                            r, findlabel(tlab)));
        else if (flab) {
            switch (generic(tp->op)) {
            case EQ:
    ...
    case CVU: {
        assert(tlab == 0 && flab == 0);
        assert(optype(tp->kids[0]->op) != optype(tp->op) ||
               tp->kids[0]->type->size != tp->type->size);
        l = listnodes(tp->kids[0], 0, 0);
...
static Node replace(Node p) {
    if (p &&
        (generic(p->op) == INDIR && generic(p->kids[0]->op) == ADDRL &&
         p->kids[0]->syms[0]->temporary && p->kids[0]->syms[0]->u.t.replace)) {
        p = p->kids[0]->syms[0]->u.t.cse;
        if (generic(p->op) == INDIR && isaddrop(p->kids[0]->op))
            p = newnode(
                p->op, newnode(p->kids[0]->op, NULL, NULL, p->kids[0]->syms[0]),
                NULL, NULL);
...
static Node prune(Node forest) {
    ...
    for (p = forest; p; p = p->link) {
        ...
        if ((generic(p->op) == ASGN && generic(p->kids[0]->op) == ADDRL &&
