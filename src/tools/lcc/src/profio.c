...
static void acaller(char *caller, char *file, int x, int y, int count,
                    struct func *callee) {
    struct caller *q;

    assert(callee);
    for (q = callee->callers;
         q && (caller != q->name || file != q->file || x != q->x || y != q->y);
         q = q->link)
        ;
    if (!q) {
        struct caller **r;
...
static struct func *afunction(char *name, char *file, int x, int y, int count) {
    ...
    if (!q) {
        ...
        q->count.count = 0;
        q->callers = 0;
        for (r = &p->funcs; *r && compare(&q->count, &(*r)->count) > 0;
             r = &(*r)->link)
            ;
        q->link = *r;
        *r = q;
...
static int gather(void) {
    ...
    for (i = 0; i < nfuncs; i++) {
        ...
        char *name, *file;
        int f, x, y, count;
        if ((name = getstr()) == 0 || (f = getd()) <= 0 || (x = getd()) < 0 ||
            (y = getd()) < 0 || (count = getd()) < 0)
            return -1;
