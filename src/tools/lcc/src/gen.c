...
void blkcopy(int dreg, int doff, int sreg, int soff, int size, int tmp[]) {
    ...
    if (size == 0)
        ...
        blkcopy(dreg, doff + (size & ~3), sreg, soff + (size & ~3), size & 3,
                tmp);
    } else
        (*IR->x.blkloop)(dreg, doff, sreg, soff, size, tmp);
}
static void blkunroll(int k, int dreg, int doff, int sreg, int soff, int size,
                      int tmp[]) {
    int i;
...
static Node reuse(Node p, int nt) {
    ...
    if (generic(p->op) == INDIR && p->kids[0]->op == VREG + P && r->u.t.cse &&
        p->x.mayrecalc && ((struct _state *)r->u.t.cse->x.state)->cost[nt] == 0)
        return r->u.t.cse;
    else
        return p;
}
...
int range(Node p, int lo, int hi) {
    ...
    case CNST + I:
        ck(s->u.c.v.i >= lo && s->u.c.v.i <= hi);
    case CNST + U:
        ck(s->u.c.v.u >= lo && s->u.c.v.u <= hi);
    case CNST + P:
        ck(s->u.c.v.p == 0 && lo <= 0 && hi >= 0);
    }
    return LBURG_MAX;
}
...
int notarget(Node p) { return p->syms[RX]->x.wildcard ? 0 : LBURG_MAX; }
...
static Symbol askreg(Symbol rs, unsigned rmask[]) {
    ...
    for (i = 31; i >= 0; i--) {
        ...
        if (r != NULL && !(r->x.regnode->mask & ~rmask[r->x.regnode->set]) &&
            askfixedreg(r))
            return r;
    }
    return NULL;
...
static Symbol spillee(Symbol set, unsigned mask[], Node here) {
    ...
    assert(bestreg); /* Must be able to spill something. Reconfigure the
            register allocator to ensure that we can allocate a register for all
            nodes without spilling the node's necessary input regs. */
    assert(bestreg->x.regnode->vbl ==
           NULL); /* Can't spill register variables because
...
static void genspill(Symbol r, Node last, Symbol tmp) {
    ...
    debug(
        fprint(stderr, "(spilling %s to local %s)\n", r->x.name, tmp->x.name));
