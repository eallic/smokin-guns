...
void definept(Coordinate *p) {
    ...
    if (ncalled > 0) {
        int n = findcount(cp->u.point.src.file, cp->u.point.src.x,
                          cp->u.point.src.y);
        if (n > 0)
...
void swcode(Swtch swp, int b[], int lb, int ub) {
    ...
    if (u - l + 1 <= 3) {
        ...
        if (k > lb && k < ub)
            ...
            assert(lolab == hilab), branch(lolab);
        walk(NULL, 0, 0);
    } else {
        Tree e;
        Type ty = signedint(swp->sym->type);
        Symbol table = genident(STATIC, array(voidptype, u - l + 1, 0), GLOBAL);
        (*IR->defsymbol)(table);
...
void retcode(Tree p) {
    ...
    if (retv) {
        if (iscallb(p))
            p = tree(RIGHT, p->type,
                     tree(CALL + B, p->type, p->kids[0]->kids[0], idtree(retv)),
                     rvalue(idtree(retv)));
        else
...
void definelab(int lab) {
    ...
    for (cp = codelist->prev; cp->kind <= Label;)
        cp = cp->prev;
    while (cp->kind == Jump && cp->u.forest->kids[0] &&
...
void branch(int lab) {
    ...
    for (cp = codelist->prev; cp->kind < Label;)
        cp = cp->prev;
    while (cp->kind == Label && cp->u.forest->op == LABEL + V &&
    ...
    if (cp->kind == Jump || cp->kind == Switch) {
        ...
        codelist->kind = Jump;
        if (cp->kind == Label && cp->u.forest->op == LABEL + V &&
            equal(cp->u.forest->syms[0], p))
...
static int foldcond(Tree e1, Tree e2) {
    ...
    if (generic(e1->op) == ASGN && isaddrop(e1->kids[0]->op) &&
        ...
        return 0;
    if ((op == LE || op == LT || op == EQ || op == NE || op == GT ||
         op == GE) &&
        generic(e2->kids[0]->op) == INDIR && e2->kids[0]->kids[0]->u.sym == v &&
        e2->kids[1]->op == e1->op) {
...
/* localaddr - returns q if p yields the address of local/parameter q; otherwise
 * returns 0 */
