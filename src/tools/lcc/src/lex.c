    ...
    for (;;) {
        ...
        case 'L':
            if (*rcp == '\'') {
                ...
                if (s - wcbuf > 2)
                    warning("excess characters in wide-character literal "
                            "ignored\n");
                tval.type = widechar;
        ...
        case '\'': {
            ...
            if (s - cbuf > 2)
                warning("excess characters in multibyte character literal "
                        "ignored\n");
            tval.type = inttype;
            if (chartype->op == INT)
        ...
        case 'w':
            ...
            goto id;
        case '_':
            if (rcp[0] == '_' && rcp[1] == 't' && rcp[2] == 'y' &&
                rcp[3] == 'p' && rcp[4] == 'e' && rcp[5] == 'c' &&
                rcp[6] == 'o' && rcp[7] == 'd' && rcp[8] == 'e' &&
                !(map[rcp[9]] & (DIGIT | LETTER))) {
                cp = rcp + 9;
                return TYPECODE;
            }
            if (rcp[0] == '_' && rcp[1] == 'f' && rcp[2] == 'i' &&
                rcp[3] == 'r' && rcp[4] == 's' && rcp[5] == 't' &&
                rcp[6] == 'a' && rcp[7] == 'r' && rcp[8] == 'g' &&
                !(map[rcp[9]] & (DIGIT | LETTER))) {
                cp = rcp + 9;
                return FIRSTARG;
            }
