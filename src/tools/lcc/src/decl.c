...
void program(void) {
    ...
    level = GLOBAL;
    for (n = 0; t != EOI; n++)
        if (kind[t] == CHAR || kind[t] == STATIC || t == ID || t == '*' ||
            t == '(') {
            decl(dclglobal);
            deallocate(STMT);
            if (!(glevel >= 3 || xref))
    ...
    if (t == ID || t == '*' || t == '(' || t == '[') {
        ...
        for (;;) {
            ...
            ty1 = dclr(ty, &id, NULL, 0);
               ...
               !(isenum(ty) ||
                 (isstruct(ty) && (*unqual(ty)->u.sym->name < '1' ||
        ...
        *q = NULL;
        ...
        if (overflow) {
            error("size of `%t' exceeds %d bytes\n", ty,
                  inttype->u.sym->u.limits.max.i);
            ty->size = inttype->u.sym->u.limits.max.i & (~(ty->align - 1));
        }
    }
}
static void funcdefn(int sclass, char *id, Type ty, Symbol params[],
                     Coordinate pt) {
    int i, n;
    ...
    if (ty->u.f.oldstyle) {
        ...
        if (p && p->scope == GLOBAL && isfunc(p->type) && p->type->u.f.proto) {
            ...
            for (i = 0; caller[i] && proto[i]; i++) {
                ...
                if (eqtype(isenum(ty) ? ty->type : ty, unqual(caller[i]->type),
                           1) == 0)
                    break;
                else if (isenum(ty) && !isenum(unqual(caller[i]->type)))
                    warning("compatibility of `%t' and `%t' is compiler "
                            "dependent\n",
...
void compound(int loop, struct swtch *swp, int lev) {
        ...
        ;
    {
        ...
        for (; (p = cp->u.block.locals[i]) != NULL; i++) {
            for (j = i; j > nregs && cp->u.block.locals[j - 1]->ref < p->ref;
                 j--)
...
static void checkref(Symbol p, void *cl) {
    ...
    assert(!(level == GLOBAL && p->sclass == STATIC && !p->defined &&
             !isfunc(p->type)));
}
static Symbol dcllocal(int sclass, char *id, Type ty, Coordinate *pos) {
    ...
    assert(level >= LOCAL);
    p = install(id, &identifiers, level,
                sclass == STATIC || sclass == EXTERN ? PERM : FUNC);
    p->type = ty;
    p->sclass = sclass;
    p->src = *pos;
    switch (sclass) {
...
static void doextern(Symbol p, void *cl) { (*IR->import)(p); }
