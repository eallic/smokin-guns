...
static void emitSymbol(Symbol p) {
    (*IR->defsymbol)(p);
    ANCHOR(name, print("%s", p->x.name));
    BEGIN(code);
    print("%s", p->name);
    END;
    END;
    BEGIN(ul);
#define xx(field, code)                                                        \
    ITEM;                                                                      \
    if (!html)                                                                 \
        print(" ");                                                            \
    print(#field "=");                                                         \
    ...
    switch (p->scope) {
    case CONSTANTS:
        xx(scope, print("CONSTANTS"));
        break;
    case LABELS:
        xx(scope, print("LABELS"));
        break;
    case GLOBAL:
        xx(scope, print("GLOBAL"));
        break;
    case PARAM:
        xx(scope, print("PARAM"));
        break;
    case LOCAL:
        xx(scope, print("LOCAL"));
        break;
    default:
        if (p->scope > LOCAL)
...
static void I(local)(Symbol p) {
    ...
    off += p->type->size;
    START;
    print(p->temporary ? "temporary " : "local ");
    emitSymbol(p);
    END;
}
...
static void I(progbeg)(int argc, char *argv[]) {
    ...
    if (html) {
        ...
        if (firstfile && *firstfile)
            BEGIN(title);
        emitString(strlen(firstfile), firstfile);
        END;
        ...
        BEGIN(P);
        BEGIN(em);
        print("Links lead from uses of identifiers and labels to their "
              "definitions.");
        END;
        END;
        print("<ul>\n");
        ...
        for (i = 1; i < argc; i++) {
            ...
            BEGIN(code);
            print("\"");
            emitString(strlen(argv[i]), argv[i]);
            print("\"");
            END;
            END;
        }
...
static void I(segment)(int s) {
    START;
    print("segment %s", &"text\0bss\0.data\0lit\0.sym\0."[5 * s - 5]);
    END;
}
...
static void I(stabend)(Coordinate *cp, Symbol p, Coordinate **cpp, Symbol *sp,
                       Symbol *stab) {
    int i;
...
Interface symbolicIR = {{1, 1, 0}, /* char */
                        {2, 2, 0}, /* short */
                        {4, 4, 0}, /* int */
                        {4, 4, 0}, /* long */
                        {4, 4, 0}, /* long long */
                        ...
                        1,         /* wants_dag */
                        0,         /* unsigned_char */
                        I(address),  I(blockbeg),  I(blockend),  I(defaddress),
                        I(defconst), I(defstring), I(defsymbol), I(emit),
                        I(export),   I(function),  I(gen),       I(global),
                        I(import),   I(local),     I(progbeg),   I(progend),
                        I(segment),  I(space),     I(stabblock), I(stabend),
                        I(stabfend), I(stabinit),  I(stabline),  I(stabsym),
                        I(stabtype)};
