...
/* appendstr - append str to the evolving format string, expanding it if
 * necessary */
...
static void tracevalue(Tree e, int lev) {
    ...
    case FLOAT:
        ...
        break;
    case POINTER:
        if (unqual(ty->type) == chartype || unqual(ty->type) == signedchar ||
...
static void tracefinis(Symbol printer) {
    ...
    *ap = tree(ARG + P, charptype, pointer(idtree(p->u.c.loc)), 0);
    walk(calltree(pointer(idtree(printer)), freturn(printer->type), args, NULL),
         0, 0);
    args = 0;
    fp = fmtend = 0;
}
...
static void tracecall(Symbol printer, Symbol f) {
    ...
    appendstr("#");
    tracevalue(
        asgn(frameno, incr(INCR, idtree(counter), consttree(1, inttype))), 0);
    appendstr("(");
    for (i = 0; f->u.f.callee[i]; i++) {
        if (i)
            appendstr(",");
        appendstr(f->u.f.callee[i]->name);
        appendstr("=");
    ...
    if (IR)
        for (i = 1; i < argc; i++)
            if (strncmp(argv[i], "-t", 2) == 0 &&
                strchr(argv[i], '=') == NULL) {
                Symbol printer =
                    mksymbol(EXTERN, argv[i][2] ? &argv[i][2] : "printf",
