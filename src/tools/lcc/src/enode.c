        ...
        for (;;) {
            ...
            if (proto && *proto && *proto != voidtype) {
                ...
                if (aty)
                    ...
                    error("type error in argument %d to %s; found `%t' "
                          "expected `%t'\n",
                          n + 1, funcname(f),
            ...
            if (!IR->wants_argb && isstruct(q->type)) {
                if (iscallb(q))
                    ...
                    q = asgn(t1, q);
                    q = tree(RIGHT, ptr(t1->type), root(q), lvalue(idtree(t1)));
                }
            }
            if (q->type->size == 0)
                q->type = inttype;
...
Tree calltree(Tree f, Type ty, Tree args, Symbol t3) {
    ...
    if (isstruct(ty))
        assert(t3),
            p = tree(RIGHT, ty, tree(CALL + B, ty, f, addrof(idtree(t3))),
                     idtree(t3));
    else {
        Type rty = ty;
        if (isenum(ty))
...
int iscallb(Tree e) {
    return e->op == RIGHT && e->kids[0] && e->kids[1] &&
           e->kids[0]->op == CALL + B && e->kids[1]->op == INDIR + B &&
...
Tree cnsttree(Type ty, ...) {
    ...
    switch (ty->op) {
    case INT:
        p->u.v.i = va_arg(ap, long);
        break;
    case UNSIGNED:
        p->u.v.u = va_arg(ap, unsigned long) & ones(8 * ty->size);
        break;
    case FLOAT:
        p->u.v.d = va_arg(ap, double);
        break;
    case POINTER:
...
static int compatible(Type ty1, Type ty2) {
    return isptr(ty1) && !isfunc(ty1->type) && isptr(ty2) &&
           !isfunc(ty2->type) &&
...
static int isnullptr(Tree e) {
    ...
    return generic(e->op) == CNST && ((ty->op == INT && e->u.v.i == 0) ||
    ...
    if (l->op == FIELD) {
        ...
        if (n > 0 && isunsigned(l->u.field->type))
            ...
            if (r->op == CNST + I) {
                ...
                r = shtree(RSH, shtree(LSH, r, cnsttree(inttype, n)),
                           cnsttree(inttype, n));
        }
    }
    if (isstruct(ty) && isaddrop(l->op) && iscallb(r))
        return tree(RIGHT, ty, tree(CALL + B, ty, r->kids[0]->kids[0], l),
                    idtree(l->u.sym));
...
Tree condtree(Tree e, Tree l, Tree r) {
    ...
    switch (e->op) {
    case CNST + I:
        return cast(e->u.v.i != 0 ? l : r, ty);
    case CNST + U:
        return cast(e->u.v.u != 0 ? l : r, ty);
    case CNST + P:
        return cast(e->u.v.p != 0 ? l : r, ty);
    case CNST + F:
...
static Tree subtree(int op, Tree l, Tree r) {
    ...
    if (isarith(l->type) && isarith(r->type)) {
        ...
        if (n == 0)
            error("unknown size for type `%t'\n", ty->type);
        l = simplify(SUB + U, unsignedptr, cast(l, unsignedptr),
                     cast(r, unsignedptr));
        return simplify(DIV + I, longtype, cast(l, longtype),
                        cnsttree(longtype, n));
    } else
        typeerror(op, l, r);
    return simplify(op, ty, l, r);
...
void typeerror(int op, Tree l, Tree r) {
    ...
    static struct {
        ...
        {ASGN, "="}, {INDIR, "*"}, {NEG, "-"},  {ADD, "+"},   {SUB, "-"},
        {LSH, "<<"}, {MOD, "%"},   {RSH, ">>"}, {BAND, "&"},  {BCOM, "~"},
        {BOR, "|"},  {BXOR, "^"},  {DIV, "/"},  {MUL, "*"},   {EQ, "=="},
        {GE, ">="},  {GT, ">"},    {LE, "<="},  {LT, "<"},    {NE, "!="},
        {AND, "&&"}, {NOT, "!"},   {OR, "||"},  {COND, "?:"}, {0, 0}};
