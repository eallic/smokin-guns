...
static int filename(char *name, char *base) {
    ...
    case 1: /* preprocessed source files */
        if (Eflag)
            break;
        if (Sflag)
            status = compile(name, outfile ? outfile
                                           : concat(base, first(suffixes[2])));
        else if ((status = compile(name, stemp ? stemp
                                               : (stemp = tempname(first(
                                                      suffixes[2]))))) == 0)
...
static void help(void) {
    static char *msgs[] = {
        "",
        " [ option | file ]...\n",
        "	except for -l, options are processed left-to-right before "
        "files\n",
        ...
        "-E	run only the preprocessor on the named C programs and "
        "unsuffixed files\n",
        "-g	produce symbol table information for debuggers\n",
        "-help or -?	print this message\n",
        "-Idir	add `dir' to the beginning of the list of #include "
        "directories\n",
        "-lx	search library `x'\n",
        ...
        "-v	show commands as they are executed; 2nd -v suppresses "
        "execution\n",
        "-w	suppress warnings\n",
        "-Woarg	specify system-specific `arg'\n",
        "-W[pfal]arg	pass `arg' to the preprocessor, compiler, assembler, "
        "or linker\n",
        0};
    int i;
    char *s;
...
/* initinputs - if LCCINPUTS or include is defined, use them to initialize
 * various lists */
static void initinputs(void) {
...
static void opt(char *arg) {
    ...
    case 'B': /* -Bdir -Bstatic -Bdynamic */
...
#endif
        {
            ...
            path = arg + 2;
            if (strstr(com[1], "win32") != NULL)
                com[0] = concat(replace(path, '/', '\\'),
                                concat("rcc", first(suffixes[4])));
            else
...
/* suffix - if one of tails[0..n-1] holds a proper suffix of name, return its
 * index */
