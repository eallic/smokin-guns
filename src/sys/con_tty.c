...
static void CON_Show(void) {
    if (ttycon_on) {
        ...
        if (ttycon_hide == 0) {
            size_t UNUSED_VAR size;
            size = write(STDOUT_FILENO, TTY_CONSOLE_PROMPT,
                         strlen(TTY_CONSOLE_PROMPT));
            if (TTY_con.cursor) {
                for (i = 0; i < TTY_con.cursor; i++) {
...
seems to lose all set attributes if user did CTRL+Z and then does fg again.
==================
*/

void CON_SigCont(int signum) { CON_Init(); }
...
char *CON_Input(void) {
    ...
    if (ttycon_on) {
        ...
        if (avail != -1) {
            // we have something
            // backspace?
            // NOTE TTimo testing a lot of values .. seems it's the only way to
            // get it to work everywhere
            ...
            if ((key) && (key) < ' ') {
                if (key == '\n') {
                    ...
                    if (clc.state != CA_ACTIVE && TTY_con.cursor &&
                        ...
                        memmove(TTY_con.buffer + 1, TTY_con.buffer,
                                sizeof(TTY_con.buffer) - 1);
                        TTY_con.buffer[0] = '\\';
                        TTY_con.cursor++;
                    }
                    ...
                    if (TTY_con.buffer[0] == '/' || TTY_con.buffer[0] == '\\') {
                        ...
                        Com_sprintf(text, sizeof(text), "cmd say %s",
                                    TTY_con.buffer);
                    } else {
                        text[0] = '\0';
                    }
