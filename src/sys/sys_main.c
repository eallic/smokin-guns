...
// in consideration Blame Tequila if you don't like this tool :P
...
void Sys_SetDefaultInstallPath(const char *path) {
    Q_strncpyz(installPath, Sys_GetSystemInstallPath(path),
               sizeof(installPath));
}
    ...
    if (!useSystemLib || !(dllhandle = Sys_LoadLibrary(name))) {
        ...
        if (!(dllhandle = Sys_LoadLibrary(libPath))) {
            ...
            if (FS_FilenameCompare(topDir, basePath)) {
                Com_Printf("Trying to load \"%s\" from \"%s\"...\n", name,
                           basePath);
                Com_sprintf(libPath, sizeof(libPath), "%s%c%s", basePath,
                            PATH_SEP, name);
...
void Sys_BinaryEngineComment(void) {
    ...
    Com_sprintf(string, sizeof(string), "%s (%s - %s %s), %s", version, release,
                date, time, platform);
    if (strlen(comment))
    ...
    Cvar_Set("sdk_engine_comment",
             va("\\version\\%s\\release\\%s\\platform\\%s\\date\\%s "
                "%s\\md5\\%s\\comment\\%s\\contact\\%s",
                version, release, platform, date, time, sum, comment, contact));
#else
    Cvar_Set("sdk_engine_comment",
             va("\\version\\%s\\release\\%s\\platform\\%s\\date\\%s "
                "%s\\md5\\%s\\diff\\%i\\comment\\%s\\contact\\%s",
                version, release, platform, date, time, sum, sdk_diff_format,
                comment, contact));
#endif
}
...
#define MINSDL_VERSION                                                         \
    XSTRING(MINSDL_MAJOR) "." XSTRING(MINSDL_MINOR) "." XSTRING(MINSDL_PATCH)

    if (SDL_VERSIONNUM(ver->major, ver->minor, ver->patch) <
        SDL_VERSIONNUM(MINSDL_MAJOR, MINSDL_MINOR, MINSDL_PATCH)) {
        Sys_Dialog(DT_ERROR,
                   va("SDL version " MINSDL_VERSION " or greater is required, "
                      "but only version %d.%d.%d was found. You may be able to "
                      "obtain a more recent copy "
    ...
    for (i = 1; i < argc; i++) {
        ...
        Q_strcat(commandLine, sizeof(commandLine), " ");
#if defined SMOKINGUNS && defined DEDICATED
        // Original code for handling the special cvars by hika AT bsdmon DOT
        // com This is where we can get some special cvars like sv_chroot,
        // sv_user
        ...
        // Either way, they will be initialized in Com_Init() as startup
        // variables
    ...
    // Create a ROM cvar to let the mod know the Smokin' Guns standalone engine
    // is in use.
