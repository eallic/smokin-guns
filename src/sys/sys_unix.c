...
==================
*/
const char *Sys_Basename(char *path) { return basename(path); }
...
#ifndef DEDICATED
    Sys_Dialog(DT_ERROR, va("%s. See \"%s\" for details.", error, ospath),
               "Error");
#endif
...
==============
*/
static void Sys_ZenityCommand(dialogType_t type, const char *message,
                              const char *title) {
    Sys_ClearExecBuffer();
...
==============
*/
static void Sys_KdialogCommand(dialogType_t type, const char *message,
                               const char *title) {
    Sys_ClearExecBuffer();
...
==============
*/
char *Sys_GetEnv(const char *name) { return getenv(name); }
...
==============
*/
int Sys_PID(void) { return getpid(); }
...
void Sys_PlatformPostInit(char *progname) {
    ...
    cv_pid = Cvar_Get(
        "sv_pidfile",
        va("/var/run/%s-%s.pid", Sys_Basename(progname), net_port->string),
        CVAR_INIT);
    ...
    if (cv_pid && cv_pid->string[0]) {
        ...
        if (fd == NULL) {
            // Try to open PID file in Smokin'Guns home directory if sytem
            // folder is not writable
            Cvar_Set("sv_pidfile",
                     va("%s/%s-%s.pid", homePath, Sys_Basename(progname),
                        net_port->string));
            fd = fopen(cv_pid->string, "w");
        }
...
==============
*/
uid_t Sys_Getuid(void) { return getuid(); }
...
==============
*/
uid_t Sys_Geteuid(void) { return geteuid(); }
...
==============
*/
int Sys_Unlink(char *file) { return unlink(file); }
...
==============
*/
pid_t Sys_Getpid(void) { return getpid(); }
