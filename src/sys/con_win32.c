...
static void CON_HistPrev(void) {
    ...
    pos = (qconsole_history_pos < 1) ? (QCONSOLE_HISTORY - 1)
...
static void CON_Show(void) {
    ...
    if (qconsole_linelen > binfo.srWindow.Right) {
        ...
        WriteConsoleOutput(qconsole_hout, line, writeSize, writePos,
                           &writeArea);
    }

    // set curor position
    cursorPos.Y = binfo.dwCursorPosition.Y;
    cursorPos.X = qconsole_linelen > binfo.srWindow.Right ? binfo.srWindow.Right
                                                          : qconsole_linelen;
...
void CON_Init(void) {
    ...
    qconsole_backgroundAttrib =
        qconsole_attrib & (BACKGROUND_BLUE | BACKGROUND_GREEN | BACKGROUND_RED |
    ...
    while (*msg) {
        ...
        if (Q_IsColorString(msg) || *msg == '\n') {
            ...
            if (*msg == '\n') {
                ...
                SetConsoleTextAttribute(qconsole_hout,
                                        CON_ColorCharToAttrib(COLOR_WHITE));
                fputs("\n", stderr);
                msg++;
            } else {
                // Set the color
                SetConsoleTextAttribute(qconsole_hout,
                                        CON_ColorCharToAttrib(*(msg + 1)));
                msg += 2;
            }
        } else {
