...
void CMod_LoadBrushes(lump_t *l) {
    ...
    cm.brushes =
        Hunk_Alloc((BOX_BRUSHES + count) * sizeof(*cm.brushes), h_high);
    cm.numBrushes = count;
...
void CMod_LoadLeafs(lump_t *l) {
    ...
    cm.areaPortals =
        Hunk_Alloc(cm.numAreas * cm.numAreas * sizeof(*cm.areaPortals), h_high);
}
...
void CMod_LoadLeafBrushes(lump_t *l) {
    ...
    cm.leafbrushes =
        Hunk_Alloc((count + BOX_BRUSHES) * sizeof(*cm.leafbrushes), h_high);
...
void CMod_LoadBrushSides(lump_t *l) {
    ...
    cm.brushsides =
        Hunk_Alloc((BOX_SIDES + count) * sizeof(*cm.brushsides), h_high);
...
unsigned CM_LumpChecksum(lump_t *lump) {
    return LittleLong(
        Com_BlockChecksum(cmod_base + lump->fileofs, lump->filelen));
}
...
void CM_LoadMap(const char *name, qboolean clientload, int *checksum) {
    ...
    cm_playerCurveClip =
        Cvar_Get("cm_playerCurveClip", "1", CVAR_ARCHIVE | CVAR_CHEAT);
#endif
...
int CM_NumInlineModels(void) { return cm.numSubModels; }
