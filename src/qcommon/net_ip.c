...
const char *NET_AdrToStringwPort(netadr_t a) {
    ...
    if (a.type == NA_LOOPBACK)
        ...
        Com_sprintf(s, sizeof(s), "[%s]:%hu", NET_AdrToString(a),
                    ntohs(a.port));
...
qboolean NET_GetPacket(netadr_t *net_from, msg_t *net_message, fd_set *fdr) {
    ...
    if (ip_socket != INVALID_SOCKET && FD_ISSET(ip_socket, fdr)) {
        fromlen = sizeof(from);
        ret =
            recvfrom(ip_socket, (void *)net_message->data, net_message->maxsize,
    ...
    if (ip6_socket != INVALID_SOCKET && FD_ISSET(ip6_socket, fdr)) {
        fromlen = sizeof(from);
        ret = recvfrom(ip6_socket, (void *)net_message->data,
    ...
    if (multicast6_socket != INVALID_SOCKET &&
        multicast6_socket != ip6_socket && FD_ISSET(multicast6_socket, fdr)) {
        fromlen = sizeof(from);
        ret = recvfrom(multicast6_socket, (void *)net_message->data,
                       net_message->maxsize, 0, (struct sockaddr *)&from,
                       &fromlen);
...
void Sys_SendPacket(int length, const void *data, netadr_t to) {
    ...
    if (usingSocks && to.type == NA_IP) {
        ...
        *(short *)&socksBuf[8] = ((struct sockaddr_in *)&addr)->sin_port;
        ...
        ret = sendto(ip_socket, socksBuf, length + 10, 0, &socksRelayAddr,
        ...
        if (addr.ss_family == AF_INET)
            ...
            ret = sendto(ip6_socket, data, length, 0, (struct sockaddr *)&addr,
...
qboolean Sys_IsLANAddress(netadr_t adr) {
    ...
    for (index = 0; index < numIP; index++) {
        if (localIP[index].type == adr.type) {
            if (adr.type == NA_IP) {
                compareip =
                    (byte *)&((struct sockaddr_in *)&localIP[index].addr)
                        ->sin_addr.s_addr;
                comparemask =
                    (byte *)&((struct sockaddr_in *)&localIP[index].netmask)
                        ->sin_addr.s_addr;
                compareadr = adr.ip;
                ...
                compareip =
                    (byte *)&((struct sockaddr_in6 *)&localIP[index].addr)
                        ->sin6_addr;
                comparemask =
                    (byte *)&((struct sockaddr_in6 *)&localIP[index].netmask)
                        ->sin6_addr;
                compareadr = adr.ip6;
...
void Sys_ShowIP(void) {
    ...
    for (i = 0; i < numIP; i++) {
        Sys_SockaddrToString(addrbuf, sizeof(addrbuf),
...
====================
*/
SOCKET NET_IP6Socket(char *net_interface, int port, struct sockaddr_in6 *bindto,
                     int *err) {
    SOCKET newsocket;
...
void NET_SetMulticast6(void) {
    ...
    if (!*net_mcast6addr->string ||
        !Sys_StringToSockaddr(net_mcast6addr->string, (struct sockaddr *)&addr,
    ...
    memcpy(&curgroup.ipv6mr_multiaddr, &addr.sin6_addr,
           sizeof(curgroup.ipv6mr_multiaddr));
...
void NET_JoinMulticast6(void) {
    ...
    if (IN6_IS_ADDR_MULTICAST(&boundto.sin6_addr) ||
        ...
        if ((multicast6_socket =
                 NET_IP6Socket(net_mcast6addr->string, ntohs(boundto.sin6_port),
                               NULL, &err)) == INVALID_SOCKET) {
            // If the OS does not support binding to multicast addresses, like
            // WinXP, at least try with the normal file descriptor.
            multicast6_socket = ip6_socket;
        }
    }
    ...
    if (curgroup.ipv6mr_interface) {
        if (setsockopt(multicast6_socket, IPPROTO_IPV6, IPV6_MULTICAST_IF,
            ...
            Com_Printf("NET_JoinMulticast6: Couldn't set scope on multicast "
                       "socket: %s\n",
                       NET_ErrorString());
...
void NET_LeaveMulticast6() {
    if (multicast6_socket != INVALID_SOCKET) {
        if (multicast6_socket != ip6_socket)
            ...
            setsockopt(multicast6_socket, IPPROTO_IPV6, IPV6_LEAVE_GROUP,
                       (char *)&curgroup, sizeof(curgroup));
...
void NET_OpenSocks(int port) {
    ...
    if (h->h_addrtype != AF_INET) {
        Com_Printf("WARNING: NET_OpenSocks: gethostbyname: address type was "
                   "not AF_INET\n");
        return;
    }
    address.sin_family = AF_INET;
    ...
    if (buf[1] == 2) {
        ...
        // send it
        if (send(socks_socket, (void *)buf, 3 + ulen + plen, 0) ==
            SOCKET_ERROR) {
            Com_Printf("NET_OpenSocks: send: %s\n", NET_ErrorString());
            return;
        }
...
=====================
*/
static void NET_AddLocalAddress(char *ifname, struct sockaddr *addr,
    ...
    if (numIP < MAX_IPS) {
        ...
        Q_strncpyz(localIP[numIP].ifname, ifname,
                   sizeof(localIP[numIP].ifname));
...
static void NET_GetLocalAddress(void) {
    ...
    if (getifaddrs(&ifap))
        Com_Printf("NET_GetLocalAddress: Unable to get list of network "
                   "interfaces: %s\n",
                   NET_ErrorString());
    else {
...
static void NET_GetLocalAddress(void) {
    ...
    if (!getaddrinfo(hostname, NULL, &hint, &res)) {
        ...
        /* On operating systems where it's more difficult to find out the
         * configured interfaces, we'll just assume a netmask with all bits set.
         */
...
void NET_OpenIP(void) {
    ...
    if (net_enabled->integer & NET_ENABLEV6) {
        for (i = 0; i < 10; i++) {
            ip6_socket =
                NET_IP6Socket(net_ip6->string, port6 + i, &boundto, &err);
...
static qboolean NET_GetCvars(void) {
...
#else
    /* End users have it enabled so they can connect to ipv6-only hosts, but
     * ipv4 will be used if available due to ping */
    ...
    // Some cvars for configuring multicast options which facilitates scanning
    // for servers on local subnets.
...
#ifdef _WIN32
    net_mcast6iface =
        Cvar_Get("net_mcast6iface", "0", CVAR_LATCH | CVAR_ARCHIVE);
#else
    net_mcast6iface =
        Cvar_Get("net_mcast6iface", "", CVAR_LATCH | CVAR_ARCHIVE);
#endif
    modified += net_mcast6iface->modified;
    net_mcast6iface->modified = qfalse;

    net_socksEnabled =
        Cvar_Get("net_socksEnabled", "0", CVAR_LATCH | CVAR_ARCHIVE);
    modified += net_socksEnabled->modified;
    net_socksEnabled->modified = qfalse;

    net_socksServer =
        Cvar_Get("net_socksServer", "", CVAR_LATCH | CVAR_ARCHIVE);
    modified += net_socksServer->modified;
    net_socksServer->modified = qfalse;

    net_socksPort =
        Cvar_Get("net_socksPort", "1080", CVAR_LATCH | CVAR_ARCHIVE);
    modified += net_socksPort->modified;
    net_socksPort->modified = qfalse;

    net_socksUsername =
        Cvar_Get("net_socksUsername", "", CVAR_LATCH | CVAR_ARCHIVE);
    modified += net_socksUsername->modified;
    net_socksUsername->modified = qfalse;

    net_socksPassword =
        Cvar_Get("net_socksPassword", "", CVAR_LATCH | CVAR_ARCHIVE);
...
void NET_Config(qboolean enableNetworking) {
    ...
    // if enable state is the same and no cvars were modified, we have nothing
    // to do
...
Called from NET_Sleep which uses select() to determine which sockets have seen
action.
====================
*/
    ...
    while (1) {
        ...
        if (NET_GetPacket(&from, &netmsg, fdr)) {
            if (net_dropsim->value > 0.0f && net_dropsim->value <= 100.0f) {
                ...
                if (rand() < (int)(((double)RAND_MAX) / 100.0 *
                                   (double)net_dropsim->value))
