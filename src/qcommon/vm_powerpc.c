...
#define opArg2IF                                                               \
    (opArg2I | opArg2F) /* requires second argument, integer or float */
...
static long int VM_AsmCall(int callSyscallInvNum, int callProgramStack) {
    ...
    if (sizeof(intptr_t) == sizeof(int)) {
        intptr_t *argPosition =
            (intptr_t *)((byte *)currentVM->dataBase + callProgramStack + 4);
        ...
        int *argPosition =
            (int *)((byte *)currentVM->dataBase + callProgramStack + 4);
 ...
 */
static void PPC_AppendInstructions(unsigned long int i_count,
    ...
    dest_instruction_t *di_now =
        PPC_Malloc(sizeof(dest_instruction_t) + iBytes);
...
static void PPC_PrepareJump(unsigned long int i_count, unsigned long int dest,
                            long int bo, long int bi, unsigned long int ext) {
 ...
 */
static void VM_AnalyzeFunction(source_instruction_t *const i_first,
                               long int *prepareStack, long int *gpr_start_pos,
    ...
    while ((i_now = i_now->next)) {
        ...
        if (opi & opArgIF) {
            ...
            if ((opi & opArgI) && (vpopi & opRetI)) {
                ...
                vp->regR |=
                    rTYPE_FLOAT; // use OR here - could be marked as static
        ...
        if ((op == OP_CALL) ||
            (op == OP_BLOCK_COPY &&
             (i_now->arg.i > SL(16, 32) || !OPTIMIZE_COPY))) {
            long int i;
            *prepareStack = 1;
    ...
    while ((i_now = i_now->next)) {
        ...
        case OP_CALL:
            if (i_const) {
                ...
                in(iRLWINM, rFIRST, rFIRST, GPRLEN_SHIFT, 0,
                   31 - GPRLEN_SHIFT);    // mul * GPRLEN
                in(iLLX, r0, rFIRST, r0); // load pointer

                in(iB, +4 * (3 + (rFIRST != r3 ? 1 : 0))); // XXX jump !

                /* syscall */
                in(iLL, r0, VM_Data_Offset(AsmCall),
                   rVMDATA); // get asmCall pointer
                /* rFIRST can be r3 or some static register */
                if (rFIRST != r3)
                    in(iMR, r3,
                       rFIRST);       // push OPSTACK top value on argument list
                in(iMR, r4, rPSTACK); // push PSTACK on argument list
                ...
                if (rFIRST != r3)
                    in(iMR, rFIRST,
                       r3); // push return value on the top of the opstack
            }
            break;
        ...
        case OP_EQ:
        case OP_NE:
            if (i_const && i_const->arg.si >= -0x8000 &&
                i_const->arg.si < 0x10000) {
                EMIT_FALSE_CONST();
        ...
        case OP_BLOCK_COPY:
...
#endif
            {
                ...
                in(iLL, r0, VM_Data_Offset(BlockCopy),
                   rVMDATA); // get blockCopy pointer
...
static void PPC_ComputeCode(vm_t *vm) {
    ...
    size_t codeLength = sizeof(vm_data_t) + sizeof(unsigned int) * data_acc +
    ...
    unsigned char *dataAndCode = mmap(NULL, codeLength, PROT_READ | PROT_WRITE,
    ...
    codeNow = codeBegin =
        (ppc_instruction_t *)(dataAndCode + VM_Data_Offset(data[data_acc]));
    ...
    while ((di_now = di_now->next)) {
        ...
        if (di_now->jump == NULL) {
            memcpy(codeNow, &(di_now->code[0]),
                   di_now->length * sizeof(ppc_instruction_t));
    ...
    while ((sj_now = sj_now->nextJump)) {
        ...
        if (jumpLength >= -8192 && jumpLength < 8192) {
            powerpc_iname_t branchConditional =
                sj_now->ext & branchExtLink ? iBCL : iBC;
            *jumpFrom =
                IN(branchConditional, sj_now->bo, sj_now->bi, jumpLength * 4);
            continue;
        }
        ...
        *jumpFrom =
            IN((sj_now->ext & branchExtLink ? iBL : iB), jumpLength * 4);
        if (sj_now->bo == branchAlways)
            continue;
...
void VM_Compile(vm_t *vm, vmHeader_t *header) {
    ...
    if (sizeof(void *) != sizeof(int)) {
        ...
        vm->instructionPointers =
            Z_Malloc(header->instructionCount * sizeof(void *));
    }
    ...
    for (i_count = 0; i_count < header->instructionCount; ++i_count) {
        ...
        if (vm_opInfo[op] & opImm4) {
            union {
                ...
                {code[pc + 3], code[pc + 2], code[pc + 1], code[pc + 0]},
            };
...
int VM_CallCompiled(vm_t *vm, int *args) {
    ...
    if (time_diff > 100) {
        printf("App clock: %ld, vm total: %ld, vm this: %ld, vm real: %ld, vm "
               "out: %ld\n"
               "Inside VM %f%% of app time\n",
               stop_time.tms_utime, time_total_vm, time_diff,
               time_diff - time_outside_vm, time_outside_vm,
