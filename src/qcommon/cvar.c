...
============
*/
static const char *Cvar_Validate(cvar_t *var, const char *value,
                                 qboolean warn) {
...
cvar_t *Cvar_Get(const char *var_name, const char *var_value, int flags) {
    ...
    if (var) {
        ...
        // Make sure the game code cannot mark engine-added variables as
        // gamecode vars
        ...
        // Make sure servers cannot mark engine-added variables as
        // SERVER_CREATED
        ...
        if (!var->resetString[0]) {
            ...
            Com_DPrintf("Warning: cvar \"%s\" given initial values: \"%s\" and "
                        "\"%s\"\n",
...
void Cvar_WriteVariables(fileHandle_t f) {
    ...
    for (var = cvar_vars; var; var = var->next) {
        ...
        if (var->flags & CVAR_ARCHIVE) {
            ...
            if (var->latchedString) {
                if (strlen(var->name) + strlen(var->latchedString) + 10 >
                    sizeof(buffer)) {
                ...
                if (strlen(var->name) + strlen(var->string) + 10 >
                    sizeof(buffer)) {
                    Com_Printf(S_COLOR_YELLOW
                               "WARNING: value of variable "
                               "\"%s\" too long to write to file\n",
...
void Cvar_Update(vmCvar_t *vmCvar) {
    ...
    if (strlen(cv->string) + 1 > MAX_CVAR_VALUE_STRING)
        Com_Error(ERR_DROP,
                  ...
                  cv->string, (unsigned int)strlen(cv->string));
