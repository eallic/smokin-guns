...
#define FMT3I(op, op3)                                                         \
    ((op) << 30) | ((op3) << 19) | IMM, (OP_MASK | OP3_MASK | IMM)
#define FMT3F(op, op3, opf)                                                    \
    ((op) << 30) | ((op3) << 19) | ((opf) << 5), (OP_MASK | OP3_MASK | OPF_MASK)
...
static void vimm(unsigned int val, int bits, int shift, int sgned,
                 int arg_index) {
...
static unsigned int sparc_assemble(enum sparc_iname iname, const int argc,
                                   const int *argv) {
...
static unsigned int sparc_push_data(struct func_info *const fp,
                                    unsigned int val) {
...
static void jump_insn_append(vm_t *vm, struct func_info *const fp,
                             enum sparc_iname iname, int dest) {
...
static void emit_jump(vm_t *vm, struct func_info *const fp,
                      enum sparc_iname iname, int dest) {
    end_emit(fp);
...
static void compile_one_insn(vm_t *vm, struct func_info *const fp,
                             struct src_insn *sp) {
    start_emit(fp, sp->i_count);
...
static void emit_vm_thunk(struct func_info *const fp) {
    /* int vm_thunk(void *vmdata, int programstack, void *database, int
     * datamask) */
...
static void sparc_compute_code(vm_t *vm, struct func_info *const fp) {
    ...
    code_length = (sizeof(vm_data_t) + (fp->data_num * sizeof(unsigned int)) +
    ...
    code_now = code_begin =
        (unsigned int *)(data_and_code + VM_Data_Offset(data[fp->data_num]));
