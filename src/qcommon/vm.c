...
vmHeader_t *VM_LoadQVM(vm_t *vm, qboolean alloc, qboolean unpure) {
    ...
    if (LittleLong(header.h->vmMagic) == VM_MAGIC_VER2) {
        ...
        // validate
        if (header.h->jtrgLength < 0 || header.h->bssLength < 0 ||
            header.h->dataLength < 0 || header.h->litLength < 0 ||
    ...
    if (header.h->vmMagic == VM_MAGIC_VER2) {
        ...
        Com_Memcpy(vm->jumpTableTargets,
...
vm_t *VM_Create(const char *module, intptr_t (*systemCalls)(intptr_t *),
    ...
    do {
        ...
        if (retval == VMI_NATIVE) {
            ...
            vm->dllHandle =
                Sys_LoadGameDll(filename, &vm->entryPoint, VM_DllSyscall);
    ...
    vm->instructionPointers = Hunk_Alloc(
        vm->instructionCount * sizeof(*vm->instructionPointers), h_high);
