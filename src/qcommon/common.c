...
void QDECL Com_Printf(const char *fmt, ...) {
    ...
    if (com_logfile && com_logfile->integer) {
        // TTimo: only open the qconsole.log if the filesystem is in an
        // initialized state
        //   also, avoid recursing in the qconsole.log opening (i.e. if fs_debug
        //   is on)
...
========================
*/
int Z_AvailableZoneMemory(memzone_t *zone) { return zone->size - zone->used; }
...
========================
*/
int Z_AvailableMemory(void) { return Z_AvailableZoneMemory(mainzone); }
...
void *Z_TagMalloc(int size, int tag) {
    ...
    do {
        if (rover == start) {
            ...
            Com_Error(ERR_FATAL,
                      "Z_Malloc: failed on allocation of %i bytes from the %s "
                      "zone: %s, line: %d (%s)",
                      size, zone == smallzone ? "small" : "main", file, line,
                      label);
#else
            Com_Error(
                ERR_FATAL,
                "Z_Malloc: failed on allocation of %i bytes from the %s zone",
...
#else
void *S_Malloc(int size) { return Z_TagMalloc(size, TAG_SMALL); }
#endif
...
memstatic_t emptystring = {
    {(sizeof(memblock_t) + 2 + 3) & ~3, TAG_STATIC, NULL, NULL, ZONEID},
    {'\0', '\0'}};
...
void Com_InitHunkMemory(void) {
    ...
    if (com_dedicated && com_dedicated->integer) {
        ...
        pMsg = "Minimum com_hunkMegs for a dedicated server is %i, allocating "
               "%i megs.\n";
    } else {
...
int Hunk_MemoryRemaining(void) {
    ...
    low =
        hunk_low.permanent > hunk_low.temp ? hunk_low.permanent : hunk_low.temp;
...
void *Hunk_Alloc(int size, ha_pref preference) {
    ...
    if (hunk_low.temp + hunk_high.temp + size > s_hunkTotal) {
        ...
        Com_Error(ERR_DROP, "Hunk_Alloc failed on %i: %s, line: %d (%s)", size,
                  file, line, label);
#else
...
sysEvent_t Com_GetEvent(void) {
    if (com_pushedEventsHead > com_pushedEventsTail) {
        com_pushedEventsTail++;
        return com_pushedEvents[(com_pushedEventsTail - 1) &
                                (MAX_PUSHED_EVENTS - 1)];
    }
...
=================
*/
static void Com_Crash_f(void) { *(volatile int *)0 = 0x12345678; }
...
void Com_ExecuteCfg(void) {
    ...
    Cbuf_Execute(); // Always execute after exec to prevent text buffer
                    // overflowing

    if (!Com_SafeMode()) {
        // skip the q3config.cfg and autoexec.cfg if "safe" is on the command
        // line
...
void Com_Init(char *commandLine) {
    ...
    com_protocol = Cvar_Get("com_protocol", va("%i", PROTOCOL_VERSION),
                            CVAR_SERVERINFO | CVAR_INIT);
#ifdef LEGACY_PROTOCOL
    com_legacyprotocol = Cvar_Get("com_legacyprotocol",
                                  va("%i", PROTOCOL_LEGACY_VERSION), CVAR_INIT);
...
static qboolean Field_Complete(void) {
    ...
    completionOffset =
        strlen(completionField->buffer) - strlen(completionString);
