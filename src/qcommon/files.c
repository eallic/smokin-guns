...
A "qpath" is a reference to game file data.  MAX_ZPATH is 256 characters, which
must include a terminating zero. "..", "\\", and ":" are explicitly illegal in
qpaths to prevent any references outside the quake directory system.

The "base path" is the path to the directory holding all the game directories
and usually the executable.  It defaults to ".", but can be overridden with a
"+set fs_basepath c:\quake3" command line to allow code debugging in a different
directory.  Basepath cannot be modified at all after startup.  Any files that
are created (demos, screenshots, etc) will be created relative to the base path,
so base path should usually be writable.

The "home path" is the path used for all write access. On win32 systems we have
"base path"
== "home path", but on *nix systems the base installation is usually readonly,
and "home path" points to ~/.q3a or similar

The user can also install custom mods and content in "home path", so it should
be searched along with "home path" and "cd path" for game content.
...
looking in the base game. This is the basis for addons.

Clients automatically set the game directory after receiving a gamestate from a
server, so only servers need to worry about +set fs_game.

No other directories outside of the base game and current game will ever be
referenced by filesystem functions.

To save disk space and speed loading, directory trees can be collapsed into zip
files. The files use a ".pk3" extension to prevent users from unzipping them
accidentally, but otherwise the are simply normal uncompressed zip files.  A
game directory can have multiple zip files of the form "pak0.pk3", "pak1.pk3",
etc.  Zip files are searched in decending order from the highest number to the
lowest, and will always take precedence over the filesystem. This allows a pk3
...
File search order: when FS_FOpenFileRead gets called it will go through the
fs_searchpaths structure and stop on the first successful hit. fs_searchpaths is
built with successive calls to FS_AddGameDirectory
...
e.g. the qpath "sound/newstuff/test.wav" would be searched for in the following
places:
...
Because the same files are loaded by both the clip model (CM_) and renderer
(TR_) subsystems, a simple single-file caching scheme is used.  The CM_
subsystems will load the file with a request to cache.  Only one file will be
kept cached at a time, so any models that are going to be referenced by both
subsystems should alternate between the CM_ load function and the ref load
function.

TODO: A qpath that starts with a leading slash will always refer to the base
game, even if another game is currently active.  This allows character models,
skins, and sounds to be downloaded to a common directory no matter which game is
active.
...
static char fs_gamedir[MAX_OSPATH]; // this will be a single file name with no
                                    // separators
...
// never load anything from pk3 files that are not present at the server when
// pure
...
qboolean FS_Initialized(void) { return (fs_searchpaths != NULL); }
...
qboolean FS_PakIsPure(pack_t *pack) {
    ...
    if (fs_numServerPaks) {
        for (i = 0; i < fs_numServerPaks; i++) {
            ...
            // NOTE TTimo: a pk3 with same checksum but different name would be
            // validated too
            //   I don't see this as allowing for any exploit, it would only
            //   happen if the client does manips of its file names 'not a bug'
    ...
    for (; ofs != NULL && *ofs; ofs++) {
        if (*ofs == PATH_SEP) {
            ...
            *ofs = 0;
            if (!Sys_Mkdir(path)) {
                Com_Error(ERR_FATAL,
                          "FS_CreatePath: failed to create path \"%s\"", path);
            }
            *ofs = PATH_SEP;
        }
    }
...
ERR_FATAL if trying to maniuplate a file with the platform library, QVM, or pk3
extension
=================
 */
...
qboolean FS_FileExists(const char *file) {
    return FS_FileInPathExists(
        FS_BuildOSPath(fs_homepath->string, fs_gamedir, file));
}
...
long FS_FOpenFileReadDir(const char *filename, searchpath_t *search,
    ...
    // make sure the q3key file is only readable by the quake3.exe at
    // initialization any other time the key should only be accessed in memory
    ...
    *file = FS_HandleForFile();
    ...
    if (search->pack) {
        ...
        if (search->pack->hashTable[hash]) {
            ...
            do {
                ...
                if (!FS_FilenameCompare(pakFile->name, filename)) {
                    ...
                    // mark the pak as having been referenced and mark specifics
                    // on cgame and ui shaders, txt, arena files  by themselves
                    // do not count as a reference as these are loaded from all
                    // pk3s from every pk3 file..
                    len = strlen(filename);
        ...
        //   this test can make the search fail although the file is in the
        //   directory
        // I had the problem on
        // https://zerowing.idsoftware.com/bugzilla/show_bug.cgi?id=8 turned out
        // I used FS_FileExists instead
...
write found DLL or QVM to "found" and return VMI_NATIVE if DLL, VMI_COMPILED if
QVM Return the searchpath in "startSearch".
=================
*/
...
int FS_FindVM(void **startSearch, char *found, int foundlen, const char *name,
    ...
    while (search) {
        if (search->dir && !fs_numServerPaks) {
            ...
            if (FS_FOpenFileReadDir(qvmName, search, NULL, qfalse, qfalse) >
                0) {
                *startSearch = search;
                return VMI_COMPILED;
            }
            ...
            if (lastSearch && lastSearch->pack) {
                ...
                if (!FS_FilenameCompare(lastSearch->pack->pakPathname,
                                        pack->pakPathname)) {
                    search = search->next;
                    continue;
                }
        ...
        while (remaining) {
            ...
            if (read == 0) {
                ...
                if (!tries) {
                    ...
                    return len - remaining; // Com_Error (ERR_FATAL, "FS_Read: 0
                                            // bytes read");
                }
            }
...
============
*/
long FS_ReadFileDir(const char *qpath, void *searchPath, qboolean unpure,
                    void **buffer) {
    fileHandle_t h;
    searchpath_t *search;
    byte *buf;
    ...
    if (h == 0) {
        ...
        // if we are journalling and it is a config file, write a zero to the
        // journal file
    ...
    *buffer = buf;
    ...
    // if we are journalling and it is a config file, write it to the journal
    // file
    if (isConfig && com_journal && com_journal->integer == 1) {
...
static pack_t *FS_LoadZipFile(const char *zipfile, const char *basename) {
    ...
    pack->checksum = Com_BlockChecksum(
        &fs_headerLongs[1], sizeof(*fs_headerLongs) * (fs_numHeaderLongs - 1));
    pack->pure_checksum = Com_BlockChecksum(
        fs_headerLongs, sizeof(*fs_headerLongs) * fs_numHeaderLongs);
...
==================
*/
static int FS_AddFileToList(char *name, char *list[MAX_FOUND_FILES],
                            int nfiles) {
    int i;
    ...
    for (search = fs_searchpaths; search; search = search->next) {
        ...
        if (search->pack) {
            ...
            if (fs_numServerPaks && !allowNonPureFilesOnDisk) {
                ...
                sysFiles = Sys_ListFiles(netpath, extension, filter,
                                         &numSysFiles, qfalse);
...
================
*/
int FS_GetFileList(const char *path, const char *extension, char *listbuf,
                   int bufsize) {
...
static char **Sys_ConcatenateFileLists(char **list0, char **list1) {
    ...
    *dst = NULL;
    ...
    // Free our old lists.
    // NOTE: not freeing their content, it's been merged in dst and still being
    // used
    if (list0)
        Z_Free(list0);
    if (list1)
...
int FS_GetModList(char *listbuf, int bufsize) {
    ...
    *listbuf = 0;
    ...
    // it is likely that we have duplicate names now, which we will cleanup
    // below
    ...
    for (i = 0; i < nPotential; i++) {
        ...
        if (Q_stricmp(name, com_basegame->string) && Q_stricmpn(name, ".", 1)) {
            ...
            // NOTE TTimo: (actually I'm not sure why .. what if it's a mod
            // under developement with no .pk3?) we didn't keep the information
            // when we merged the directory names, as to what OS Path it was
            // found under
            ...
            Sys_FreeFileList(pPaks); // we only use Sys_ListFiles to check
                                     // wether .pk3 files are present
...
void FS_AddGameDirectory(const char *path, const char *dir) {
    ...
    if (fs_numServerPaks) {
        ...
        // Get top level directories (we'll filter them later since the
        // Sys_ListFiles filtering is terrible)
    ...
    while ((pakfilesi < numfiles) || (pakdirsi < numdirs)) {
        ...
        if (pakwhich) {
            ...
            // But wait, this could be any directory, we're filtering to only
            // ending with ".pk3dir" here.
            ...
            Q_strncpyz(search->dir->path, curpath,
                       sizeof(search->dir->path)); // c:\quake3\baseq3
            Q_strncpyz(
                search->dir->fullpath, pakfile,
...
traversal bugs and return qtrue if it does.
================
*/
...
Returns a list of pak files that we should download from the server. They all
get stored in the current gamedir and an FS_Restart will be fired up after we
download them all.
...
we are not interested in a download string format, we want something
human-readable (this is used for diagnostics while connecting to a pure server)
    ...
    for (i = 0; i < fs_numServerReferencedPaks; i++) {
        ...
        if (!havepak && fs_serverReferencedPakNames[i] &&
            *fs_serverReferencedPakNames[i]) {
            ...
            if (dlstring) {
                // We need this to make sure we won't hit the end of the buffer
                // or the server could overwrite non-pk3 files on clients by
                // writing so much crap into neededpaks that Q_strcat cuts off
                // the .pk3 extension.
                ...
                if (FS_SV_FileExists(
                        va("%s.pk3", fs_serverReferencedPakNames[i]))) {
                    char st[MAX_ZPATH];
                    // We already have one called this, we need to download it
                    // to another name Make something up with the checksum in it
                    Com_sprintf(st, sizeof(st), "%s.%08x.pk3",
                                fs_serverReferencedPakNames[i],
                ...
                // Find out whether it might have overflowed the buffer and
                // don't add this file to the list if that is the case.
...
================
FS_ReorderPurePaks
NOTE TTimo: the reordering that happens here is not reflected in the cvars
(\cvarlist *pak*) this can lead to misleading situations, see
...
static void FS_ReorderPurePaks(void) {
    ...
    for (i = 0; i < fs_numServerPaks; i++) {
        ...
        for (s = *p_insert_index; s; s = s->next) {
            // the part of the list before p_insert_index has been sorted
            // already
...
static void FS_Startup(const char *gameName) {
    ...
    fs_basepath = Cvar_Get("fs_basepath", Sys_DefaultInstallPath(),
    ...
    fs_gamedirvar =
        Cvar_Get("fs_game", fs_basegame->string, CVAR_INIT | CVAR_SYSTEMINFO);
...
files. Servers with sv_pure set will get this string and pass it to clients.
=====================
*/
...
Returns a space separated string containing the pure checksums of all loaded pk3
files. Servers with sv_pure use these checksums to compare with the checksums
the clients send back to the server.
=====================
*/
...
Returns a space separated string containing the checksums of all referenced pk3
files. The server will send this to the clients so they can check which files
    ...
    for (search = fs_searchpaths; search; search = search->next) {
        ...
        if (search->pack) {
            if (search->pack->referenced ||
                Q_stricmpn(search->pack->pakGamename, com_basegame->string,
...
Returns a space separated string containing the pure checksums of all referenced
pk3 files. Servers with sv_pure set will get this string back from clients for
pure validation
...
const char *FS_ReferencedPakPureChecksums(void) {
    ...
    for (nFlags = FS_CGAME_REF; nFlags; nFlags = nFlags >> 1) {
        ...
        for (search = fs_searchpaths; search; search = search->next) {
            // is the element a pak file and has it been referenced based on
            // flag?
...
Returns a space separated string containing the names of all referenced pk3
files. The server will send this to the clients so they can check which files
should be auto-downloaded.
=====================
*/
...
void FS_Restart(int checksumFeed) {
    ...
    // graphics screen when the font fails to load
    if (FS_ReadFile("default.cfg", NULL) <= 0) {
        // this might happen when connecting to a pure server not using
        // BASEGAME/pak0.pk3 (for instance a TA demo server)
        if (lastValidBase[0]) {
            FS_PureServerSetLoadedPaks("", "");
...
qboolean FS_ConditionalRestart(int checksumFeed, qboolean disconnect) {
    if (fs_gamedirvar->modified) {
        if (FS_FilenameCompare(lastValidGame, fs_gamedirvar->string) &&
            (*lastValidGame ||
             FS_FilenameCompare(fs_gamedirvar->string, com_basegame->string)) &&
...
void FS_FilenameCompletion(const char *dir, const char *ext, qboolean stripExt,
    ...
    filenames =
        FS_ListFilteredFiles(dir, ext, NULL, &nfiles, allowNonPureFilesOnDisk);
