...
void COM_ParseError(char *format, ...) {
    ...
    Com_Printf("ERROR: %s, line %d: %s\n", com_parsename,
               COM_GetCurrentParseLine(), string);
}
    ...
    if (in) {
        while ((c = *in) != 0) {
            ...
            if (c == '/' && in[1] == '/') {
                ...
                // if we have a pending newline, emit it (and it counts as
                // whitespace)
                if (newline) {
                    *out++ = '\n';
                    newline = qfalse;
...
int QDECL Com_sprintf(char *dest, int size, const char *fmt, ...) {
    ...
    if (len >= size)
        Com_Printf(
            "Com_sprintf: Output length %d too short, require %d bytes.\n",
            size, len + 1);
...
void Info_SetValueForKey(char *s, const char *key, const char *value) {
    ...
    if (strlen(newi) + strlen(s) >= MAX_INFO_STRING) {
        Com_Printf(S_COLOR_YELLOW
                   "Info string length exceeded, can't add %s = %s\n",
                   key, value);
        return;
    }
...
void Info_SetValueForKey_Big(char *s, const char *key, const char *value) {
    ...
    if (strlen(newi) + strlen(s) >= BIG_INFO_STRING) {
        Com_Printf(S_COLOR_YELLOW
                   "BIG Info string length exceeded, can't add %s = %s\n",
                   key, value);
        return;
    }
