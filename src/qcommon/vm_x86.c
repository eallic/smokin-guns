...
typedef enum { VM_JMP_VIOLATION = 0, VM_BLOCK_COPY = 1 } ESysCallType;

static ELastCommand LastCommand;

static int iss8(int32_t v) { return (SCHAR_MIN <= v && v <= SCHAR_MAX); }
...
static int NextConstant4(void) {
    return (code[pc] | (code[pc + 1] << 8) | (code[pc + 2] << 16) |
            (code[pc + 3] << 24));
}
...
static void EmitMovEAXStack(vm_t *vm, int andit) {
    if (!jlabel) {
        if (LastCommand ==
            LAST_COMMAND_MOV_STACK_EAX) { // mov [edi + ebx * 4], eax
            compiledOfs -= 3;
            vm->instructionPointers[instruction - 1] = compiledOfs;
        } else if (pop1 == OP_CONST && buf[compiledOfs - 7] == 0xC7 &&
                   buf[compiledOfs - 6] == 0x04 &&
                   buf[compiledOfs - 5] ==
                       0x9F) { // mov [edi + ebx * 4], 0x12345678
            compiledOfs -= 7;
            vm->instructionPointers[instruction - 1] = compiledOfs;
            ...
            return;
        } else if (pop1 != OP_DIVI && pop1 != OP_DIVU && pop1 != OP_MULI &&
                   pop1 != OP_MULU && pop1 != OP_STORE4 && pop1 != OP_STORE2 &&
                   pop1 != OP_STORE1) {
...
void EmitMovECXStack(vm_t *vm) {
    if (!jlabel) {
        ...
        if (pop1 == OP_DIVI || pop1 == OP_DIVU || pop1 == OP_MULI ||
            pop1 == OP_MULU || pop1 == OP_STORE4 || pop1 == OP_STORE2 ||
            pop1 == OP_STORE1) {
            EmitString("89 C1"); // mov ecx, eax
            return;
...
void EmitMovEDXStack(vm_t *vm, int andit) {
    if (!jlabel) {
        if (LastCommand ==
            LAST_COMMAND_MOV_STACK_EAX) { // mov dword ptr [edi + ebx * 4], eax
            compiledOfs -= 3;
            vm->instructionPointers[instruction - 1] = compiledOfs;
            ...
            EmitString("8B D0"); // mov edx, eax
                   ...
                   buf[compiledOfs - 6] == 0x07 &&
                   buf[compiledOfs - 5] ==
                       0x9F) { // mov dword ptr [edi + ebx * 4], 0x12345678
            compiledOfs -= 7;
            vm->instructionPointers[instruction - 1] = compiledOfs;
...
static void DoSyscall(void) {
    ...
    if (vm_syscallNum < 0) {
        ...
        vm_opStackBase[vm_opStackOfs + 1] =
            savedVM->systemCall((intptr_t *)data);
#endif
    } else {
        ...
        case VM_BLOCK_COPY:
            ...
            VM_BlockCopy(vm_opStackBase[(vm_opStackOfs - 1)],
                         vm_opStackBase[vm_opStackOfs], vm_arg);
            break;
        default:
...
#else
    EmitString(
        "FF 14 85"); // call dword ptr [vm->instructionPointers + eax * 4]
...
=================
ConstOptimize
Constant values for immediately following instructions may be translated to
immediate values instead of opStack operations, which will save expensive
...
#if idx64
        EmitRexString(0x41,
                      "0F B7 81"); // movzx eax, word ptr [r9 + 0x12345678]
        Emit4(Constant4() & vm->dataMask);
#else
        EmitString("B8"); // mov eax, 0x12345678
        EmitPtr(vm->dataBase + (Constant4() & vm->dataMask));
...
#if idx64
        EmitRexString(0x41,
                      "0F B6 81"); // movzx eax, byte ptr [r9 + 0x12345678]
        Emit4(Constant4() & vm->dataMask);
#else
        EmitString("B8"); // mov eax, 0x12345678
        EmitPtr(vm->dataBase + (Constant4() & vm->dataMask));
    ...
    for (pass = 0; pass < 3; pass++) {
        ...
        while (instruction < header->instructionCount) {
            ...
            case OP_CONST:
                ...
                EmitPushStack(vm);
                EmitString(
                    "C7 04 9F"); // mov dword ptr [edi + ebx * 4], 0x12345678
                lastConst = Constant4();
            ...
            case OP_LOCAL:
                ...
                oc1 = Constant4();
                Emit4(oc1);
                EmitCommand(LAST_COMMAND_MOV_STACK_EAX); // mov dword ptr [edi +
                                                         // ebx * 4], eax
                break;
            case OP_ARG:
...
#if idx64
                EmitRexString(0x41,
                              "89 04 11"); // mov dword ptr [r9 + edx], eax
#else
            ...
            case OP_LEAVE:
                ...
                break;
            case OP_LOAD4:
                if (code[pc] == OP_CONST && code[pc + 5] == OP_ADD &&
                    code[pc + 6] == OP_STORE4) {
                    ...
                    if (v == 1 && oc0 == oc1 && pop0 == OP_LOCAL &&
                        pop1 == OP_LOCAL) {
#if idx64
                        EmitRexString(0x41,
                                      "FF 04 11"); // inc dword ptr [r9 + edx]
#else
...
#if idx64
                        EmitRexString(
                            0x41, "8B 04 11"); // mov eax, dword ptr [r9 + edx]
#else
                        EmitString(
                            "8B 82"); // mov eax, dword ptr [edx + 0x12345678]
                        Emit4((intptr_t)vm->dataBase);
#endif
                        EmitString("05"); // add eax, v
                        Emit4(v);

                        if (oc0 == oc1 && pop0 == OP_LOCAL &&
                            pop1 == OP_LOCAL) {
#if idx64
                            EmitRexString(
                                0x41,
                                "89 04 11"); // mov dword ptr [r9 + edx], eax
#else
                            EmitString("89 82"); // mov dword ptr [edx +
                                                 // 0x12345678], eax
                            Emit4((intptr_t)vm->dataBase);
#endif
                        } else {
                            EmitCommand(LAST_COMMAND_SUB_BL_1); // sub bl, 1
                            EmitString("8B 14 9F"); // mov edx, dword ptr [edi +
                                                    // ebx * 4]
                            MASK_REG("E2", vm->dataMask); // and edx, 0x12345678
#if idx64
                            EmitRexString(
                ...
                if (code[pc] == OP_CONST && code[pc + 5] == OP_SUB &&
                    code[pc + 6] == OP_STORE4) {
                    if (oc0 == oc1 && pop0 == OP_LOCAL && pop1 == OP_LOCAL) {
                        compiledOfs -= 12;
                        vm->instructionPointers[instruction - 1] = compiledOfs;
                ...
                if (buf[compiledOfs - 3] == 0x89 &&
                    buf[compiledOfs - 2] == 0x04 &&
                    buf[compiledOfs - 1] == 0x9F) {
                    compiledOfs -= 3;
                    vm->instructionPointers[instruction - 1] = compiledOfs;
                    MASK_REG("E0", vm->dataMask); // and eax, 0x12345678
#if idx64
                    EmitRexString(0x41,
                                  "8B 04 01"); // mov eax, dword ptr [r9 + eax]
#else
                    EmitString("8B 80"); // mov eax, dword ptr [eax + 0x1234567]
                    Emit4((intptr_t)vm->dataBase);
#endif
                    EmitCommand(
                        LAST_COMMAND_MOV_STACK_EAX); // mov dword ptr [edi + ebx
                                                     // * 4], eax
                    break;
                }
...
#if idx64
                EmitRexString(0x41,
                              "0F B7 04 01"); // movzx eax, word ptr [r9 + eax]
#else
                EmitString(
                    "0F B7 80"); // movzx eax, word ptr [eax + 0x12345678]
                Emit4((intptr_t)vm->dataBase);
#endif
                EmitCommand(LAST_COMMAND_MOV_STACK_EAX); // mov dword ptr [edi +
                                                         // ebx * 4], eax
...
#if idx64
                EmitRexString(0x41,
                              "0F B6 04 01"); // movzx eax, byte ptr [r9 + eax]
#else
                EmitString(
                    "0F B6 80"); // movzx eax, byte ptr [eax + 0x12345678]
                Emit4((intptr_t)vm->dataBase);
#endif
                EmitCommand(LAST_COMMAND_MOV_STACK_EAX); // mov dword ptr [edi +
                                                         // ebx * 4], eax
            ...
            case OP_STORE4:
                EmitMovEAXStack(vm, 0);
                EmitString(
                    "8B 54 9F FC"); // mov edx, dword ptr -4[edi + ebx * 4]
            ...
            case OP_GEU:
                ...
                EmitCommand(LAST_COMMAND_SUB_BL_2); // sub bl, 2
                EmitString(
                    "39 44 9F 04"); // cmp	eax, dword ptr 4[edi + ebx * 4]
            ...
            case OP_ADD:
                EmitMovEAXStack(vm, 0); // mov eax, dword ptr [edi + ebx * 4]
                EmitString(
                    "01 44 9F FC"); // add dword ptr -4[edi + ebx * 4], eax
                EmitCommand(LAST_COMMAND_SUB_BL_1); // sub bl, 1
                break;
            case OP_SUB:
                EmitMovEAXStack(vm, 0); // mov eax, dword ptr [edi + ebx * 4]
                EmitString(
                    "29 44 9F FC"); // sub dword ptr -4[edi + ebx * 4], eax
                EmitCommand(LAST_COMMAND_SUB_BL_1); // sub bl, 1
                break;
            case OP_DIVI:
                EmitString(
                    "8B 44 9F FC");     // mov eax,dword ptr -4[edi + ebx * 4]
                EmitString("99");       // cdq
                EmitString("F7 3C 9F"); // idiv dword ptr [edi + ebx * 4]
                EmitString(
                    "89 44 9F FC"); // mov dword ptr -4[edi + ebx * 4],eax
                EmitCommand(LAST_COMMAND_SUB_BL_1); // sub bl, 1
                break;
            case OP_DIVU:
                EmitString(
            ...
            case OP_MODI:
                ...
                EmitString("F7 3C 9F"); // idiv dword ptr [edi + ebx * 4]
                EmitString(
                    "89 54 9F FC"); // mov dword ptr -4[edi + ebx * 4],edx
                EmitCommand(LAST_COMMAND_SUB_BL_1); // sub bl, 1
                break;
            case OP_MODU:
                EmitString(
            ...
            case OP_BAND:
                EmitMovEAXStack(vm, 0); // mov eax, dword ptr [edi + ebx * 4]
                EmitString(
                    "21 44 9F FC"); // and dword ptr -4[edi + ebx * 4],eax
                EmitCommand(LAST_COMMAND_SUB_BL_1); // sub bl, 1
                break;
            case OP_BOR:
                EmitMovEAXStack(vm, 0);    // mov eax, dword ptr [edi + ebx * 4]
            ...
            case OP_BXOR:
                EmitMovEAXStack(vm, 0); // mov eax, dword ptr [edi + ebx * 4]
                EmitString(
                    "31 44 9F FC"); // xor dword ptr -4[edi + ebx * 4],eax
                EmitCommand(LAST_COMMAND_SUB_BL_1); // sub bl, 1
                break;
            case OP_BCOM:
            ...
            case OP_LSH:
                EmitMovECXStack(vm);
                EmitString(
                    "D3 64 9F FC"); // shl dword ptr -4[edi + ebx * 4], cl
                EmitCommand(LAST_COMMAND_SUB_BL_1); // sub bl, 1
                break;
            case OP_RSHI:
                EmitMovECXStack(vm);
                EmitString(
                    "D3 7C 9F FC"); // sar dword ptr -4[edi + ebx * 4], cl
                EmitCommand(LAST_COMMAND_SUB_BL_1); // sub bl, 1
                break;
            case OP_RSHU:
                EmitMovECXStack(vm);
                EmitString(
                    "D3 6C 9F FC"); // shr dword ptr -4[edi + ebx * 4], cl
                EmitCommand(LAST_COMMAND_SUB_BL_1); // sub bl, 1
                break;
            case OP_NEGF:
            ...
            case OP_SEX8:
                EmitString(
                    "0F BE 04 9F"); // movsx eax, byte ptr [edi + ebx * 4]
                EmitCommand(LAST_COMMAND_MOV_STACK_EAX); // mov dword ptr [edi +
                                                         // ebx * 4], eax
                break;
            case OP_SEX16:
                EmitString(
                    "0F BF 04 9F"); // movsx eax, word ptr [edi + ebx * 4]
                EmitCommand(LAST_COMMAND_MOV_STACK_EAX); // mov dword ptr [edi +
                                                         // ebx * 4], eax
                break;
            ...
            case OP_JUMP:
                EmitCommand(LAST_COMMAND_SUB_BL_1); // sub bl, 1
                EmitString(
                    "8B 44 9F 04");  // mov eax, dword ptr 4[edi + ebx * 4]
                EmitString("81 F8"); // cmp eax, vm->instructionCount
                Emit4(vm->instructionCount);
#if idx64
                ...
                EmitString("FF 24 85"); // jmp dword ptr [instructionPointers +
                                        // eax * 4]
                Emit4((intptr_t)vm->instructionPointers);
#endif
            ...
            default:
                VMFREE_BUFFERS();
                Com_Error(ERR_DROP, "VM_CompileX86: bad opcode %i at offset %i",
                          op, pc);
            }
            pop0 = pop1;
            pop1 = op;
...
#ifdef VM_X86_MMAP
    vm->codeBase =
        mmap(NULL, compiledOfs, PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    if (vm->codeBase == MAP_FAILED)
        Com_Error(ERR_FATAL, "VM_CompileX86: can't mmap memory");
#elif _WIN32
    // allocate memory with EXECUTE permissions under windows.
    vm->codeBase =
        VirtualAlloc(NULL, compiledOfs, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
    if (!vm->codeBase)
...
extern uint8_t qvmcall64(int *programStack, int *opStack,
                         intptr_t *instructionPointers, byte *dataBase);
#endif
...
#ifdef _MSC_VER
#if idx64
    opStackOfs = qvmcall64(&programStack, opStack, vm->instructionPointers,
                           vm->dataBase);
#else
    __asm
    {
