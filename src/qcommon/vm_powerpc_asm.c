...
ppc_instruction_t asm_instruction(powerpc_iname_t sname, const int argc,
                                  const long int *argv) {
        ...
        if (operand->insert) {
            ...
            if (operand->flags & PPC_OPERAND_SIGNED) {
                ...
                if ((opu & ~bitm_full) != 0 && (opu | bitm_full) != -1)
                    printf("%s: signed operand nr.%d to wide. op: %.8lx, mask: "
                           "%.8lx\n",
                           name, argi, opu, bitm);
            } else {
                if ((opu & ~bitm_full) != 0)
                    printf("%s: unsigned operand nr.%d to wide. op: %.8lx, "
                           "mask: %.8lx\n",
                           name, argi, opu, bitm);
            }
            if ((bitm & 1) == 0) {
                if (opu & 0xf & ~bitm)
                    printf("%s: operand nr.%d not aligned correctly. op: "
                           "%.8lx, mask: %.8lx\n",
                           name, argi, opu, bitm);
            }
...
static unsigned long insert_bdm(unsigned long insn, long value, int dialect,
...
static unsigned long insert_bo(unsigned long insn, long value, int dialect,
                               const char **errmsg) {
...
static unsigned long insert_rbs(unsigned long insn, long value ATTRIBUTE_UNUSED,
                                int dialect ATTRIBUTE_UNUSED,
                                const char **errmsg ATTRIBUTE_UNUSED) {
...
#define SC_MASK                                                                \
    (OP_MASK | (((unsigned long)0x3ff) << 16) | (((unsigned long)1) << 1) | 1)
...
#define XS(op, xop, rc)                                                        \
    (OP(op) | ((((unsigned long)(xop)) & 0x1ff) << 2) |                        \
     (((unsigned long)(rc)) & 1))
...
#define XSPR(op, xop, spr)                                                     \
    (X((op), (xop)) | ((((unsigned long)(spr)) & 0x1f) << 16) |                \
...
#define COM32                                                                  \
    PPC_OPCODE_POWER | PPC_OPCODE_PPC | PPC_OPCODE_COMMON | PPC_OPCODE_32
