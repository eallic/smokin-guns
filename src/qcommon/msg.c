...
void MSG_ReadDeltaEntity(msg_t *msg, entityState_t *from, entityState_t *to,
    ...
    if (MSG_ReadBits(msg, 1) == 1) {
        ...
        if (cl_shownet &&
            (cl_shownet->integer >= 2 || cl_shownet->integer == -1)) {
...
void MSG_NUinitHuffman() {
        ...
        for(i=0;i<size;i++) {
                ch = data[i];
                Huff_addRef(&msgHuff.compressor,	ch);
// Do update Huff_addRef(&msgHuff.decompressor,	ch);			// Do
update array[ch]++;
        }
        Com_Printf("msg_hData {\n");
        for(i=0;i<256;i++) {
                if (array[i] == 0) {
                        Huff_addRef(&msgHuff.compressor,	i);
// Do update Huff_addRef(&msgHuff.decompressor,	i);			// Do
update
                }
                Com_Printf("%d,			// %d\n", array[i], i);
        }
