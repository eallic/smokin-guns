...
void Cbuf_Execute(void) {
    ...
    // a semicolon.  It will keep /* ... */ style comments all on one line by
    // not breaking it for semicolon or newline.
    ...
    while (cmd_text.cursize) {
            ...
            if (!(quotes & 1)) {
                if (i < cmd_text.cursize - 1) {
                    if (!in_star_comment && text[i] == '/' &&
                        ...
                        // If we are in a star comment, then the part after it
                        // is valid Note: This will cause it to NUL out the
                        // terminating '/' but ExecuteString doesn't require it
                        // anyway.
                        i++;
                        break;
                    }
        ...
        // down this is necessary because commands (exec) can insert data at the
...
===============
*/
void Cmd_Echo_f(void) { Com_Printf("%s\n", Cmd_Args()); }
...
static char cmd_tokenized[BIG_INFO_STRING +
                          MAX_STRING_TOKENS]; // will have 0 bytes inserted
static char cmd_cmd[BIG_INFO_STRING]; // the original command we received (no
                                      // token processing)
...
void Cmd_RemoveCommandSafe(const char *cmd_name) {
    ...
    if (cmd->function) {
        Com_Error(ERR_DROP,
                  "Restricted source tried to remove "
                  "system command \"%s\"",
                  cmd_name);
        return;
    }
