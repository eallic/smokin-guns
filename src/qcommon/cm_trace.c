...
void CM_TestBoxInBrush(traceWork_t *tw, cbrush_t *brush) {
    ...
    if (tw->sphere.use) {
        ...
        for (i = 6; i < brush->numsides; i++) {
            ...
            dist = plane->dist -
                   DotProduct(tw->offsets[plane->signbits], plane->normal);
...
void CM_TraceThroughLeaf(traceWork_t *tw, cLeaf_t *leaf) {
    ...
    for (k = 0; k < leaf->numLeafBrushes; k++) {
        ...
        if (!CM_BoundsIntersect(tw->bounds[0], tw->bounds[1], b->bounds[0],
                                b->bounds[1])) {
            continue;
        }
...
================
*/
void CM_TraceThroughSphere(traceWork_t *tw, vec3_t origin, float radius,
    ...
    //origin[0])^2 + (start[1] - origin[1])^2 + (start[2] - origin[2])^2 -
    //radius^2;
    //
    ...
    if (d > 0) {
        ...
        if (fraction < tw->trace.fraction) {
            ...
            tw->trace.plane.dist =
                DotProduct(tw->trace.plane.normal, intersection);
...
void CM_TraceCapsuleThroughCapsule(traceWork_t *tw, clipHandle_t model) {
    ...
    if (tw->start[0] != tw->end[0] || tw->start[1] != tw->end[1]) {
        // height of the expanded cylinder is the height of both cylinders minus
        ...
        if (h > 0) {
            ...
            CM_TraceThroughVerticalCylinder(tw, offset, radius, h, tw->start,
                                            tw->end);
        }
    }
...
void CM_Trace(trace_t *results, const vec3_t start, const vec3_t end,
              vec3_t mins, vec3_t maxs, clipHandle_t model, const vec3_t origin,
    ...
    if (tw.trace.fraction == 1) {
        ...
        for (i = 0; i < 3; i++) {
            tw.trace.endpos[i] =
                start[i] + tw.trace.fraction * (end[i] - start[i]);
        }
    }

    // If allsolid is set (was entirely inside something solid), the plane is
    // not valid. If fraction == 1.0, we never hit anything, and thus the plane
    // is not valid. Otherwise, the normal on the plane should have unit length
    assert(tw.trace.allsolid || tw.trace.fraction == 1.0 ||
...
void CM_TransformedBoxTrace(trace_t *results, const vec3_t start,
    ...
    if (rotated) {
        ...
        // bounding
        //		 box that is swept through the model is not rotated. We
        //cannot rotate 		 the bounding box or the bmodel because that would make
        //all the brush 		 bevels invalid. 		 However this is correct for capsules
        //since a capsule itself is rotated too.
