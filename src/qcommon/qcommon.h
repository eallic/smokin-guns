...
typedef struct {
    ...
    qboolean overflowed;    // set to true if the buffer size failed (with
                            // allowoverflow set)
    qboolean
...
void MSG_WriteDeltaUsercmdKey(msg_t *msg, int key, usercmd_t *from,
                              usercmd_t *to);
void MSG_ReadDeltaUsercmdKey(msg_t *msg, int key, usercmd_t *from,
                             usercmd_t *to);
...
#define NET_ADDRSTRMAXLEN                                                      \
    48 // maximum length of an IPv6 address string including trailing '\0'
typedef struct {
    netadrtype_t type;
...
void QDECL NET_OutOfBandData(netsrc_t sock, netadr_t adr, byte *format,
                             int len);
...
// will properly create any needed paths and deal with seperater character
// issues
...
// closed It is generally safe to always set uniqueFILE to true, because the
// majority of file IO goes through FS_ReadFile, which Does The Right Thing
// already.
...
char *Com_MD5File(const char *filename, int length, const char *prefix,
                  int prefix_len);
char *Com_MD5Text(const char *text, int length, const char *prefix,
                  int prefix_len);
char *Com_MD5TextArray(const char **array, int length, const char *prefix,
                       int prefix_len);
...
qboolean Com_IsVoipTarget(uint8_t *voipTargets, int voipTargetsSize,
                          int clientNum);
...
#ifdef ZONE_DEBUG
#define Z_TagMalloc(size, tag)                                                 \
    Z_TagMallocDebug(size, tag, #size, __FILE__, __LINE__)
#define Z_Malloc(size) Z_MallocDebug(size, #size, __FILE__, __LINE__)
#define S_Malloc(size) S_MallocDebug(size, #size, __FILE__, __LINE__)
void *Z_TagMallocDebug(int size, int tag, char *label, char *file,
                       int line); // NOT 0 filled memory
void *Z_MallocDebug(int size, char *label, char *file,
                    int line); // returns 0 filled memory
void *S_MallocDebug(int size, char *label, char *file,
                    int line); // returns 0 filled memory
#else
...
typedef enum { DR_YES = 0, DR_NO = 1, DR_OK = 0, DR_CANCEL = 1 } dialogResult_t;
